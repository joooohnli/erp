...
<form method="post" action="gatherPeriodExpiry_remind.jsp">
...
%>
 <table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
<tr <%=TR_STYLE1%> class="TR_STYLE1">
			 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","督办")%>"></div></td>
 </tr> 
 </table>
<%
int m=0;
...
nseer_grid.columns =[
                       ...
                       {name: '<%=demo.getLang("erp","订单执行时间")%>'},
                       {name: '<%=demo.getLang("erp","要求回款期限（天）")%>'},
                       {name: '<%=demo.getLang("erp","未回款时间（天）")%>'},
					   {name: '<%=demo.getLang("erp","超期（天）")%>'},
					   {name: '<%=demo.getLang("erp","点选")%>'}
]
nseer_grid.column_width=[180,100,160,120,120,100,40];
...
%>
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../order/query.jsp?order_ID=<%=rs.getString("order_ID")%>")><span style="color:<%=color1%>"><%=rs.getString("order_ID")%></span></div>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../file/query.jsp?customer_ID=<%=rs.getString("customer_ID")%>")><span style="color:<%=color1%>"><%=exchange.toHtml(rs.getString("customer_name"))%></span></div>','<span style="color:<%=color1%>"><%=exchange.toHtml(rs.getString("order_check_time"))%></span>','<span style="color:<%=color1%>"><%=exchange.toHtml(rs.getString("gather_period_limit"))%></span>','<span style="color:<%=color1%>"><%=exchange.toHtml(rs.getString("gather_period_expiry"))%></span>','<span style="color:<%=color1%>"><%=exchange.toHtml(rs.getString("period_expiry_over"))%></span>','<span style="color:<%=color1%>"><input type="checkbox" <%=CHECKBOX_STYLE1%> class="CHECKBOX_STYLE1" name="<%=m%>" value="<%=rs.getString("order_ID")%>/<%=rs.getString("id")%>">&nbsp;</span>'],
<%
m++;
}
crm_db.close();
...
<page:updowntag num="<%=intRowCount%>" file="gatherPeriodExpiry.jsp"/> 
