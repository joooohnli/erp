...
%>
<form method="post" action="gatherSumLimit_remind_1.jsp">
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3" colspan="2"><%=demo.getLang("erp","欠款报警")%>：<%=intRowCount%><%=demo.getLang("erp","例")%></td>
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
%>
['<div style="text-decoration : underline;color:#3366FF" onclick=winopen("../file/query.jsp?customer_ID=<%=rs.getString("customer_ID")%>")><span style="color:<%=color1%>"><%=rs.getString("customer_ID")%></span></div>','<span style="color:<%=color1%>"><input type="checkbox" <%=CHECKBOX_STYLE1%> class="CHECKBOX_STYLE1" name="<%=m%>" value="<%=rs.getString("customer_ID")%>/<%=rs.getString("id")%>">&nbsp;</span>'],
<%
m++;
}
crm_db.close();
