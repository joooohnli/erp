...
<div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","添加")%>" onClick=location="strategyClass_register.jsp">&nbsp;<input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","删除")%>"></div></td>
</tr>
</table>
<% 
 String sql = "select * from crm_config_public_char where kind='客户级别' order by type_ID";
...
nseer_grid.columns =[
                       {name: '&nbsp;'},
                       {name: '<%=demo.getLang("erp","客户优质级别名称")%>'}
                  ]
nseer_grid.column_width=[40,180];
nseer_grid.auto='<%=demo.getLang("erp","客户优质级别名称")%>';
nseer_grid.data = [
