...
function id_link1(customer_ID,customer_name,contact_person1,customer_tel1){
var link1='register_getpara.jsp?customer_ID='+customer_ID+'&&customer_name='+customer_name+'&&contact_person1='+contact_person1+'&&customer_tel1='+customer_tel1;
...
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../file/query.jsp?customer_ID=<%=rs.getString("customer_ID")%>")><%=rs.getString("customer_ID")%></div>','<%=exchange.toHtml(rs.getString("customer_name"))%>','<%=exchange.toHtml(rs.getString("customer_class"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("sales_name"))%>','<div style="text-decoration : underline;color:#3366FF"  onclick=id_link1("<%=rs.getString("customer_ID")%>","<%=toUtf8String.utf8String(exchange.toURL(rs.getString("customer_name")))%>","<%=toUtf8String.utf8String(exchange.toURL(rs.getString("contact_person1")))%>","<%=toUtf8String.utf8String(exchange.toURL(rs.getString("customer_tel1")))%>")><%=demo.getLang("erp","登记")%></div>'],
<%}else{%>
