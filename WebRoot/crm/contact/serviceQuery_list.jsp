...
String nickName="出入库明细";
String fileName="serviceQuery_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的记录总数");
%>
...
nseer_grid.columns =[
                       ...
                       {name: '<%=demo.getLang("erp","批号或序号")%>'},
                       {name: '<%=demo.getLang("erp","出库时间")%>'}
]
nseer_grid.column_width=[180,200,200,160];
...
%>
['<%=rs1.getString("product_ID")%>','<%=exchange.toHtml(rs1.getString("product_name"))%>','<%=rs1.getString("serial_number")%>','<%=exchange.toHtml(register_time)%>'],
</page:pages>
['']];
nseer_grid.init();
</script>
