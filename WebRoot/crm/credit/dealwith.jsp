...
String crediter_name=exchange.unURL(request.getParameter("customer_name")) ;
...
String condition="payer_ID='"+crediter_ID+"' and reason='销售赊货' and cost_price_sum!='0' and check_tag='1'";
...
String rowSummary=demo.getLang("erp","客户")+exchange.toHtml(crediter_name)+demo.getLang("erp","的赊货单总数");
%>
...
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../stock/apply_pay/query.jsp?pay_ID=<%=rs1.getString("pay_ID")%>")><%=rs1.getString("pay_ID")%></div>','<%=exchange.toHtml(rs1.getString("payer_name"))%>','<%=exchange.toHtml(rs1.getString("sales_name"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs1.getDouble("demand_amount"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("list_price_sum"))%>',<%if(rs1.getString("pay_tag").equals("2")){%>'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("dealwith_reconfirm.jsp?pay_ID=<%=rs1.getString("pay_ID")%>&&sales_ID=<%=sales_ID%>&&sales_name=<%=toUtf8String.utf8String(exchange.toURL(sales_name))%>&&salecredit_list_price_sum=<%=toUtf8String.utf8String(exchange.toURL(salecredit_list_price_sum))%>")><%=demo.getLang("erp","开始")%></div>'<%}else{%>'<%=demo.getLang("erp","出库未完成")%>'<%}%>],
</page:pages>
['']];
nseer_grid.init();
</script>
