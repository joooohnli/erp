...
String rowSummary=demo.getLang("erp","当前等待审核的客户报价总数");
%>
...
nseer_grid.column_width=[180,200,70,70,140,70,100,
<%
for(int i=1;i<workflow_amount;i++){
%>					   70,
<%
...
sql1="select id from crm_workflow where object_ID='"+rs.getString("discussion_ID")+"' and check_tag='0'";
ResultSet rs2=crm_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from crm_workflow where object_ID='"+rs.getString("discussion_ID")+"' order by id";
	rs2=crm_db1.executeQuery(sql1);
%>
  ['<span style="color:<%=color%>" ><%=rs.getString("discussion_ID")%></span>','<span style="color:<%=color%>" ><%=exchange.toHtml(rs.getString("customer_name"))%></span>','<span style="color:<%=color%>" ><%=exchange.toHtml(rs.getString("sales_name"))%></span>','<span style="color:<%=color%>" ><%=exchange.toHtml(rs.getString("demand_contact_person_tel"))%></span>','<span style="color:<%=color%>" ><%=exchange.toHtml(rs.getString("demand_pay_time"))%></span>','<span style="color:<%=color%>" ><%=exchange.toHtml(rs.getString("type"))%></span>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("sale_price_sum"))%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
