...
if(!vt.validata((String)session.getAttribute("unit_db_name"),"crm_discussion","id",id,"discussion_tag").equals("2")){
try{
...
form.action = "../../crm_discussion_dealwith_ok?id=<%=rs.getString("id")%>";
}else{
form.action = "dealwith_delete_reconfirm.jsp?id=<%=rs.getString("id")%>";
}
}
</script>
...
<form id="mutiValidation" method="POST" onSubmit="return doValidate('../../xml/crm/crm_discussion.xml','mutiValidation')&&TwoSubmit(this)">
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><INPUT name="Ref" type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1" value=cop checked><%=demo.getLang("erp","转订单")%> <INPUT name="Ref" type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1" value=Ind> 
 <%=demo.getLang("erp","转垃圾箱")%> <input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","确认")%>" name="B1"> <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></div></td>
 </tr>
 </table>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","提货时间")%>：</td>
...
<input name="demand_customer_mailing_address" type="hidden" value="<%=exchange.toHtml(rs.getString("demand_customer_mailing_address"))%>">
...
<input name="process_time" type="hidden" value="<%=exchange.toHtml(time)%>">
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" name="amount<%=i%>" value="<%=exchange.toHtml(rs6.getString("amount"))%>"></td>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1"> 
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="processer" value="<%=exchange.toHtml(processer)%>"></td>
 ...
 <input name="processer_ID" type="hidden" value="<%=processer_ID%>">
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","该记录已处理，请返回")%></td>
 <td <%=TD_STYLE3%> class="TD_STYLE3">&nbsp;</td>
 </tr>
</table>
</div>
