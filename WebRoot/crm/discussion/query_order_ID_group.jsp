...
color2="3333FF";
}else if(rs.getString("discussion_tag").equals("3")){
...
%>
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3">
<%=demo.getLang("erp","报价单编号")%>：<font color="<%=color%>"><%=rs.getString("discussion_ID")%></font> <%=demo.getLang("erp","其中审核状态")%>：<font color="<%=color1%>"><%=check_tag%></font>
<%=demo.getLang("erp","处理状态")%>：<font color="<%=color2%>"><%=discussion_tag%></font>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><%=exchange.toHtml(rs.getString("customer_name"))%>&nbsp;</td>
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","客户编号")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><%=rs.getString("customer_ID")%>&nbsp;</td>
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","电话")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><%=exchange.toHtml(rs.getString("demand_contact_person_tel"))%>&nbsp;</td>
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","拟订货时间")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%" colspan="2"><%=exchange.toHtml(rs.getString("demand_pay_time"))%>&nbsp;</td>
 
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","销售人")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><%=exchange.toHtml(rs.getString("sales_name"))%>&nbsp;</td>
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","销售人编号")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><%=rs.getString("sales_ID")%>&nbsp;</td>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%" colspan="2"><%=exchange.toHtml(rs.getString("checker"))%>&nbsp;</td>
 
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE11%> class="TD_STYLE1" height="65" width="11%"><%=demo.getLang("erp","备注")%>&nbsp;</td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" colspan="8" width="86.5%"><%=rs.getString("remark")%>&nbsp;</td>
 </tr>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
<%
String[] attachment_name1=DealWithString.divide(rs.getString("attachment_name"),20);
%>
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","总计")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" colspan="3" width="37.4%"><%=exchange.toHtml(rs.getString("sale_price_sum"))%>&nbsp;</td>
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","报价单附件")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" colspan="4" width="37.4%"><a href="javascript:winopenm('query_attachment.jsp?id=<%=rs.getString("id")%>&tablename=crm_discussion&fieldname=attachment_name')"><%=attachment_name1[1]%></a>&nbsp;</td>
 </tr>

 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","序号")%></td>
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="13%"><%=demo.getLang("erp","商品名称")%> </td>
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","商品编号")%> </td>
...
%>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2"><%=exchange.toHtml(rs6.getString("product_name"))%>&nbsp;</td>
 <td <%=TD_STYLE21%> class="TD_STYLE2"><%=rs6.getString("product_ID")%>&nbsp;</td>
 <td <%=TD_STYLE21%> class="TD_STYLE2"><%=rs6.getString("product_describe")%>&nbsp;</td>
 <td <%=TD_STYLE21%> class="TD_STYLE2"><%=exchange.toHtml(rs6.getString("amount"))%>&nbsp;</td>
 <td <%=TD_STYLE21%> class="TD_STYLE2"><%=exchange.toHtml(rs6.getString("amount_unit"))%>&nbsp;</td>
 <td <%=TD_STYLE21%> class="TD_STYLE2"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("list_price"))%>&nbsp;</td>
 <td <%=TD_STYLE21%> class="TD_STYLE2"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("subtotal"))%>&nbsp;</td>
 <td <%=TD_STYLE21%> class="TD_STYLE2"><%=rs6.getDouble("off_discount")%>&nbsp;</td>
 </tr>
<%
	i++;
	}
