...
String sql9="select * from crm_workflow where object_ID='"+customer_ID+"' and check_tag='0' and config_id<'"+config_id+"'";
ResultSet rs9=crmdb.executeQuery(sql9);
if(!rs9.next()){
if(vt.validata((String)session.getAttribute("unit_db_name"),"crm_file","customer_ID",customer_ID,"check_tag").equals("0")){
try{
...
<div id="nseerGround" class="nseerGround">
 <form id="mutiValidation" class="x-form" method="post" onSubmit="return doValidate('../../xml/crm/crm_file.xml','mutiValidation')&&TwoSubmit(this)">
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" colspan="3" width="13%"><input name="customer_ID" type="hidden" value="<%=rss.getString("customer_ID")%>"><%=rss.getString("customer_ID")%>　&nbsp; 
 &nbsp;&nbsp;&nbsp;&nbsp;</td>
 </tr>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><font color=red>*</font><%=demo.getLang("erp","客户名称")%> </td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="37.4%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="customer_name" value="<%=exchange.toHtml(rss.getString("customer_name"))%>" style="width:80%"></td>
 <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","地址")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="37.4%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="customer_address" style="width:80%" value="<%=exchange.toHtml(rss.getString("customer_address"))%>"></td>
 </tr>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="39%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" id="fileKind_chain" name="fileKind_chain" style="width:80%" onFocus="showKind('nseer1',this,showTree)" onkeyup="search_suggest(this.id)" value="<%=rss.getString("chain_id")%> <%=exchange.toHtml(rss.getString("chain_name"))%>">
 <input id="fileKind_chain_table" type="hidden" value="crm_config_file_kind"></td>
...
%>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%">
 <select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="gather_sum_limit" style="width:49%">
  <%
  String sql6 = "select * from crm_config_public_double where kind='欠款' order by type_ID" ;
	 ResultSet rs6 = crm_db.executeQuery(sql6) ;
while(rs6.next()){
	if(rs6.getDouble("type_value")==rss.getDouble("gather_sum_limit")){	
%>
		<option value="<%=rs6.getDouble("type_value")%>" selected><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs6.getDouble("type_value"))%></option>
<%}else{%>
		<option value="<%=rs6.getDouble("type_value")%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs6.getDouble("type_value"))%></option>
<%
}
}
%>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%">
 <select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="gather_period_limit" style="width:49%">
  <%
  String sql7 = "select * from crm_config_public_double where kind='回款' order by type_ID" ;
	 ResultSet rs7 = crm_db.executeQuery(sql7) ;
while(rs7.next()){
	if(rs7.getDouble("type_value")==rss.getDouble("gather_period_limit")){	
%>
		<option value="<%=rs7.getDouble("type_value")%>" selected><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs7.getDouble("type_value"))%></option>
<%}else{%>
		<option value="<%=rs7.getDouble("type_value")%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs7.getDouble("type_value"))%></option>
<%
}
}
%>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%">
 <select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="contact_period_limit" style="width:49%">
  <%
  String sql8 = "select * from crm_config_public_double where kind='联络' order by type_ID" ;
	 ResultSet rs8 = crm_db.executeQuery(sql8) ;
while(rs8.next()){
		if(rs8.getDouble("type_value")==rss.getDouble("contact_period_limit")){
	%>
		<option value="<%=rs8.getDouble("type_value")%>" selected><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs8.getDouble("type_value"))%></option>
<%}else{%>
		<option value="<%=rs8.getDouble("type_value")%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs8.getDouble("type_value"))%></option>
<%
}
}
%>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="used_customer_name" style="width:80%" value="<%=exchange.toHtml(rss.getString("used_customer_name"))%>"></td>
 <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","开户银行")%> </td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="customer_bank" style="width:49%" value="<%=exchange.toHtml(rss.getString("customer_bank"))%>"></td>
 </tr>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","银行账号")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="customer_account" style="width:49%" value="<%=exchange.toHtml(rss.getString("customer_account"))%>"></td>
 <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","网址")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="customer_web" style="width:49%" value="<%=exchange.toHtml(rss.getString("customer_web"))%>"></td>
 
 </tr>

<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","电话1")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="customer_tel1" style="width:49%" value="<%=exchange.toHtml(rss.getString("customer_tel1"))%>"></td>
 <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","电话2")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="customer_tel2" style="width:49%" value="<%=exchange.toHtml(rss.getString("customer_tel2"))%>"></td>
 </tr>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","传真")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="customer_fax" style="width:49%" value="<%=exchange.toHtml(rss.getString("customer_fax"))%>"></td>
 <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","邮政编码")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="customer_postcode" style="width:49%" value="<%=exchange.toHtml(rss.getString("customer_postcode"))%>"></td>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" colspan="3" width="13%"><a href="javascript:winopenm('query_attachment.jsp?id=<%=rss.getString("id")%>&tablename=crm_file&fieldname=attachment_name')">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="sales_ID" style="width:49%" value="<%=rss.getString("sales_ID")%>"></td>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="checker" style="width:49%" value="<%=exchange.toHtml(checker)%>"></td>
