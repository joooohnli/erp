...
String sql="select count(id) from crm_config_workflow where type_id='01'";
...
nseer_grid.column_width=[180,200,70,70,70,
<%
for(int i=1;i<workflow_amount;i++){
%>					   70,
<%
...
sql1="select id from crm_workflow where object_ID='"+rs.getString("customer_ID")+"' and check_tag='0'";
ResultSet rs2=crm_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from crm_workflow where object_ID='"+rs.getString("customer_ID")+"' order by id";
	rs2=crm_db1.executeQuery(sql1);
%>
['<%=rs.getString("customer_ID")%>','<%=exchange.toHtml(rs.getString("customer_name"))%>','<%=exchange.toHtml(rs.getString("customer_class"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("sales_name"))%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
