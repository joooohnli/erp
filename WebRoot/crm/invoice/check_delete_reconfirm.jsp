...
<form class="x-form" method="post" action="../../crm_invoice_check_delete_ok">  
...
<input type="hidden" name="which_time" value="<%=which_time%>">
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check.jsp?order_ID=<%=order_ID%>&config_id=<%=config_id%>&which_time=<%=which_time%>"></div></td>
 </tr>
 </table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<%
int i=1;
String sql6="select config_id from crm_workflow where object_ID='"+order_ID+"' and config_id<'"+config_id+"' and which_time='"+which_time+"' order by id";
