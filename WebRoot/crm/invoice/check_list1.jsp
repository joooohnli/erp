...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?order_ID=<%=rs.getString("order_ID")%>&config_id=<%=rs2.getString("config_id")%>&which_time=<%=rs2.getString("which_time")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
