...
String sql9="select * from crm_workflow where object_ID='"+order_ID+"' and check_tag='0' and which_time='0' and config_id<'"+config_id+"'";
ResultSet rs9=crmdb.executeQuery(sql9);
if(!rs9.next()){
if(vt.validata((String)session.getAttribute("unit_db_name"),"crm_order","order_ID",order_ID,"check_tag").equals("0")){
try{
...
form.action = "check_delete_reconfirm.jsp?config_id=<%=config_id%>&order_ID=<%=rs.getString("order_ID")%>";
}else{
form.action = "../../crm_order_check_ok?config_id=<%=config_id%>&order_ID=<%=rs.getString("order_ID")%>";
}
}
</script>
...
<form id="mutiValidation" method="POST" onSubmit="return doValidate('../../xml/crm/crm_order.xml','mutiValidation')&&TwoSubmit(this)">
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=demo.getLang("erp","该记录已审核，请返回")%></td>
