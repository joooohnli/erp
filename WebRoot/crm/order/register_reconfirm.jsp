...
<form id="register2" method="POST" action="../../crm_order_register_ok">
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
	 <tr <%=TR_STYLE1%> class="TR_STYLE1">
	 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDraft("'register2','../../crm_order_register_draft_ok'",request)%>&nbsp;<input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","确认")%>">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="register.jsp?customer_ID=<%=customer_ID%>&&customer_name=<%=toUtf8String.utf8String(exchange.toURL(customer_name))%>&&sales_name=<%=toUtf8String.utf8String(exchange.toURL(sales_name))%>&&sales_ID=<%=sales_ID%>">&nbsp;<input type="reset" <%=RESET_STYLE1%> class="RESET_STYLE1" value="<%=demo.getLang("erp","清除")%>"></div></td>
	 </tr>
</table>
