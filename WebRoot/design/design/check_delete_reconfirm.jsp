...
<form class="x-form" method="post" action="../../design_design_check_delete_ok">
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check.jsp?design_ID=<%=design_ID%>&config_id=<%=config_id%>"></div></td>
 </tr>
 </table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","该物料组成设计单审核未通过，您确认吗？请选择处理：")%></td>
 </tr>
 </table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
<%
int i=1;
String sql6="select config_id from design_workflow where type_id='03' and object_ID='"+design_ID+"' and config_id<'"+config_id+"' order by id";
