...
%>
<%
	nseer_db design_db1 = new nseer_db((String) session
			.getAttribute("unit_db_name"));
%>
...
<%
	...
	String rowSummary = demo.getLang("erp", "当前等待审核的产品档案总数：");
%>
...
<%
	try {
		...
		String sql = "select count(id) from design_config_workflow where type_id='01'";
		ResultSet rs = design_db.executeQuery(sql);
		if (rs.next()) {
			workflow_amount = rs.getInt("count(id)");
		}
		//sql_search ?? added by john
...
nseer_grid.column_width=[180,200,70,140,180,
<%for (int i = 1; i < workflow_amount; i++) {%>					   70,
<%}%> 
                           70
	];
...
<%String sql1 = "select id from design_workflow where object_ID='"
							+ rs.getString("product_ID")
							+ "' and check_tag='0' and type_id='01'";
					ResultSet rs2 = design_db1.executeQuery(sql1);
					if (rs2.next()) {
						sql1 = "select check_tag,describe1,config_id from design_workflow where type_id='01' and object_ID='"
								+ rs.getString("product_ID") + "' order by id";
						rs2 = design_db1.executeQuery(sql1);%>

['<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs
										.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs
								.getString("responsible_person_name"))%>'
<%for (int i = 1; i <= workflow_amount; i++) {
							String status = "";
							if (rs2.next()) {
								if (rs2.getString("check_tag").equals("0")) {
									status = "无权限";
								} else if (rs2.getString("check_tag").equals(
										"1")) {
									status = "通过";
								} else if (rs2.getString("check_tag").equals(
										"9")) {
								...
								if (rs2.getString("check_tag").equals("0")
										&& rs2.getString("describe1").indexOf(
												register_ID) != -1) {%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?product_ID=<%=rs.getString("product_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp", "审核")%></div>'<%} else {%>,'<%=demo.getLang("erp", status)%>'<%}
							} else {%>,''<%}
						}%>],
<%}%>
</page:pages>
