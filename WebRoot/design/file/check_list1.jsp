...
nseer_grid.auto='<%=demo.getLang("erp","产品编号")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>"> 
<%
String sql1="select id from design_workflow where object_ID='"+rs.getString("product_ID")+"' and check_tag='0' and type_id='01'";
ResultSet rs2=design_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from design_workflow where type_id='01' and object_ID='"+rs.getString("product_ID")+"' order by id";
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?product_ID=<%=rs.getString("product_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
