...
%>
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../file/query.jsp?product_ID=<%=rs1.getString("product_ID")%>")><span style="color:<%=color%>"><%=rs1.getString("product_ID")%></span></div>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("product_name"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("type"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("chain_name"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("responsible_person_name"))%></span>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("register.jsp?product_ID=<%=rs1.getString("product_ID")%>")><span style="color:<%=color%>"><%=demo.getLang("erp","价格调整")%></span></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
