...
<form id="mutiValidation" class="x-form" method="post" action="../../design_price_change_register_ok?product_ID=<%=product_ID%>" onSubmit="return doValidate('../../xml/design/design_file.xml','mutiValidation')">
<div id="nseerGround" class="nseerGround">
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value=<%=demo.getLang("erp","确认")%>>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value=<%=demo.getLang("erp","返回")%> onClick=location="register.jsp?product_ID=<%=product_ID%>"></div></td>
 </tr>
 </table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
try{
	...
	String product_describe=exchange.unHtml(request.getParameter("product_describe"));
...
%>
	<td <%=TD_STYLE3%> class="TD_STYLE3"><font color="red"><%=demo.getLang("erp","成本单价与参考成本单价的差额超出报警比例，您确认提交吗？")%>
<%
		 }else{	
%>
...
<td <%=TD_STYLE21%> class="TD_STYLE2" width="18%"><%=exchange.toHtml(rss.getString("chain_id"))%> <%=exchange.toHtml(rss.getString("chain_name"))%>&nbsp;</td>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input name="product_nick" type="hidden" value="<%=exchange.toHtml(rss.getString("product_nick"))%>"><%=exchange.toHtml(rss.getString("product_nick"))%>&nbsp;</td>
</tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input name="select4" type="hidden" value="<%=rss.getString("responsible_person_ID")%>/<%=exchange.toHtml(rss.getString("responsible_person_name"))%>"><%=rss.getString("responsible_person_ID")%>/<%=exchange.toHtml(rss.getString("responsible_person_name"))%>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="changer_ID" value="<%=changer_ID%>" style="width:49%"></td>
