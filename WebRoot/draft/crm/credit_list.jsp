...
String rowSummary=demo.getLang("erp","符合条件的赊货单总数");
int k=1;
%>
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("pay_ID")%>','<%=exchange.toHtml(rs.getString("payer_name"))%>','<%=exchange.toHtml(rs.getString("sales_name"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=exchange.toHtml(demand_return_time)%>','<%=demo.qformat(rs.getDouble("demand_amount"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("list_price_sum"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("credit.jsp?pay_ID=<%=rs.getString("pay_ID")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("credit.jsp?pay_ID=<%=rs.getString("pay_ID")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%
}
k++;
%>
