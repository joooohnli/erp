...
if(vt.validata((String)session.getAttribute("unit_db_name"),"crm_file","customer_ID",customer_ID,"check_tag").equals("5")||vt.validata((String)session.getAttribute("unit_db_name"),"crm_file","customer_ID",customer_ID,"check_tag").equals("9")){
try{
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_crm_file_draft_ok','../../xml/crm/crm_file.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_crm_file_check_ok','../../xml/crm/crm_file.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></div>
 </td>
 </tr>
</table>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="39%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" id="fileKind_chain" name="fileKind_chain" style="width:80%" onFocus="showKind('nseer1',this,showTree)" onkeyup="search_suggest(this.id)" value="<%=rss.getString("chain_id")%> <%=exchange.toHtml(rss.getString("chain_name"))%>"><input id="fileKind_chain_table" type="hidden" value="crm_config_file_kind"> </td>
 ...
 <select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="type" style="width:49%">
  <%
  String sql4 = "select type_name from crm_config_public_char where kind='客户类型' order by type_ID" ;
...
%>
 ...
 <select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="class" style="width:49%">
  <%
  String sql5 = "select type_name from crm_config_public_char where kind='客户级别' order by type_ID" ;
...
%>
 ...
 <select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="gather_sum_limit" style="width:49%">
  <%
  String sql6 = "select type_value from crm_config_public_double where kind='欠款' order by type_ID" ;
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="gather_period_limit" style="width:49%">
  <%
  String sql7 = "select type_value from crm_config_public_double where kind='回款' order by type_ID" ;
	 ResultSet rs7 = crm_db.executeQuery(sql7) ;
while(rs7.next()){
	if(rs7.getDouble("type_value")==rss.getDouble("gather_period_limit")){	
%>
		<option value="<%=exchange.toHtml(rs7.getString("type_value"))%>" selected><%=demo.qformat(rs7.getDouble("type_value"))%></option>
<%}else{%>
		<option value="<%=exchange.toHtml(rs7.getString("type_value"))%>"><%=demo.qformat(rs7.getDouble("type_value"))%></option>
<%
}
}
%>
 ...
 <select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="contact_period_limit" style="width:49%">
  <%
  String sql8 = "select type_value from crm_config_public_double where kind='联络' order by type_ID" ;
	 ResultSet rs8 = crm_db.executeQuery(sql8) ;
while(rs8.next()){
		if(rs8.getDouble("type_value")==rss.getDouble("contact_period_limit")){
	%>
		<option value="<%=exchange.toHtml(rs8.getString("type_value"))%>" selected><%=demo.qformat(rs8.getDouble("type_value"))%></option>
<%}else{%>
		<option value="<%=exchange.toHtml(rs8.getString("type_value"))%>"><%=demo.qformat(rs8.getDouble("type_value"))%></option>
<%
}
}
%>
...
<input name="register_time" type="hidden" width="100%" value="<%=exchange.toHtml(register_time)%>">
 </table>
