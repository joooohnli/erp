...
if(rs8.next()){
try{
String sql="select * from crm_ordering where reasonexact='"+reasonexact+"' and which_time='"+which_time+"'";
...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_crm_invoice_draft_ok??reasonexact="+reasonexact+"&which_time="+rs.getString("which_time")+"','../../xml/crm/crm_ordering.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_crm_invoice_check_ok?reasonexact=<%=reasonexact%>&which_time=<%=rs.getString("which_time")%>','../../xml/crm/crm_ordering.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back()"></div></td>
 </tr>
 </table>
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input name="register_time" type="hidden" value="<%=exchange.toHtml(time)%>"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="real_invoice_time" onFocus="" id="date_start" value="<%=rs.getString("real_invoice_time")%>"></td>
 </tr>
 </table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
