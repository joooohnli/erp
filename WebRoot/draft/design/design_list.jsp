...
String nickName="产品物料设计";
String fileName="design_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的产品物料设计总数");
int k=1;
%>
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("design_ID")%>','<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("designer"))%>','<%=exchange.toHtml(register_time)%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("cost_price_sum"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("design.jsp?design_ID=<%=rs.getString("design_ID")%>&&id=<%=rs.getString("id")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("design.jsp?design_ID=<%=rs.getString("design_ID")%>&&id=<%=rs.getString("id")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%
}
k++;
%>
