...
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("responsible_person_name"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("file.jsp?product_ID=<%=rs.getString("product_ID")%>")><%=demo.getLang("erp","重新登记")%></div>'],
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("file.jsp?product_ID=<%=rs.getString("product_ID")%>")><%=demo.getLang("erp","继续登记")%></div>'],
<%
}
k++;
%>
