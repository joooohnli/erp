...
String nickName="反馈信息发布管理";
String fileName="comment_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的反馈信息发布草稿总数");
int k=1;
%>
...
nseer_grid.auto='<%=demo.getLang("erp","地址")%>';
nseer_grid.data = [                     
                       
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">

['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../ecommerce/comment/query.jsp?comment_ID=<%=rs.getString("comment_ID")%>")><%=rs.getString("comment_ID")%></div>','<%=exchange.toHtml(rs.getString("name"))%>','<%=exchange.toHtml(rs.getString("company"))%>','<%=exchange.toHtml(rs.getString("address"))%>','<%=exchange.toHtml(rs.getString("tel"))%>',

<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("comment.jsp?comment_ID=<%=rs.getString("comment_ID")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("comment.jsp?comment_ID=<%=rs.getString("comment_ID")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
