...
String nickName="友情链接";
String fileName="link_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的友情链接草稿总数");
int k=1;
%>
...
nseer_grid.auto='<%=demo.getLang("erp","链接图片")%>';
nseer_grid.data = [       
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../ecommerce/website/link/query.jsp?link_ID=<%=rs.getString("link_ID")%>")><%=rs.getString("link_ID")%></div>','<%=rs.getString("topic")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("<%=url%>")><%=exchange.toHtml(rs.getString("url"))%></div>',
<%
if(rs.getString("attachment1").equals("")){	
%>'',
	<%}else{%>'<img src="../../ecommerce/file_attachments/<%=rs.getString("attachment1")%>" width="50" height="50" hspace="1">',
<%}%>
<%if(rs.getString("check_tag").equals("9")){%>
	'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("link.jsp?link_ID=<%=rs.getString("link_ID")%>")><%=demo.getLang("erp","重新登记")%></div>'
	<%}else{%>
		'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("link.jsp?link_ID=<%=rs.getString("link_ID")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
