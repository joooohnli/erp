...
String nickName="新闻发布";
String fileName="news_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的新闻发布草稿总数");
int k=1;
%>
...
nseer_grid.auto='<%=demo.getLang("erp","新闻分类")%>';
nseer_grid.data = [    

<page:pages rs="<%=rs%>" strPage="<%=strPage%>">

['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../ecommerce/news/query.jsp?news_ID=<%=rs.getString("news_ID")%>")><%=rs.getString("news_ID")%></div>','<%=exchange.toHtml(rs.getString("topic"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("news.jsp?news_ID=<%=rs.getString("news_ID")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("news.jsp?news_ID=<%=rs.getString("news_ID")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
