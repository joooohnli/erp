...
if(vt.validata((String)session.getAttribute("unit_db_name"),"ecommerce_other","other_ID",other_ID,"check_tag").equals("5")||vt.validata((String)session.getAttribute("unit_db_name"),"ecommerce_other","other_ID",other_ID,"check_tag").equals("9")){
try{
	String sqll = "select * from ecommerce_other where other_ID='"+other_ID+"'" ;
...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 ...
 <div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_ecommerce_other_draft_ok?other_ID="+rss.getString("other_ID")+"&"+Globals.TOKEN_KEY+"="+session.getAttribute(Globals.TOKEN_KEY)+"','../../xml/ecommerce/ecommerce_other.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_ecommerce_other_check_ok?other_ID=<%=rss.getString("other_ID")%>&<%=Globals.TOKEN_KEY%>=<%=session.getAttribute(Globals.TOKEN_KEY)%>','../../xml/ecommerce/ecommerce_other.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();" /></div>
 </td>
 </tr>
</table>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 	<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="11%"><%=demo.getLang("erp","信息标题")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="other_ID" type="hidden" value="<%=rss.getString("other_ID")%>"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="topic" value="<%=exchange.toHtml(rss.getString("topic"))%>">&nbsp;</td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="11%"><%=demo.getLang("erp","信息分类")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input id="filekind_chain_table" type="hidden" value="ecommerce_config_other_kind"><input type="text" id="filekind_chain" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="filekind_chain" onFocus="showKind('nseer1',this,showTree)" onkeyup="search_suggest(this.id)" autocomplete="off" value="<%=rss.getString("chain_id")+" "+exchange.toHtml(rss.getString("chain_name"))%>"></td>
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<%=iisPrint.hasOrNot7(rss.getString("attachment1"),"附件1","1",response,rss.getString("id"),FILE_STYLE1,"ecommerce_other")%>
<%=iisPrint.hasOrNot7(rss.getString("attachment2"),"附件2","2",response,rss.getString("id"),FILE_STYLE1,"ecommerce_other")%>
<%=iisPrint.hasOrNot7(rss.getString("attachment3"),"附件3","3",response,rss.getString("id"),FILE_STYLE1,"ecommerce_other")%>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="other_list.jsp"></div></td>
 </tr>
</table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
