...
String sql8="select id from fund_fund where fund_ID='"+fund_ID+"' and (check_tag='9' or check_tag='5') and pay_time='"+pay_time+"'";
	ResultSet rs8=fund_db1.executeQuery(sql8);
	if(rs8.next()){
String register_ID="";
String fund_chain_ID="";
try{
String sql3="select file_ID from fund_config_fund_kind where responsible_person_ID like '%"+register_ID+"%'";
...
String sql="select * from fund_fund where fund_ID='"+fund_ID+"' and pay_time='"+pay_time+"'";
...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_fund_pay_draft_ok?pay_time="+rs.getString("pay_time")+"','../../xml/fund/fund_fund.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_fund_pay_check_ok?fund_ID=<%=rs.getString("fund_ID")%>&pay_time=<%=rs.getString("pay_time")%>','../../xml/fund/fund_fund.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回" )%>" onClick="history.back()"></div></td> 
 </tr>
</table>
...
<td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","金额")%>&nbsp;&nbsp;&nbsp;&nbsp;：</td>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="register" type="text" value="<%=checker%>"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="register_ID" type="hidden" value="<%=checker_ID%>"></td>
