...
String condition="(check_tag='5' or check_tag='9') and reason='付款'";
...
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("pay.jsp?fund_ID=<%=rs.getString("fund_ID")%>&pay_time=<%=rs.getString("pay_time")%>")><%=demo.getLang("erp","重新登记")%></div>'],
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("pay.jsp?fund_ID=<%=rs.getString("fund_ID")%>&pay_time=<%=rs.getString("pay_time")%>")><%=demo.getLang("erp","继续登记")%></div>'],
<%}%>
<%
k++;
%>
