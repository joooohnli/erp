...
String rowSummary=demo.getLang("erp","符合条件的人力资源档案草稿总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,100,100,100,100,70];
nseer_grid.auto='<%=demo.getLang("erp","机构")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">

['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("human_ID")%>','<%=exchange.toHtml(rs.getString("human_name"))%>','<%=exchange.toHtml(rs.getString("sex"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("human_title_class"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("file.jsp?human_id=<%=rs.getString("human_id")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("file.jsp?human_id=<%=rs.getString("human_id")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
