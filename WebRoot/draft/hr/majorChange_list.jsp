...
String rowSummary=demo.getLang("erp","符合条件的调动草稿总数");
int k=1;
%>
...
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("human_ID")%>','<%=exchange.toHtml(rs.getString("human_name"))%>','<%=exchange.toHtml(rs.getString("new_chain_name"))%>','<%=exchange.toHtml(rs.getString("new_human_major_second_kind_name"))%>',

<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("majorChange.jsp?human_ID=<%=rs.getString("human_ID")%>&&major_time=<%=rs.getString("major_time")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("majorChange.jsp?human_ID=<%=rs.getString("human_ID")%>&&major_time=<%=rs.getString("major_time")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
