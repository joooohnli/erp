...
<%
 ...
 String sql = "select * from hr_file where check_tag='1' and gar_tag='0' and salary_standard_id!='未定义' and salary_tag='1' order by id";
try{
...
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
 ...
 <div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_hr_salary_draft_ok','../../xml/hr/hr_salary.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_hr_salary_check_ok','../../xml/hr/hr_salary.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();" /></div> 
 </td>
 </tr>
 </table>
<%
...
nseer_grid.column_width=[150,60,60,<%rs1.first();for(int i=0;i<k;i++){%>60,<%rs1.next();}%>100,100,100];
...
if(rs2.next()){%>'<%=demo.aformat(rs2.getDouble("salary"))%>',<%}else{%>'',<%}rs1.next();}
...
if(rs3.next()){
%>'<input type="text" name="bonus_sum" value="<%=demo.aformat(rs3.getDouble("bonus_sum"))%>">','<input type="text" name="sale_bonus_sum" value="<%=demo.aformat(rs3.getDouble("sale_bonus_sum"))%>">','<input type="text" name="deduct_sum" value="<%=demo.aformat(rs3.getDouble("deduct_sum"))%>">'<%}else{%>'<input type="text" name="bonus_sum">','<input type="text" name="sale_bonus_sum">','<input type="text" name="deduct_sum">'<%}%>],
<%}%>
['']];
/*************************************************************/
nseer_grid.init();
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="salary_list.jsp"></div></td>
 </tr>
</table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
