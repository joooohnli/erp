...
String rowSummary=demo.getLang("erp","符合条件的培训草稿总数");
int k=1;
%>
...
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("training.jsp?human_ID=<%=rs.getString("human_ID")%>&&training_time=<%=rs.getString("training_time")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("training.jsp?human_ID=<%=rs.getString("human_ID")%>&&training_time=<%=rs.getString("training_time")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
