...
String nickName="委外报价单";
String fileName="discussion_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的委外报价单总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,200,100,100,100,70];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("discussion_ID")%>','<%=exchange.toHtml(rs.getString("provider_name"))%>','<%=exchange.toHtml(rs.getString("demand_contact_person_tel"))%>','<%=exchange.toHtml(demand_pay_time)%>','<%=demo.aformat(rs.getDouble("sale_price_sum"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("discussion.jsp?discussion_ID=<%=rs.getString("discussion_ID")%>&&id=<%=rs.getString("id")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("discussion.jsp?discussion_ID=<%=rs.getString("discussion_ID")%>&&id=<%=rs.getString("id")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
