...
if(vt.validata((String)session.getAttribute("unit_db_name"),"intrmanufacture_file","provider_ID",provider_ID,"check_tag").equals("9")||vt.validata((String)session.getAttribute("unit_db_name"),"intrmanufacture_file","provider_ID",provider_ID,"check_tag").equals("5")){
try{
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_intrmanufacture_file_draft_ok','../../xml/intrmanufacture/intrmanufacture_file.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_intrmanufacture_file_check_ok?provider_ID=<%=rss.getString("provider_ID")%>','../../xml/intrmanufacture/intrmanufacture_file.xml');">&nbsp;
...
<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input id="kind_chain" <%=INPUT_STYLE1%> class="INPUT_STYLE1" style="width:80%" name="kind_chain" onFocus="showKind('nseer1',this,showTree)" onkeyup="search_suggest(this.id)" autocomplete="off" value="<%=rss.getString("chain_id")%> <%=exchange.toHtml(rss.getString("chain_name"))%>"><input id="kind_chain_table" type="hidden" value="design_config_file_kind"></td>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input name="register_time" type="hidden" value="<%=exchange.toHtml(register_time)%>"><%=exchange.toHtml(register_time)%></td>
 </tr>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
