...
<form id="mutiValidation"  method="POST">
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_intrmanufacture_intrmanufacture_draft_ok?intrmanufacture_ID="+intrmanufacture_ID+"','../../xml/intrmanufacture/intrmanufacture_intrmanufacture.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_intrmanufacture_intrmanufacture_check_ok?intrmanufacture_ID=<%=intrmanufacture_ID%>','../../xml/intrmanufacture/intrmanufacture_intrmanufacture.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="intrmanufacture_list.jsp"></div></td> 
 </tr>
 </table>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4" id=theObjTable>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="product_name" type="hidden" value="<%=exchange.toHtml(rs.getString("product_name"))%>"><%=exchange.toHtml(rs.getString("product_name"))%></td>
	...
	<tr <%=TR_STYLE1%> class="TR_STYLE1">	 
	 ...
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="89%" colspan="3"><input name="balance_amount" type="hidden" value="<%=getAmount.balanceAmount((String)session.getAttribute("unit_db_name"),product_ID)%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(getAmount.balanceAmount((String)session.getAttribute("unit_db_name"),product_ID))%></td>	 
	</tr>
</table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
	<td <%=TD_STYLE2%> class="TD_STYLE2"><input name="provider_name" type="hidden" value="<%=exchange.toHtml(rs6.getString("provider_name"))%>"><%=exchange.toHtml(rs6.getString("provider_name"))%>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input name="provider_ID" type="hidden" value="<%=rs6.getString("provider_ID")%>"><a href="javascript:winopen('../../intrmanufacture/file/query.jsp?provider_ID=<%=rs6.getString("provider_ID")%>')"><%=rs6.getString("provider_ID")%></a>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" name="demand_amount" value="<%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs6.getDouble("demand_amount"))%>"></td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" name="demand_price" value="<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("demand_price"))%>"></td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2">&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" name="demand_gather_time1" id="date_start<%=i%>" value="<%=rs6.getString("demand_gather_time")%>"></td>

	<td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" name="contact_person1" value="<%=exchange.toHtml(rs6.getString("demand_contact_person"))%>"></td>
	<td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" name="provider_tel1" value="<%=exchange.toHtml(rs6.getString("demand_contact_person_tel"))%>"></td>
 </tr>
<%i++;}
%>
...
<div id="nseerGround" class="nseerGround">
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="intrmanufacture_list.jsp"></div></td> 
 </tr>
 </table>
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
