...
String nickName="配送单位推荐";
String fileName="productProviders_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的配送单位推荐草稿总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,180,180,100,160,70];
nseer_grid.auto='<%=demo.getLang("erp","产品分类")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("product_logistics_recommend_ID")%>','<%=exchange.toHtml(rs.getString("product_ID"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("productProviders.jsp?product_logistics_recommend_ID=<%=rs.getString("product_logistics_recommend_ID")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("productProviders.jsp?product_logistics_recommend_ID=<%=rs.getString("product_logistics_recommend_ID")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
