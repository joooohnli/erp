...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8"><%=DgButton.getDsend("'register1','../../draft_manufacture_designModule_draft_ok?design_ID="+rs.getString("design_ID")+"','../../xml/manufacture/manufacture_design_procedure.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('register1','../../draft_manufacture_designModule_check_ok?design_ID=<%=rs.getString("design_ID")%>','../../xml/manufacture/manufacture_design_procedure.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="designModule_list.jsp"></td> 
 </tr>
 </table>
...
<table <%=TABLE_STYLE5%> class="TABLE_STYLE5" id=theObjTable>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%"><%=demo.getLang("erp","工时成本小计(元)")%></td>
	 ...
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%"><%=demo.getLang("erp","设计")%></td>
 </tr>
<%
int i=1;
...
while(rs6.next()){
	if(rs6.getString("design_module_change_tag").equals("1")){
%>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("module_subtotal"))%>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="designModule_module_again_reconfirm.jsp?design_ID=<%=rs.getString("design_ID")%>&&procedure_name=<%=toUtf8String.utf8String(exchange.toURL(rs6.getString("procedure_name")))%>&&product_ID=<%=rs.getString("product_ID")%>"><font color="forestgreen"><%=demo.getLang("erp","重新设计")%></a>&nbsp;</td>
 </tr>
<%}else{%>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=i%></td>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("subtotal"))%>&nbsp;</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("module_subtotal"))%>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="designModule_module_list.jsp?design_ID=<%=rs.getString("design_ID")%>&&procedure_name=<%=toUtf8String.utf8String(exchange.toURL(rs6.getString("procedure_name")))%>&&product_ID=<%=rs.getString("product_ID")%>"><%=demo.getLang("erp","设计")%></a>&nbsp;</td>
 </tr>
<%
}
	i++;
