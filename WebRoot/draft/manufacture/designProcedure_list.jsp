...
String condition="(check_tag='5' or check_tag='9') and excel_tag='2'";
...
String nickName="工序设计单";
String fileName="designProcedure_list.jsp";
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("design_ID")%>','<%=exchange.toHtml(rs.getString("product_ID"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=rs.getString("designer")%>','<%=exchange.toHtml(rs.getString("register_time"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<%=demo.aformat(rs.getDouble("cost_price_sum"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("designProcedure.jsp?design_ID=<%=rs.getString("design_ID")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<%=demo.aformat(rs.getDouble("cost_price_sum"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("designProcedure.jsp?design_ID=<%=rs.getString("design_ID")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
