...
//*********************************************
function addGoodsItem(product_IDa,product_namea,product_describea,amounta,amount_unita,cost_pricea) {
...
+ createAddQueryString(product_ID,product_name,product_describe,amount,amount_unit,cost_price, "add") 
...
//*********************************************
function createAddQueryString( product_ID,product_name, product_describe,amount,amount_unit,cost_price, action) {
...
deleteButton3.setAttribute("type", "text");

deleteButton3.setAttribute("name", "product_name");

deleteButton3.setAttribute("value", product_name);

deleteButton3.setAttribute("onFocus","this.blur()");

deleteButton3.setAttribute("class","INPUT_STYLE4");
...
deleteButton4.setAttribute("onFocus","this.blur()");
...
deleteButton6.setAttribute("onFocus","this.blur()");

deleteButton6.setAttribute("class","INPUT_STYLE4");
...
deleteButton7.setAttribute("onFocus","this.blur()");

deleteButton7.setAttribute("class","INPUT_STYLE4");
...
String condition="check_tag='1' and type!='商品' and type!='服务型产品'";
String queue="order by register_time";
String validationXml="../../xml/design/design_file.xml";
String nickName="产品档案";
String fileName="moduleBalance_module_list.jsp";
...
nseer_grid.auto='<%=demo.getLang("erp","物料名称")%>';
nseer_grid.data = [
...
<%if(rs1.getString("price_alarm_tag").equals("0")){%>
	'<div style="text-decoration : underline;color:#3366FF" onclick=addGoodsItem("<%=rs1.getString("product_ID")%>","<%=exchange.unHtmls(exchange.toHtml(rs1.getString("product_name")))%>","<%=exchange.unHtmls(rs1.getString("product_describe"))%>","1","<%=exchange.unHtmls(exchange.toHtml(rs1.getString("amount_unit")))%>","<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("cost_price"))%>")><%=demo.getLang("erp","添加")%></div>'
<%}else{%>
...
var names=['product_ID','product_name','product_describe1','product_describe','amount','amount_unit','cost_price'];
function addGoodsItem(product_IDa,product_namea,product_describea,amounta,amount_unita,cost_pricea) {

var product_name=product_namea.replace(/★/g,"<br>").replace(/☆/g," ");
var product_describe=product_describea.replace(/★/g,"<br>").replace(/☆/g," ");
var amount_unit=amount_unita.replace(/★/g,"<br>").replace(/☆/g," ");

var values=[product_IDa,product_name,product_describe,product_describe,amounta,amount_unit,cost_pricea];
