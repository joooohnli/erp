...
String nickName="知识管理";
String fileName="knowledge_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的知识草稿总数");
int k=1;
%>
...
nseer_grid.columns =[
                       ...
                       {name: '<%=demo.getLang("erp","知识类型")%>'},
                       {name: '<%=demo.getLang("erp","机构")%>'},				   
					   {name: '<%=demo.getLang("erp","收录")%>'}
	]
nseer_grid.column_width=[40,200,180,180,70];
nseer_grid.auto='<%=demo.getLang("erp","机构")%>';
nseer_grid.data = [    				   					 
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=exchange.toHtml(rs.getString("subject"))%>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("knowledge.jsp?knowledge_ID=<%=rs.getString("knowledge_ID")%>")><%=demo.getLang("erp","重新收录")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("knowledge.jsp?knowledge_ID=<%=rs.getString("knowledge_ID")%>")><%=demo.getLang("erp","继续收录")%></div>'
<%}%>
],
<%
k++;
