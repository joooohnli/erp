...
String sql="select * from oa_publicize where publicize_ID='"+publicize_ID+"' and (check_tag='5' or check_tag='9')";
...
%>
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_oa_publicize_draft_ok?publicize_ID="+publicize_ID+"&"+Globals.TOKEN_KEY+"="+session.getAttribute(Globals.TOKEN_KEY)+"','../../xml/oa/oa_publicize.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_oa_publicize_check_ok?publicize_ID=<%=publicize_ID%>&<%=Globals.TOKEN_KEY%>=<%=session.getAttribute(Globals.TOKEN_KEY)%>','../../xml/oa/oa_publicize.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();" /></div></td>
 </tr>
 </table>
...
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
 ...
 <div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="publicize_list.jsp"></div>
 </td>
 </tr>
</table>
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
