...
String rowSummary=demo.getLang("erp","符合条件的宣传草稿总数");
int k=1;
%>
...
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("publicize.jsp?publicize_ID=<%=rs.getString("publicize_ID")%>")><%=demo.getLang("erp","重新制定")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("publicize.jsp?publicize_ID=<%=rs.getString("publicize_ID")%>")><%=demo.getLang("erp","继续制定")%></div>'
<%}%>
],
<%
k++;
