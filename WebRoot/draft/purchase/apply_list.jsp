...
String nickName="采购计划单";
String fileName="apply_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的采购计划单总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,100,100,160,100,100];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("apply_ID")%>','<%=exchange.toHtml(rs.getString("designer"))%>','<%=exchange.toHtml(rs.getString("register"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=rs.getString("remark")%>',
