...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("discussion_ID")%>','<%=exchange.toHtml(rs.getString("provider_name"))%>','<%=exchange.toHtml(rs.getString("demand_contact_person_tel"))%>','<%=exchange.toHtml(demand_pay_time)%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<%=demo.aformat(rs.getDouble("sale_price_sum"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("discussion.jsp?discussion_ID=<%=rs.getString("discussion_ID")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<%=rs.getString("sale_price_sum")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("discussion.jsp?discussion_ID=<%=rs.getString("discussion_ID")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
