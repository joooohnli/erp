...
if(vt.validata((String)session.getAttribute("unit_db_name"),"purchase_file","provider_ID",provider_ID,"check_tag").equals("5")||vt.validata((String)session.getAttribute("unit_db_name"),"purchase_file","provider_ID",provider_ID,"check_tag").equals("9")){
try{
	String sqll = "select * from purchase_file where provider_ID='"+provider_ID+"'" ;
	ResultSet rss = purchasedb.executeQuery(sqll) ;
	if(rss.next()){
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_purchase_file_draft_ok','../../xml/purchase/purchase_file.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_purchase_file_check_ok?provider_ID=<%=rss.getString("provider_ID")%>','../../xml/purchase/purchase_file.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></div>
 </td>
 </tr>
</table>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" id="fileKind_chain" name="fileKind_chain" style="width:80%" value="<%=exchange.toHtml(rss.getString("chain_id"))%> <%=exchange.toHtml(rss.getString("chain_name"))%>" onFocus="showKind('nseer1',this,showTree)" onkeyup="search_suggest(this.id)" autocomplete="off"><input id="fileKind_chain_table" type="hidden" value="design_config_file_kind"></td>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td colspan="3" <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><a href="javascript:winopenm('query_attachment.jsp?id=<%=rss.getString("id")%>&tablename=purchase_file&fieldname=attachment_name')">
...
<textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="invoice_info"><%=invoice_info%></textarea>
 ...
 <td height="65" <%=TD_STYLE4%> class="TD_STYLE7" width="13%"><%=demo.getLang("erp","可供应产品集合")%> &nbsp; </td>
<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%">
<textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="demand_products"><%=demand_products%></textarea>
</td>
 </tr>
	 <tr <%=TR_STYLE1%> class="TR_STYLE7">
 <td <%=TD_STYLE4%> class="TD_STYLE7" width="13%"><%=demo.getLang("erp","推荐供应产品集合")%> &nbsp; </td>
<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%">
...
<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","采购人")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="purchaser" style="width:45%" value="<%=exchange.toHtml(rss.getString("purchaser"))%>"></td>
	 <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","采购人编号")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="text" <%=INPUT_STYLE1%> class="INPUT_STYLE1" name="purchaser_ID" style="width:45%" value="<%=rss.getString("purchaser_ID")%>"></td>
 </tr>
