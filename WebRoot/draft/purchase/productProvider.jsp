...
if(vt.validata((String)session.getAttribute("unit_db_name"),"purchase_product_providers_recommend","product_providers_recommend_ID",product_providers_recommend_ID,"check_tag").equals("5")||vt.validata((String)session.getAttribute("unit_db_name"),"purchase_product_providers_recommend","product_providers_recommend_ID",product_providers_recommend_ID,"check_tag").equals("9")){
try{
	String sqll = "select * from purchase_product_providers_recommend where product_providers_recommend_ID='"+product_providers_recommend_ID+"'" ;
...
<form id="mutiValidation" method="POST" action="../../purchase_product_providers_uncheck_ok?product_providers_recommend_ID=<%=rs.getString("product_providers_recommend_ID")%>">
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8"><%=DgButton.getDsend("'mutiValidation','../../draft_purchase_productProvider_draft_ok','../../xml/purchase/purchase_product_providers_recommend.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_purchase_productProvider_check_ok?product_providers_recommend_ID=<%=rs.getString("product_providers_recommend_ID")%>','../../xml/purchase/purchase_product_providers_recommend.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onclick="winopen('productProvider_provider_list.jsp')" value="<%=demo.getLang("erp","新添加")%>"> 
  <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onclick="delSelect()" value="<%=demo.getLang("erp","新删除")%>">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onclick="history.back()" value="<%=demo.getLang("erp","返回")%>"></td>
 </tr>
 </table>
...
<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input name="product_providers_recommend_ID" type="hidden" value="<%=rs.getString("product_providers_recommend_ID")%>"><%=rs.getString("product_providers_recommend_ID")%>&nbsp;</td>
...
<thead>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="5%"><%=demo.getLang("erp","所在区域")%></td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="5%"><%=demo.getLang("erp","优质级别")%></td>
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=exchange.toHtml(rs7.getString("provider_tel1"))%>&nbsp;</td>
	 ...
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="productProvider_delete_details.jsp?id=<%=rs6.getString("id")%>&&product_providers_recommend_ID=<%=product_providers_recommend_ID%>"><%=demo.getLang("erp","删除")%></a></td>
<%
}else{
%>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1"> 
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input name="register" type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" value="<%=exchange.toHtml(register)%>" readonly/></td>
...
<textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="recommend_describe" value="<%=recommend_describe%>"><%=recommend_describe%></textarea>
</td>
 </tr>
   </table>
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="productProvider_list.jsp"></div></td>
 </tr>
</table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
