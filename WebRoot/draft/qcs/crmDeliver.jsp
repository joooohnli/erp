...
<script type="text/javascript" src="../../javascript/draft/qcs/crmDeliver.js"></script>
...
String config_id=request.getParameter("config_id");
	String sql = "select id,apply_id,stock_name,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,register,register_time,qcs_result,attachment1,remark from qcs_crm_deliver where qcs_id='"+qcs_id+"' and (check_tag='5' or check_tag='9')";
...
<div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_qcs_crmDeliver_draft_ok','../../xml/qcs/qcs_crm_deliver.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_qcs_crmDeliver_check_ok','../../xml/qcs/qcs_crm_deliver.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();" /></td>
</tr>
</table>
...
<td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","质检方案")%>：</td>
 ...
 <%}else{%>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","抽样标准")%>&nbsp;&nbsp;：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="sampling_standard" name="sampling_standard" type="text" width="37%"  onclick="loadAjaxDiv('sampling_standard','<%=rs.getString("qcs_amount")%>');this.blur();" value="<%=sampling_standard%>" readonly/><div class="select_div_l" id="sampling_standard_div"  onclick="loadAjaxDiv('sampling_standard','<%=rs.getString("qcs_amount")%>')"></td>
 <%}%>
