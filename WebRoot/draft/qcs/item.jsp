...
<link rel="stylesheet" type="text/css" href="../../css/qcs/item/item.css" />
...
<script language="javascript" src="../../javascript/draft/qcs/item.js"></script>
<!--  弹出 计数器层的样式 -->
...
<STYLE type=text/css>  
.result_div1{
position:absolute;background:yellow;height:80px; filter:alpha(opacity=80); overflow-y: auto; overflow-x: hidden;z-index:100
}
.select_div_l{
cursor:hand;padding-top:3px;left:500px;width:15px;position:absolute;background-image:url(/erp/images/selectArrow.gif);
}
.select_div_r{
cursor:hand;padding-top:3px;left:360px;width:15px;position:absolute;background-image:url(/erp/images/selectArrow.gif);
}
 </STYLE>
...
<div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_qcs_item_draft_ok','../../xml/qcs/qcs_item.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" id="QV_button" onclick="showQVDiv('QV_hidden','QV_table2',0,'uncheck',event);" value="质检值" disabled>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_qcs_item_check_ok','../../xml/qcs/qcs_item.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onClick="history.back()" value=<%=demo.getLang("erp","返回")%>></div></td>
</tr>
</table>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="36%"><input class="INPUT_STYLE3" id="analyse_method" name="analyse_method" onclick="loadAjaxDiv('analyse_method','分析方法')" type="text" value="<%=rs.getString("analyse_method")%>" readonly/><div class="select_div_r" id="analyse_method_div" onclick="loadAjaxDiv('analyse_method','分析方法')"></div></td>
</tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
  <td <%=TD_STYLE1%> class="TD_STYLE8" width="14%"><%=demo.getLang("erp","首选质检依据")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="36%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="default_basis" name="default_basis" onclick="loadAjaxDiv('default_basis','质检依据')" type="text" width="100%" value="<%=rs.getString("default_basis")%>"  readonly/><div class="select_div_l" id="default_basis_div"  onclick="loadAjaxDiv('default_basis','质检依据')"></div></td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="14%"><%=demo.getLang("erp","备选质检依据")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="36%"><input class="INPUT_STYLE3" id="ready_basis" name="ready_basis" onclick="loadAjaxDiv('ready_basis','质检依据')" type="text" value="<%=rs.getString("ready_basis")%>"  readonly/><div class="select_div_r" id="ready_basis_div" onclick="loadAjaxDiv('ready_basis','质检依据')"></div></td>
</tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
  <td <%=TD_STYLE1%> class="TD_STYLE8" width="14%"><%=demo.getLang("erp","质检方法")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="36%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="quality_method" name="quality_method" onclick="loadAjaxDiv('quality_method','质检方法')" type="text" width="100%" value="<%=rs.getString("quality_method")%>"  readonly/><div class="select_div_l" id="quality_method_div"  onclick="loadAjaxDiv('quality_method','质检方法')"></div></td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="14%"><%=demo.getLang("erp","质检设备")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="36%"><input class="INPUT_STYLE3" id="quality_equipment" name="quality_equipment" onclick="loadAjaxDiv('quality_equipment','质检设备')" type="text" value="<%=rs.getString("quality_equipment")%>"  readonly/><div class="select_div_r" id="quality_equipment_div"  onclick="loadAjaxDiv('quality_equipment','质检设备')"></div></td>
</tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
  <td <%=TD_STYLE1%> class="TD_STYLE8" width="14%"><%=demo.getLang("erp","抽样标准")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="36%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="sampling_standard" name="sampling_standard" onclick="loadAjaxDiv('sampling_standard','抽样标准')" type="text" width="100%" value="<%=rs.getString("sampling_standard")%>"  readonly/><div class="select_div_l" id="sampling_standard_div"  onclick="loadAjaxDiv('sampling_standard','抽样标准')"></div></td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="14%"><%=demo.getLang("erp","质量问题等级")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="36%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="defect_class" name="defect_class" onclick="loadAjaxDiv('defect_class','质量问题等级')" type="text" width="100%" value="<%=rs.getString("defect_class")%>"  readonly/><div class="select_div_r" id="defect_class_div"  onclick="loadAjaxDiv('defect_class','质量问题等级')"></div></td>
</tr>
<%
String check_tag1="";
String check_tag2="";
...
<td <%=TD_STYLE3%> class="TD_STYLE3" colspan="3" width="89%"></td>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="36%"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="register_time" value="<%=register_time%>" readonly></td>
 </tr>
...
<div id="QV_div" class="nseer_div" nseerDef="dragAble" style="width:440px;height:280px;display:none;position:absolute;top:200px;left:100px" >
...
<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","增加")%>" onClick="addRow('QV_table2',0,event);">&nbsp;&nbsp;    
<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","删除")%>" onClick="deleteSelect('QV_table2');">&nbsp;&nbsp;
<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","确认")%>" name="B1" width="20" onClick="setQV('QV_hidden','QV_table2')">&nbsp;&nbsp;    
<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","退出")%>" name="B3" width="20" onClick="document.getElementById('QV_div').style.display='none';">  
</p>
</td>
</tr>
</table>
