...
String nickName="质检项目草稿";
String fileName="item_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的质检项目草稿总数");
int k=1;
%>
...
nseer_grid.column_width=[40,150,100,100,100,100,100,80,180,70];
nseer_grid.auto='<%=demo.getLang("erp","项目名称")%>';
nseer_grid.data = [

<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../qcs/item/query.jsp?item_id=<%=rs.getString("item_id")%>")><%=rs.getString("item_id")%></div>','<%=exchange.toHtml(rs.getString("item_name"))%>','<%=exchange.toHtml(rs.getString("designer"))%>','<%=exchange.toHtml(rs.getString("analyse_method"))%>','<%=exchange.toHtml(rs.getString("default_basis"))%>','<%=exchange.toHtml(rs.getString("quality_method"))%>','<%=exchange.toHtml(rs.getString("quality_equipment"))%>','<%=exchange.toHtml(rs.getString("sampling_standard"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("item.jsp?item_id=<%=rs.getString("item_id")%>")><%=demo.getLang("erp","重新登记")%></div>'],
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("item.jsp?item_id=<%=rs.getString("item_id")%>")><%=demo.getLang("erp","继续登记")%></div>'],
<%}%>
<%
k++;
%>
