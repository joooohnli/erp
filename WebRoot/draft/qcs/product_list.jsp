...
String rowSummary=demo.getLang("erp","符合条件的产品质检方式草稿总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,180,150,100,160,60];
nseer_grid.auto='<%=demo.getLang("erp","产品名称")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("qcs_id")%>','<%=rs.getString("product_id")%>','<%=rs.getString("product_name")%>','<%=rs.getString("register")%>','<%=rs.getString("register_time")%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("product.jsp?qcs_id=<%=rs.getString("qcs_id")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("product.jsp?qcs_id=<%=rs.getString("qcs_id")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
