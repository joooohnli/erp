...
<script language="javascript" src="../../javascript/draft/qcs/qualityStandard.js"></script>
<!--  弹出 计数器层的样式和js -->
...
<div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_qcs_qualityStandard_draft_ok','../../xml/qcs/qcs_quality_standard.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" id="add_button" onclick="addRow('bill_body');" value="添加">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" id="del_button" onclick="deleteSelect('bill_body')" value=<%=demo.getLang("erp","删除")%>>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_qcs_qualityStandard_check_ok','../../xml/qcs/qcs_quality_standard.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onClick="history.back()" value=<%=demo.getLang("erp","返回")%>></div></td>
</tr>
</table>
...
<%
sql="select item,analyse_method,default_basis,ready_basis,quality_method,standard_value,standard_max,standard_min from qcs_quality_standard_details where standard_id='"+standard_id+"'";
...
<tr <%=TR_STYLE2%> class="TR_STYLE2">
 ...
 <%
 sql="select item_name,analyse_method,default_basis,ready_basis,quality_method from qcs_item order by id";
 rs=qcs_db.executeQuery(sql);
 while(rs.next()){
 %>
 <tr <%=TR_STYLE1%> class="TR_STYLE1" ondblclick="selectIn(this);" bgcolor="#ffffff">
