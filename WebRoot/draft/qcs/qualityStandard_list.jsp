...
String fileName="qualityStandard_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的质量标准草稿总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,200,180,100,100,70];
nseer_grid.auto='<%=demo.getLang("erp","标准名称")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../qcs/quality_standard/query.jsp?standard_id=<%=rs.getString("standard_id")%>")><%=rs.getString("standard_id")%></div>','<%=exchange.toHtml(rs.getString("standard_name"))%>','<%=exchange.toHtml(rs.getString("product_id"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("designer"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("qualityStandard.jsp?standard_id=<%=rs.getString("standard_id")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("qualityStandard.jsp?standard_id=<%=rs.getString("standard_id")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
