...
String nickName="抽样标准草稿";
String fileName="samplingStandard_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的抽样标准草稿总数");
int k=1;
%>
...
nseer_grid.column_width=[40,150,100,100,100,100,100,100,70];
nseer_grid.auto='<%=demo.getLang("erp","标准名称")%>';
nseer_grid.data = [
/*****************************************************************************/                      
  
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../qcs/sampling_standard/query.jsp?standard_id=<%=rs.getString("standard_id")%>")><%=rs.getString("standard_id")%></div>','<%=exchange.toHtml(rs.getString("standard_name"))%>','<%=exchange.toHtml(rs.getString("sampling_method"))%>','<%=exchange.toHtml(rs.getString("designer"))%>','<%=exchange.toHtml(rs.getString("quality_level"))%>','<%=exchange.toHtml(rs.getString("mil_std"))%>','<%=exchange.toHtml(rs.getString("aql"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("samplingStandard.jsp?standard_id=<%=rs.getString("standard_id")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("samplingStandard.jsp?standard_id=<%=rs.getString("standard_id")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
