...
String rowSummary=demo.getLang("erp","符合条件的库存质检草稿总数");
int k=1;
%>
...
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("stock.jsp?qcs_id=<%=rs.getString("qcs_id")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("stock.jsp?qcs_id=<%=rs.getString("qcs_id")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
