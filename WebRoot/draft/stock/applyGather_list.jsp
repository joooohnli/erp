...
String nickName="入库申请";
String fileName="applyGather_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的入库申请总数");
int k=1;
%>
...
nseer_grid.columns =[
                       ...
                       {name: '<%=demo.getLang("erp","退回时间")%>'},
	                   {name: '<%=demo.getLang("erp","总件数")%>'},
                       {name: '<%=demo.getLang("erp","总金额（元）")%>'},
                       {name: '<%=demo.getLang("erp","登记")%>'}
                       ]
nseer_grid.column_width=[40,180,100,160,100,100,100,70];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("gather_ID")%>','<%=exchange.toHtml(rs.getString("reason"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=demand_return_time%>','<%=demo.qformat(rs.getDouble("demand_amount"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("cost_price_sum"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("applyGather.jsp?gather_ID=<%=rs.getString("gather_ID")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("applyGather.jsp?gather_ID=<%=rs.getString("gather_ID")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
