...
try{
	...
	String id_chain="";	
	sql="select subtotal,amount,id from stock_apply_pay_details where pay_ID='"+pay_ID+"' order by details_number";
	...
	sql="update stock_apply_pay set cost_price_sum='"+cost_price_sum+"',demand_amount='"+demand_amount+"' where pay_ID='"+pay_ID+"'";
	stock_db.executeUpdate(sql) ;
    stock_db.close();
}
catch(Exception ex){
...
response.sendRedirect("applyPay.jsp?pay_ID="+pay_ID+"");
}else{
%>
 ...
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2" width="100%">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="applyPay.jsp?pay_ID=<%=pay_ID%>">
 </div>
 </td>
 </tr>
 </table>
