...
String nickName="安全库存配置";
String fileName="cell_list.jsp";
...
nseer_grid.auto='<%=demo.getLang("erp","分类")%>';
nseer_grid.data = [
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("design_ID")%>','<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<%=exchange.toHtml(register_time)%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("cell.jsp?design_ID=<%=rs.getString("design_ID")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<%=exchange.toHtml(register_time)%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("cell.jsp?design_ID=<%=rs.getString("design_ID")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
