...
String stock_ID="";
String sql8="select id from stock_pre_gathering where gather_ID='"+gather_ID+"' and (gather_check_tag='9' or gather_check_tag='5') and gather_tag='0' order by details_number";
...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_stock_gather_draft_ok?gather_ID="+rs.getString("gather_ID")+"&gathering_time="+rs.getString("gathering_time")+"','../../xml/stock/stock_pre_gathering.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_stock_gather_check_ok?gather_ID=<%=rs.getString("gather_ID")%>&gathering_time=<%=rs.getString("gathering_time")%>','../../xml/stock/stock_pre_gathering.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back()"></div></td> 
 </tr>
</table>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="register" value="<%=exchange.toHtml(register)%>" readonly/><input name="register_id" type="hidden" value="<%=register_id%>"></td>
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td align="left"><%=demo.getLang("erp","该记录已提交审核，请返回！")%></td>
 </tr>
</table>
</div>
<%}
