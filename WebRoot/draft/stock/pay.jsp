...
String stock_ID="";
String sql8="select id from stock_pre_paying where pay_ID='"+pay_ID+"' and (pay_check_tag='9' or pay_check_tag='5') and pay_tag='0' order by details_number";
...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_stock_pay_draft_ok?pay_ID="+rs.getString("pay_ID")+"&paying_time="+rs.getString("paying_time")+"','../../xml/stock/stock_pre_paying.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_stock_pay_check_ok?pay_ID=<%=rs.getString("pay_ID")%>&paying_time=<%=rs.getString("paying_time")%>','../../xml/stock/stock_pre_paying.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></div></td>
 </tr>
</table>
...
String sql9="select qualified_amount from stock_pay_details where product_ID='"+rs6.getString("product_ID")+"' and pay_id='"+pay_ID+"'";
ResultSet rs9=stock_db1.executeQuery(sql9);
if(rs9.next()){
...
<%}else{%>
	...
	<td <%=TD_STYLE2%> class="TD_STYLE2"><%=available.qualifiedAmount((String)session.getAttribute("unit_db_name"),rs6.getString("product_ID"),pay_ID)%>
	</td><input type="hidden" id="qualified_amount<%=j%>" value="<%=qualified_amount%>">
<%}%>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="checker" value="<%=exchange.toHtml(checker)%>" readonly/><input name="checker_id" type="hidden" value="<%=checker_id%>"></td>
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="pay_list.jsp"></td>
 </tr>
</table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
