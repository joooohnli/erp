...
if(vt.validata((String)session.getAttribute("unit_db_name"),"stock_transfer_gather","gather_ID",gather_ID,"check_tag").equals("5")||vt.validata((String)session.getAttribute("unit_db_name"),"stock_transfer_gather","gather_ID",gather_ID,"check_tag").equals("9")){
try{
...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDsend("'mutiValidation','../../draft_stock_transferGather_draft_ok','../../xml/stock/stock_transfer_gather.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交审核")%>" onClick="sendOk('mutiValidation','../../draft_stock_transferGather_check_ok?gather_ID=<%=rs.getString("gather_ID")%>','../../xml/stock/stock_transfer_gather.xml');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onclick="winopen('transferGather_product_list.jsp')" value="<%=demo.getLang("erp","添加产品")%>">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onclick="delSelect()" value="<%=demo.getLang("erp","删除产品")%>">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></div></td>
 </tr>
</table>
<!-- ############################################ -->
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input name="gather_ID" type="hidden" value="<%=rs.getString("gather_ID")%>"><%=rs.getString("gather_ID")%>&nbsp;</td>
...
<%
int i=1;
String sql6="select * from stock_transfer_gather_details where gather_ID='"+gather_ID+"'";
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="transferGather_delete_details.jsp?id=<%=rs6.getString("id")%>&&gather_ID=<%=rs.getString("gather_ID")%>"><%=demo.getLang("erp","删除")%></a></td>
 </tr>
<%
	i++;
	}
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="transferGather_list.jsp"></td>
 </tr>
</table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","该记录保存为草稿，请返回")%></td>
 <td <%=TD_STYLE3%> class="TD_STYLE3">&nbsp;</td>
 </tr>
</table>
</div>
