...
String nickName="申请单";
String fileName="transferGather_list.jsp";
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<%=rs.getString("gather_ID")%>','<%=exchange.toHtml(rs.getString("reasonexact"))%>','<%=exchange.toHtml(register_time)%>','<%=demo.qformat(rs.getDouble("demand_amount"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("cost_price_sum"))%>',
<%if(rs.getString("check_tag").equals("9")){%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("transferGather.jsp?gather_ID=<%=rs.getString("gather_ID")%>")><%=demo.getLang("erp","重新登记")%></div>'
<%}else{%>
'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("transferGather.jsp?gather_ID=<%=rs.getString("gather_ID")%>")><%=demo.getLang("erp","继续登记")%></div>'
<%}%>
],
<%
k++;
