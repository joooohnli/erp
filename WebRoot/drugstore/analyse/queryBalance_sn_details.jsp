...
nseer_grid.column_width=[180,200,100,100,160];
nseer_grid.auto='<%=demo.getLang("erp","S/N")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">

['<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("stock_name"))%>','<%=rs.getString("serial_number")%>','<%=exchange.toHtml(rs.getString("register_time"))%>'],
