...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE5%> class="TD_STYLE5"><font size="4"><b><%=demo.getLang("erp","库存盘点单明细")%></b></font></td>
 </tr>
 </table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4" id=theObjTable>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
	 <td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","盘点单编号")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><%=static_report_ID%></td>
...
int i=1;
try{
	String sql1="select * from stock_balance_static_report_details where static_report_ID='"+static_report_ID+"' and product_ID='"+product_ID+"' order by stock_ID";
...
<%}else if(serial_number_tag==1){%>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="staticQuery_sn_details.jsp?static_report_ID=<%=static_report_ID%>&&product_ID=<%=product_ID%>&&product_name=<%=toUtf8String.utf8String(exchange.toURL(product_name))%>&&stock_name=<%=toUtf8String.utf8String(exchange.toURL(rs1.getString("stock_name")))%>&&amount=<%=rs1.getDouble("amount")%>"><%=rs1.getDouble("amount")%></a>&nbsp;</td>
	<%}else{%>
<td <%=TD_STYLE2%> class="TD_STYLE2"><a href="staticQuery_bn_details.jsp?static_report_ID=<%=static_report_ID%>&&product_ID=<%=product_ID%>&&product_name=<%=toUtf8String.utf8String(exchange.toURL(product_name))%>&&stock_name=<%=toUtf8String.utf8String(exchange.toURL(rs1.getString("stock_name")))%>&&amount=<%=rs1.getDouble("amount")%>"><%=rs1.getDouble("amount")%></a>&nbsp;</td>
<%}%>
