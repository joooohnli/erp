...
%>
 ...
 <table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
<tr <%=TR_STYLE1%> class="TR_STYLE1">
<td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","产品：")%><%=product_name%><%=demo.getLang("erp","在")%><%=stock_name%><%=demo.getLang("erp","中共有")%><%=amount%><%=demo.getLang("erp","件。")%></td>
...
nseer_grid.column_width=[180,200,160,100,160];
nseer_grid.auto='<%=demo.getLang("erp","库房名称")%>';
nseer_grid.data = [
<%
while(rs.next()){
%>
['<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("stock_name"))%>','<%=exchange.toHtml(rs.getString("serial_number"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>'],
<%
}
%>
['']];
