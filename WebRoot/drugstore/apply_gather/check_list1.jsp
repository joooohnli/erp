...
String sql="select count(id) from stock_config_workflow where type_id='01'";
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?gather_ID=<%=rs.getString("GATHER_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
