...
['<%if(delete_tag.equals("0")){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs1.getString("id")%>⊙<%=delete_tag%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?gather_ID=<%=rs1.getString("gather_ID")%>")><span style="color:<%=color%>"><%=rs1.getString("gather_ID")%></span></div>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("reason"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("register_time"))%></span>','<span style="color:<%=color%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs1.getDouble("demand_amount"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(apply_tag)%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(check_tag)%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(gather_tag)%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(return_tag)%></span>',

	<%if(rs1.getString("check_tag").equals("9")){%>
	'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("register.jsp?gather_ID=<%=rs1.getString("gather_ID")%>")><%=demo.getLang("erp","重新申请")%></span></div>'],<%}else{%>''],<%}%>
<%k++;%>
</page:pages>
['']];
nseer_grid.init();
