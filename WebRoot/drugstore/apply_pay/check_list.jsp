...
String sql="select count(id) from stock_config_workflow where type_id='02'";
...
nseer_grid.column_width=[180,180,160,100,70,100,
<%
for(int i=1;i<workflow_amount;i++){
%>					   70,
<%
...
nseer_grid.auto='<%=demo.getLang("erp","申请单编号")%>';
nseer_grid.data = [
...
String sql1="select id from stock_workflow where object_ID='"+rs.getString("pay_ID")+"' and check_tag='0'";//rs.getString("pay_ID")
ResultSet rs2=stock_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from stock_workflow where object_ID='"+rs.getString("pay_ID")+"' order by id";
	rs2=stock_db1.executeQuery(sql1);
%>

['<%=rs.getString("pay_ID")%>','<%=exchange.toHtml(rs.getString("reason"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=exchange.toHtml(demand_return_time)%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs.getDouble("demand_amount"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("cost_price_sum"))%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
