...
nseer_grid.columns =[
                       ...
                       {name: '<%=demo.getLang("erp","借领状态")%>'},
	                   {name: '<%=demo.getLang("erp","归还状态")%>'},
					   {name: '&nbsp;'}
]
nseer_grid.column_width=[40,180,70,160,70,70,70,70,70];
...
if(rs1.getString("return_tag").equals("0")){
return_tag="等待";
}else if(rs1.getString("return_tag").equals("1")){
return_tag="执行";
}else if(rs1.getString("return_tag").equals("2")){
return_tag="完成";
}
if(rs1.getString("not_return_tag").equals("1")){
return_tag="不归还";
}
%>
['<%if(delete_tag.equals("0")){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs1.getString("id")%>⊙<%=delete_tag%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?pay_ID=<%=rs1.getString("pay_ID")%>")><span style="color:<%=color%>"><%=rs1.getString("pay_ID")%></span></div>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("reason"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("register_time"))%></span>','<span style="color:<%=color%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs1.getDouble("demand_amount"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(apply_tag)%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(check_tag)%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(pay_tag)%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(return_tag)%></span>',
	<%if(rs1.getString("check_tag").equals("9")){%>
	'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("register.jsp?pay_ID=<%=rs1.getString("pay_ID")%>")><%=demo.getLang("erp","重新申请")%></span></div>'],<%}else{%>''],<%}%>
<%k++;%>
</page:pages>
['']];
nseer_grid.init();
