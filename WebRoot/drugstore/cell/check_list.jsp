...
String sql="select count(id) from stock_config_workflow where type_id='07'";
...
nseer_grid.column_width=[180,180,70,100,70,160,
<%
for(int i=1;i<workflow_amount;i++){
%>					   70,
<%
...
String sql1="select id from stock_workflow where object_ID='"+rs.getString("design_ID")+"' and check_tag='0'";
ResultSet rs2=stock_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from stock_workflow where object_ID='"+rs.getString("design_ID")+"' order by id";
	rs2=stock_db1.executeQuery(sql1);
%>

['<%=rs.getString("design_ID")%>','<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(register_time)%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
