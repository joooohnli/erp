...
<form class="x-form" method="post" action="../../stock_gather_check_delete_ok">
...
<input type="hidden" name="gathering_time" value="<%=gathering_time%>">
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check.jsp?gather_ID=<%=gather_ID%>&config_id=<%=config_id%>&gathering_time=<%=gathering_time%>"></div></td>
 </tr>
 </table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","该入库登记未通过审核，您确认吗？请选择处理：")%></td>
 </tr>
 </table>
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
<%
int i=1;
String sql6="select config_id from stock_workflow where object_ID='"+gather_ID+"' and config_id<'"+config_id+"' and gathering_time='"+gathering_time+"' order by id";
