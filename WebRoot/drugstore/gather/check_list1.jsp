...
nseer_grid.auto='<%=demo.getLang("erp","入库单编号")%>';
nseer_grid.data = [
<%	
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?gather_ID=<%=rs6.getString("gather_ID")%>&config_id=<%=rs2.getString("config_id")%>&gathering_time=<%=rs6.getString("gathering_time")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
	}
m++;
