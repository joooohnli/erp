...
nseer_grid.columns =[
                       ...
                       {name: '<%=demo.getLang("erp","应入库总件数")%>'},
	                   {name: '<%=demo.getLang("erp","已入库总件数")%>'},
                       {name: '<%=demo.getLang("erp","入库单状态")%>'}
]
...
%>
['<%if(delete_tag.equals("0")){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs1.getString("id")%>⊙<%=delete_tag%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?gather_ID=<%=rs1.getString("gather_ID")%>")><span style="color:<%=color%>"><%=rs1.getString("gather_ID")%></span></div>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("reason"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(finish_time)%></span>','<span style="color:<%=color%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs1.getDouble("demand_amount"))%></span>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs1.getDouble("gathered_amount"))%>','<span style="color:<%=color%>"><%=exchange.toHtml(gather_tag)%></span>'],
<%k++;%>
</page:pages>
['']];
nseer_grid.init();
