...
<%nseer_db drugstore_db3 = new nseer_db((String)session.getAttribute("unit_db_name"));%>
...
String prescription_id="11111111111";
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8"><input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","确认")%>" name="B1">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></td>
 </tr>
 </table>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE5%> class="TD_STYLE5"><font size="4"><b><%=demo.getLang("erp","药库提药单")%></b></font></td>
 </tr>
 </table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
	 <td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp","制作人")%>&nbsp;&nbsp;：</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="gatherer_name"></td>
	 <td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp","送入药房")%>&nbsp;&nbsp;：</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="reason">
  <%
  String sql5 = "select * from drugstore_config_public_char where describe1='药房' order by id" ;
	 ResultSet rs5 = drugstore_db3.executeQuery(sql5) ;
while(rs5.next()){%>
		<option value="<%=exchange.toHtml(rs5.getString("drugstore_name"))%>"><%=exchange.toHtml(rs5.getString("drugstore_name"))%></option>
<%
}
drugstore_db3.close();
%>
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../design/file/query.jsp?product_ID=<%=rs2.getString("drug_id")%>")><%=rs2.getString("drug_id")%></div></td>
  <td <%=TD_STYLE2%> class="TD_STYLE2"><%=rs3.getString("product_name")%></td>
    <td <%=TD_STYLE2%> class="TD_STYLE2"><%=rs3.getString("product_describe")%></td>
  <td <%=TD_STYLE2%> class="TD_STYLE2"><input style="width: 100%;background-color:#C9E7DC" value=<%=rs2.getString("drug_amount")%>></td>
