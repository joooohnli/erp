...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="gather_product_details.jsp?gather_ID=<%=gather_ID%>&&product_ID=<%=product_ID%>&&reasonexact=<%=toUtf8String.utf8String(exchange.toURL(reasonexact))%>&&reason=<%=toUtf8String.utf8String(exchange.toURL(reason))%>&&reasonexact_details=<%=toUtf8String.utf8String(exchange.toURL(reasonexact_details))%>"></div></td>
 </tr>
 </table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","入库数量有误，请返回确认！")%></td>
 </tr>
 </table>
 </div>