...
String sql6="select * from ecommerce_workflow where type_id='01' and object_id='"+product_ID+"' and check_tag='0' and config_id<'"+config_id+"'";
...
form.action = "../../ecommerce_design_check_ok?config_id=<%=config_id%>&product_ID=<%=product_ID%>";
}
}
</script>
<form id="mutiValidation" method="post" onSubmit="return doValidate('../../xml/design/design_file.xml','mutiValidation')&&TwoSubmit(this)">
 ...
 <%=demo.getLang("erp","通过")%> <input <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","确认")%>" name="B1">
 <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check_list.jsp"></div>
 </td>
 </tr>
</table>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="hidden" name="register"  style="width:60%" value="<%=exchange.toHtml(rss.getString("register"))%>"><%=rss.getString("register")%></td>
 </tr>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","审核人")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input type="hidden" name="checker_ID" value="<%=checker_ID%>"><input type="hidden" name="checker" value="<%=exchange.toHtml(checker)%>"><%=exchange.toHtml(checker)%></td>
