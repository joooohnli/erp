...
String rowSummary=demo.getLang("erp","当前可变更的职位总数");
%>
...
ResultSet rs=hr_db1.executeQuery(sql_search);
%>
...
ResultSet rs2=hr_db.executeQuery(sql1);
...
%>
	'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("query_details.jsp?release_id=<%=rs2.getString("object_id")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","查看")%></div>',<%}else{%>'',<%}}%>'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?release_id=<%=rs.getString("release_id")%>")><%=demo.getLang("erp","变更")%></div>'],
