...
<%nseer_db engage_db = new nseer_db((String)session.getAttribute("unit_db_name"));%>
...
%>
<form class="x-form" method="post" action="../../ecommerce_engage_check_delete_ok">
     
<input type="hidden" name="config_id" value="<%=config_id%>">
<input type="hidden" name="release_id" value="<%=release_id%>">
<input type="hidden" name="remark1" value="<%=remark1%>">
<input type="hidden" name="remark2" value="<%=remark2%>">
<input type="hidden" name="opinion" value="<%=exchange.toHtml(opinion)%>">
...
<input type="hidden" name="check_time" value="<%=exchange.toHtml(check_time)%>">
 ...
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2" width="100%">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3" height="10"><%=demo.getLang("erp","该职位未通过发布审核，您确认吗？请选择处理：")%></td>
 </tr>
 
<%
int i=1;
String sql6="select * from ecommerce_workflow where object_id='"+release_id+"' and config_id<'"+config_id+"' order by id";
ResultSet rs6=engage_db.executeQuery(sql6);
while(rs6.next()){
