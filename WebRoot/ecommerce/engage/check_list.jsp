...
String rowSummary=demo.getLang("erp","当前等待发布审核的职位总数：");
%>
...
<%
String sql1="select * from ecommerce_workflow where object_id='"+rs.getString("release_id")+"' and check_tag='0'";
ResultSet rs2=ecommerce_db.executeQuery(sql1);
if(rs2.next()){
	sql1="select * from ecommerce_workflow where object_id='"+rs.getString("release_id")+"' order by id";
...
%>
	,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?release_id=<%=rs2.getString("object_id")%>&&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],  
<%
}
%>
</page:pages>
