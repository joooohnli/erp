...
String condition="check_tag='1' and excel_tag4='3' and type!='物料' and type!='服务型产品'";
...
String sql="select count(id) from ecommerce_config_workflow where type_id='05'";
...
<%
String sql1="select * from ecommerce_workflow where object_ID='"+rs.getString("product_ID")+"' and type_id='05'";
...
%>
	,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("query_details.jsp?product_ID=<%=rs.getString("product_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","查看")%></div>'<%}else{%>,''<%}}%>],
