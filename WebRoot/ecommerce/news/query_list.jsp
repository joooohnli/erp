...
String rowSummary=demo.getLang("erp","符合条件的新闻总数");
...
<%
String sql1="select * from ecommerce_workflow where object_id='"+rs.getString("news_ID")+"'";
ResultSet rs2=ecommerce_db.executeQuery(sql1);
%>
  ['<%if(rs.getString("check_tag").equals("1")){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?news_ID=<%=rs.getString("news_ID")%>")><%=rs.getString("news_ID")%></div>','<%=exchange.toHtml(rs.getString("topic"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>',
<%
for(int i=1;i<=workflow_amount;i++){
	if(rs2.next()){
%>
	'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("query_details.jsp?news_ID=<%=rs.getString("news_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","查看")%></div>',<%}else{%>'',
		<%}}
	String status="";
