...
String sql9="select * from ecommerce_workflow where object_ID='"+other_ID+"' and check_tag='0' and config_ID<'"+config_ID+"'";
...
form.action = "check_delete_reconfirm.jsp?other_ID=<%=other_ID%>&config_ID=<%=config_ID%>";
}else{
form.action = "../../ecommerce_other_check_ok?other_ID=<%=other_ID%>&config_ID=<%=config_ID%>";
}
}
</script>
<form id="mutiValidation" method="post" onSubmit="return doValidate('../../xml/ecommerce/ecommerce_other.xml','mutiValidation')&&TwoSubmit(this)" ENCTYPE="multipart/form-data">
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input type="text" id="filekind_chain" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="filekind_chain" onFocus="showKind('nseer1',this,showTree)" onkeyup="search_suggest(this.id)" autocomplete="off" value="<%=rss.getString("chain_id")%> <%=exchange.toHtml(rss.getString("chain_name"))%>"><input id="filekind_chain_table" type="hidden" value="ecommerce_config_other_kind"></td>
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="opinion"><%=rss.getString("opinion")%></textarea>
</td>
 </tr>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
