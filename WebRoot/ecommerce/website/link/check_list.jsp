...
String rowSummary=demo.getLang("erp","当前等待审核的链接总数：");
%>
...
nseer_grid.column_width=[200,200,200,100,<%for(int i=1;i<workflow_amount;i++){%>70,<%}%>70];
...
String sql1="select id from ecommerce_workflow where object_ID='"+rs.getString("link_ID")+"' and check_tag='0'";
ResultSet rs2=ecommerce_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from ecommerce_workflow where object_ID='"+rs.getString("link_ID")+"' order by id";
	rs2=ecommerce_db1.executeQuery(sql1);
String url=rs.getString("url");
if(url.indexOf("http://")==-1) url="http://"+rs.getString("url");

%>
    ['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?link_ID=<%=rs.getString("link_ID")%>")><%=rs.getString("link_ID")%></div>','<%=rs.getString("topic")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("<%=url%>")><%=exchange.toHtml(rs.getString("url"))%></div>',
<%
...
%>
	,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?link_ID=<%=rs.getString("link_ID")%>&config_ID=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
