...
<jsp:setProperty name="mask" property="file" value="xml/ecommerce/ecommerce_web_template.xml"/>
...
String key="";
if(rs.next()){
	key=rs.getString("home_page");
}

String tablename="ecommerce_web_template";
String realname=(String)session.getAttribute("realeditorc");
String condition="";
String queue="order by id";
String validationXml="../../../xml/ecommerce/ecommerce_web_template.xml";
String nickName="模板";
String fileName="homePage.jsp";
...
<%@include file="../../../include/search_top.jsp"%>
 <form name="selectList" id="keyRegister" method="POST" action="../../../ecommerce_website_template_homePage_ok">
...
nseer_grid.columns =[
                       {name: '<%=demo.getLang("erp","请选择首页模版")%>'}
]

nseer_grid.column_width=[200];
nseer_grid.auto='<%=demo.getLang("erp","请选择首页模版")%>';
nseer_grid.data = [
<page:pages rs="<%=rs1%>" strPage="<%=strPage%>"> 
<% 
	if(key.equals(rs1.getString("filename"))){
%>
  ['<input type="radio" name="col" value="<%=rs1.getString("filename")%>" checked><%=exchange.toHtml(rs1.getString("topic"))%>'],
 <%}else{%>
	['<input type="radio" name="col" value="<%=rs1.getString("filename")%>"><%=exchange.toHtml(rs1.getString("topic"))%>'],
<%
}	
n++;
if(rs1.next()){
