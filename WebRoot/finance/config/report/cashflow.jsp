...
<script language="javascript" src="../../../javascript/finance/config/report/cashflow.js"></script>
...
<% 
String sql = "select * from finance_config_report_itemc order by id";
...
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
	['<input type="checkbox" <%=CHECKBOX_STYLE1%> class="CHECKBOX_STYLE1" name="<%=k%>" value="<%=rs.getString("id")%>">','<%=rs.getString("number_in_cash_table")%>','<%=exchange.toHtml(rs.getString("first_kind_name"))%>'],
<%
k++;
%>
</page:pages>
...
<td <%=TD_STYLE21%> class="TD_STYLE2" width="70%"><input <%=INPUT_STYLE1%> class="INPUT_STYLE1" style="width:100%" type="text" id="number_in_cash_table" name="number_in_cash_table" value="" onkeyup="inputControl(this.id)"/></td>
</tr>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<page:updowntag num="<%=intRowCount%>" file="cashflow.jsp"/>
