...
out.print(search);
	...
	sql="select file_name,start_time,specification,remnant_value_rate,remnant_value,original_value,net_value from finance_fa_file where file_id='"+keyword+"'";
	rs=finance_db.executeQuery(sql);
if(rs.next()){
	search=rs.getString("file_name")+"⊙"+rs.getString("start_time")+"⊙"+rs.getString("specification")+"⊙"+rs.getString("remnant_value_rate")+"⊙"+rs.getString("remnant_value")+"⊙"+rs.getString("original_value")+"⊙"+rs.getString("net_value");
	}
 	search=!search.equals("")?search:"179206725";
 out.print(search);
}
