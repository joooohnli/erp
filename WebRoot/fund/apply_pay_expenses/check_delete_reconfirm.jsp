...
<form id="check1" method="POST" action="../../fund_apply_pay_expenses_check_delete_ok">
<input name="apply_pay_ID" type="hidden" value="<%=apply_pay_ID%>">
...
<div id="nseerGround" class="nseerGround">
 ...
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","确认")%>" name="B1">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check.jsp?apply_pay_ID=<%=apply_pay_ID%>&config_id=<%=config_id%>"></div></td>
 </tr>
 </table>
  <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3">
  <%=demo.getLang("erp","该费用申请单未通过审核，您确认吗？请选择处理：")%></td>
 <td <%=TD_STYLE3%> class="TD_STYLE3">&nbsp;</td>
 </tr>
 <%
int i=1;
String sql6="select config_id from fund_workflow where object_ID='"+apply_pay_ID+"' and config_id<'"+config_id+"' order by id";
