...
%>
 ...
 <td <%=TD_HANDBOOK_STYLE1%> class="TD_HANDBOOK_STYLE1"><div class="div_handbook"><%=handbook%></div></td>
  ...
  <table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
	 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="打印" onClick="javascript:winopen('query_print.jsp?apply_pay_ID=<%=apply_pay_ID%>')">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="返回" onClick="history.back()"></div></td>
 </tr>
 </table>
 <table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=demo.getLang("erp","费用申请单编号")%>：<font color="<%=color%>"><%=rs.getString("apply_pay_ID")%><%=apply_tag%></font><%=demo.getLang("erp","其中审核状态")%>：<font color="<%=color1%>"><%=check_tag%></font><a><%=demo.getLang("erp"," 费用状态")%>：<font color="<%=color2%>"><%=pay_tag%></font></a></td>
 </tr>
 </table>
...
<%}else{%>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><%=rs.getString("human_ID")%>/<%=rs.getString("human_name")%>&nbsp;</td>
<%}%>
 </tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","币种/单位")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><%=rs.getString("currency_name")%>/<%=exchange.toHtml(rs.getString("personal_unit"))%>&nbsp;</td>
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" colspan="2"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs2.getDouble("cost_price_subtotal"))%>&nbsp;</td>
 </tr>
<%
	}
%>
