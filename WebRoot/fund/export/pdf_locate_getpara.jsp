...
try{
include.excel_export.Solid writer=new include.excel_export.Solid("xml/"+export_mod+"/pdf_export.xml");
include.operateXML.Reading mask=new include.operateXML.Reading("xml/"+export_mod+"/"+tablename+".xml");
 ...
 String[] colsNick=new String[cols.length];
 for(int i=0;i<cols.length;i++) {
 colsName[i]=cols[i].split("⊙")[1];
 colsNick[i]=cols[i].split("⊙")[2];
 }
 writer.update(tablename,tablenick,colsName,colsNick);
 response.sendRedirect("../../"+export_mod+"_export_pdf_ok?tablename="+tablename+"&&tablenick="+toUtf8String.utf8String(exchange.toURL(tablenick)));
