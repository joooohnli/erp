...
choice_group=choice_group+choice[i];
String sql = "select distinct chain_ID,chain_name,funder,funder_ID,currency_name,personal_unit,file_chain_ID,file_chain_name from fund_fund where "+condition+"" ;
...
if(row==1){
	...
	String sql1="select sum(demand_cost_price_sum) from fund_fund where ("+condition+") and reason='收款'";
	ResultSet rs1 = fund_db.executeQuery(sql1) ;
	if(rs1.next()){
		cost_price_sum1=rs1.getDouble("sum(demand_cost_price_sum)");
	}
	String sql3="select sum(demand_cost_price_sum) from fund_fund where ("+condition+") and reason='付款'";
...
<form id="register1" method="POST" action="../../fund_fund_registerGather_ok" onSubmit="return doValidate('../../xml/fund/fund_fund.xml','register1')">
...
<input name="choice_group" type="hidden" style="width: 50%; " value="<%=exchange.toHtml(choice_group)%>">
<input name="apply_ID_group" type="hidden" style="width: 50%; " value="<%=exchange.toHtml(apply_ID_group)%>">
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE5%> class="TD_STYLE5"><font size="4"><b><%=demo.getLang("erp","收款执行单")%></b></font></td>
 </tr>
 </table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="designer" type="text"></td>
	 <td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","收款理由")%>：</td>
<%
if(file_chain_name.indexOf("应收账款")!=-1){
%>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><a href="javascript:winopen('../../crm/order/query_order_ID_group.jsp?order_ID=<%=apply_ID_group%>')"><%=exchange.toHtml(apply_ID_group)%></td>
<%}else if(file_chain_name.indexOf("应付账款")!=-1){%>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><a href="javascript:winopen('../../purchase/purchase/query_purchase_ID_group.jsp?purchase_ID=<%=apply_ID_group%>')"><%=exchange.toHtml(apply_ID_group)%></td>
<%}else{%>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><a href="javascript:winopen('../apply_pay_expenses/query_apply_pay_ID_group.jsp?apply_pay_ID=<%=apply_ID_group%>')"><%=exchange.toHtml(apply_ID_group)%></td>
<%}%>
	</tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<%}}else if(funder.equals("")&&!chain_name.equals("")){%>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><a href="javascript:winopen('../../hr/config/file/thirdKind_details.jsp?chain_ID=<%=chain_ID%>')"><%=exchange.toHtml(chain_name)%></td>
<%}%>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td width="30%" <%=TD_STYLE2%> class="TD_STYLE2"><%=demo.getLang("erp","账户")%></td>
 ...
 <td width="10%" <%=TD_STYLE2%> class="TD_STYLE2"><%=demo.getLang("erp","币种/单位")%></td>
	 <td width="10%" <%=TD_STYLE2%> class="TD_STYLE2"><%=demo.getLang("erp","收款金额")%></td>
 </tr>
<%
i=1;
...
<div id="nseerGround" class="nseerGround">
 ...
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","请选择付款人分类、付款人编号、科目、币种一致的收款计划单，请返回。")%></td>
 <td <%=TD_STYLE3%> class="TD_STYLE3">&nbsp;</td>
 </tr>
 </table>
  </div>
...
<div id="nseerGround" class="nseerGround">
 ...
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","您没有选择收款计划单，请返回。")%></td>
 <td <%=TD_STYLE3%> class="TD_STYLE3">&nbsp;</td>
 </tr>
 </table>
  </div>
