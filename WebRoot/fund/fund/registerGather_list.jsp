...
String condition="fund_tag!='1' and check_tag='0' and excel_tag='2' and fund_pre_tag='0' and fund_execute_tag='0' and reason='收款'";
String queue="order by funder,reason,register_time";
String validationXml="../../xml/fund/fund_fund.xml";
String nickName="收款执行单";
String fileName="registerGather_list.jsp";
String rowSummary=demo.getLang("erp","当前等待调度的收款计划单总数：");
%>

<form id="register_list" method="post" action="registerGather.jsp">
...
<%}else if(rs.getString("file_chain_name").indexOf("工资")!=-1){%>
 '<div style="text-decoration : underline;color:#3366FF" onclick=winopen("../../hr/salary/query_unit_list.jsp?salary_ID=<%=rs.getString("reasonexact")%>")><span style="color:<%=color1%>"><%=exchange.toHtml(rs.getString("reasonexact"))%></span></div>'
<%}else{%>
