...
nseer_grid.auto='<%=demo.getLang("erp","收款执行单编号")%>';
nseer_grid.data = [
<%	
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?fund_ID=<%=rs6.getString("fund_ID")%>&config_id=<%=rs2.getString("config_id")%>&gather_time=<%=rs6.getString("gather_time")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
	}
m++;
