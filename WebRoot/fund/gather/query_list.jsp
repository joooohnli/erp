...
String condition0="excel_tag='2' and gar_tag='0' and reason='收款' and fund_execute_tag='1'";
...
String condition1="(check_tag='0' or check_tag='9') and gar_tag='0' and excel_tag='2' and reason='收款' and fund_execute_tag='1'";
String condition2="check_tag='1' and gar_tag='0' and excel_tag='2' and fund_tag='1' and reason='收款' and fund_execute_tag='1'";
String condition3="check_tag='1' and gar_tag='0' and excel_tag='2' and fund_tag='0' and reason='收款' and fund_execute_tag='1'";
int k=1;
%>
...
%>
['<%if(delete_tag.equals("0")){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs1.getString("id")%>⊙<%=delete_tag%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?fund_ID=<%=rs1.getString("fund_ID")%>")><%=rs1.getString("fund_ID")%></div>','<%=exchange.toHtml(rs1.getString("funder"))%>','<%=exchange.toHtml(rs1.getString("designer"))%>','<%=exchange.toHtml(finish_time)%>','<%=exchange.toHtml(rs1.getString("apply_ID_group"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("demand_cost_price_sum"))%>','<%=exchange.toHtml(rs1.getString("currency_name"))%>','<span style="color:<%=color%>"><%=fund_tag%></span>'],
<%k++;%>
</page:pages>
['']];
nseer_grid.init();
