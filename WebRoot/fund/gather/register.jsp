...
try{
String sql3="select file_ID from fund_config_fund_kind where responsible_person_ID like ''";
...
<form id="mutiValidation" method="POST" action="../../fund_gather_register_ok" onSubmit="return doValidate('../../xml/fund/fund_fund.xml','mutiValidation')">
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDraft("'mutiValidation','../../fund_gather_register_draft_ok','../../xml/fund/fund_fund.xml'",request)%>&nbsp;<input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value=<%=demo.getLang("erp","提交" )%>>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value=<%=demo.getLang("erp","打印")%> onClick="javascript:winopen('register_print.jsp?fund_ID=<%=fund_ID%>')">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value=<%=demo.getLang("erp","返回")%> onClick="history.back()"></div></td> 
 </tr>
</table>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input name="designer" type="hidden" value="<%=exchange.toHtml(rs.getString("designer"))%>"><%=exchange.toHtml(rs.getString("designer"))%>&nbsp;</td>
	 ...
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input name="apply_ID_group" type="hidden" value="<%=exchange.toHtml(rs.getString("apply_ID_group"))%>"><%=exchange.toHtml(rs.getString("apply_ID_group"))%>&nbsp;</td>
	 
	</tr>
	<input name="demand_cost_price_sum" type="hidden" value="<%=rs.getString("demand_cost_price_sum")%>">
	<input name="personal_unit" type="hidden" value="<%=exchange.toHtml(rs.getString("personal_unit"))%>">
	<input name="executed_cost_price_sum" type="hidden" value="<%=rs.getString("executed_cost_price_sum")%>">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="20%"><%=demo.getLang("erp","科目")%></td>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("executed_cost_price_sum"))%>&nbsp;</td>
 </tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","登记人")%>&nbsp;&nbsp;：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="register" type="text" value="<%=exchange.toHtml(rs.getString("register"))%>"></td>
