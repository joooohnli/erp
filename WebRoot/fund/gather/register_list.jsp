...
String[] fund_ID_group1=new String[10000];
	String sql = "select * from fund_fund where check_tag='1' and fund_tag!='1' and reason='收款' and fund_execute_tag='1' order by register_time" ;
	...
	while(rs.next()){
		...
		while(rs4.next()){
			if(rs4.getString("execute_check_tag").equals("0")&&!rs4.getString("execute_details_tag").equals("2")){
...
<form id=form1 action="register_list.jsp">
 ...
 <%=demo.getLang("erp","当前第")%><%=intPage%><%=demo.getLang("erp","页")%>&nbsp;&nbsp;&nbsp; <%if(intPage<intPageCount){%><a href="register_list.jsp?page=<%=intPage+1%>"><%=demo.getLang("erp","下页")%></a>&nbsp;&nbsp;&nbsp;<%}%> <%=demo.getLang("erp","共")%><%=intPageCount%><%=demo.getLang("erp","页")%>&nbsp;&nbsp;<%=demo.getLang("erp","跳到第")%> <input type="text" name=page size=1> <%=demo.getLang("erp","页")%>&nbsp;&nbsp; 
