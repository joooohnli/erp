...
String sql9="select * from fund_workflow where object_ID='"+fund_ID+"' and check_tag='0' and pay_time='"+pay_time+"' and config_id<'"+config_id+"'";
...
function TwoSubmit(){	
		...
		if (form.Ref[0].checked){
			form.action = "check_delete_reconfirm.jsp?config_id=<%=config_id%>&fund_ID=<%=rs.getString("fund_ID")%>&pay_time=<%=rs.getString("pay_time")%>";
			form.submit();
		}else{
			if(doValidate('../../xml/fund/fund_fund.xml','mutiValidation')){
					form.action = "../../fund_pay_check_ok?config_id=<%=config_id%>&fund_ID=<%=rs.getString("fund_ID")%>&pay_time=<%=rs.getString("pay_time")%>";
					form.submit();
			}
		}	
}
...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 ...
 <%=demo.getLang("erp","通过")%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","确认")%>" name="B1" onClick="TwoSubmit();">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回" )%>" onClick="history.back()"></div></td> 
 </tr>
</table>
<input name="file_chain_name" type="hidden" value="<%=exchange.toHtml(rs.getString("file_chain_name"))%>">
...
<%if(rs.getString("file_chain_name").indexOf("工资")!=-1){%>
...
%>
	 ...
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE5%> class="INPUT_STYLE5" id="subtotal" name="subtotal" type="text" value="<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("subtotal"))%>"></td>
 </tr>
<%
	i++;
		}
