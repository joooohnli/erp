...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?fund_ID=<%=rs6.getString("fund_ID")%>&config_id=<%=rs2.getString("config_id")%>&pay_time=<%=rs6.getString("pay_time")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
	}
i++;
...
<form id=form1 action="check_list.jsp">
 ...
 <%=demo.getLang("erp","当前第")%><%=intPage%><%=demo.getLang("erp","页")%>&nbsp;&nbsp;&nbsp; <%if(intPage<intPageCount){%><a href="check_list.jsp?page=<%=intPage+1%>"><%=demo.getLang("erp","下页")%></a>&nbsp;&nbsp;&nbsp;<%}%> <%=demo.getLang("erp","共")%><%=intPageCount%><%=demo.getLang("erp","页")%>&nbsp;&nbsp;<%=demo.getLang("erp","跳到第")%> <input type="text" name=page size=1> <%=demo.getLang("erp","页")%>&nbsp;&nbsp;<input type=image src="../../images/go.bmp" width=18 border=0></td></form>
 </tr> 
 </table> 
</div> 
<%	
