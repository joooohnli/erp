...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../crm/contact/query.jsp?contact_ID=<%=rs.getString("contact_ID")%>")><%=rs.getString("contact_ID")%></div>','<%=exchange.toHtml(rs.getString("customer_name"))%>','<%=exchange.toHtml(rs.getString("reason"))%>','<%=rs.getString("reasonexact")%>','<%=exchange.toHtml(rs.getString("person_contacted"))%>','<%=exchange.toHtml(rs.getString("contact_person"))%>','<%=exchange.toHtml(contact_time)%>'],
<%
k++;}
%>
['']];
