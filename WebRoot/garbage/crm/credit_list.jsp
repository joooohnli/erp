...
String rowSummary=demo.getLang("erp","符合条件的赊货申请单总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,100,100,160,100,100,100];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../crm/credit/query.jsp?customer_ID=<%=rs.getString("payer_ID")%>&&salecredit_list_price_sum=<%=rs.getDouble("list_price_sum")%>")><%=rs.getString("pay_ID")%></div>','<%=exchange.toHtml(rs.getString("payer_name"))%>','<%=exchange.toHtml(rs.getString("sales_name"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=exchange.toHtml(demand_return_time)%>','<%=demo.qformat(rs.getDouble("demand_amount"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("list_price_sum"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="gar_tag" value="">
