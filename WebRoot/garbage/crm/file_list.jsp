...
nseer_grid.column_width=[40,160,200,100,100,100];
nseer_grid.auto='<%=demo.getLang("erp","客户分类")%>';
nseer_grid.data = [

<%while(rs.next()){%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../crm/file/query.jsp?customer_ID=<%=rs.getString("customer_ID")%>")><%=rs.getString("customer_ID")%></div>','<%=exchange.toHtml(rs.getString("customer_name"))%>','<%=exchange.toHtml(rs.getString("customer_class"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("sales_name"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="field1" value="customer_name">
<input type="hidden" id="gar_tag" value="check_tag='1'">
<input type="hidden" id="field" value="customer_id">
