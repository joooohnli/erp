...
<script type="text/javascript" src="../../javascript/garbage/crm/invoiceReconfirm.js"></script>
...
String condition="(invoice_check_tag='2' or invoice_gar_tag='1') and gar_tag='0'";
...
nseer_grid.column_width=[40,180,100,100,100,100,100];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../crm/invoice/query_details.jsp?reasonexact=<%=rs.getString("order_ID")%>")><%=rs.getString("order_ID")%></div>','<%=exchange.toHtml(rs.getString("customer_name"))%>','<%=exchange.toHtml(rs.getString("demand_contact_person_tel"))%>','<%=exchange.toHtml(demand_pay_time)%>','<%=exchange.toHtml(rs.getString("sales_name"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("sale_price_sum"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="gar_recovery_tag" value="invoice_check_tag='2' and gar_tag='0'">
<input type="hidden" id="field1" value="order_id">
<input type="hidden" id="gar_tag" value="invoice_gar_tag='1' and gar_tag='0'">
<input type="hidden" id="field" value="order_id">
