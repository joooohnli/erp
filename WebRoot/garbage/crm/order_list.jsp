...
nseer_grid.column_width=[40,180,100,100,100,100,100,80];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="color:<%=color%>;text-decoration : underline;color:#3366FF" onclick=id_link("../../crm/order/query.jsp?order_ID=<%=rs.getString("order_ID")%>")><%=rs.getString("order_ID")%></div>','<div style="color:<%=color%>"><%=exchange.toHtml(rs.getString("customer_name"))%></div>','<div style="color:<%=color%>"><%=exchange.toHtml(rs.getString("sales_name"))%></div>','<div style="color:<%=color%>"><%=exchange.toHtml(rs.getString("demand_contact_person_tel"))%></div>','<div style="color:<%=color%>"><%=exchange.toHtml(demand_pay_time)%></div>','<div style="color:<%=color%>"><%=exchange.toHtml(rs.getString("type"))%></div>','<div style="color:<%=color%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("sale_price_sum"))%></div>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="gar_tag" value="order_tag='2'">
