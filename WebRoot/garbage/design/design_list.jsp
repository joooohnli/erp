...
<script type="text/javascript" src="../../javascript/garbage/design/garReconfirm.js"></script>
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../design/design/query.jsp?design_ID=<%=rs.getString("design_ID")%>")><%=rs.getString("design_ID")%></div>','<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("designer"))%>','<%=exchange.toHtml(register_time)%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("cost_price_sum"))%>'],
<%
k++;}
%>
['']];
