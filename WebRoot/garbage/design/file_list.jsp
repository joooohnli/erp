...
nseer_grid.column_width=[40,180,100,100,100,100];
nseer_grid.auto='<%=demo.getLang("erp","产品分类")%>';
nseer_grid.data = [

<%while(rs.next()){%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../design/file/query.jsp?product_ID=<%=rs.getString("product_ID")%>")><%=rs.getString("product_ID")%></div>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="field1" value="product_name">
<input type="hidden" id="gar_tag" value="check_tag='1'">
<input type="hidden" id="field" value="product_id">
