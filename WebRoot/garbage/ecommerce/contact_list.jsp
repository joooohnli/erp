...
String rowSummary=demo.getLang("erp","符合条件的联络垃圾总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,200,200];
nseer_grid.auto='<%=demo.getLang("erp","机构")%>';
nseer_grid.data = [
<%while(rs.next()){%>

['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../ecommerce/website/contact/query.jsp?id=<%=rs.getString("id")%>")><%=exchange.toHtml(rs.getString("chain_name"))%></div>','<%=exchange.toHtml(rs.getString("tel"))%>','<%=exchange.toHtml(rs.getString("email"))%>'
],
<%
k++;}
%>
...
<input type="hidden" id="field1" value="chain_name">
<input type="hidden" id="gar_tag" value="check_tag='1'">
<input type="hidden" id="field" value="chain_name">
