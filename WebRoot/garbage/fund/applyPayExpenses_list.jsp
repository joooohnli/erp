...
nseer_grid.column_width=[40,180,100,100,160,100];
nseer_grid.auto='<%=demo.getLang("erp","机构")%>';
nseer_grid.data = [

<%while(rs.next()){%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../fund/apply_pay_expenses/query.jsp?apply_pay_ID=<%=rs.getString("apply_pay_ID")%>")><%=rs.getString("apply_pay_ID")%></div>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("human_name"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("demand_cost_price_sum"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="field1" value="apply_pay_id">
<input type="hidden" id="gar_tag" value="apply_tag='1'">
<input type="hidden" id="field" value="apply_pay_id">
