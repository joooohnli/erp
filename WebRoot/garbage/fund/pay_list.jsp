...
String condition="(check_tag='2' or gar_tag='1') and reason='付款'";
...
nseer_grid.column_width=[40,180,100,100,160,180,100,100];
nseer_grid.auto='<%=demo.getLang("erp","收款人")%>';
nseer_grid.data = [

<%while(rs.next()){%>

['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../fund/pay/query.jsp?fund_ID=<%=rs.getString("fund_ID")%>")><%=rs.getString("fund_ID")%></div>','<%=exchange.toHtml(rs.getString("funder"))%>','<%=exchange.toHtml(rs.getString("designer"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=exchange.toHtml(rs.getString("apply_ID_group"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("demand_cost_price_sum"))%>','<%=exchange.toHtml(rs.getString("currency_name"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="field1" value="fund_id">
<input type="hidden" id="gar_tag" value="fund_tag='1'">
<input type="hidden" id="field" value="fund_id">
