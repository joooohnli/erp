...
String condition="(check_tag='3' or gar_tag='1')";
...
nseer_grid.column_width=[40,180,180,100,100,180,160];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../intrmanufacture/intrmanufacture/query.jsp?intrmanufacture_ID=<%=rs.getString("intrmanufacture_ID")%>")><%=rs.getString("intrmanufacture_ID")%></div>','<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=demo.qformat(rs.getDouble("demand_amount"))%>','<%=rs.getString("pay_ID_group")%>','<%=exchange.toHtml(register_time)%>'],
<%
k++;}
%>
['']];
