...
<%while(rs.next()){%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../intrmanufacture/product_providers/query.jsp?product_providers_recommend_ID=<%=rs.getString("product_providers_recommend_ID")%>")><%=rs.getString("product_providers_recommend_ID")%></div>','<%=exchange.toHtml(rs.getString("product_ID"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>'],
<%
k++;}
%>
['']];
