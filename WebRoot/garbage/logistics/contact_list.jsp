...
String nickName="配送单位联络垃圾";
String fileName="contact_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的配送单位联络垃圾总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,150,100,180,100,100,160];
nseer_grid.auto='<%=demo.getLang("erp","物流单位名称")%>';
nseer_grid.data = [
<%while(rs.next()){

String contact_time="";
if(!rs.getString("contact_time").equals("1800-01-01 00:00:00.0"))  contact_time=rs.getString("contact_time");

%>

['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../logistics/contact/query.jsp?contact_ID=<%=rs.getString("contact_ID")%>")><%=rs.getString("contact_ID")%></div>','<%=exchange.toHtml(rs.getString("provider_name"))%>','<%=exchange.toHtml(rs.getString("reason"))%>','<%=rs.getString("reasonexact")%>','<%=exchange.toHtml(rs.getString("person_contacted"))%>','<%=exchange.toHtml(rs.getString("contact_person"))%>','<%=exchange.toHtml(contact_time)%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="field1" value="contact_id">
<input type="hidden" id="gar_tag" value="gar_tag='1'">
<input type="hidden" id="field" value="contact_id">
