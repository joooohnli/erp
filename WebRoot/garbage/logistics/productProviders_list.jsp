...
<script type="text/javascript" src="../../javascript/garbage/logistics/ppReconfirm.js"></script>
...
String nickName="配送单位推荐垃圾";
String fileName="productProviders_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的配送单位推荐垃圾总数");
int k=1;
%>
...
nseer_grid.column_width=[50,180,180,180,180,160];
nseer_grid.auto='<%=demo.getLang("erp","产品分类")%>';
nseer_grid.data = [
<%while(rs.next()){%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../logistics/product_providers/query.jsp?product_logistics_recommend_ID=<%=rs.getString("product_logistics_recommend_ID")%>")><%=rs.getString("product_logistics_recommend_ID")%></div>','<%=exchange.toHtml(rs.getString("product_ID"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="field1" value="product_logistics_recommend_id">
<input type="hidden" id="gar_tag" value="gar_tag='1'">
<input type="hidden" id="field" value="product_logistics_recommend_id">
