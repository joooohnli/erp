...
<script type="text/javascript" src="../../javascript/garbage/manufacture/apply/garReconfirm.js"></script>
...
String nickName="生产计划单";
String fileName="apply_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的生产计划单总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,200,100,160,100];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("apply_ID")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../manufacture/apply/query.jsp?apply_ID=<%=rs.getString("apply_ID")%>")><%=rs.getString("apply_ID")%></div>','<%=exchange.toHtml(rs.getString("pay_id_group"))%>','<%=exchange.toHtml(rs.getString("register"))%>','<%=rs.getString("register_time")%>','<%=rs.getString("remark")%>'],
<%
k++;}
}
%>
...
<input type="hidden" id="field1" value="apply_id">
<input type="hidden" id="gar_tag" value="gar_tag='1'">
<input type="hidden" id="field" value="apply_id">
