...
<script type="text/javascript" src="../../javascript/garbage/manufacture/design_module/garReconfirm.js"></script>
...
String condition="(design_module_tag='4' or module_gar_tag='1') and gar_tag='0'";
...
nseer_grid.column_width=[40,180,180,100,100,160,100];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("design_ID")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../manufacture/design_module/query.jsp?design_ID=<%=rs.getString("design_ID")%>")><%=rs.getString("design_ID")%></div>','<%=exchange.toHtml(rs.getString("product_ID"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=rs.getString("designer")%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=demo.aformat(rs.getDouble("cost_price_sum"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="gar_recovery_tag" value="design_module_tag='4' and gar_tag='0'">
<input type="hidden" id="field1" value="design_id">
<input type="hidden" id="gar_tag" value="module_gar_tag='1' and gar_tag='0'">
<input type="hidden" id="field" value="design_id">
