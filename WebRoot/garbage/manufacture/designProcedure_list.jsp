...
<script type="text/javascript" src="../../javascript/garbage/manufacture/design_procedure/garReconfirm.js"></script>
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../manufacture/design_procedure/query.jsp?design_ID=<%=rs.getString("design_ID")%>")><%=rs.getString("design_ID")%></div>','<%=exchange.toHtml(rs.getString("product_ID"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=rs.getString("designer")%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=demo.aformat(rs.getDouble("cost_price_sum"))%>'],
<%
k++;}
%>
['']];
