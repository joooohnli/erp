...
String fileName="manufacture_list.jsp";
...
nseer_grid.column_width=[40,180,180,100,100,200,160];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../manufacture/manufacture/query.jsp?manufacture_ID=<%=rs.getString("manufacture_ID")%>")><%=rs.getString("manufacture_ID")%></div>','<%=exchange.toHtml(rs.getString("product_ID"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=demo.qformat(rs.getDouble("amount"))%>','<%=rs.getString("pay_ID_group")%>','<%=exchange.toHtml(register_time)%>'],
<%
k++;}
%>
['']];
