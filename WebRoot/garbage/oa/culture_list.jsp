...
String rowSummary=demo.getLang("erp","符合条件的企业文化垃圾总数");
int k=1;
%>
...
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../oa/culture/query.jsp?culture_ID=<%=rs.getString("culture_ID")%>")><%=exchange.toHtml(rs.getString("subject"))%></div>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>'],
<%
k++;}
%>
['']];
