...
String nickName="计划管理";
String fileName="planing_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的计划垃圾总数");
int k=1;
%>
...
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../oa/planing/query.jsp?planing_ID=<%=rs.getString("planing_ID")%>")><%=exchange.toHtml(rs.getString("subject"))%></div>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("begin_time"))%>','<%=exchange.toHtml(rs.getString("end_time"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="gar_tag" value="check_tag='8'">
