...
String rowSummary=demo.getLang("erp","符合条件的制度垃圾总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,200,200,200];
nseer_grid.auto='<%=demo.getLang("erp","制度主题")%>';
nseer_grid.data = [
<%while(rs.next()){%>

['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../oa/regulation/query.jsp?regulation_ID=<%=rs.getString("regulation_ID")%>")><%=exchange.toHtml(rs.getString("subject"))%></div>',
