...
String condition="check_tag='2' and gatherer_type='采购放货'";
...
String nickName="放货单";
String fileName="credit_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的放货单总数");
int k=1;
%>
...
otherButtons="&nbsp;<input type=\"button\" "+BUTTON_STYLE1+" class=\"BUTTON_STYLE1\" id=\"select_all\" value=\""+demo.getLang("erp","全选")+"\" name=\"check\" onclick=\"selAll()\">&nbsp;<input type=\"button\" "+BUTTON_STYLE1+" class=\"BUTTON_STYLE1\" value=\""+demo.getLang("erp","转入草稿箱")+"\" onclick=\"showDraDiv(document.getElementById('rows_num').value,'"+tablename+"');\">&nbsp;<input type=\"button\" "+BUTTON_STYLE1+" class=\"BUTTON_STYLE1\" value=\""+demo.getLang("erp","永久删除")+"\" onclick=\"showDeleteDiv(document.getElementById('rows_num').value,'"+tablename+"');\">";
%>
...
function id_link1(provider_ID,purchasecredit_cost_price_sum){ 
 var link1='../../purchase/credit/query.jsp?provider_ID='+provider_ID+'&&purchasecredit_cost_price_sum='+purchasecredit_cost_price_sum;
...
nseer_grid.column_width=[40,180,120,120,160,100,60,100];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link1("<%=rs.getString("gatherer_ID")%>","<%=rs.getString("cost_price_sum")%>")><%=rs.getString("gather_ID")%></div>','<%=exchange.toHtml(rs.getString("gatherer_name"))%>','<%=exchange.toHtml(rs.getString("purchaser"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=exchange.toHtml(demand_return_time)%>','<%=exchange.toHtml(demo.qformat(rs.getDouble("demand_amount")))%>',' <%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("cost_price_sum"))%>'
],
<%
k++;}
%>
...
<input type="hidden" id="gar_recovery_tag" value="check_tag='2' and gatherer_type='采购放货'">
<input type="hidden" id="field1" value="gather_id">
<input type="hidden" id="gar_tag" value="gar_tag='1'">
<input type="hidden" id="field" value="gather_id">
