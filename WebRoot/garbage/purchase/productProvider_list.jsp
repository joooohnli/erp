...
<script type="text/javascript" src="../../javascript/garbage/purchase/ppReconfirm.js"></script>
...
nseer_grid.column_width=[40,180,180,100,100,160];
nseer_grid.auto='<%=demo.getLang("erp","产品分类")%>';
nseer_grid.data = [
                       
<%while(rs.next()){%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../purchase/product_providers/query.jsp?product_providers_recommend_ID=<%=rs.getString("product_providers_recommend_ID")%>")><%=rs.getString("product_providers_recommend_ID")%>','<%=exchange.toHtml(rs.getString("product_ID"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="field1" value="product_providers_recommend_id">
<input type="hidden" id="gar_tag" value="gar_tag='1'">
<input type="hidden" id="field" value="product_providers_recommend_id">
