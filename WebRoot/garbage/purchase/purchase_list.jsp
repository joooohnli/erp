...
String condition="(check_tag='3' or gar_tag='1')";//3表示数据来自于草稿箱
...
String fileName="purchase_list.jsp";
...
otherButtons="&nbsp;<input type=\"button\" "+BUTTON_STYLE1+" class=\"BUTTON_STYLE1\" id=\"select_all\" value=\""+demo.getLang("erp","全选")+"\" name=\"check\" onclick=\"selAll()\">&nbsp;<input type=\"button\" "+BUTTON_STYLE1+" class=\"BUTTON_STYLE1\" value=\""+demo.getLang("erp","永久删除")+"\" onclick=\"showDeleteDiv(document.getElementById('rows_num').value,'"+tablename+"');\">&nbsp;<input type=\"button\" "+BUTTON_STYLE1+" class=\"BUTTON_STYLE1\" value=\""+demo.getLang("erp","还原")+"\" onclick=\"showRecovery(document.getElementById('rows_num').value,'"+tablename+"');\">";
%>
...
nseer_grid.column_width=[40,180,180,100,60,100,160];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../purchase/purchase/query.jsp?purchase_ID=<%=rs.getString("purchase_ID")%>")><%=rs.getString("purchase_ID")%></div>','<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=demo.qformat(rs.getDouble("demand_amount"))%>','<%=rs.getString("pay_ID_group")%>','<%=exchange.toHtml(register_time)%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="gar_recovery_tag" value="check_tag='3'">
<input type="hidden" id="field1" value="purchase_id">
<input type="hidden" id="gar_tag" value="gar_tag='1'">
<input type="hidden" id="field" value="purchase_id">
