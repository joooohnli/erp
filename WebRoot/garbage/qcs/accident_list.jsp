...
String rowSummary=demo.getLang("erp","符合条件的质量事故垃圾总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,160,180,180,100];
nseer_grid.auto='<%=demo.getLang("erp","产品名称")%>';
nseer_grid.data = [
<%while(rs.next()){%>

['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../qcs/accident/query.jsp?accident_id=<%=rs.getString("accident_id")%>")><%=rs.getString("accident_id")%></div>','<%=rs.getString("accident_time")%>','<%=exchange.toHtml(rs.getString("product_id"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("accident_name"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="field1" value="accident_id">
<input type="hidden" id="gar_tag" value="dealwith_tag!='0'">
<input type="hidden" id="field" value="accident_id">
