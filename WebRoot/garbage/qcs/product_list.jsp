...
String nickName="产品质检方式";
String fileName="product_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的产品质检方式垃圾总数");
int k=1;
%>
...
otherButtons="&nbsp;<input type=\"button\" "+BUTTON_STYLE1+" class=\"BUTTON_STYLE1\" id=\"select_all\" value=\""+demo.getLang("erp","全选")+"\" name=\"check\" onclick=\"selAll()\">&nbsp;<input type=\"button\" "+BUTTON_STYLE1+" class=\"BUTTON_STYLE1\" value=\""+demo.getLang("erp","转入草稿箱")+"\" onclick=\"showDraDiv(document.getElementById('rows_num').value,'"+tablename+"');\">&nbsp;<input type=\"button\" "+BUTTON_STYLE1+" class=\"BUTTON_STYLE1\" value=\""+demo.getLang("erp","永久删除")+"\" onclick=\"showDeleteDiv(document.getElementById('rows_num').value,'"+tablename+"','../../garbage_qcs_product_delete_ok');\">&nbsp;<input type=\"button\" "+BUTTON_STYLE1+" class=\"BUTTON_STYLE1\" value=\""+demo.getLang("erp","还原")+"\" onclick=\"showRecovery(document.getElementById('rows_num').value,'"+tablename+"');\">";
%>
...
nseer_grid.column_width=[40,180,200,180,180,160];
nseer_grid.auto='<%=demo.getLang("erp","产品名称")%>';
nseer_grid.data = [
<%while(rs.next()){%>

['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../qcs/product/query.jsp?qcs_id=<%=rs.getString("qcs_id")%>")><%=rs.getString("qcs_id")%></div>','<%=rs.getString("product_id")%>','<%=rs.getString("product_name")%>','<%=rs.getString("register")%>','<%=rs.getString("register_time")%>'],
<%
k++;}
%>
['']];
