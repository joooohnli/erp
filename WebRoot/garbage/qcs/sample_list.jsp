...
String rowSummary=demo.getLang("erp","符合条件的样品垃圾总数");
int k=1;
%>
...
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../qcs/sample/query.jsp?sample_id=<%=rs.getString("sample_id")%>")><%=exchange.toHtml(rs.getString("sample_id"))%></div>','<%=rs.getString("quality_type")%>','<%=rs.getString("apply_id")%>','<%=exchange.toHtml(rs.getString("sampling_person"))%>','<%=exchange.toHtml(rs.getString("sampling_time"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="field1" value="sample_id">
<input type="hidden" id="gar_tag" value="check_tag='1'">
<input type="hidden" id="field" value="sample_id">
