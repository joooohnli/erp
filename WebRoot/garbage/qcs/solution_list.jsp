...
String nickName="质检方案垃圾";
String fileName="solution_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的质检方案垃圾总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,200,180,180,100,100];
nseer_grid.auto='<%=demo.getLang("erp","标准名称")%>';
nseer_grid.data = [
<%while(rs.next()){%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../qcs/solution/query.jsp?solution_id=<%=rs.getString("solution_id")%>")><%=exchange.toHtml(rs.getString("solution_id"))%></div>','<%=exchange.toHtml(rs.getString("solution_name"))%>','<%=exchange.toHtml(rs.getString("product_id"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("quality_standard_name"))%>','<%=exchange.toHtml(rs.getString("designer"))%>'],
<%
k++;}
%>
['']];
...
<input type="hidden" id="field1" value="solution_id">
<input type="hidden" id="gar_tag" value="check_tag='1'">
<input type="hidden" id="field" value="solution_id">
