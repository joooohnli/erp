...
<script type="text/javascript" src="../../javascript/garbage/stock/garReconfirm.js"></script>
...
String rowSummary=demo.getLang("erp","符合条件的安全库存配置单总数");
int k=1;
%>
...
nseer_grid.column_width=[40,180,180,100,100,100,160];
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../stock/cell/query.jsp?design_ID=<%=rs.getString("design_ID")%>")><%=rs.getString("design_ID")%></div>','<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(register_time)%>'],
<%
k++;}
%>
['']];
