...
String nickName="出库垃圾";
String fileName="pay_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的出库垃圾总数");
int k=1;
%>
...
<%while(rs.next()){%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../stock/pay/query.jsp?pay_ID=<%=rs.getString("pay_ID")%>")><%=rs.getString("pay_ID")%></div>','<%=exchange.toHtml(rs.getString("reason"))%>','<%=exchange.toHtml(rs.getString("reasonexact"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=demo.aformat(rs.getDouble("demand_amount"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("cost_price_sum"))%>'],
<%
k++;}
%>
['']];
