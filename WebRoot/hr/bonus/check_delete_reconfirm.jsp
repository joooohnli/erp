...
<form id="timeLocateValidation"  class="x-form" method="post" action="../../hr_bonus_check_delete_ok" onSubmit="return doValidate('../../xml/hr/hr_bonus.xml','timeLocateValidation')">
<%
...
<input name="remark" type="hidden" value="<%=remark%>">
 ...
 <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check.jsp?human_ID=<%=human_ID%>&&config_id=<%=config_id%>&&bonus_time=<%=bonus_time%>"></div></td>
 </tr> 
 </table>
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<%
int i=1;
String sql6="select config_id from hr_workflow where object_ID='"+human_ID+"' and bonus_time='"+bonus_time+"' and type_id='06' and config_id<'"+config_id+"' order by id";
