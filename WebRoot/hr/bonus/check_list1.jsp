...
String sql="select count(id) from hr_config_workflow where type_id='06'";
ResultSet rs=hr_db.executeQuery(sql);
if(rs.next()){
	workflow_amount=rs.getInt("count(id)");
}
sql = "select * from hr_file left join hr_bonus on hr_file.human_ID=hr_bonus.human_ID where hr_bonus.check_tag='0' order by hr_file.register_time" ;
...
nseer_grid.column_width=[160,100,50,100,70,
<%
for(int i=1;i<workflow_amount;i++){
%>					   70,
<%
...
<page:pages rs="<%=rs%>" strPage="<%=strPage%>"> 
 <%
String sql1="select id,bonus_time from hr_workflow where object_ID='"+rs.getString("human_ID")+"' and check_tag='0' and type_id='06'";
ResultSet rs2=hr_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id,bonus_time from hr_workflow where object_ID='"+rs.getString("human_ID")+"' and bonus_time='"+rs2.getString("bonus_time")+"' and type_id='06' order by id";
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?human_ID=<%=rs.getString("human_ID")%>&&config_id=<%=rs2.getString("config_id")%>&&bonus_time=<%=rs2.getString("bonus_time")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
