...
String validationXml="../../xml/hr/hr_bonus.xml";
String nickName="激励";
String fileName="query_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的激励登记总数：");
%>
...
nseer_grid.columns =[
					   ...
					   {name: '<%=demo.getLang("erp","奖励项目")%>'},
					   {name: '<%=demo.getLang("erp","奖励价值(元)")%>'},
					   {name: '<%=demo.getLang("erp","奖励等级")%>'}
]
nseer_grid.column_width=[40,180,100,100,100,160,100,100,70];
nseer_grid.auto='<%=demo.getLang("erp","姓名")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>"> 
  ['<%if(rs.getString("check_tag").equals("1")){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link1("<%=rs.getString("human_ID")%>","<%=toUtf8String.utf8String(exchange.toURL(rs.getString("check_time").substring(0,rs.getString("check_time").indexOf(" "))))%>","<%=toUtf8String.utf8String(exchange.toURL(rs.getString("check_time").substring(rs.getString("check_time").indexOf(" ")+1)))%>")><%=rs.getString("human_ID")%></div>','<%=exchange.toHtml(rs.getString("human_name"))%>','<%=exchange.toHtml(rs.getString("human_major_first_kind_name"))%>','<%=exchange.toHtml(rs.getString("human_major_second_kind_name"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=exchange.toHtml(rs.getString("bonus_item"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("bonus_worth"))%>','<%=exchange.toHtml(rs.getString("bonus_degree"))%>'],
  <%k++;%>
 </page:pages>
['']];
nseer_grid.init();
