...
nseer_grid.column_width=[200,100,100,100,160,100,100,70];
nseer_grid.auto='<%=demo.getLang("erp","题干")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>"> 
['<%=rs.getString("content")%>','<%=exchange.toHtml(rs.getString("first_kind_name"))%>','<%=exchange.toHtml(rs.getString("second_kind_name"))%>','<%=exchange.toHtml(rs.getString("register"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=exchange.toHtml(rs.getString("derivation"))%>','<%=exchange.toHtml(rs.getString("correctkey"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?id=<%=rs.getString("id")%>")><%=demo.getLang("erp","变更")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
