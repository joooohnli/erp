...
nseer_grid.column_width=[100,60,60,100,100,100,100];
nseer_grid.auto='<%=demo.getLang("erp","姓名")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>"> 
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?details_number=<%=rs.getString("details_number")%>")><%=exchange.toHtml(rs.getString("human_name"))%></div>','<%=exchange.toHtml(rs.getString("sex"))%>','<%=exchange.toHtml(rs.getString("age"))%>','<%=exchange.toHtml(rs.getString("human_major_first_kind_name"))%>','<%=exchange.toHtml(rs.getString("human_major_second_kind_name"))%>','<%=exchange.toHtml(rs.getString("college"))%>','<%=exchange.toHtml(rs.getString("educated_major"))%>'],
</page:pages>
['']];
nseer_grid.init();
</script>
