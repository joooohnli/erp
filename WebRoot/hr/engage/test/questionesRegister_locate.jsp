...
<form id="mutiValidation" method="POST" action="questionesRegister_ok.jsp" onSubmit="return doValidate('../../../xml/hr/hr_test.xml','mutiValidation')">
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
 ...
 <div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","提交")%>" name="B1"> <input type="reset" <%=RESET_STYLE1%> class="RESET_STYLE1" value="<%=demo.getLang("erp","清除")%>"> <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="questionesRegister_list.jsp"></div>
 </td>
 </tr>
</table>
<%
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="select5"><script language = "JavaScript">
...
<%
 ...
 while(rsa.next()) {
  String sqll1 = "select * from hr_config_question_second_kind where first_kind_ID='"+rsa.getString("first_kind_ID")+"' and second_kind_name!='' order by id" ;
	 ResultSet rss1 = hrdb.executeQuery(sqll1) ;
				if(rss1.last()){
...
<table <%=TABLE_STYLE5%> class="TABLE_STYLE5">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=rsa.getString("first_kind_name")%>
 <input name="kind<%=j%>" type="hidden" value="<%=rsa.getString("first_kind_name")%>">　</td>
<td <%=TD_STYLE3%> class="TD_STYLE3" width="87%"> 
 <table <%=TABLE_STYLE6%> class="TABLE_STYLE6" style="width:100%">
...
<%for(int k=1;k<=countRow;k++){%>
 ...
 <input name="<%="amounta"+j+k%>" type="hidden" size="33" value="<%=getAmount.available((String)session.getAttribute("unit_db_name"),rsa.getString("first_kind_name"),rss1.getString("second_kind_name"))%>"><%=getAmount.available((String)session.getAttribute("unit_db_name"),rsa.getString("first_kind_name"),rss1.getString("second_kind_name"))%>&nbsp;</td>
<td <%=TD_STYLE2%> class="TD_STYLE2">
<input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="<%="amount"+j+k%>">　</td>
  </tr>
