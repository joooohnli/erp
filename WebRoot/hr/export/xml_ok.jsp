...
queue="order by register_time";
condition="where check_tag='0' and excel_tag='2'";
}
sql="select * from "+tablename+" "+condition+" "+queue;
int a=sql.indexOf("where");
sql=sql.substring(a,sql.length());
XmlFactory xml= new XmlFactory("conf/xml/hr/file_export.xml",dbase,tablename,colsNames,"conf/xml/hr/data_excel.xml",sql);
xml.makeElement();
xml.Xmlwrite();
