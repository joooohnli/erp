...
String sql90="select id from hr_workflow where object_ID='"+human_ID+"' and check_tag='0' and type_id='01' and config_id<'"+config_id+"'";
ResultSet rs90=hrdb.executeQuery(sql90);
if(!rs90.next()){
if(vt.validata((String)session.getAttribute("unit_db_name"),"hr_file","human_ID",human_ID,"check_tag").equals("0")){
try{
...
form.action = "check_delete_reconfirm.jsp?config_id=<%=config_id%>&human_ID=<%=human_ID%>";
}else{
...
<form id="mutiValidation" method="POST" class="x-form" onSubmit="return doValidate('../../xml/hr/hr_file.xml','mutiValidation')&&TwoSubmit(this)">
...
<script type="text/javascript" src="../../javascript/include/validate/validation-framework.js"></script>
 ...
 <%=demo.getLang("erp","通过")%>&nbsp;<input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","确认")%>" name="B1">&nbsp;<input type="reset" <%=RESET_STYLE1%> class="RESET_STYLE1" value="<%=demo.getLang("erp","清除")%>">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back()">
 </div></td>
 </tr>
</table>
...
<%if(rss.getString("change_tag").equals("0")){%>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="36%"><input <%=INPUT_STYLE1%> class="INPUT_STYLE1" type="text" id="fileKind_chain" name="kind_chain" style="width:49%" value="<%=exchange.toHtml(rss.getString("chain_id"))%> <%=exchange.toHtml(rss.getString("chain_name"))%>" onFocus="showKind('nseer1',this,showTree)" onkeyup="search_suggest(this.id)" autocomplete="off"><input id="fileKind_chain_table" type="hidden" value="hr_config_file_kind"></td>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><font color=red>*</font><%=demo.getLang("erp","机构")%></td>
<td <%=TD_STYLE21%> class="TD_STYLE2" width="39%"><input <%=INPUT_STYLE1%> class="INPUT_STYLE1" type="hidden" name="kind_chain" value="<%=exchange.toHtml(rss.getString("chain_id"))%> <%=exchange.toHtml(rss.getString("chain_name"))%>"><%=exchange.toHtml(rss.getString("chain_id"))%> <%=exchange.toHtml(rss.getString("chain_name"))%></td>
<td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","职位分类")%></td>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="39%"><input <%=INPUT_STYLE1%> class="INPUT_STYLE1" type="text" name="human_account" style="width:49%" value="<%=exchange.toHtml(rss.getString("human_account"))%>"></td> <td <%=TD_STYLE4%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","审核人")%></td>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="39%"><input name="check_time" type="hidden" style="width:49%" value="<%=exchange.toHtml(time)%>"><%=exchange.toHtml(time)%></td>
