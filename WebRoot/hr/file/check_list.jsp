...
nseer_grid.column_width=[180,200,60,200,100,<%for(int i=1;i<workflow_amount;i++){%>70,<%}%>70];
...
%>
	,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?human_id=<%=rs.getString("human_id")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
