...
<%
  String sql7 = "select * from hr_salary_standard where check_tag='1' order by standard_ID" ;
	 ...
	 String selected="";
while(rs7.next()){
	if(rs.getString("new_salary_standard_ID").equals(rs7.getString("standard_ID"))){selected="selected";}else{selected="";}
%>
		<option value="<%=rs7.getString("standard_ID")%>/<%=exchange.toHtml(rs7.getString("standard_name"))%>/<%=rs7.getString("salary_sum")%>" <%=selected%>><%=exchange.toHtml(rs7.getString("standard_name"))%>/<%=rs7.getDouble("salary_sum")%></option>
<%
}
%>
  </select>
