...
<form id="timeLocateValidation"  class="x-form" method="post" action="../../hr_major_change_check_delete_ok" onSubmit="return doValidate('../../xml/hr/hr_major_change.xml','timeLocateValidation')">
<%
...
<input name="remark2" type="hidden" value="<%=remark2%>">
 ...
 <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check.jsp?human_ID=<%=human_ID%>&&config_id=<%=config_id%>&&major_time=<%=major_time%>"></div></td>
 </tr>
 </table>
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3" height="10"><%=demo.getLang("erp","该调动未通过审核，您确认吗？请选择处理：")%></td>
 </tr> 
<%
int i=1;
String sql6="select config_id from hr_workflow where object_ID='"+human_ID+"' and major_time='"+major_time+"' and type_id='04' and config_id<'"+config_id+"' order by id";
