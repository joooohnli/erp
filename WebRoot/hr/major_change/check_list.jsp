...
nseer_grid.column_width=[200,200,200,100,100,<%for(int i=1;i<workflow_amount;i++){%>70,<%}%>70];
...
%> ['<%=rs.getString("human_ID")%>','<%=exchange.toHtml(rs.getString("human_name"))%>','<%=exchange.toHtml(rs.getString("new_chain_name"))%>','<%=exchange.toHtml(rs.getString("new_human_major_first_kind_name"))%>','<%=exchange.toHtml(rs.getString("new_human_major_second_kind_name"))%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?human_ID=<%=rs.getString("human_ID")%>&&config_id=<%=rs2.getString("config_id")%>&&major_time=<%=rs.getString("major_time")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
