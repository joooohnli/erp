...
if(rs.getString("intrmanufacture_tag").equals("1")){
intrmanufacture_tag="执行";
color="mediumseagreen";
}else if(rs.getString("intrmanufacture_tag").equals("2")){
...
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","打印")%> " onClick="javascript:winopen('../intrmanufacture/query_print.jsp?intrmanufacture_ID=<%=intrmanufacture_ID%>')">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%> " onClick="history.back();"></div></td>
 </tr>
</table>
 <table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=demo.getLang("erp","执行单编号")%>：<font color="<%=color%>"><%=rs.getString("intrmanufacture_ID")%><%=intrmanufacture_tag%></font> <%=demo.getLang("erp","委外加工费用：")%>&nbsp;<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("demand_cost_price_sum"))%></td>
 </tr>
 </table>
...
<table <%=TABLE_STYLE5%> class="TABLE_STYLE5">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="11%"><%=demo.getLang("erp","委外厂商编号")%></td>
	 ...
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="11%"><%=demo.getLang("erp","委外数量")%></td>
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=rs6.getString("demand_amount")%>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="javascript:winopen('../intrmanufacture/query_intrmanufacture_details.jsp?intrmanufacture_ID=<%=intrmanufacture_ID%>&&provider_ID=<%=rs6.getString("provider_ID")%>')"><%=demo.getLang("erp","查看")%> </a></td>
 </tr>
<%
	}
%>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
	 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","委外数量")%>：</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><%=exchange.toHtml(rs.getString("demand_amount"))%>&nbsp;</td>
