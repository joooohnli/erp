...
nseer_grid.column_width=[200,200,200,100,70,120];
...
%>
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("queryCost.jsp?intrmanufacture_ID=<%=rs1.getString("intrmanufacture_ID")%>")><%=rs1.getString("intrmanufacture_ID")%></div>','<span style="color:<%=color%>"><%=rs1.getString("product_ID")%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("product_name"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(intrmanufacture_tag)%></span>','<%=rs1.getDouble("demand_amount")%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("demand_cost_price_sum"))%>'],
</page:pages>
['']];
nseer_grid.init();
</script>
