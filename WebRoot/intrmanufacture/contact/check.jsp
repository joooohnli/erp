...
String sql9="select id from intrmanufacture_workflow where object_ID='"+contact_ID+"' and check_tag='0' and config_id<'"+config_id+"'";
ResultSet rs9=intrmanufacture_db.executeQuery(sql9);
if(!rs9.next()){
if(vt.validata((String)session.getAttribute("unit_db_name"),"intrmanufacture_contact","contact_ID",contact_ID,"check_tag").equals("0")){
try{
...
form.action = "../../intrmanufacture_contact_check_ok?config_id=<%=config_id%>&contact_ID=<%=rss.getString("contact_ID")%>";
}
}
</script>
...
<form id="mutiValidation" method="post" onSubmit="return doValidate('../../xml/intrmanufacture/intrmanufacture_contact.xml','mutiValidation')&&TwoSubmit(this)">
