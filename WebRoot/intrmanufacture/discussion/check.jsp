...
String sql9="select * from intrmanufacture_workflow where object_ID='"+discussion_ID+"' and check_tag='0' and config_id<'"+config_id+"'";
ResultSet rs9=intrmanufacture_db.executeQuery(sql9);
if(!rs9.next()){
if(!vt.validata((String)session.getAttribute("unit_db_name"),"intrmanufacture_discussion","discussion_ID",discussion_ID,"check_tag").equals("1")){
try{
...
form.action = "check_delete_reconfirm.jsp?id=<%=rs.getString("id")%>";
}else{
form.action = "../../intrmanufacture_discussion_check_ok?id=<%=rs.getString("id")%>";
}
}
</script>
...
%>
 ...
 <input name="config_id" type="hidden" value="<%=exchange.toHtml(config_id)%>">
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<div id="nseerGround" class="nseerGround">
<form id="register2" class="x-form" method="POST">    
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
