 ...
 *version 2 of the License, or (at your option) any later version.
 -->
<%@page contentType="text/html; charset=UTF-8" language="java" import="java.sql.*,include.nseer_cookie.*" import="java.util.*" import="java.io.*" import="include.nseer_cookie.exchange" import ="include.nseer_db.*,include.nseerdb.*,java.text.*,include.nseer_cookie.*"%>
...
try{
String sql="select * from intrmanufacture_discussion where discussion_ID='"+discussion_ID+"' and discussion_tag!='2'";
...
form.action = "../../intrmanufacture_discussion_dealwith_ok?discussion_ID=<%=rs.getString("discussion_ID")%>";
}else{
...
<form id="mutiValidation" method="POST" onSubmit="return doValidate('../../xml/intrmanufacture/intrmanufacture_discussion.xml','mutiValidation')&&TwoSubmit(this)">
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><INPUT name="Ref" type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1" value=cop checked><%=demo.getLang("erp","处理完成")%>&nbsp;<INPUT name="Ref" type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1" value=Ind><%=demo.getLang("erp","转垃圾箱")%>&nbsp;<input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","确认")%>">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="dealwith_list.jsp"></div></td>
 </tr>
 </table>
...
<TABLE <%=TABLE_STYLE5%> class="TABLE_STYLE5">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="11%"><%=demo.getLang("erp","推荐委外厂商")%></td>
 </tr>
<%
int i=1;
String sql6="select * from intrmanufacture_discussion_details where discussion_ID='"+rs.getString("discussion_ID")+"'";
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE4%> class="INPUT_STYLE4" name="amount<%=i%>" value="<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("amount"))%>"></td>
...
<td <%=TD_STYLE2%> class="TD_STYLE2"><a href="../../intrmanufacture_discussion_dealwith_recommend_ok?discussion_ID=<%=rs.getString("discussion_ID")%>&provider_ID=<%=rs.getString("provider_ID")%>&product_ID=<%=rs6.getString("product_ID")%>&processer=<%=toUtf8String.utf8String(exchange.toURL(processer))%>&processer_ID=<%=processer_ID%>"><%=demo.getLang("erp","推荐委外厂商")%></a></td>
<%}else{%>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1"> 
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="processer" value="<%=exchange.toHtml(processer)%>"></td>
