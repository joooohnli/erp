...
%>
 ...
 <table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=demo.getLang("erp","报价单编号")%>：<font color="<%=color%>"><%=rs.getString("discussion_ID")%></font>&nbsp;<%=demo.getLang("erp","其中审核状态")%>：<font color="<%=color1%>"><%=check_tag%></font>&nbsp;<%=demo.getLang("erp","处理状态")%>：<font color="<%=color2%>"><%=exchange.toHtml(discussion_tag)%></font></td> 
 </tr>
 </table>
