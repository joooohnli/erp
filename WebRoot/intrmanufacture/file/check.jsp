...
String sql9="select * from intrmanufacture_workflow where object_ID='"+provider_ID+"' and check_tag='0' and config_id<'"+config_id+"'";
ResultSet rs9=intrmanufacture_db.executeQuery(sql9);
if(!rs9.next()){

if(vt.validata((String)session.getAttribute("unit_db_name"),"intrmanufacture_file","provider_ID",provider_ID,"check_tag").equals("0")){
	String sqll = "select * from intrmanufacture_file where provider_ID='"+provider_ID+"'";
...
<form id="mutiValidation" method="POST" class="x-form" action="../../intrmanufacture_file_check_ok?id=<%=rss.getString("id")%>" onSubmit="return doValidate('../../xml/intrmanufacture/intrmanufacture_file.xml','mutiValidation')">
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3">
 <div <%=DIV_STYLE1%> class="DIV_STYLE1"><INPUT name="Ref" type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1" value="0" checked><%=demo.getLang("erp","未通过")%> <INPUT name="Ref" type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1" value="1"> 
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%" colspan="7"><a href="javascript:winopenm('query_attachment.jsp?id=<%=rss.getString("id")%>&tablename=intrmanufacture_file&fieldname=attachment_name')">
<%=attachment_name1[1]%></a>&nbsp;</td>
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE4%> class="TD_STYLE7" width="11%"><%=demo.getLang("erp","开票信息")%> &nbsp; </td>
<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%" height="65">
...
<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%" height="65">&nbsp;</td>
	 ...
	 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE4%> class="TD_STYLE7" width="11%"><%=demo.getLang("erp","可委托生产产品集合")%> &nbsp; </td>
<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%" height="65">
<textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="demand_products"><%=demand_products%></textarea>
</td>
 <td <%=TD_STYLE4%> class="TD_STYLE7" width="11%"><%=demo.getLang("erp","推荐委托生产产品集合")%> &nbsp; </td>
<td <%=TD_STYLE21%> class="TD_STYLE2" width="13%" height="65">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input name="checker" type="hidden" value="<%=exchange.toHtml(checker)%>"><input name="checker_ID" type="hidden" value="<%=exchange.toHtml(checker_ID)%>"><input name="config_id" type="hidden" value="<%=exchange.toHtml(config_id)%>"><%=exchange.toHtml(checker)%></td>
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="返回" onClick=location="check_list.jsp"></div></td>
 </tr>
</table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
