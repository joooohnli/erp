...
<jsp:setProperty name="mask" property="file" value="xml/intrmanufacture/intrmanufacture_details.xml"/>
...
<%
String tablename="intrmanufacture_details";
String realname=(String)session.getAttribute("realeditorc");
String condition="qcs_dealwith_tag='2' and change_tag='0'";
String queue="order by id";
String validationXml="../../xml/intrmanufacture/intrmanufacture_details.xml";
...
nseer_grid.column_width=[200,200,100,100,70];
nseer_grid.auto='<%=demo.getLang("erp","委外厂商名称")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
['<%=rs.getString("intrmanufacture_ID")%>','<%=rs.getString("provider_id")%>','<%=exchange.toHtml(rs.getString("provider_name"))%>','<%=rs.getString("demand_amount")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?intrmanufacture_id=<%=rs.getString("intrmanufacture_id")%>&&provider_id=<%=rs.getString("provider_id")%>")><%=demo.getLang("erp","变更")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
