...
for(int i=1;i<=workflow_amount;i++){
	...
	if(rs2.next()){
			...
			status="未审核";	
%>
	,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?intrmanufacture_ID=<%=rs.getString("intrmanufacture_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
