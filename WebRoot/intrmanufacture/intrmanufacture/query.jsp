...
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","打印")%>" onClick="javascript:winopen('query_print.jsp?intrmanufacture_ID=<%=intrmanufacture_ID%>')">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></div></td>
 </tr>
</table>
 <table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE2%> class="TD_STYLE2">
 <%=demo.getLang("erp","执行单编号：")%><font color="<%=color%>"><%=rs.getString("intrmanufacture_ID")%><%=exchange.toHtml(intrmanufacture_tag)%></font>
 <%=demo.getLang("erp","其中审核状态：")%><font color="<%=color1%>"><%=exchange.toHtml(check_tag)%></font>
 <%=demo.getLang("erp","入库状态：")%><font color="<%=color2%>"><%=exchange.toHtml(stock_gather_tag)%></font><%=demo.getLang("erp","发票状态：")%><font color="<%=color5%>"><%=exchange.toHtml(invoice_tag)%></font>
 <%=demo.getLang("erp","质检状态：")%><font color="<%=color4%>"><%=exchange.toHtml(gather_tag)%></td>
 </tr>
 </table>
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("demand_amount"))%>&nbsp;</td>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
	 ...
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("demand_amount"))%>&nbsp;</td>
