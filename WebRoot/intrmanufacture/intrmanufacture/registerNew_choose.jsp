...
String mod=request.getRequestURI();
 ...
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","完成")%>" style="width: 50; " onClick=location="registerNew_list.jsp"></div></td>
 </tr>									 
 </table> <table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
%>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="registerNew_choose_attachment.jsp?provider_ID=<%=rs6.getString("provider_ID")%>&&intrmanufacture_ID=<%=intrmanufacture_ID%>"><%=demo.getLang("erp","上传附件")%></a></td>
<%}else{%>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="registerNew_choose_attachment.jsp?provider_ID=<%=rs6.getString("provider_ID")%>&&intrmanufacture_ID=<%=intrmanufacture_ID%>"><font color="#0099cc"><%=demo.getLang("erp","重新上传附件")%></a></td>
<%}%>
 </tr>
<%
	}
