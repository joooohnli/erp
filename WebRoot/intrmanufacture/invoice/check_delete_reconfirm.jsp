...
<form class="x-form" method="post" action="../../intrmanufacture_invoice_check_delete_ok"> 
<input type="hidden" name="config_ID" value="<%=config_ID%>">
<input type="hidden" name="intrmanufacture_ID" value="<%=intrmanufacture_ID%>">
...
<input type="hidden" name="invoice_time" value="<%=invoice_time%>">
 ...
 <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check.jsp?intrmanufacture_ID=<%=intrmanufacture_ID%>&config_ID=<%=config_ID%>&invoice_time=<%=invoice_time%>"></div></td>
 </tr>
 </table>
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<%
int i=1;
String sql6="select config_id from intrmanufacture_workflow where object_ID='"+intrmanufacture_ID+"' and config_id<'"+config_ID+"' and invoice_time='"+invoice_time+"' order by id";
