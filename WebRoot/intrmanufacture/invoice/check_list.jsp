...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?intrmanufacture_ID=<%=rs.getString("intrmanufacture_ID")%>&invoice_time=<%=rs2.getString("invoice_time")%>&config_ID=<%=rs2.getString("config_ID")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%}%>
</page:pages>
['']];
nseer_grid.init();
