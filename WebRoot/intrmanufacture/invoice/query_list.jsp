...
<script type="text/javascript" src="../../javascript/intrmanufacture/invoice/query_list.js"></script>
...
%>
['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs1.getString("id")%>⊙<%=rs1.getString("invoice_tag")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?intrmanufacture_ID=<%=rs1.getString("intrmanufacture_ID")%>")><%=rs1.getString("intrmanufacture_ID")%></div>','<span style="color:<%=color%>"><%=rs1.getString("product_ID")%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("product_name"))%></span>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("demand_cost_price_sum"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("invoiced_sum"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("demand_cost_price_sum")-rs1.getDouble("invoiced_sum"))%>','<span style="color:<%=color%>"><%=exchange.toHtml(invoice_tag)%></span>'],
<% k++;%>
</page:pages>
['']];
nseer_grid.init();
