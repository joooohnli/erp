...
<form class="x-form" id="form1" method="post" action="../../intrmanufacture_product_providers_check_delete_ok">
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check.jsp?product_providers_recommend_ID=<%=product_providers_recommend_ID%>"></div></td>
 </tr>
</table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3" height="10"><%=demo.getLang("erp","该委外厂商推荐单未通过审核，您确认吗？")%></td>
 <td <%=TD_STYLE3%> class="TD_STYLE3" height="10">&nbsp;</td>
 </tr>
<%
int i=1;
String sql6="select config_id from intrmanufacture_workflow where object_ID='"+product_providers_recommend_ID+"' and config_id<'"+config_id+"' order by id";
