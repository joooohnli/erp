...
String sql="select count(id) from intrmanufacture_config_workflow where type_id='03'";
...
nseer_grid.column_width=[180,180,100,100,160,<%for(int i=1;i<workflow_amount;i++){%>70,<%}%>70];
...
String sql1="select id from intrmanufacture_workflow where object_ID='"+rs.getString("product_providers_recommend_ID")+"' and check_tag='0'";
ResultSet rs2=intrmanufacture_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from intrmanufacture_workflow where object_ID='"+rs.getString("product_providers_recommend_ID")+"' order by id";
