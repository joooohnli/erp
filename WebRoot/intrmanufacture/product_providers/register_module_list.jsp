...
%>
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
  ...
  <table width="100%">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
	<td <%=TD_STYLE1%> class="TD_STYLE1" width="17%"><%=demo.getLang("erp","物料编号")%></td>
 <td <%=TD_STYLE1%> class="TD_STYLE1" width="21%"><%=demo.getLang("erp","物料名称")%></td>
	 <td <%=TD_STYLE1%> class="TD_STYLE1" width="9%"><%=demo.getLang("erp","用途类型")%></td>
 <td <%=TD_STYLE1%> class="TD_STYLE1" width="9%"><%=demo.getLang("erp","物料描述")%></td>
 <td <%=TD_STYLE1%> class="TD_STYLE1" width="9%"><%=demo.getLang("erp","单位")%></td>
	 <td <%=TD_STYLE1%> class="TD_STYLE1" width="9%"><%=demo.getLang("erp","市场单价")%></td>
	 <td <%=TD_STYLE1%> class="TD_STYLE1" width="9%"><%=demo.getLang("erp","添加")%></td>
 </tr>
<page:pages rs="<%=rs1%>" strPage="<%=strPage%>">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
	<td <%=TD_STYLE2%> class="TD_STYLE2" width="17%"><%=rs1.getString("product_ID")%></td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="21%"><%=exchange.toHtml(rs1.getString("product_name"))%>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="9%"><%=exchange.toHtml(rs1.getString("type"))%>&nbsp;</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="9%"><%=rs1.getString("product_describe")%>&nbsp;</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="9%"><%=exchange.toHtml(rs1.getString("amount_unit"))%>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="9%"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("cost_price"))%>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="9%"><a href="javascript:addGoodsItem(['<%=rs1.getString("product_name")%>','<%=rs1.getString("product_ID")%>','<%=rs1.getString("type")%>','<%=rs1.getString("product_describe")%>','1','<%=rs1.getString("amount_unit")%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("cost_price"))%>'])"><%=demo.getLang("erp","添加")%></a></td>
 </tr>
</page:pages>
  </table>
  </div>
