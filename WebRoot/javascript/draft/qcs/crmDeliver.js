...
function search_suggest(input_id,div_obj,param1){//根据输入内容快速查询下拉层
	...
	if (param1==''||param1==null){
		...
		if(window.XMLHttpRequest){xmlhttp=new XMLHttpRequest();}else if (window.ActiveXObject){xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");}if(xmlhttp) {xmlhttp.onreadystatechange = function() {
			...
			xmlhttp.open("POST", "crmDeliver_search.jsp", true);
...
function formSubmit(){
	...
	if (action_tag == 'register'){
	    form_id='crmDeliver_register';
		form = document.getElementById('crmDeliver_register');
		form.action = '../../qcs_crm_deliver_register_ok';
	}else if (action_tag == 'change') {
	    form_id='crmDeliver_change';
		form = document.getElementById('crmDeliver_change');
		form.action = '../../qcs_crm_deliver_change_ok';
	}else if(action_tag == 'dealwith'){
	    form_id='crmDeliver_dealwith';
		form = document.getElementById('crmDeliver_dealwith');
		form.action = '../../qcs_crm_deliver_dealwith_ok';
     }
