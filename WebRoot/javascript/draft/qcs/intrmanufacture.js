...
function search_suggest(input_id,div_obj,param1){//根据输入内容快速查询下拉层
	...
	if (param1==''||param1==null){
		...
		if(window.XMLHttpRequest){xmlhttp=new XMLHttpRequest();}else if (window.ActiveXObject){xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");}if(xmlhttp) {xmlhttp.onreadystatechange = function() {
			...
			xmlhttp.open("POST", "intrmanufacture_search.jsp", true);
...
function formSubmit(){
	...
	if (action_tag == 'register') {
	    form_id='intrmanufacture_register';
		form = document.getElementById('intrmanufacture_register');
		form.action = '../../qcs_intrmanufacture_register_ok';
	}else if (action_tag == 'change') {
	    form_id='intrmanufacture_change';
		form = document.getElementById('intrmanufacture_change');
		form.action = '../../qcs_intrmanufacture_change_ok';
	}else if (action_tag == 'dealwith') {
	    form_id='intrmanufacture_dealwith';
		form = document.getElementById('intrmanufacture_dealwith');
		form.action = '../../qcs_intrmanufacture_dealwith_ok';
	...
	if (doValidate('../../xml/qcs/qcs_intrmanufacture.xml',form_id)){
	form.submit();
	}
}
function showFormSubmit(this_tag){
	...
	if(action_tag == 'dealwith'){
		if(document.getElementsByName('dealwith_tag')[1].checked){
			readXml('../../css/include/nseer_cookie/xml-css.css','../../xml/qcs/intrmanufacture/dealwith_ok.xml','0');
		}
		else if(document.getElementsByName('dealwith_tag')[2].checked&&document.getElementById('quality_way').value.split('/')[0]!='02'){
			readXml('../../css/include/nseer_cookie/xml-css.css','../../../xml/qcs/intrmanufacture/dealwith_ok1.xml','0');
		}
		else if(!document.getElementsByName('dealwith_tag')[2].checked&&document.getElementById('limit_tag').value=='1'){readXml('../../css/include/nseer_cookie/xml-css.css','../../xml/qcs/intrmanufacture/dealwith_ok2.xml','0');}
		else{
			if(parseInt(document.getElementById('unqualified').value)>parseInt(document.getElementById('reject').value)&&document.getElementById('qcs_result').value.split('/')[0]=='01'){
				readXml('../../css/include/nseer_cookie/xml-css.css','../../xml/qcs/intrmanufacture/register_ok.xml','0');
			}
			else{formSubmit();}
		}
	}
