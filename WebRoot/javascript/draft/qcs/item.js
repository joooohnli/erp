...
xmlhttp3.open("POST", "item_search.jsp", true);
...
function addRow(table_id,query_tag,event) {//动态添加行
    ...
    if(query_tag==0){
    td.innerHTML = '<input name=\"'+td_array[i].id+'\" id=\"'+td_array[i].id+row_num+'\" type=\"text\" style=\"width: 100%; height: 100%;\" onkeydown=\"return false;\" onFocus=\"fo(this.id,\''+td_array[i].id+'\')\"><div class="select_div_r" id="defect_class_div"  onclick="loadAjaxDiv(\''+td_array[i].id+row_num+'\',\'质检值\',event)"></div>';
    }else{
