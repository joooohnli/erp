 ...
 */
function loadAjaxDiv(input_id,product_id){//弹出ajax动态查询数据库层
...
function search_suggest(input_id,div_obj,param1){//根据输入内容快速查询下拉层
	...
	if (input_id == 'sampling_standard'){
		...
		search_tag = 6;
	}else if(input_id.indexOf('provider')!=-1){
		if(document.getElementById('dealwith_type').value.split('/')[0]=='01'){search_tag = 7;}
		else if(document.getElementById('dealwith_type').value.split('/')[0]=='02'){search_tag = 8;}
	}else if(input_id.indexOf('dealwith_type')!=-1){
		var div_options =['01/样品送检','02/退货'];
		var conter='';
	...
	if (param1==''||param1==null){
		...
		if(window.XMLHttpRequest){xmlhttp=new XMLHttpRequest();}else if (window.ActiveXObject){xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");}if(xmlhttp) {xmlhttp.onreadystatechange = function() {
			if(xmlhttp.readyState==4){try {if(xmlhttp.status==200){
				...
				if(search_tag==5||search_tag==6||search_tag==7||search_tag==8){
			...
			xmlhttp.open("POST", "other_search.jsp", true);
...
function setSearch(input_value,input_id,div_id,param1){
	...
	if(input_id=='dealwith_type'){
	document.getElementById('provider').value='';
	if(input_value.split('/')[0]=='01'){document.getElementById('provider_td').innerHTML='<font color=red>*</font>供应商：';}
	if(input_value.split('/')[0]=='02'){document.getElementById('provider_td').innerHTML='<font color=red>*</font>客户：';}	
	}
...
function formSubmit(){
	...
	if (action_tag == 'register') {
	    form_id='other_register';
		form = document.getElementById('other_register');
		form.action = '../../qcs_other_register_ok';
	}else if (action_tag == 'change') {
	    form_id='other_change';
		form = document.getElementById('other_change');
		form.action = '../../qcs_other_change_ok';
	}else if (action_tag == 'dealwith'){
	    form_id='other_dealwith';
		form = document.getElementById('other_dealwith');
		form.action = '../../qcs_other_dealwith_ok';
	}
	if (doValidate('../../xml/qcs/qcs_other.xml',form_id)){
	form.submit();
	}
}
function showFormSubmit(this_tag){
	...
	if(parseInt(document.getElementById('unqualified').value)>parseInt(document.getElementById('reject').value)&&document.getElementById('qcs_result').value.split('/')[0]=='01'){
		...
		var dealwith_type=document.getElementById('dealwith_type').value;
		if(dealwith_type==''||dealwith_type==null){alert('请选择处理类型');return false;}
		var provider=document.getElementById('provider').value;
		if(provider==''||provider==null){alert('请填写供应商或客户');return false;}
		if(document.getElementsByName('dealwith_tag')[1].checked&&dealwith_type.split('/')[0]!='01'){readXml('../../css/include/nseer_cookie/xml-css.css','../../xml/qcs/other/dealwith_ok.xml','0');}
		else if(document.getElementsByName('dealwith_tag')[2].checked&&dealwith_type.split('/')[0]!='02'){readXml('../../css/include/nseer_cookie/xml-css.css','../../xml/qcs/other/dealwith_ok1.xml','0');}
		else{formSubmit();}
	}else{
		formSubmit();
	}
