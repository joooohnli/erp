...
function search_suggest(input_id,div_obj,param1){//根据输入内容快速查询下拉层
	...
	if (param1==''||param1==null){
		...
		if(window.XMLHttpRequest){xmlhttp=new XMLHttpRequest();}else if (window.ActiveXObject){xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");}if(xmlhttp) {xmlhttp.onreadystatechange = function() {
			...
			xmlhttp.open("POST", "stock_search.jsp", true);
...
function formSubmit(){
	...
	if (action_tag == 'register'){
	    form_id='stock_register';
		form = document.getElementById('stock_register');
		form.action = '../../qcs_stock_register_ok';
	}else if (action_tag == 'check'){
	    form_id='stock_check';
		form = document.getElementById('stock_check');
		form.action = '../../qcs_stock_check_ok';
	}else if (action_tag == 'change'){
	    form_id='stock_change';
		form = document.getElementById('stock_change');
		form.action = '../../qcs_stock_change_ok';
	}else if(action_tag == 'dealwith'){
	    form_id='stock_dealwith';
        form = document.getElementById('stock_dealwith');
	    form.action = '../../qcs_stock_dealwith_ok';
	}
	if (doValidate('../../xml/qcs/qcs_stock.xml',form_id)){
	form.submit();
	}
}
