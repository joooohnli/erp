...
xmlhttp3.open("POST", "kind_search.jsp", true);
...
var options =xmlhttp3.responseText.split('⊙');
	...
	document.getElementById('cb_type').value=options[3];
}else {
alert( xmlhttp3.status + '=' + xmlhttp3.statusText);}} catch(exception) {alert(exception);}}};
xmlhttp3.open("POST", "kind_search.jsp", true);
xmlhttp3.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
function kind_ok(){//提交数据库
var file_id=document.getElementById('file_id1').value;
if(file_id==''){
	alert('固定资产编号不能为空');
	return false;
}
var file_name=document.getElementById('file_name1').value;
var start_time=document.getElementById('start_time').value;
var specification=document.getElementById('specification').value;
var cb_type=document.getElementById('cb_type').value;
var ca_type=document.getElementById('type_name2').value;
...
alert( xmlhttp3.status + '=' + xmlhttp3.statusText);}} catch(exception) {alert(exception);}}};
xmlhttp3.open("POST", "../../../finance_fixed_assets_change_kind_ok", true);
xmlhttp3.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
xmlhttp3.send('file_id='+file_id+'&file_name='+encodeURI(file_name)+'&start_time='+start_time+'&specification='+encodeURI(specification)+'&cb_type='+cb_type+'&ca_type='+encodeURI(ca_type)+'&change_reason='+encodeURI(change_reason)+'&change_date='+change_date+'&changer='+encodeURI(changer));
}
