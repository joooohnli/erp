...
xmlhttp3.open("POST", "life_search.jsp", true);
...
var options =xmlhttp3.responseText.split('⊙');
	...
	document.getElementById('cb_lifecycle1').value=options[3];
	document.getElementById('cb_lifecycle2').value=options[4];
}else {
alert( xmlhttp3.status + '=' + xmlhttp3.statusText);}} catch(exception) {alert(exception);}}};
xmlhttp3.open("POST", "life_search.jsp", true);
xmlhttp3.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
var cb_lifecycle1=document.getElementById('cb_lifecycle1').value;
var cb_lifecycle2=document.getElementById('cb_lifecycle2').value;
var ca_lifecycle1=document.getElementById('ca_lifecycle1').value==''?'0':document.getElementById('ca_lifecycle1').value;
var ca_lifecycle2=document.getElementById('ca_lifecycle2').value==''?'0':document.getElementById('ca_lifecycle2').value;
var cb_lifecycle=parseInt(cb_lifecycle1)*12+parseInt(cb_lifecycle2);
var ca_lifecycle=parseInt(ca_lifecycle1)*12+parseInt(ca_lifecycle2);
...
alert( xmlhttp3.status + '=' + xmlhttp3.statusText);}} catch(exception) {alert(exception);}}};
xmlhttp3.open("POST", "../../../finance_fixed_assets_change_life_ok", true);
xmlhttp3.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
xmlhttp3.send('file_id='+file_id+'&file_name='+encodeURI(file_name)+'&start_time='+start_time+'&specification='+encodeURI(specification)+'&cb_lifecycle='+cb_lifecycle+'&ca_lifecycle='+ca_lifecycle+'&change_reason='+encodeURI(change_reason)+'&change_date='+change_date+'&changer='+encodeURI(changer));
}
}
