...
xmlhttp3.open("POST", "reduceReady_search.jsp", true);
...
function pre_sub1(input_id){//减值准备金额框调用的计算方法
	...
	document.getElementById('sum_presub').value=FormatNumberPoint(parseFloat(input_value)+parseFloat(cb_sum_presub),2);
	if(input_value=='') 		document.getElementById('sum_presub').value=FormatNumberPoint(parseFloat(cb_sum_presub),2);
...
var remnant_value=document.getElementById('remnant_value').value;
var ca_net_value=document.getElementById('ca_net_value').value;
var pre_sub=document.getElementById('pre_sub').value;
...
alert( xmlhttp3.status + '=' + xmlhttp3.statusText);}} catch(exception) {alert(exception);}}};
xmlhttp3.open("POST","../../../finance_fixed_assets_change_reduceReady_ok", true);
xmlhttp3.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
xmlhttp3.send('file_id='+file_id+'&file_name='+encodeURI(file_name)+'&start_time='+start_time+'&specification='+encodeURI(specification)+'&currency='+encodeURI(currency)+'&currency_rate='+currency_rate+'&original_value='+original_value+'&caled_sum='+caled_sum+'&sum_presub='+sum_presub+'&sum_re_presub='+sum_re_presub+'&ca_net_value='+ca_net_value+'&pre_sub='+pre_sub+'&change_reason='+encodeURI(change_reason)+'&change_date='+change_date+'&changer='+encodeURI(changer));
}
