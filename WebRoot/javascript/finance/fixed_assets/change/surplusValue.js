...
xmlhttp3.open("POST", "surplusValue_search.jsp", true);
...
var options =xmlhttp3.responseText.split('⊙');
	...
	options[3]=parseFloat(options[3])*100;
	document.getElementById('cb_remnant_value_rate').value=options[3];
	document.getElementById('cb_remnant_value').value=options[4];
	document.getElementById('original_value').value=options[5];
	document.getElementById('net_value').value=options[6];
}else {
alert( xmlhttp3.status + '=' + xmlhttp3.statusText);}} catch(exception) {alert(exception);}}};
xmlhttp3.open("POST", "surplusValue_search.jsp", true);
xmlhttp3.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
function ca_remnant_value_rate1(input_id){//变动后的净残值率框调用的计算方法
	var input_value=decimalControl(input_id);
	document.getElementById('ca_remnant_value').value=FormatNumberPoint(parseFloat(document.getElementById('original_value').value)*parseFloat(input_value)/100,2);
	if(input_value=='') document.getElementById('ca_remnant_value').value='';
} 

function ca_remnant_value_rate2(input_id){//验证净残值率数据的合法性
	var input_value=decimalControl(input_id);
	if(input_value<0||input_value>=100){
	alert('净残值率必须大于"0"且小于"100"！');
	document.getElementById('ca_remnant_value_rate').focus();
	}
}

function ca_remnant_value1(input_id){//变动后的净残值框调用的计算方法
	var input_value=decimalControl(input_id);
	document.getElementById('ca_remnant_value_rate').value=FormatNumberPoint(parseFloat(input_value)/parseFloat(document.getElementById('original_value').value)*100,2);
	if(input_value=='') document.getElementById('ca_remnant_value_rate').value='';
}

function ca_remnant_value2(input_id){//验证净残值数据的合法性
	var input_value=decimalControl(input_id);
	if(parseFloat(input_value)>parseFloat(document.getElementById('net_value').value)){
		alert('净残值不能大于净值！');
		document.getElementById('ca_remnant_value').focus();
	}
}
...
function surplusValue_ok(){//提交数据库
...
var net_value=document.getElementById('net_value').value;
var cb_remnant_value_rate=document.getElementById('cb_remnant_value_rate').value;
var ca_remnant_value_rate=parseFloat(document.getElementById('ca_remnant_value_rate').value)/100;
...
alert( xmlhttp3.status + '=' + xmlhttp3.statusText);}} catch(exception) {alert(exception);}}};
xmlhttp3.open("POST", "../../../finance_fixed_assets_change_surplusValue_ok", true);
xmlhttp3.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
xmlhttp3.send('file_id='+file_id+'&file_name='+encodeURI(file_name)+'&start_time='+start_time+'&specification='+specification+'&net_value='+net_value+'&cb_remnant_value_rate='+cb_remnant_value_rate+'&ca_remnant_value_rate='+ca_remnant_value_rate+'&cb_remnant_value='+cb_remnant_value+'&ca_remnant_value='+ca_remnant_value+'&original_value='+original_value+'&change_reason='+encodeURI(change_reason)+'&change_date='+change_date+'&changer='+encodeURI(changer));
}
