...
function changeTag(){//判断使用状态，是否需要计算
	...
	if(!document.getElementById('unit_cal').value=='') document.getElementById('unit_cal').value=0;
	}else{
	if(!document.getElementById('caled_subtotal_rate').value=='')  document.getElementById('caled_subtotal_rate').value='0';
	if(!document.getElementById('caled_subtotal').value=='')  document.getElementById('caled_subtotal').value='0';
	}
}
...
xmlhttp3.open("POST", "registerInit_getpara.jsp", true);
...
if(array1[i]=='单位折旧'){
    if(document.getElementById('unit_cal').value==''){
    	array1[i]='√';
    	}else{
    	array1[i]=document.getElementById('unit_cal').value;
    	}
    }
    }
    return array1;
...
function net_validate(obj){//验证净值合法性
	if(parseFloat(obj.value)>parseFloat(document.getElementById('original_value').value)){
		alert('净值不能大于原值！');
		obj.focus();
	}else if(parseFloat(obj.value)<parseFloat(document.getElementById('remnant_value').value)){
		alert('净残值不能大于净值！');
		obj.focus();
...
function remnant_validate(obj){//验证净残值合法性
	...
	if(parseFloat(obj.value)>=parseFloat(document.getElementById('original_value').value)){
		alert('净残值不能大于等于原值！');
		obj.focus();
	}else if(parseFloat(obj.value)>parseFloat(document.getElementById('net_value').value)){
		alert('净残值不能大于净值！');
		obj.focus();
	}
}

function registerInit_ok(){//提交数据库
 //if (doValidate("")!=false){
...
alert( xmlhttp3.status + '=' + xmlhttp3.statusText);}} catch(exception) {alert(exception);}}};
xmlhttp3.open("POST", "../../finance_fixed_assets_registerInit_ok", true);
xmlhttp3.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
/*
function toolTip(msg, fg, bg,id){
if(id=='0'){
	document.getElementById('tooltip').innerHTML='<table id="msg1" name="msg1" cellspacing="0" cellpadding="1" bgcolor="' + fg + '"><tr><td <%=TD_STYLE3%> class="TD_STYLE3">' +
 '<table cellspacing="0" cellpadding="3" bgcolor="' + bg + 
 '"><tr style=\"height:25px\"><td <%=TD_STYLE3%> class="TD_STYLE3"><div align="center"><font face="Arial" width=100% color="' + fg +
 '">' + msg + '</td></tr></table></td></tr></table>';
 	document.getElementById('tooltip').style.top='85px';
 	document.getElementById('tooltip').style.left='255px';
 ...
 '">' + msg + '</td></tr></table></td></tr></table>';
 	document.getElementById('tooltip').style.top='85px';
 	document.getElementById('tooltip').style.left='580px';
 	document.getElementById('tooltip').style.display='block';
 	}
 }
 */
function closefile(tag1){
if(tag1=='1') document.getElementById('nseer1').style.display='none';
if(tag1=='2') document.getElementById('nseer2').style.display='none';
if(tag1=='3') document.getElementById('nseer3').style.display='none';
if(tag1=='4') document.getElementById('nseer4').style.display='none';
