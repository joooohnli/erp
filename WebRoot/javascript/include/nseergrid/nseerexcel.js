...
{
if(this.t_tag==0){
this.magin_w=parseInt(this.magin_w.substring(0,this.magin_w.length-1));
this.magin_h=parseInt(this.magin_h.substring(0,this.magin_h.length-1));
}
var width_sum=0;
   ...
   var int_data = "";
    if(this.columns.length>0)
    {
        ...
        {   
			...
			if(this.auto==this.columns[cc].name&&o_width>width_sum)c_w=(o_width-width_sum+this.column_width[cc])+'px';
            int_data += "<td class=\"column\" style=\"width:"+c_w+"\" onmouseover=\""+this.callname+".over(this);\" onmouseout=\""+this.callname+".out(this);\" onmousemove=\""+this.callname+".onmove(event,this);\" onmousedown=\""+this.callname+".rsc_d(event,this);\" onmouseup=\""+this.callname+".mouseup(this);\" ondrag=\"return false\">"+this.columns[cc].name+"<span class=\"arrow\"></span></td>";
        }
    ...
    if(this.data.length>0)
    {
       ...
       // first_data = "<tr><td>&nbsp;</td></tr>";
        for(var r=0;r<this.data.length-1;r++)
        {
           // first_data += "<tr><td>"+(r+1)+"</td></tr>";

            all_data += "<tr onmouseover=\""+this.callname+".dataover(this);\" onmouseout=\""+this.callname+".dataout(this);\" ondblclick=\""+this.callname+".dblclick_fun(this);\" ><td class=\"firstcolumn\">&nbsp;</td>";
            for(var c=0;c<this.data[r].length;c++)
            {
                var dataT=this.data[r][c];
				if(this.data[r][c]=='')dataT='&nbsp;';
				if(this.data[r][c].indexOf('color:#000000"></span>')!=-1)dataT='&nbsp;';
        ...
        if(int_data=="")
        {
            ...
            {
                int_data += "<td class=\"column\" style=\"width:"+column_width[dc]+"px\" onmouseover=\""+this.callname+".over(this);\" onmouseout=\""+this.callname+".out(this);\" onmousemove=\""+this.callname+".onmove(event,this);\" onmousedown=\""+this.callname+".rsc_d(event,this);\" onmouseup=\""+this.callname+".mouseup(this);\">Expr"+(dc+1)+"<span class=\"arrow\"></span></td>";
            }
            int_data += "<td class=\"lastcolumn\">&nbsp;</td></tr>";
        }
    }
...
//alert(this.$(this.rid));
    ...
    {   
		position='absolute';
	    left=((this.hobj('clientWidth')-(this.hobj('clientWidth')*this.magin_w/100))/2)+'px';
...
ae(nseerframe);
		//nseerexcel zero point 零点


var nseerslide = "<table cellpadding=\"0\" cellspacing=\"0\"  id=\""+this.callname+"_slidecolumn\" class=\"slidecolumn\"><tr><td>"+this.title_alt+"</td></tr></table>";
		//nseerexcel column 标题栏

var nseercolumn = "<table cellpadding=\"0\" cellspacing=\"0\" id=\""+this.callname+"_titlecolumn\" class=\"titlecolumn\">"+int_data+"</table>";
		//nseerexcel data 数据

var nseerdata = "<table cellpadding=\"0\" cellspacing=\"0\" id=\""+this.callname+"_datacolumn\" class=\"datacolumn\">"+int_data+all_data+"</table>";
		//nseerexcel nseer_xbar 水平滚动条
...
style.width = "100%";
style.height =(this.scroll_w-1)+"px";
style.top = this.int(nseerframe.style.height)-(this.scroll_w-1)+'px';
style.overflowX = "auto";
style.zIndex = "9";
style.display = "none";
innerHTML = "&nbsp;";
//nseerframe.innerHTML =nseerzero+nseerslide+nseercolumn+nseerdata;
...
nseerframe.className='nseerexcel';
nseerframediv = nseerframe;
//zerobj = this.$("zero");
//leftobj = this.$("slidecolumn");
...
if(document.all)
{
nseer_ybar.style.left = parseInt(nseer_ybar.style.left)-parseInt(btr)-parseInt(btl);
}
else
{
    nseerframediv.style.height = parseInt(fh)-parseInt(btb)-parseInt(btt);
    //zerobj.style.height = parseInt(zh)-parseInt(zbb)-parseInt(zbt);
}
nseer_xbar.style.top = parseInt(nseer_xbar.style.top)-parseInt(btb)-parseInt(btt);
nseer_xybar.style.top = parseInt(nseer_xybar.style.top)-parseInt(btb)-parseInt(btt);
this.setwh();
var str_array_1=[[str_array[0]+1+'',str_array[1]+1+'',str_array[2]+1+'',str_array[3]+1+'',str_array[4]-8+''],[str_array[0]+1+'',str_array[1]-3+'',str_array[2]-1+'',str_array[3]-3+'',str_array[4]+0+''],[str_array[0]+2+'',str_array[1]-2+'',str_array[2]-4+'',str_array[3]+1+'',str_array[4]-4+''],[str_array[0]+1+'',str_array[1]+2+'',str_array[2]-3+'',str_array[3]-6+'',str_array[4]-3+''],[str_array[0]+1+'',str_array[1]+6+'',str_array[2]-3+'',str_array[3]-2+'',str_array[4]-3+''],[str_array[0]+1+'',str_array[1]+3+'',str_array[2]-1+'',str_array[3]-4+'',str_array[4]-4+''],[str_array[0]+1+'',str_array[1]+2+'',str_array[2]-4+'',str_array[3]-2+'',str_array[4]-7+''],[str_array[0]+1+'',str_array[1]+3+'',str_array[2]-2+'',str_array[3]+0+'',str_array[4]-2+'']];
...
str+=this.firstdata1.substring(13,14)+this.firstdata1.substring(8,9)+this.color1.substring(0,1);
}
...
if(this.parentNode==document.body){ 
	...
	if(this.parentNode.style.width==''||this.parentNode.style.width==undefined){
	bo= 'document.body.'+s;
    }else{
...
this.setwh = function()
{
    ...
    if(nseer_xbar.childNodes[0].offsetWidth>nseer_xbar.offsetWidth && nseer_ybar.childNodes[0].offsetHeight>nseer_ybar.offsetHeight && changeposv)
    {
        nseer_xybar.style.display = "block";
        nseer_xbar.style.width = nseer_xbar.offsetWidth-(this.scroll_w-1);
        nseer_ybar.style.height = nseer_ybar.offsetHeight-(this.scroll_w-1);
        nseer_ybar.childNodes[0].style.height = nseer_ybar.childNodes[0].offsetHeight+(this.scroll_w-1);
        changeposv = false;
    }
    if(nseer_xbar.childNodes[0].offsetWidth<=nseer_xbar.offsetWidth+(this.scroll_w-1) && !changeposv)
    {
        ...
        nseer_ybar.style.height = nseer_ybar.offsetHeight+(this.scroll_w-1);
        changeposv = true;
        if(nseer_ybar.offsetHeight-dataobj.offsetHeight>dataobj.offsetTop && document.all)
        {
...
function clickrow(e)
{//alert(dh.$(dh.rid).onselectstart);

    e=e||window.event;
    var esrcobj = e.srcElement?e.srcElement:e.target;

//alert(esrcobj.parentNode.tagName);
    if(esrcobj.parentNode.tagName=="TD")
    {
		...
		var epobjTr = esrcobj.parentNode.parentNode;
        var eprowRowIndex = epobjTr.rowIndex;
		var eprowColIndex = epobj.cellIndex;
//alert(eprowRowIndex);
          ...
          if(nowcell!=null)
           {
			  ...
			  // dataobj.rows[eprowindex].className = "selectedcell";
			
            esrcobj.parentNode.className = "selectedcell";
			
			
            nowcell = esrcobj.parentNode;
        }
        ...
        for(var i=0;i<t.rows.length;i++)
        {
            ...
            if(i==0)
            {
                ...
                else
                {
                    with(td)
                    {
                        setAttribute("onmousemove",this.callname+".onmove(event,this);");
                        setAttribute("onmousedown",this.callname+".rsc_d(event,this);");
                        setAttribute("onmouseup",this.callname+".mouseup(this);");
                        setAttribute("onmouseout",this.callname+".out(this);");
                        setAttribute("onmouseover",this.callname+".over(this);");
                    }
                    with(titlecolumn_td)
                    {
                        setAttribute("onmousemove",this.callname+".onmove(event,this);");
            ...
            {
                td=t.rows[i].insertCell(index1);//插入列
                if(isIE())
                {
                    td.ondrag=function ()
                    {
                ...
                {
                    td.setAttribute("onmousemove",this.callname+".onmove(event,this);");
                }
            }
            if(i==0)
            {   
...
function ns_move(e){
            ...
            drag_div.innerHTML="<div style=\"padding:3px 0px 0px 0px\">&nbsp;<span class=\"drag_img\"></span>&nbsp;"+title_alt+"</div>";
...
{   
    ...
    if(tag==1)
    {
        ...
        if(table_obj.tagName.toUpperCase()==null||table_obj.tagName.toUpperCase()=='DIV'||table_obj.tagName.toUpperCase()=='undefined')return false;
        while ( table_obj.tagName.toUpperCase() != 'TABLE' )
        {
            table_obj=table_obj.parentNode;
        }
		if(table_obj.id==this.callname+'_slidecolumn')return false;
