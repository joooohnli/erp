...
for(var i=1;i<rows_num;i++){
	if(document.getElementById('draft_gar'+i).checked){
			if(document.getElementById('draft_gar'+i).value.split('⊙')[1]=='3'){
...
xmlhttp3.open("POST","../../intrmanufacture_invoice_querySendToGar_ok", true);
xmlhttp3.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
xmlhttp3.send('ids_str='+encodeURI(ids_str.substring(1))+'&&tableName='+encodeURI(tableName));
}
}

function showCompulsion(value,tableName){/*强制完成*/
readXml('../../css/include/nseer_cookie/xml-css.css','../../xml/include/draft_gar/compulsion.xml','0');
//multiLangValidate.dwrGetLang("erp",value,{callback:function(msg){document.getElementById('prompt_span').innerHTML=msg;}});
document.getElementById('div_rows_num').value=value;
document.getElementById('tableName').value=tableName;
} 
...
if(ids_str==''&&ids_str1==''){
	multiLangValidate.dwrGetLang("erp",'请您选择要强制完成的记录',{callback:function(msg){n_A.divShow(msg);}});
	n_D.closeDiv('remove');
	return false;
}else if(ids_str==''&&ids_str1!=''){
	multiLangValidate.dwrGetLang("erp",'您所选记录均不能强制完成',{callback:function(msg){n_A.divShow(msg);}});
	n_D.closeDiv('remove');
	return false;
}else{
...
if (xmlhttp3.readyState==4){try {if(xmlhttp3.status==200){
if(parseInt(xmlhttp3.responseText)==1){
multiLangValidate.dwrGetLang("erp",'您所选记录已强制完成',{callback:function(msg){n_A.divShow(msg); window.location.reload();}});
}
}else{alert( xmlhttp3.status + '=' + xmlhttp3.statusText);}} catch(exception) {alert(exception);}}};
xmlhttp3.open("POST","../../intrmanufacture_invoice_compulsion_ok", true);
xmlhttp3.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
xmlhttp3.send('ids_str='+encodeURI(ids_str.substring(1))+'&&tableName='+encodeURI(tableName));
}
}
