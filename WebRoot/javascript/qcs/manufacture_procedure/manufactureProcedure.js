...
function search_suggest(input_id,div_obj,param){//根据输入内容快速查询下拉层
	var search_tag=0;
	if(input_id=='sampling_standard'){
		search_tag=1;
	}else if(input_id=='quality_solution'){
		search_tag=3;
	}else if(input_id.indexOf('quality_result')!=-1){
		search_tag =6;
	}else if(input_id=='qcs_result'){
         search_tag=4;
	}else if(input_id=='procedure_name'){
         search_tag=7;
	}
	var xmlhttp;
		if(window.XMLHttpRequest){xmlhttp=new XMLHttpRequest();}else if (window.ActiveXObject){xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");}if(xmlhttp) {xmlhttp.onreadystatechange = function() {
			if(xmlhttp.readyState==4){try {if(xmlhttp.status==200){
				...
				suggest += 'onclick=\"javascript:setSearch(this.innerHTML,\''+input_id+'\',\''+div_obj.id+'\',\''+param+'\');\" ';
			...
			xmlhttp.send('param='+encodeURI(param)+'&search_tag='+encodeURI(search_tag));
		}
	}

function setSearch(input_value,input_id,div_id,param){
	...
	if(window.XMLHttpRequest){xmlhttp=new XMLHttpRequest();}else if (window.ActiveXObject){xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");}if(xmlhttp) {xmlhttp.onreadystatechange = function() {
		...
		xmlhttp.open("POST", "register_search.jsp", true);
		xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		xmlhttp.send('solution_id='+encodeURI(solution_id)+'&search_tag=2');}
	...
	if(window.XMLHttpRequest){xmlhttp=new XMLHttpRequest();}else if (window.ActiveXObject){xmlhttp=new ActiveXObject("Microsoft.XMLHTTP");}if(xmlhttp) {xmlhttp.onreadystatechange = function() {
		...
		xmlhttp.open("POST", "register_search.jsp", true);
		xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		xmlhttp.send('standard_id='+encodeURI(standard_id)+'&quality_amount='+encodeURI(param)+'&search_tag=5');}
	}else{
    document.getElementById(input_id).value = input_value;
	document.body.removeChild(document.getElementById(div_id));
	}	
...
function formSubmit(){
	...
	if (action_tag == 'register') {
	    form_id='manufactureProcedure_register';
		form = document.getElementById('manufactureProcedure_register');
		form.action = '../../qcs_manufacture_procedure_register_ok';
	}else if (action_tag == 'change') {
	    form_id='manufactureProcedure_change';
		form = document.getElementById('manufactureProcedure_change');
		form.action = '../../qcs_manufacture_procedure_change_ok';
	}else if (action_tag == 'dealwith') {
	    form_id='manufactureProcedure_dealwith';
		form = document.getElementById('manufactureProcedure_dealwith');
		form.action = '../../qcs_manufacture_procedure_dealwith_ok';
	}else if (action_tag == 'uncheck') {
	    form_id='manufactureProcedure_uncheck';
		form = document.getElementById('manufactureProcedure_uncheck');
		form.action = '../../qcs_manufacture_procedure_uncheck_ok';
	}else if (action_tag == 'uncheck_key') {
	    form_id='manufactureProcedure_uncheck_key';
		form = document.getElementById('manufactureProcedure_uncheck_key');
		form.action = '../../qcs_manufacture_procedure_uncheck_key_ok';
	}
