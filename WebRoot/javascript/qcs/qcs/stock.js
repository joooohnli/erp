...
function showBill(num){
	...
	document.getElementById('div_stock_id_hidden').value=document.getElementById('stock_id_hidden'+num).value;
	...
	document.getElementById('div_nick_name').value=document.getElementById('nick_name'+num).innerHTML;
	document.getElementById('div_amount').value=document.getElementById('amount'+num).innerHTML;
	document.getElementById('div_label').value=num.length==1?'0'+num:num;	
}
function registerOk(form_id){
	var form=document.getElementById(form_id);
	form.action='../../qcs_qcs_stock_register_ok';
	form.submit();
}
function showApplyOk(id){
if(document.getElementById('stock_qcs_way').value.substring(0,2)=='03'){
	product_id=id;
	readXml('../../css/include/nseer_cookie/xml-css.css','../../xml/qcs/qcs/apply_ok.xml','0');
	}else{
	var alert_sentence='该产品为非免检产品，请完成质检申请';
...
window.location='stock_register_ok.jsp?finished_tag=0';
}else {
alert( xmlhttp3.status + '=' + xmlhttp3.statusText);}} catch(exception) {alert(exception);}}};
xmlhttp3.open("POST", "../../qcs_qcs_stock_apply_ok", true);
xmlhttp3.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
xmlhttp3.send('product_id='+encodeURI(product_id));
}
}