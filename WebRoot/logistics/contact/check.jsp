...
String sql9="select id from logistics_workflow where object_ID='"+contact_ID+"' and check_tag='0' and config_id<'"+config_id+"'";
ResultSet rs9=logistics_db.executeQuery(sql9);
if(!rs9.next()){
if(vt.validata((String)session.getAttribute("unit_db_name"),"logistics_contact","contact_ID",contact_ID,"check_tag").equals("0")){
try{
	String sqll = "select * from logistics_contact where contact_ID='"+contact_ID+"'" ;
	ResultSet rss = logistics_db.executeQuery(sqll) ;
	if(rss.next()){
...
form.action = "../../logistics_contact_check_ok?config_id=<%=config_id%>&contact_ID=<%=rss.getString("contact_ID")%>";
}
}
</script>
...
<form id="mutiValidation" method="post" onSubmit="return doValidate('../../xml/logistics/logistics_contact.xml','mutiValidation')&&TwoSubmit(this);">
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 ...
 <%=demo.getLang("erp","通过")%> <input type="submit" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","确认")%>" name="B1">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></div></td>
 </tr>
</table>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="provider_ID" type="hidden" value="<%=rss.getString("provider_ID")%>"><input name="reasonexact" type="hidden" value="<%=exchange.toHtml(rss.getString("reasonexact"))%>"><%=rss.getString("reasonexact")%>&nbsp;</td>
 </tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
