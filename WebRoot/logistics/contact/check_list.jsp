...
String rowSummary=demo.getLang("erp","当前等待审核的物流单位联络总数");
%>
...
String sql="select count(id) from logistics_config_workflow where type_id='04'";
...
nseer_grid.column_width=[180,200,80,180,80,80,160,<%for(int i=1;i<workflow_amount;i++){%>70,<%}%>70];
...
String sql1="select id from logistics_workflow where object_ID='"+rs.getString("contact_ID")+"' and check_tag='0'";
ResultSet rs2=logistics_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from logistics_workflow where object_ID='"+rs.getString("contact_ID")+"' order by id";
	rs2=logistics_db1.executeQuery(sql1);
%>
	['<span style="color:<%=color%>"><%=rs.getString("contact_ID")%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs.getString("provider_name"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs.getString("reason"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs.getString("reasonexact"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs.getString("person_contacted"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs.getString("contact_person"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(contact_time)%></span>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
