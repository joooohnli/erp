...
<form class="x-form" method="post" action="../../logistics_file_check_delete_ok">
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="submit" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","确认")%>"> 
 <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></div></td>
 </tr>
 </table>
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3" height="10"><%=demo.getLang("erp","该物流单位档案审核未通过，您确认吗？请选择处理：")%></td>
 <td <%=TD_STYLE3%> class="TD_STYLE3" height="10">
...
<%
int i=1;
String sql6="select config_id from logistics_workflow where object_ID='"+provider_ID+"' and config_id<'"+config_id+"' order by id";
