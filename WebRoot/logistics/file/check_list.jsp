...
String sql="select count(id) from logistics_config_workflow where type_id='01'";
...
nseer_grid.column_width=[180,200,100,100,100,<%for(int i=1;i<workflow_amount;i++){%>70,<%}%>70];
nseer_grid.auto='<%=demo.getLang("erp","产品分类")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>"> 
<%
String sql1="select id from logistics_workflow where object_ID='"+rs.getString("provider_ID")+"' and check_tag='0'";
ResultSet rs2=logistics_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from logistics_workflow where object_ID='"+rs.getString("provider_ID")+"' order by id";
	rs2=logistics_db1.executeQuery(sql1);
%>
['<%=rs.getString("provider_ID")%>','<%=exchange.toHtml(rs.getString("provider_name"))%>','<%=exchange.toHtml(rs.getString("provider_class"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("logisticser"))%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
