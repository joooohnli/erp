...
String sql9="select * from logistics_workflow where object_ID='"+product_logistics_recommend_ID+"' and check_tag='0' and config_id<'"+config_id+"'";
ResultSet rs9=logisticsdb.executeQuery(sql9);
if(!rs9.next()){
if(vt.validata((String)session.getAttribute("unit_db_name"),"logistics_product_providers_recommend","product_logistics_recommend_ID",product_logistics_recommend_ID,"check_tag").equals("0")){
try{
...
form.action = "check_delete_reconfirm.jsp?product_logistics_recommend_ID=<%=rs.getString("product_logistics_recommend_ID")%>&config_id=<%=config_id%>";
}else{
form.action = "../../logistics_product_providers_check_ok?product_logistics_recommend_ID=<%=rs.getString("product_logistics_recommend_ID")%>&config_id=<%=config_id%>";
}
}
</script>
...
<form id="mutiValidation" method="POST" onSubmit="return doValidate('../../xml/logistics/logistics_product_providers_recommend.xml','mutiValidation')&&TwoSubmit(this)">
