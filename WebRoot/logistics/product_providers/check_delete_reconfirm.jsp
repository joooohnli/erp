...
<form class="x-form" id="form1" method="post" action="../../logistics_product_providers_check_delete_ok">
<input type="hidden" name="product_logistics_recommend_ID" value="<%=product_logistics_recommend_ID%>">
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="submit" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","确认")%>" name="B1"> 
 <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check.jsp?product_logistics_recommend_ID=<%=product_logistics_recommend_ID%>"></div></td>
 </tr>
</table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3" height="10"><%=demo.getLang("erp","该产品物流单位推荐单未通过审核，您确认吗？")%></td>
 </tr>
<%
int i=1;
String sql6="select config_id from logistics_workflow where object_ID='"+product_logistics_recommend_ID+"' and config_id<'"+config_id+"' order by id";
