...
String condition="check_tag='1' and recommend_logistics_tag='0' and (type!='物料')";
...
nseer_grid.columns =[
                       ...
                       {name: '<%=demo.getLang("erp","推荐物流单位")%>'}
]
nseer_grid.column_width=[180,200,80,100,150,80];
nseer_grid.auto='<%=demo.getLang("erp","产品分类")%>';
nseer_grid.data = [
<page:pages rs="<%=rs1%>" strPage="<%=strPage%>"> 
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../design/file/query.jsp?product_ID=<%=rs1.getString("product_ID")%>")><%=rs1.getString("product_ID")%></div>','<%=exchange.toHtml(rs1.getString("product_name"))%>','<%=exchange.toHtml(rs1.getString("type"))%>','<%=exchange.toHtml(rs1.getString("product_class"))%>','<%=exchange.toHtml(rs1.getString("chain_name"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("register.jsp?product_ID=<%=rs1.getString("product_ID")%>&&product_name=<%=toUtf8String.utf8String(exchange.toURL(rs1.getString("product_name")))%>")><%=demo.getLang("erp","推荐物流单位")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
