...
<form id="mutiValidation" method="POST" action="../../manufacture_apply_register_ok" onSubmit="return doValidate('../../xml/manufacture/manufacture_apply.xml','mutiValidation')">
...
choice_group=", "+choice_group+", ";
	...
	while(rs.next()){
		double amount1=0.0d;
		String sql1="select sum(apply_manufacture_amount) from stock_pay_details where product_ID='"+rs.getString("product_ID")+"' and ("+condition+")";
		ResultSet rs1 = stock_db.executeQuery(sql1) ;
		if(rs1.next()){
			amount1=rs1.getDouble("sum(apply_manufacture_amount)");
		}
...
for(i=0;i<m;i++){
	String sql2="select distinct pay_ID,product_ID,product_name,product_describe,type from stock_pay_details where product_ID='"+product_ID1[i]+"'";
	...
	while(rs2.next()){
		...
		type[i]=rs2.getString("type");
		String sql3="select sum(apply_manufacture_amount) from stock_pay_details where product_ID='"+product_ID1[i]+"' and pay_ID='"+rs2.getString("pay_ID")+"'";
		ResultSet rs3 = stock_db.executeQuery(sql3) ;
		if(rs3.next()){
			amount2=rs3.getDouble("sum(apply_manufacture_amount)");
		}
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="89%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="designer" type="text" style="width:44.8%"></td>
</tr>
</table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<TABLE <%=TABLE_STYLE5%> class="TABLE_STYLE5">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%" colspan="3"><%=demo.getLang("erp","出库单编号集合")%></td>
 </tr>
<%
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="hidden" name="product_name" value="<%=exchange.toHtml(product_name[j])%>"><%=exchange.toHtml(product_name[j])%>&nbsp;</td>
	 ...
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="hidden" name="type" value="<%=exchange.toHtml(type[j])%>"><%=exchange.toHtml(type[j])%>&nbsp;</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="hidden" name="amount" value="<%=amount[j]%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(amount[j])%>&nbsp;</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" colspan="3"><input type="hidden" name="pay_ID_group" value="<%=pay_ID_group[j]%>"><%=pay_ID_group[j]%>&nbsp;</td>
 </tr>
<%
	}
%>
...
<textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="remark" cols="115" rows="4"></textarea>
</td>
 </tr>
</table>
