...
String condition="check_tag='1' and excel_tag='2' and module_gar_tag='0' and (design_module_tag='2' or design_module_tag='3')";
...
if(rs1.getString("design_module_change_tag").equals("1")){
	color="#FF9A31";
}
%>
	['<span style="color:<%=color%>" ><%=rs1.getString("design_ID")%></span>','<span style="color:<%=color%>"><%=rs1.getString("product_ID")%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("product_name"))%></span>','<span style="color:<%=color%>" ><%=exchange.toHtml(rs1.getString("designer"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(register_time)%></span>','<span style="color:<%=color%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("cost_price_sum"))%></span>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?design_ID=<%=rs1.getString("design_ID")%>")><%=demo.getLang("erp","变更")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
