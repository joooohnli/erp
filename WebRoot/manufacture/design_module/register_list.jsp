...
String condition="check_tag='1' and module_gar_tag='0' and design_module_tag='0'";
...
String rowSummary=demo.getLang("erp","符合条件的工序设计单总数：");
%>
...
<page:pages rs="<%=rs1%>" strPage="<%=strPage%>"> 
	['<%=rs1.getString("design_ID")%>','<%=rs1.getString("product_ID")%>','<%=exchange.toHtml(rs1.getString("product_name"))%>','<%=exchange.toHtml(rs1.getString("designer"))%>','<%=exchange.toHtml(rs1.getString("register_time"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("cost_price_sum"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("register.jsp?design_ID=<%=rs1.getString("design_ID")%>")><%=demo.getLang("erp","制定")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
