...
String condition="check_tag='1' and design_procedure_tag='0' and (type='商品' or type='部件')";
...
<page:pages rs="<%=rs1%>" strPage="<%=strPage%>"> 
	['<div style="text-decoration : underline;color:#3366FF" onclick=winopen("../../design/file/query.jsp?product_ID=<%=rs1.getString("product_ID")%>")><%=rs1.getString("product_ID")%></div>','<%=exchange.toHtml(rs1.getString("product_name"))%>','<%=exchange.toHtml(rs1.getString("product_class"))%>','<%=exchange.toHtml(rs1.getString("chain_name"))%>','<%=exchange.toHtml(rs1.getString("responsible_person_name"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link1("<%=rs1.getString("product_ID")%>","<%=toUtf8String.utf8String(exchange.toURL(rs1.getString("product_name")))%>","<%=toUtf8String.utf8String(exchange.toURL(rs1.getString("responsible_person_name")))%>")><%=demo.getLang("erp","制定设计单")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
