...
String rowSummary=demo.getLang("erp","当前等待审核的生产派工单总数：");
%>
...
String sql="select count(id) from manufacture_config_workflow where type_id='04'";
...
nseer_grid.column_width=[180,180,200,70,180,160,
<%
for(int i=1;i<workflow_amount;i++){
%>					   70,
<%
...
String sql1="select id from manufacture_workflow where object_ID='"+rs.getString("manufacture_ID")+"' and check_tag='0'";
ResultSet rs2=manufacture_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from manufacture_workflow where object_ID='"+rs.getString("manufacture_ID")+"' order by id";
	rs2=manufacture_db1.executeQuery(sql1);
%>
	['<%=rs.getString("manufacture_ID")%>','<%=exchange.toHtml(rs.getString("product_ID"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs.getDouble("amount"))%>','<%=rs.getString("pay_ID_group")%>','<%=exchange.toHtml(register_time)%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?manufacture_ID=<%=rs.getString("manufacture_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
