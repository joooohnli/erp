...
nseer_grid.columns =[
                       ...
                       {name: '<%=demo.getLang("erp","投产数量")%>'},
	                   {name: '<%=demo.getLang("erp","合格数量")%>'},
                       {name: '<%=demo.getLang("erp","派工单状态")%>'},
	                   {name: '<%=demo.getLang("erp","审核状态")%>'},
	                   {name: '<%=demo.getLang("erp","生产状态")%>'}
]
nseer_grid.column_width=[40,180,180,70,70,70,70,70,70];
...
manufacture_tag="等待";
}else if(rs1.getString("check_tag").equals("1")&&rs1.getString("manufacture_tag").equals("0")){
manufacture_tag="执行";
color="mediumseagreen";
}else if(rs1.getString("check_tag").equals("1")&&rs1.getString("manufacture_tag").equals("1")){
manufacture_tag="完成";
delete_tag="0";
color="3333FF";
}
...
if(rs1.getDouble("tested_amount")!=0){
	tested_amount=rs1.getString("tested_amount");
}
%>
	['<%if(manufacture_tag.equals("完成")){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs1.getString("id")%>⊙<%=delete_tag%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?manufacture_ID=<%=rs1.getString("manufacture_ID")%>")><%=rs1.getString("manufacture_ID")%></div>',' <span style="color:<%=color%>" ><%=rs1.getString("product_ID")%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("product_name"))%></span>','<span style="color:<%=color%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs1.getDouble("amount"))%></span>','<span style="color:<%=color%>" ><%=tested_amount%></span>','<span style="color:<%=color%>" ><%=manufacture_tag%></span>','<span style="color:<%=color%>" ><%=check_tag%></span>','<span style="color:<%=color%>" ><%=manufacture_procedure_tag%></span>'],
	<%k++;%>
</page:pages>
['']];
nseer_grid.init();
