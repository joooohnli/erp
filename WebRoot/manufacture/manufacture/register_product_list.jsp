...
<form id="register1" method="POST" action="register.jsp">
 ...
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2" width="100%">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","预览计划单")%>">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back()"></div></td> 
 </tr>
 </table>
...
choice_group=choice_group+choice[i];
	...
	String sql = "select distinct product_ID from stock_order_details "+condition+"" ;
	ResultSet rs = stockdb.executeQuery(sql) ;
	while(rs.next()){
		double amount1=0.0d;
		String sql1="select sum(amount) from stock_order_details where product_ID='"+rs.getString("product_ID")+"'";
...
for(i=0;i<m;i++){
	String sql4="select * from stock_order_details where product_ID='"+product_ID1[i]+"'";
	...
	String sql2="select distinct pay_ID from stock_order_details where product_ID='"+product_ID1[i]+"'";
	ResultSet rs2 = stockdb.executeQuery(sql2) ;
	while(rs2.next()){
		double amount2=0.0d;
		String sql3="select sum(amount) from stock_order_details where product_ID='"+product_ID1[i]+"' and pay_ID='"+rs2.getString("pay_ID")+"'";
...
<table <%=TABLE_STYLE1%> class="TABLE_STYLE1" width="100%" id=theObjTable>
 ...
 <td <%=TD_STYLE1%> class="TD_STYLE1"><font class="resizeDivClass" onmousedown="MouseDownToResize(this);" onmousemove="MouseMoveToResize(this);" onmouseup="MouseUpToResize(this);"></font><%=demo.getLang("erp","描述")%> </td>
	 <td <%=TD_STYLE1%> class="TD_STYLE1"><font class="resizeDivClass" onmousedown="MouseDownToResize(this);" onmousemove="MouseMoveToResize(this);" onmouseup="MouseUpToResize(this);"></font><%=demo.getLang("erp","erp")%></td>
