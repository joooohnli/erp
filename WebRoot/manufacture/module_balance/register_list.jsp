...
nseer_grid.column_width=[180,180,180,70,70,70,70];
...
%>
	['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../manufacture/query.jsp?manufacture_ID=<%=rs1.getString("manufacture_ID")%>")><%=rs1.getString("manufacture_ID")%></div>',' <span style="color:<%=color%>"><%=rs1.getString("product_ID")%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("product_name"))%></span>','<span style="color:<%=color%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs1.getDouble("amount"))%></span>','<span style="color:<%=color%>" ><%=manufacture_tag%></span>','<span style="color:<%=color%>" ><%=manufacture_procedure_tag%></span>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("register.jsp?manufacture_ID=<%=rs1.getString("manufacture_ID")%>")><%=demo.getLang("erp","登记")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
