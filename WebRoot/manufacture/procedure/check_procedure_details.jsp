...
String sql8="select * from manufacture_proceduring where manufacture_ID='"+manufacture_ID+"' and procedure_name='"+procedure_name+"' and check_tag='0' and details_number='"+details_number+"'";
ResultSet rs8=manufacture_db.executeQuery(sql8);
if(rs8.next()){
	String register_ID_group="";
	String sql="select * from manufacture_config_public_char where kind='工序' and type_name='"+procedure_name+"'";
	...
	double real_labour_hour_amount=0.0d;
	...
	if(rs2.next()){
		labour_hour_amount=rs2.getDouble("labour_hour_amount");
		real_labour_hour_amount=rs2.getDouble("real_labour_hour_amount");
	}
	String sql3="select * from manufacture_proceduring where manufacture_ID='"+manufacture_ID+"' and procedure_name='"+procedure_name+"' and check_tag='0' and details_number='"+details_number+"'";
...
<form id="mutiValidation" method="POST" action="../../manufacture_procedure_check_procedure_details_ok">
<table <%=TABLE_STYLE3%> class="TABLE_STYLE3">
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8"><input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","审核通过")%>">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="返回" onClick="history.back()"></td> 
 </tr>
</table>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input name="details_number1" type="hidden" value="<%=details_number%>"><input name="manufacture_ID" type="hidden" value="<%=manufacture_ID%>"><%=manufacture_ID%></td>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="procedure_responsible_person" type="text" value="<%=exchange.toHtml(rs3.getString("procedure_responsible_person"))%>"></td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","设计工时数")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><%=labour_hour_amount%></td>
 </tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
	 <td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","已用工时数")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input name="real_labour_hour_amount" type="hidden" value="<%=real_labour_hour_amount%>"><%=real_labour_hour_amount%>&nbsp;</td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","本次工时数")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="labour_hour_amount" type="text" value="<%=rs3.getDouble("labour_hour_amount")%>"></td>
 </tr>
<%}%>
</table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
...
<TABLE <%=TABLE_STYLE5%> class="TABLE_STYLE5">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="11%"><%=demo.getLang("erp","本次使用数量")%></td>
...
<input name="product_describe" type="hidden" value="<%=rs1.getString("product_describe")%>">
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input name="type" type="hidden" value="<%=exchange.toHtml(rs1.getString("type"))%>"><input name="design_amount" type="hidden" value="<%=rs1.getString("amount")%>"><%=rs1.getDouble("amount")%>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input name="extra_amount" type="hidden" value="<%=rs1.getString("extra_amount")%>"><%=rs1.getDouble("extra_amount")%>&nbsp;</td>
...
<%
	String sql4="select * from manufacture_procedure_moduling where manufacture_ID='"+manufacture_ID+"' and procedure_name='"+procedure_name+"' and details_number='"+rs1.getString("details_number")+"' and check_tag='0' order by id desc";
	ResultSet rs4=manufacturedb.executeQuery(sql4);
	if(rs4.next()){
	 %>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input <%=INPUT_STYLE5%> class="INPUT_STYLE5" name="amount" type="text" value="<%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs4.getDouble("amount"))%>"></td>
...
<div id="nseerGround" class="nseerGround">
 ...
 <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","对不起，您没有被授权进行本工序的审核，请返回。")%></td>
 </tr>
 </table>
</div>
<%}
