...
String sql9="select * from oa_workflow where object_ID='"+culture_ID+"' and type_id='05' and check_tag='0' and config_id<'"+config_id+"'";
ResultSet rs9=oadb.executeQuery(sql9);
if(!rs9.next()){
String sql="select * from oa_culture where culture_ID='"+culture_ID+"' and check_tag='0'";
...
form.action = "check_delete_reconfirm.jsp?culture_ID=<%=rs.getString("culture_ID")%>&config_id=<%=config_id%>";
}else{
form.action = "../../oa_culture_check_ok?culture_ID=<%=rs.getString("culture_ID")%>&config_id=<%=config_id%>";
}
}
</script>
<form id="subjectRegister" method="POST" onSubmit="return doValidate('../../xml/oa/oa_culture.xml','mutiValidation')&&TwoSubmit(this)">
...
<input type="hidden" name="culture_ID" value="<%=culture_ID%>">
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
	 <td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","该文化已审核，请返回！")%></td>
 </tr>
</table>
</div>
<%}}else{
