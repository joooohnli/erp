...
String rowSummary=demo.getLang("erp","当前等待审核的文化总数：");
%>
...
String sql="select count(id) from oa_config_workflow where type_id='05'";
...
nseer_grid.auto='<%=demo.getLang("erp","文化主题")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
	
<%
	String sql1="select id from oa_workflow where object_ID='"+rs.getString("culture_ID")+"' and type_id='05' and check_tag='0'";
ResultSet rs2=oa_db1.executeQuery(sql1);
if(rs2.next()){

	sql1="select check_tag,describe1,config_id from oa_workflow where object_ID='"+rs.getString("culture_ID")+"' and type_id='05' order by id";
...
%>
	,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?culture_ID=<%=rs.getString("culture_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}	
%>
</page:pages>
