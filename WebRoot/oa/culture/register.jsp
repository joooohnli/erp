...
<form id="mutiValidation" method="POST" action="../../oa_culture_register_ok" onSubmit="return doValidate('../../xml/oa/oa_culture.xml','mutiValidation')" ENCTYPE="multipart/form-data">
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
<tr <%=TR_STYLE1%> class="TR_STYLE1">
<td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><%=DgButton.getDraft("'mutiValidation','../../oa_culture_register_draft_ok','../../xml/oa/oa_culture.xml'",request)%>&nbsp;<input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","提交")%>"></div></td>
</tr>
</table>
<%
...
<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="culture_type">
<%
String sql5 = "select * from oa_config_public_char where kind='文化分类' order by type_name" ;
...
<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp","机构")%>：</td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="13%"><input id="fileKind_chain" <%=INPUT_STYLE3%> class="INPUT_STYLE3" style="width:100%;height:22;top:0px;" name="fileKind_chain" onFocus="showKind('nseer1',this,showTree)" onkeyup="search_suggest(this.id)" autocomplete="off"><input id="fileKind_chain_table" type="hidden" value="hr_config_file_kind"></td>
 </tr>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<script type='text/javascript' src="../../javascript/oa/culture/treeBusiness.js"></script>
...
function showTree(){
 ...
 Nseer_tree1.setModuleName('../../xml/oa/culture');
 Nseer_tree1.addRootNode('No0','<%=demo.getLang("erp","全部分类")%>',false,'1',[]);
initMyTree(Nseer_tree1);
createButton('../../xml/oa/culture','hr_config_file_kind','treeButton');
}
</script>
</div>
</TD>
