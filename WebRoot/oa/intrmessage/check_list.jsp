...
%>
 ...
 [' <div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?intrmessage_ID=<%=rs.getString("intrmessage_ID")%>")><%=exchange.toHtml(rs.getString("subject"))%></div>','<%=exchange.toHtml(rs.getString("type"))%>','<%=rs.getString("content")%>','<%=rs.getString("check_amount")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check_choose.jsp?intrmessage_ID=<%=rs.getString("intrmessage_ID")%>")><%=demo.getLang("erp","发送")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
