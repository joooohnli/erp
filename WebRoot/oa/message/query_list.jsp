...
%>
  ...
  ['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?message_ID=<%=rs.getString("message_ID")%>")><%=exchange.toHtml(rs.getString("subject"))%></div>','<%=exchange.toHtml(rs.getString("type"))%>','<%=rs.getString("content")%>','<%=rs.getString("check_amount")%>','<%=exchange.toHtml(status)%>'],
</page:pages>
['']];
nseer_grid.init();
</script>
