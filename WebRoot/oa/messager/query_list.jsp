...
String rowSummary=demo.getLang("erp","符合条件的群发对象总数：");
String status="";
%>
...
nseer_grid.column_width=[70,100,140,70,140,70,140];
...
%>
	['<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("tool_type"))%>','<%=rs.getString("content")%>','<%=exchange.toHtml(rs.getString("register"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<%=exchange.toHtml(rs.getString("changer"))%>','<%=exchange.toHtml(change_time)%>'],
