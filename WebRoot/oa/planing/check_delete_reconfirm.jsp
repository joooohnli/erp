...
<form class="x-form" method="post" action="../../oa_planing_check_delete_ok">
...
<input type="hidden" name="check_time" value="<%=check_time%>">
 ...
 <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></div></td>
  ...
  <table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3" height="10"><%=demo.getLang("erp","计划审核未通过，您确认吗？请选择处理：")%></td>
 <td <%=TD_STYLE3%> class="TD_STYLE3" height="10">&nbsp;</td>
 </tr>
<%
int i=1;
String sql6="select config_id from oa_workflow where object_ID='"+planing_ID+"' and type_id='01' and config_id<'"+config_id+"' order by id";
