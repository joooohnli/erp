...
String condition="check_tag='0' and subject!=''";
String queue="order by register_time";
String validationXml="../../xml/oa/oa_planing.xml";
String nickName="计划";
String fileName="check_list.jsp";
String rowSummary=demo.getLang("erp","当前等待审核的计划总数：");
%>
...
String sql="select count(id) from oa_config_workflow where type_id='01'";
...
<%
	String sql1="select id from oa_workflow where object_ID='"+rs.getString("planing_ID")+"' and type_id='01' and check_tag='0'";
ResultSet rs2=oa_db1.executeQuery(sql1);
if(rs2.next()){

	sql1="select check_tag,describe1,config_id from oa_workflow where object_ID='"+rs.getString("planing_ID")+"' and type_id='01' order by id";
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?planing_ID=<%=rs.getString("planing_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}	
%>
</page:pages>
