...
nseer_grid.columns =[
					   ...
					   {name: '<%=demo.getLang("erp","执行")%>'}
]
nseer_grid.column_width=[200,100,140,140,70];
nseer_grid.auto='<%=demo.getLang("erp","计划主题")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
	['<%=exchange.toHtml(rs.getString("subject"))%>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("begin_time"))%>','<%=exchange.toHtml(rs.getString("end_time"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("exec.jsp?planing_ID=<%=rs.getString("planing_ID")%>")><%=demo.getLang("erp","执行")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
