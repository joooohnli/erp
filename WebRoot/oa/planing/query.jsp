...
String sql="select * from oa_planing where planing_ID='"+planing_ID+"'";
...
if(rs.next()){
	...
	String exec_time=rs.getString("exec_time");
...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
<tr <%=TR_STYLE1%> class="TR_STYLE1">
<td <%=TD_STYLE2%> class="TD_STYLE2"><%=demo.getLang("erp","本计划状态：")%><%=status%></td>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
<td <%=TD_STYLE1%> class="TD_STYLE8" colspan="4" width="100%"><div align="left"><fieldset><legend><%=demo.getLang("erp","执行情况")%></legend><%=rs.getString("exec")%></fieldset></div></td>
</tr>
<%=isPrint.printOrNot3(rs.getString("attachment2"),"附件2&nbsp;&nbsp;&nbsp;",response,TR_STYLE1,TD_STYLE1,TD_STYLE2,rs.getString("id"),"oa_planing","attachment2")%>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><%=exchange.toHtml(rs.getString("execer"))%></td>
<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp","执行时间")%>：</td>
<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><%=exchange.toHtml(exec_time)%></td>
</tr>
</table>
