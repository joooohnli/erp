...
String sql="select * from oa_publicize_dig where publicize_ID='"+publicize_ID+"' and change_time='"+change_time+"'";
...
<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><a href="javascript:winopen('change_dig.jsp?time=<%=exchange.toURL(rs.getString("lately_change_time"))%>&<%=demo.getLang("erp","publicize_ID")%>=<%=rs.getString("publicize_ID")%>')">
<%=lately_change_time%></a>&nbsp;</td>
<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp","变更次数")%>：</td>
<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><a href="javascript:winopen('change_dig.jsp?time=<%=exchange.toURL(rs.getString("lately_change_time"))%>&&publicize_ID=<%=rs.getString("publicize_ID")%>')">
