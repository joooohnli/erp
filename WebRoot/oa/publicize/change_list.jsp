...
String rowSummary=demo.getLang("erp","当前可变更的宣传总数：");
String status="";
%>
...
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
    ...
    ['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?publicize_ID=<%=rs.getString("publicize_ID")%>")><%=exchange.toHtml(rs.getString("subject"))%></div>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?publicize_ID=<%=rs.getString("publicize_ID")%>")><%=demo.getLang("erp","变更")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
