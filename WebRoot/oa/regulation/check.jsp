...
String sql9="select * from oa_workflow where object_ID='"+regulation_ID+"' and type_id='03' and check_tag='0' and config_id<'"+config_id+"'";
ResultSet rs9=oadb.executeQuery(sql9);
if(!rs9.next()){
String sql="select * from oa_regulation where regulation_ID='"+regulation_ID+"' and check_tag='0'";
...
form.action = "check_delete_reconfirm.jsp?regulation_ID=<%=rs.getString("regulation_ID")%>&config_id=<%=config_id%>";
}else{
form.action = "../../oa_regulation_check_ok?regulation_ID=<%=rs.getString("regulation_ID")%>&config_id=<%=config_id%>";
}
}
</script>
<form id="subjectRegister" method="POST" onSubmit="return doValidate('../../xml/oa/oa_regulation.xml','mutiValidation')&&TwoSubmit(this)">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
<td <%=TD_STYLE1%> class="TD_STYLE8" colspan="4" width="100%"><div align="left"><fieldset><legend><%=demo.getLang("erp","制度内容")%></legend><%=rs.getString("content")%></fieldset></div></td>
</tr>
<%=isPrint.printOrNot3(rs.getString("attachment1"),"附件1&nbsp;&nbsp;&nbsp;",response,TR_STYLE1,TD_STYLE1,TD_STYLE2,rs.getString("id"),"oa_regulation","attachment1")%>
<%=isPrint.printOrNot3(rs.getString("attachment2"),"附件2&nbsp;&nbsp;&nbsp;",response,TR_STYLE1,TD_STYLE1,TD_STYLE2,rs.getString("id"),"oa_regulation","attachment2")%>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<td <%=TD_STYLE1%> class="TD_STYLE8" width="20%"><%=demo.getLang("erp","审核时间")%>：</td>
<td <%=TD_STYLE2%> class="TD_STYLE2" width="80%"><%=exchange.toHtml(time)%></td>
</tr>
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
	 <td <%=TD_STYLE3%> class="TD_STYLE3"><%=demo.getLang("erp","该制度已审核，请返回！")%></td>
 </tr>
</table>
</div>
<%}}else{
