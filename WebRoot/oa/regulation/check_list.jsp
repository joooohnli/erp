...
<jsp:setProperty name="mask" property="file" value="xml/oa/oa_regulation.xml"/>
...
<%
String tablename="oa_regulation";
String realname=(String)session.getAttribute("realeditorc");
String condition="check_tag='0'";
String queue="order by register_time";
String validationXml="../../xml/oa/oa_regulation.xml";
String nickName="制度";
String fileName="check_list.jsp";
String rowSummary=demo.getLang("erp","当前等待审核的制度总数：");
%>
...
String sql="select count(id) from oa_config_workflow where type_id='03'";
...
nseer_grid.columns =[
                       {name: '<%=demo.getLang("erp","制度主题")%>'},
                       {name: '<%=demo.getLang("erp","制度分类")%>'},
...
<%
	String sql1="select id from oa_workflow where object_ID='"+rs.getString("regulation_ID")+"' and type_id='03' and check_tag='0'";
ResultSet rs2=oa_db1.executeQuery(sql1);
if(rs2.next()){

	sql1="select check_tag,describe1,config_id from oa_workflow where object_ID='"+rs.getString("regulation_ID")+"' and type_id='03' order by id";
	rs2=oa_db1.executeQuery(sql1);

%>
   ['<%=exchange.toHtml(rs.getString("subject"))%>',
'<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>'
...
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?regulation_ID=<%=rs.getString("regulation_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}	
%>
</page:pages>
