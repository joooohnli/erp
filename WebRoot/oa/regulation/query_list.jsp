...
String rowSummary=demo.getLang("erp","符合条件的制度总数：");
%>
...
nseer_grid.column_width=[40,200,100,140,70];
...
%>
	['<%if(rs.getInt("check_tag")==1){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?regulation_ID=<%=rs.getString("regulation_ID")%>")><%=exchange.toHtml(rs.getString("subject"))%></div>','<%=exchange.toHtml(rs.getString("type"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(status)%>'],
<%k++;%>
</page:pages>
['']];
nseer_grid.init();
