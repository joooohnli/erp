...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <td <%=TD_HANDBOOK_STYLE1%> class="TD_HANDBOOK_STYLE1"><div class="div_handbook"><%=demo.getLang("erp","发消息")%></div></td>
 </tr>
</table>
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 ResultSet rs1 = db.executeQuery(sql1) ;
	 while(rs1.next()){if(!rs1.getString("type_name").equals("QQ")&&!rs1.getString("type_name").equals("msn")){%>
	 <INPUT name="check_type" type="checkbox" value="<%=exchange.toHtml(rs1.getString("type_name"))%>"><%=exchange.toHtml(rs1.getString("type_name"))%>&nbsp;<%}}db.close();%>&nbsp;<input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","提交")%>" name="B1"> <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="colleague.jsp"></div></td>
 </tr>
</table>
