...
<table width="930" height="500" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  <tr>
	...
	<tr>
        <td width="930" class="STYLE1">&nbsp;&nbsp;<%=demo.getLang("erp","首页")%>&gt;&gt;<%=demo.getLang("erp","注册配送单位信息")%></td>
      </tr>
      <tr>
...
if(logistics_db.conn("ondemand1")){
...
%>
 ...
 <td <%=TD_STYLE3%> class="TD_STYLE3">
  <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" style="width:50" onClick=location="register1_choose_attachment.jsp?provider_ID=<%=provider_ID%>"></td>
 </tr>
 </table>
 <%}
}catch(Exception ex){
...
<td <%=TD_STYLE3%> class="TD_STYLE3"> <%=demo.getLang("erp","附件类型错误或附件容量太大(最大")%><%=d/1024%>KB)，<%=demo.getLang("erp","请返回。")%></td>
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" style="width:50" onClick=location="register1_choose_attachment.jsp?provider_ID=<%=provider_ID%>"></div></td>
 </tr>
 </table>
<%}%>
