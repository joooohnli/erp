...
<table <%=TABLE_STYLE1%> class="TABLE_STYLE1" width="100%" id=theObjTable>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37.4%" colspan="3"><%=purchase_ID%></td>
...
<td <%=TD_STYLE2%> class="TD_STYLE2" width="86.5%" colspan="7"><%=rs.getString("remark")%>&nbsp;</td>
 ...
 <td <%=TD_STYLE1%> class="TD_STYLE1" width="13%"><%=demo.getLang("erp","供应商名称")%></td>
...
<%
	...
	int count=0;
	String sql5="select count(*) from purchase_details where purchase_ID='"+purchase_ID+"'";
...
if(count>20){
	int m=1;
String sql6="select * from purchase_details where purchase_ID='"+purchase_ID+"'";
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=exchange.toHtml(rs6.getString("demand_amount"))%>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="query_purchase_details.jsp?purchase_ID=<%=purchase_ID%>&&provider_ID=<%=rs6.getString("provider_ID")%>" target="_blank"><%=demo.getLang("erp","查看")%></a></td>
 </tr>
<%
}
	 	purchase_db.close();
