...
if(a!=1){
%>
<form id="mutiValidation" method="POST" action="../../purchase_apply_newRegister_ok" onSubmit="return CheckForm(this)">
...
<input name="payer_ID" type="hidden" value="">
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","采购理由")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="reason" type="hidden" value="<%=exchange.toHtml(reason)%>"><%=exchange.toHtml(reason)%></td>	 
	 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","要求供货时间")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="demand_gather_time" type="hidden" value="<%=exchange.toHtml(demand_gather_time)%>"><%=exchange.toHtml(demand_gather_time)%></td>
</tr>
</table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input name="real_cost_price<%=i%>" type="hidden" value="<%=exchange.toHtml(real_cost_price[i])%>"><%=exchange.toHtml(real_cost_price[i])%></td>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","总件数")%>&nbsp;&nbsp;：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="demand_amount" type="hidden" value="<%=demand_amount%>"><%=demand_amount%></td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","总金额")%>&nbsp;&nbsp;：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="real_cost_price_sum" type="hidden" width="100%" value="<%=real_cost_price_sum%>"><%=real_cost_price_sum%></td>
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="register_time" type="hidden" onfocus="setday(this)" value="<%=exchange.toHtml(register_time)%>"><%=exchange.toHtml(register_time)%></td>
	 ...
	 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%" height="65"><%=demo.getLang("erp","备注")%>&nbsp;&nbsp;&nbsp;&nbsp;：</td>
