...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE5%> class="TD_STYLE5"><font size="4"><b><%=demo.getLang("erp","采购数量确认单")%></b></font></td>
 </tr>
 </table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<table <%=TABLE_STYLE5%> class="TABLE_STYLE5">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="11%"><%=demo.getLang("erp","拟采购数量")%> </td>
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=rs6.getDouble("amount")%><input name="amount<%=i%>" type="hidden" value="<%=rs6.getDouble("amount")%>">&nbsp;</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=rs6.getDouble("paid_amount")%><input name="paid_amount<%=i%>" type="hidden" value="<%=exchange.toHtml(rs6.getString("paid_amount"))%>"></td>
<%if(!rs6.getString("type").equals("物料")&&!rs6.getString("type").equals("外购商品")){%>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE4%> class="INPUT_STYLE4" name="apply_purchase_amount<%=i%>" onFocus="this.blur()" value="0"></td>
<%}else{
if(rs6.getDouble("apply_purchase_amount")==0){%>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE4%> class="INPUT_STYLE4" name="apply_purchase_amount<%=i%>" value="<%=rs6.getDouble("amount")-rs6.getDouble("paid_amount")%>"></td>
<%}else{%>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" name="apply_purchase_amount<%=i%>" value="<%=rs6.getDouble("apply_purchase_amount")%>"></td>
<%}}%>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><%=new java.text.DecimalFormat("###,##0.0").format(rs.getDouble("demand_amount"))%>&nbsp;</td>
	 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","已出库总件数")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><%=new java.text.DecimalFormat("###,##0.0").format(rs.getDouble("paid_amount"))%>&nbsp;</td>
 </tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
