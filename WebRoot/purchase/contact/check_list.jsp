...
String validationXml="../../xml/purchase/purchase_contact.xml";
String nickName="供应商联络";
String fileName="check_list.jsp";
String rowSummary=demo.getLang("erp","当前等待审核的供应商联络总数");
%>
<%@include file="../../include/search.jsp"%>
<%
try{
int workflow_amount=0;
String sql="select count(id) from purchase_config_workflow where type_id='04'";
...
nseer_grid.column_width=[180,180,70,180,70,70,160,
<%
for(int i=1;i<workflow_amount;i++){
%>					   70,
<%
...
String sql1="select id from purchase_workflow where object_ID='"+rs.getString("contact_ID")+"' and check_tag='0'";
ResultSet rs2=purchase_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from purchase_workflow where object_ID='"+rs.getString("contact_ID")+"' order by id";
	rs2=purchase_db1.executeQuery(sql1);
%>
['<%=rs.getString("contact_ID")%>','<%=exchange.toHtml(rs.getString("provider_name"))%>','<%=exchange.toHtml(rs.getString("reason"))%>','<%=exchange.toHtml(rs.getString("reasonexact"))%>','<%=exchange.toHtml(rs.getString("person_contacted"))%>','<%=exchange.toHtml(rs.getString("contact_person"))%>','<%=exchange.toHtml(contact_time)%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
