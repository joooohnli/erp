...
nseer_grid.auto='<%=demo.getLang("erp","联络单编号")%>';
nseer_grid.data = [
...
for(int i=1;i<=workflow_amount;i++){
	...
	if(rs2.next()){
			...
			status="未审核";	
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?contact_ID=<%=rs.getString("contact_ID")%>&config_ID=<%=rs2.getString("config_ID")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%}%>
</page:pages>
['']];
nseer_grid.init();
