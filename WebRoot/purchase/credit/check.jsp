...
String sql9="select * from purchase_workflow where object_ID='"+gather_ID+"' and check_tag='0' and config_ID<'"+config_ID+"'";
ResultSet rs9=purchasedb.executeQuery(sql9);
if(!rs9.next()){
if(vt.validata((String)session.getAttribute("unit_db_name"),"stock_apply_gather","gather_ID",gather_ID,"check_tag").equals("0")){
try{
...
form.action = "check_delete_reconfirm.jsp?gather_ID=<%=rs.getString("gather_ID")%>&config_ID=<%=config_ID%>";
}else{
form.action = "../../purchase_credit_check_ok?gather_ID=<%=rs.getString("gather_ID")%>&config_ID=<%=config_ID%>";
}
}
</script>
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input name="cost_price<%=i%>" type="hidden" value="<%=rs6.getDouble("cost_price")%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("cost_price"))%></td>
	 <input name="cost_price<%=i%>" type="hidden" value="<%=rs6.getDouble("cost_price")%>"><input name="type<%=i%>" type="hidden" value="<%=exchange.toHtml(rs6.getString("type"))%>"></td>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input name="check_time" type="hidden" value="<%=exchange.toHtml(time)%>"><%=exchange.toHtml(time)%><input name="checker_ID" type="hidden" value="<%=exchange.toHtml(checker_ID)%>"></td>
 </tr>
	 <tr <%=TR_STYLE1%> class="TR_STYLE1">
