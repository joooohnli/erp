...
String sql9="select * from purchase_workflow where object_ID='"+discussion_ID+"' and check_tag='0' and config_id<'"+config_id+"'";
ResultSet rs9=purchasedb.executeQuery(sql9);
if(!rs9.next()){
if(!vt.validata((String)session.getAttribute("unit_db_name"),"purchase_discussion","discussion_ID",discussion_ID,"check_tag").equals("1")){
try{
...
form.action = "../../purchase_discussion_check_ok?discussion_ID=<%=rs.getString("discussion_ID")%>&config_id=<%=config_id%>";
}
}
</script>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4" id=theObjTable>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input name="provider_ID" type="hidden" value="<%=rs.getString("provider_ID")%>"><%=rs.getString("provider_ID")%>&nbsp;</td>
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="checker" value="<%=exchange.toHtml(checker)%>"><input name="check_time" type="hidden" width="100%" value="<%=exchange.toHtml(time)%>"></td>
