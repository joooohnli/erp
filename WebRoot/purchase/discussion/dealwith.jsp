...
form.action = "../../purchase_discussion_dealwith_ok?discussion_ID=<%=rs.getString("discussion_ID")%>";
}else{
...
<form  id="mutiValidation" method="POST" onSubmit="return doValidate('../../xml/purchase/purchase_discussion.xml','mutiValidation')&&TwoSubmit(this)">
...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8"><INPUT name="Ref" type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1" value=cop checked><%=demo.getLang("erp","处理完成")%> <INPUT name="Ref" type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1" value=Ind> 
 <%=demo.getLang("erp","转垃圾箱")%> <input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","确认")%>"> <input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="dealwith_list.jsp"></td>
 </tr>
 </table>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4" id=theObjTable>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input name="provider_name" type="hidden" value="<%=exchange.toHtml(rs.getString("provider_name"))%>"><%=exchange.toHtml(rs.getString("provider_name"))%>&nbsp;</td>
...
<TABLE <%=TABLE_STYLE5%> class="TABLE_STYLE5">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%"><%=demo.getLang("erp","推荐供应商")%></td>
 </tr>
<%
int i=1;
...
<td <%=TD_STYLE2%> class="TD_STYLE2"><a href="../../purchase_discussion_dealwith_recommend_ok?discussion_ID=<%=rs.getString("discussion_ID")%>&provider_ID=<%=rs.getString("provider_ID")%>&product_ID=<%=rs6.getString("product_ID")%>&processer=<%=toUtf8String.utf8String(exchange.toURL(processer))%>&processer_ID=<%=processer_ID%>"><%=demo.getLang("erp","推荐供应商")%></a></td>
<%}else{%>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">  
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","处理人")%>&nbsp;&nbsp;：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input type="text" <%=INPUT_STYLE4%> class="INPUT_STYLE4" name="processer" value="<%=exchange.toHtml(processer)%>"></td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="10%"><%=demo.getLang("erp","处理人编号")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input type="text" <%=INPUT_STYLE4%> class="INPUT_STYLE4" name="processer_ID"></td>
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
