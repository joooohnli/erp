...
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../file/query.jsp?provider_ID=<%=rs1.getString("provider_ID")%>")><%=rs1.getString("provider_ID")%></div>','<%=rs1.getString("provider_name")%>','<%=rs1.getString("provider_class")%>','<%=exchange.toHtml(rs1.getString("chain_name"))%>','<%=rs1.getString("type")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("register.jsp?provider_ID=<%=rs1.getString("provider_ID")%>&&provider_name=<%=toUtf8String.utf8String(exchange.toURL(rs1.getString("provider_name")))%>")><%=demo.getLang("erp","登记")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
