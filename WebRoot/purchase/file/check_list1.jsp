...
String rowSummary=demo.getLang("erp","当前等待审核的供应商档案总数");
%>
<%@include file="../../include/search.jsp"%>
<%
try{
int workflow_amount=0;
String sql="select count(id) from purchase_config_workflow where type_id='01'";
...
nseer_grid.auto='<%=demo.getLang("erp","供应商编号")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
<%
String sql1="select id from purchase_workflow where object_ID='"+rs.getString("provider_ID")+"' and check_tag='0'";
ResultSet rs2=purchase_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from purchase_workflow where object_ID='"+rs.getString("provider_ID")+"' order by id";
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?provider_ID=<%=rs.getString("provider_ID")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
