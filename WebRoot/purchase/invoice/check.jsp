...
String sql9="select * from purchase_workflow where object_ID='"+purchase_ID+"' and check_tag='0' and invoice_time='"+invoice_time+"' and config_ID<'"+config_ID+"'";
...
if(rs8.next()){
	try{
		...
		ResultSet rs7 = purchase_db.executeQuery(sql) ;
		if(rs7.next()){
...
form.action = "check_delete_reconfirm.jsp?purchase_ID=<%=rs7.getString("purchase_ID")%>&config_ID=<%=config_ID%>&invoice_time=<%=rs7.getString("invoice_time")%>";
}else{
form.action = "../../purchase_invoice_check_ok?purchase_ID=<%=rs7.getString("purchase_ID")%>&config_ID=<%=config_ID%>&invoice_time=<%=rs7.getString("invoice_time")%>";
}
}
</script>
...
<form id="mutiValidation" method="POST" onSubmit="return doValidate('../../xml/purchase/purchase_purchasing.xml','mutiValidation')&&TwoSubmit(this)">
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4" id=theObjTable>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="87%" colspan="3"><input type="hidden" name="purchase_ID" value="<%=purchase_ID%>"><%=purchase_ID%></td>
 </tr>
 	<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<%
	int i=1;
String sql6="select * from purchase_purchasing where purchase_ID='"+purchase_ID+"' and check_tag='0' and kind='发票' order by provider_ID,id";
...
%>
 ...
 <input name="provider_name" type="hidden" value="<%=exchange.toHtml(rs6.getString("provider_name"))%>"><%=exchange.toHtml(rs6.getString("provider_name"))%>&nbsp;</td>
	 ...
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input name="demand_cost_price_sum" type="hidden" value="<%=rs6.getString("demand_cost_price_sum")%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("demand_cost_price_sum"))%>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input name="invoiced_sum" type="hidden" value="<%=rs6.getString("invoiced_sum")%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs6.getDouble("invoiced_sum"))%>&nbsp;</td>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="check_time" type="hidden" style="width: 50%; " value="<%=exchange.toHtml(time)%>"><%=exchange.toHtml(time)%>&nbsp;<input name="checker_ID" type="hidden" style="width: 50%; " value="<%=exchange.toHtml(checker_ID)%>"></td>
	</tr>
 </table>
