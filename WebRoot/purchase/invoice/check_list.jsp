...
String nickName="采购执行单";
String fileName="check_list.jsp";
String rowSummary=demo.getLang("erp","等待发票信息登记审核的采购执行单总数");
%>
...
String sql="select count(id) from purchase_config_workflow where type_id='08'";
...
nseer_grid.column_width=[180,180,100,100,100,100,<%for(int i=1;i<workflow_amount;i++){%>70,<%}%>70];
nseer_grid.auto='<%=demo.getLang("erp","产品名称")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
<%
String sql1="select id from purchase_workflow where object_ID='"+rs.getString("purchase_ID")+"' and check_tag='0' and invoice_time='"+rs.getString("invoice_time")+"'";
ResultSet rs2=purchase_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id,invoice_time from purchase_workflow where object_ID='"+rs.getString("purchase_ID")+"' and invoice_time='"+rs.getString("invoice_time")+"' order by id";
	rs2=purchase_db1.executeQuery(sql1);
%>
['<%=rs.getString("purchase_ID")%>','<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("demand_cost_price_sum"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("invoiced_sum"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("demand_cost_price_sum")-rs.getDouble("invoiced_sum"))%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?purchase_ID=<%=rs.getString("purchase_ID")%>&invoice_time=<%=rs2.getString("invoice_time")%>&config_ID=<%=rs2.getString("config_ID")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%}%>
</page:pages>
['']];
nseer_grid.init();
