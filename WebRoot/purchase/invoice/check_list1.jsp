...
nseer_grid.column_width=[100,<%for(int i=1;i<workflow_amount;i++){%>70,<%}%>70];
nseer_grid.auto='<%=demo.getLang("erp","采购执行单编号")%>';
nseer_grid.data = [
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?purchase_ID=<%=rs.getString("purchase_ID")%>&invoice_time=<%=rs2.getString("invoice_time")%>&config_ID=<%=rs2.getString("config_ID")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%}%>
</page:pages>
['']];
nseer_grid.init();
