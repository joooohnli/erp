...
<script type="text/javascript" src="../../javascript/purchase/invoice/query_list.js"></script>
...
String rowSummary=demo.getLang("erp","符合条件的执行单总数");
int k=1;
String condition="excel_tag='2' and invoice_tag!='0' and gar_tag='0' and invoice_gar_tag='0'";
String condition1="excel_tag='2' and invoice_tag='1' and gar_tag='0' and invoice_gar_tag='0'";
String condition2="excel_tag='2' and invoice_tag='3' and gar_tag='0' and invoice_gar_tag='0'";
String condition3="excel_tag='2' and invoice_tag='2' and gar_tag='0' and invoice_gar_tag='0'";
%>
...
%>
<%
condition="excel_tag='2' and invoice_tag!='0' and gar_tag='0' and invoice_gar_tag='0'";
%>
...
nseer_grid.columns =[
                       ...
                       {name: '<%=demo.getLang("erp","已收到发票总额")%>'},
	                   {name: '<%=demo.getLang("erp","未收到发票总额")%>'},
					   {name: '<%=demo.getLang("erp","收发票状态")%>'}
]
nseer_grid.column_width=[40,180,180,100,100,100,100,100];
...
%>
 ['<input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs1.getString("id")%>⊙<%=rs1.getString("invoice_tag")%>" style="height:10">','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?purchase_ID=<%=rs1.getString("purchase_ID")%>")><%=rs1.getString("purchase_ID")%></div>','<span style="color:<%=color%>"><%=rs1.getString("product_ID")%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("product_name"))%></span>','<span style="color:<%=color%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("demand_cost_price_sum"))%></span>',
 '<span style="color:<%=color%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("invoiced_sum"))%>',
 '<span style="color:<%=color%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs1.getDouble("demand_cost_price_sum")-rs1.getDouble("invoiced_sum"))%></span>',
