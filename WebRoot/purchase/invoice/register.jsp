...
String sql7="select * from purchase_purchase where purchase_ID='"+purchase_ID+"'";
...
<form id="mutiValidation" method="POST" action="../../purchase_invoice_register_ok" onSubmit="return doValidate('../../xml/purchase/purchase_purchasing.xml','mutiValidation')">
 <table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8"><%=DgButton.getDraft("'mutiValidation','../../purchase_invoice_register_draft_ok','../../xml/purchase/purchase_purchasing.xml'",request)%>&nbsp;<input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","提交")%>">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="register_list.jsp"></td> 
 </tr>
 </table> 
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><input name="provider_name" type="hidden" value="<%=exchange.toHtml(rs6.getString("provider_name"))%>"><%=exchange.toHtml(rs6.getString("provider_name"))%>&nbsp;</td>
	 ...
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" name="real_contact_person_tel" value="<%=exchange.toHtml(rs6.getString("demand_contact_person_tel"))%>"></td>
...
<%}else if(rs6.getString("invoice_check_tag").equals("1")&&rs6.getString("invoice_tag").equals("0")){
	String sql8="select * from purchase_purchasing where purchase_ID='"+purchase_ID+"' and provider_ID='"+rs6.getString("provider_ID")+"' and check_tag='0' and kind='发票'";
