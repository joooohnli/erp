...
String sql9="select * from purchase_workflow where object_ID='"+product_providers_recommend_ID+"' and check_tag='0' and config_id<'"+config_id+"'";
ResultSet rs9=purchasedb.executeQuery(sql9);
if(!rs9.next()){
if(vt.validata((String)session.getAttribute("unit_db_name"),"purchase_product_providers_recommend","product_providers_recommend_ID",product_providers_recommend_ID,"check_tag").equals("0")){
try{
...
form.action = "check_delete_reconfirm.jsp?product_providers_recommend_ID=<%=rs.getString("product_providers_recommend_ID")%>&config_id=<%=config_id%>";
}else{
form.action = "../../purchase_product_providers_check_ok?product_providers_recommend_ID=<%=rs.getString("product_providers_recommend_ID")%>&config_id=<%=config_id%>";
}
}
</script>
...
<form id="mutiValidation" method="POST" onSubmit="return doValidate('../../xml/purchase/purchase_product_providers_recommend.xml','mutiValidation')&&TwoSubmit(this)">
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="../file/query.jsp?provider_ID=<%=rs6.getString("provider_ID")%>"><%=rs6.getString("provider_ID")%></a>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="40%"><input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="checker" value="<%=exchange.toHtml(checker)%>"><input type="hidden" <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="checker_ID" value="<%=exchange.toHtml(checker_ID)%>"></td>
