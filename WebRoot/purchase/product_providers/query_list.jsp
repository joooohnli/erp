...
%>
['<%if(recommend_provider_tag.equals("完成")){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs1.getString("id")%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?product_providers_recommend_ID=<%=rs1.getString("product_providers_recommend_ID")%>")><span style="color:<%=color%>"><%=rs1.getString("product_providers_recommend_ID")%></span></div>','<span style="color:<%=color%>"><%=rs1.getString("product_ID")%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("product_name"))%></span>','<span style="color:<%=color%>"><%=recommend_provider_tag%></span>','<%=check_tag%>'],
<%k++;%>
</page:pages>
['']];
nseer_grid.init();
