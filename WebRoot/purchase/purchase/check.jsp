...
String sql9="select * from purchase_workflow where object_ID='"+purchase_ID+"' and check_tag='0' and config_ID<'"+config_ID+"'";
ResultSet rs9=purchasedb.executeQuery(sql9);
if(!rs9.next()){

if(vt.validata((String)session.getAttribute("unit_db_name"),"purchase_purchase","purchase_ID",purchase_ID,"check_tag").equals("1")){
try{
...
form.action = "check_delete_reconfirm.jsp?purchase_ID=<%=rs.getString("purchase_ID")%>&config_ID=<%=config_ID%>";
}else{
form.action = "../../purchase_purchase_check_ok?purchase_ID=<%=rs.getString("purchase_ID")%>&config_ID=<%=config_ID%>";
}
...
<form id="mutiValidation" method="POST" onSubmit="return doValidate('../../xml/purchase/purchase_purchase.xml','mutiValidation')&&TwoSubmit(this)">
...
%>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs6.getDouble("demand_amount"))%>&nbsp;</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="javascript:winopen('check_purchase_details.jsp?purchase_ID=<%=purchase_ID%>&&provider_ID=<%=rs6.getString("provider_ID")%>')"><%=demo.getLang("erp","查看")%></a></td>
 </tr>
<%
	}
%>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%" colspan="3"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs.getDouble("demand_amount"))%><input type="hidden" name="amount" value="<%=rs.getString("demand_amount")%>">&nbsp;</td>
 </tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
