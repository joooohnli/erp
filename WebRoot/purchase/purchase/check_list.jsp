...
String rowSummary=demo.getLang("erp","当前等待审核的采购执行单总数");
%>
<%@include file="../../include/search.jsp"%>
<%
try{
int workflow_amount=0;
String sql="select count(id) from purchase_config_workflow where type_id='06'";
...
nseer_grid.column_width=[180,180,100,70,180,160,
<%
for(int i=1;i<workflow_amount;i++){
%>					   70,
<%
...
String sql1="select id from purchase_workflow where object_ID='"+rs.getString("purchase_ID")+"' and check_tag='0'";
ResultSet rs2=purchase_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from purchase_workflow where object_ID='"+rs.getString("purchase_ID")+"' order by id";
	rs2=purchase_db1.executeQuery(sql1);
%>
['<%=rs.getString("purchase_ID")%>','<%=rs.getString("product_ID")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs.getDouble("demand_amount"))%>','<%=rs.getString("pay_ID_group")%>','<%=exchange.toHtml(register_time)%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?purchase_ID=<%=rs.getString("purchase_ID")%>&config_ID=<%=rs2.getString("config_ID")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%}%>
</page:pages>
['']];
nseer_grid.init();
