...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?purchase_ID=<%=rs.getString("purchase_ID")%>&config_ID=<%=rs2.getString("config_ID")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%}%>
</page:pages>
['']];
nseer_grid.init();
