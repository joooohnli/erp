 ...
 *version 2 of the License, or (at your option) any later version.
 -->
<%@page contentType="text/html; charset=UTF-8" language="java" import="java.sql.*" import="java.util.*" import="java.io.*" import="include.nseer_cookie.exchange" import ="include.nseer_db.*,java.text.*,include.nseer_cookie.*"%>
...
<table <%=TABLE_STYLE7%> class="TABLE_STYLE5" style="width:100%;" id=theObjTable>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="86.5%" colspan="3"><%=purchase_ID%></td>
 </tr>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","采购数量")%></td>
...
%>
 ...
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="37.4%"><a href="javascript:winopenm('query_attachment.jsp?id=<%=rs6.getString("id")%>&tablename=purchase_details&fieldname=attachment_name')">
<%=attachment_name1[1]%></a>&nbsp;</td>
 <td <%=TD_STYLE11%> class="TD_STYLE1" width="11%"><%=demo.getLang("erp","已入库数量")%></td>
 <td <%=TD_STYLE21%> class="TD_STYLE2" width="37.4%"><input name="gathered_amount" type="hidden" value="<%=rs6.getString("stock_gathered_amount")%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs6.getDouble("stock_gathered_amount"))%>&nbsp;</td>
 </tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
