...
nseer_grid.auto='<%=demo.getLang("erp","事故单编号")%>';
nseer_grid.data = [
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?accident_id=<%=rs.getString("accident_id")%>&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
