...
nseer_grid.column_width=[50,180,180,180,150,80,80,80];
...
%>
['<%if(!rs.getString("dealwith_tag").equals("0")){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>⊙<%=delete_tag%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?accident_id=<%=rs.getString("accident_id")%>")><%=rs.getString("accident_id")%></div>','<%=rs.getString("accident_time")%>','<%=exchange.toHtml(rs.getString("product_id"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("accident_name"))%>','<span style="color:<%=color%>"><%=check_tag%></span>','<span style="color:<%=color1%>"><%=dealwith_tag%></span>'],
<%k++;%>
</page:pages>
['']];
nseer_grid.init();
