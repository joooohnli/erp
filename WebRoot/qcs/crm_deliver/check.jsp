...
if(!rsa.next()){
	String sql = "select id,apply_id,stock_name,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,register,register_time,qcs_result,attachment1,remark from qcs_crm_deliver where qcs_id='"+qcs_id+"' and check_tag='0'";
...
form.action = "check_delete_reconfirm.jsp?qcs_id=<%=qcs_id%>&&config_id=<%=config_id%>";
}else{
form.action ='../../qcs_crm_deliver_check_ok';
}
if (doValidate('../../xml/qcs/qcs_crm_deliver.xml',form_id)){
form.submit();
}
}
</script>
<body onload="locateSelectDiv()">
<form id="crmDeliver_check" METHOD="POST" ENCTYPE="multipart/form-data">
...
<div <%=DIV_STYLE1%> class="DIV_STYLE1"><input name="Ref" type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1" value=cop checked> <%=demo.getLang("erp","未通过")%> <INPUT name="Ref" type="radio" <%=RADIO_STYLE1%> class="RADIO_STYLE1" value=Ind><%=demo.getLang("erp","通过")%><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","确认")%>" onClick="TwoSubmit('crmDeliver_check');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check_list.jsp"></div></td>
</tr>
</table>
...
<input <%=INPUT_STYLE3%> class="INPUT_STYLE3" name="config_id" type="hidden" width="37%" value="<%=config_id%>">
</td>
 </tr>
 </table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
