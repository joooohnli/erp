...
try{
	String sql = "select apply_id,product_id,product_name,stock_id,stock_name,demand_amount from qcs_apply_details where qcs_tag='0' and reason='3' and apply_id='"+apply_id+"'";
	ResultSet rs = qcs_db.executeQuery(sql);
    if(rs.next()){
%>
<%String table_width1="820px";%>
<body onload="locateSelectDiv()">
<form id="crmDeliver_register" METHOD="POST" ENCTYPE="multipart/form-data">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE1%> class="TD_STYLE8"><%=DgButton.getDraft("'crmDeliver_register','../../qcs_crm_deliver_register_draft_ok','../../xml/qcs/qcs_crm_deliver.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","保存")%>" onClick="showFormSubmit('register');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></td>
 </tr>
 </table>
...
<td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","不合格数")%>：</td>
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="stock_name" name="stock_name" type="text" width="37%" value="<%=rs.getString("stock_name")%>" readonly/><input id="stock_id" name="stock_id" type="hidden" value="<%=rs.getString("stock_id")%>" /></td>
