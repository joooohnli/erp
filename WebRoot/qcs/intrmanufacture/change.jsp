...
<script type="text/javascript" src="../../javascript/qcs/intrmanufacture/intrmanufacture.js"></script>
...
try{
	String sql = "select id,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,register,register_time,qcs_result,attachment1,remark from qcs_intrmanufacture where qcs_id='"+qcs_id+"' and check_tag='1'";
...
<form id="intrmanufacture_change" METHOD="POST" ENCTYPE="multipart/form-data">
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE5%> class="TD_STYLE5"><font size="4"><b><%=demo.getLang("erp","委外质检单")%></b></font></td>
 </tr>
 </table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
%>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="sampling_standard" name="sampling_standard" type="text" width="37%" value="<%=sampling_standard%>" readonly/><div class="select_div_l" id="sampling_standard_div"  readonly/></div></td>
...
<script>
<%
sql="select * from qcs_intrmanufacture_details where qcs_id='"+qcs_id+"' order by details_number";
...
<td <%=TD_STYLE2%> class="TD_STYLE2" width="87%" colspan="3"><textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="remark" /><%=exchange.unURL(remark)%></textarea></td>
 ...
 <%=isPrint.hasOrNot3d(attachment1,"附件&nbsp;&nbsp;&nbsp;","1",response,TR_STYLE1,TD_STYLE1,TD_STYLE2,FILE_STYLE1,id,"qcs_intrmanufacture")%>
</table>
