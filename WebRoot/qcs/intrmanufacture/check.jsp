...
<link rel="stylesheet" type="text/css" href="../../css/qcs/intrmanufacture/intrmanufacture.css" />
...
if(!rsa.next()){
	String sql = "select id,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,register,register_time,qcs_result,attachment1,remark from qcs_intrmanufacture where qcs_id='"+qcs_id+"' and check_tag='0'";
...
function TwoSubmit(){
var form=document.getElementById('intrmanufacture_check');
if (form.Ref[0].checked){
form.action = "check_delete_reconfirm.jsp";
}else{
form.action = "../../qcs_intrmanufacture_check_ok";
}
if (doValidate('../../xml/qcs/qcs_intrmanufacture.xml','intrmanufacture_check')){
form.submit();
}
}
</script>
<body onload="locateSelectDiv()">
<form id="intrmanufacture_check" METHOD="POST" ENCTYPE="multipart/form-data">
...
<%=demo.getLang("erp","通过")%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onClick="TwoSubmit();" value=<%=demo.getLang("erp","确定")%>>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();" /> 
