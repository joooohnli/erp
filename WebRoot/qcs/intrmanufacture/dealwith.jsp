...
try{
	String sql = "select id,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,register,register_time,qcs_result,attachment1,remark,limit_tag from qcs_intrmanufacture where qcs_id='"+qcs_id+"'";
    ...
    String limit_tag=rs.getString("limit_tag");
%>
<%String table_width1="820px";%>
<form id="intrmanufacture_dealwith" METHOD="POST">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <%
 sql="select type_name,describe2 from qcs_config_public_char where kind_id='07' and describe1='2' and used_tag='1' order by describe1";
 ...
 <input type="hidden" id="price_percent" name="price_percent">
 <input type="hidden" id="limit_tag" value="<%=limit_tag%>">
...
<td <%=TD_STYLE2%> class="TD_STYLE2" width="87%" colspan="3"><textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="remark" readonly/><%=exchange.unHtml(remark)%></textarea></td>
</tr>
<%=isPrint.printOrNot3(attachment1,"附件&nbsp;&nbsp;&nbsp;",response,TR_STYLE1,TD_STYLE1,TD_STYLE2,id,"qcs_purchase","attachment1")%>
</table>
