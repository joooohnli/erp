...
String condition="check_tag='1' and dealwith_tag='0' and excel_tag='2'";
...
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?qcs_id=<%=rs.getString("qcs_id")%>")><%=rs.getString("qcs_id")%></div>','<%=rs.getString("apply_id")%>','<%=exchange.toHtml(rs.getString("product_id"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("qcs_result"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("dealwith.jsp?qcs_id=<%=rs.getString("qcs_id")%>")><%=demo.getLang("erp","处理")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
