...
String rowSummary=demo.getLang("erp","当前等待审核的质检项目总数：");
%>
...
String sql="select count(id) from qcs_config_workflow where type_id='02'";
...
nseer_grid.column_width=[180,60,100,100,100,100,70,200,<%for(int i=1;i<workflow_amount;i++){%>70,<%}%>70];
nseer_grid.auto='<%=demo.getLang("erp","项目名称")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
<%
String sql1="select id from qcs_workflow where object_ID='"+rs.getString("item_id")+"' and check_tag='0'";
ResultSet rs2=qcs_db1.executeQuery(sql1);
if(rs2.next()){
	sql1="select check_tag,describe1,config_id from qcs_workflow where object_ID='"+rs.getString("item_id")+"' order by id";
	rs2=qcs_db1.executeQuery(sql1);
%>
['<%=exchange.toHtml(rs.getString("item_id"))%>','<%=exchange.toHtml(rs.getString("item_name"))%>','<%=exchange.toHtml(rs.getString("designer"))%>','<%=exchange.toHtml(rs.getString("analyse_method"))%>','<%=exchange.toHtml(rs.getString("default_basis"))%>','<%=exchange.toHtml(rs.getString("quality_method"))%>','<%=exchange.toHtml(rs.getString("quality_equipment"))%>','<%=exchange.toHtml(rs.getString("sampling_standard"))%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?item_id=<%=rs.getString("item_id")%>&&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
