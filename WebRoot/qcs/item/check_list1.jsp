...
nseer_grid.auto='<%=demo.getLang("erp","质检项目编号")%>';
nseer_grid.data = [
...
%>
['<%=exchange.toHtml(rs.getString("item_id"))%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?item_id=<%=rs.getString("item_id")%>&&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
