...
nseer_grid.columns =[
                       {name: '<%=demo.getLang("erp","生产工序质检单编号")%>'},
<%
...
nseer_grid.auto='<%=demo.getLang("erp","生产工序质检单编号")%>';
nseer_grid.data = [
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?qcs_id=<%=rs.getString("qcs_id")%>&&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
