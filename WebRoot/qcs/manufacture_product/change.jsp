...
<script type="text/javascript" src="../../javascript/qcs/manufacture_product/manufactureProduct.js"></script>
...
try{
	String sql = "select id,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,register,register_time,qcs_result,attachment1,remark from qcs_manufacture_product where qcs_id='"+qcs_id+"' and check_tag='1'";
...
<form id="manufactureProduct_change" METHOD="POST" ENCTYPE="multipart/form-data">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE1%> class="TD_STYLE8"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","确定")%>" onClick="showFormSubmit('change');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();" /></td>
 </tr>
 </table>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE5%> class="TD_STYLE5"><font size="4"><b><%=demo.getLang("erp","生产入库质检单")%></b></font></td>
 </tr>
 </table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","质检方案")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="quality_solution" name="quality_solution" type="text" width="37%"  onclick="loadAjaxDiv('quality_solution','<%=product_id%>');this.blur();" value="<%=quality_solution%>" readonly/><div class="select_div_r" id="quality_solution_div"  onclick="loadAjaxDiv('quality_solution','<%=product_id%>')"></td>
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<%}else{%>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","抽样标准")%>&nbsp;&nbsp;：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="sampling_standard" name="sampling_standard" type="text" width="37%"  onfocus="this.blur();" value="<%=sampling_standard%>" readonly/><div class="select_div_l" id="sampling_standard_div"></td>
 <%}%>
...
<script>
<%
sql="select * from qcs_manufacture_product_details where qcs_id='"+qcs_id+"' order by details_number";
...
<td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="changer" name="changer" type="text" width="37%" value="<%=changer%>" readonly /><input id="changer_id" name="changer_id" type="hidden" value="<%=changer_id%>" readonly /></td>
<td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","变更时间")%>：</td>
<td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="change_time" name="change_time" type="text" width="37%" value="<%=time%>" readonly /></td>
</tr>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<td <%=TD_STYLE2%> class="TD_STYLE2" width="87%" colspan="3"><textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="remark" /><%=exchange.unURL(remark)%></textarea></td>
 ...
 <%=isPrint.hasOrNot3d(attachment1,"附件&nbsp;&nbsp;&nbsp;","1",response,TR_STYLE1,TD_STYLE1,TD_STYLE2,FILE_STYLE1,id,"qcs_manufacture_product")%>
</table>
