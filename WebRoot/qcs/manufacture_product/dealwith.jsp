...
try{
	String sql = "select id,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_solution,sampling_standard,sampling_amount,qualified,unqualified,register,register_time,qcs_result,attachment1,remark from qcs_manufacture_product where qcs_id='"+qcs_id+"'";
...
<form id="manufactureProduct_dealwith" METHOD="POST">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <%
 sql="select type_name,describe2 from qcs_config_public_char where kind_id='07' and describe1='5' and used_tag='1' order by describe1";
 ...
 <% a++;}%><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交")%>" onClick="dealwithSubmit('dealwith');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();" /></td>
...
<%=isPrint.printOrNot3(attachment1,"附件&nbsp;&nbsp;&nbsp;",response,TR_STYLE1,TD_STYLE1,TD_STYLE2,id,"qcs_manufacture_procedure","attachment1")%>
</table>
