...
try{
	String sql = "select apply_id,product_id,product_name,purchase_time,demand_amount from qcs_apply_details where qcs_tag='0' and apply_id='"+apply_id+"' and reason='5'";
	ResultSet rs = qcs_db.executeQuery(sql);
    if(rs.next()){
	getNameFromID getnamefromid=new getNameFromID();
String quality_way=getnamefromid.getNameFromID((String)session.getAttribute("unit_db_name"),"design_file","product_id",rs.getString("product_id"),"manu_product_qcs_way");
if(!quality_way.equals("")){
%>
<%String table_width1="820px";%>
<body onload="locateSelectDiv()">
<form id="manufactureProduct_register" METHOD="POST" ENCTYPE="multipart/form-data">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE1%> class="TD_STYLE8"><%=DgButton.getDraft("'manufactureProduct_register','../../qcs_manufacture_product_register_draft_ok','../../xml/qcs/qcs_manufacture_product.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","保存")%>" onClick="showFormSubmit('register');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></td>
 </tr>
 </table>
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 ...
 <td <%=TD_STYLE3%> class="TD_STYLE3">
 <div <%=DIV_STYLE1%> class="DIV_STYLE1"></td>
 </tr>
</table>
</div>
<%
