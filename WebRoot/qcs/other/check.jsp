...
if(!rsa.next()){
	String sql = "select id,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,register,register_time,qcs_result,attachment1,remark from qcs_other where qcs_id='"+qcs_id+"' and check_tag='0'";
...
function TwoSubmit(){
var form=document.getElementById('other_check');
if (form.Ref[0].checked){
form.action = "check_delete_reconfirm.jsp?config_id=<%=config_id%>";
}else{
form.action = "../../qcs_other_check_ok?config_id=<%=config_id%>";
}
if (doValidate('../../xml/qcs/qcs_other.xml','other_check')){
form.submit();
}
}
</script>
<body onload="locateSelectDiv()">
<form id="other_check" METHOD="POST" ENCTYPE="multipart/form-data">
...
<%=demo.getLang("erp","通过")%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onClick="TwoSubmit();" value=<%=demo.getLang("erp","保存")%>>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();" /></td>
 </tr>
 </table>
