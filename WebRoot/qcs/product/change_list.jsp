...
nseer_grid.column_width=[180,180,100,100,180,70];
nseer_grid.auto='<%=demo.getLang("erp","产品名称")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>"> 
['<%=rs.getString("qcs_id")%>','<%=rs.getString("product_id")%>','<%=rs.getString("product_name")%>','<%=rs.getString("register")%>','<%=rs.getString("register_time")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?qcs_id=<%=rs.getString("qcs_id")%>")><%=demo.getLang("erp","变更")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
