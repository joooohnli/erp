...
String rowSummary=demo.getLang("erp","当前等待审核的配置单总数：");
%>
...
String sql="select count(id) from qcs_config_workflow where type_id='05'";
...
nseer_grid.column_width=[180,180,100,100,180,<%for(int i=1;i<workflow_amount;i++){%>70,<%}%>70];
...
%>
['<%=rs.getString("qcs_id")%>','<%=rs.getString("product_id")%>','<%=rs.getString("product_name")%>','<%=rs.getString("register")%>','<%=rs.getString("register_time")%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
