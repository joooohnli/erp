...
String condition="check_tag='2' and intrmanufacture_tag='1' and qcs_apply_tag='0'";
String queue="order by register_time";
String validationXml="../../xml/intrmanufacture/intrmanufacture_intrmanufacture.xml";
String nickName="委外执行单";
String fileName="intrmanufacture.jsp";
...
%>
['<%=rs.getString("intrmanufacture_id")%>','<%=rs.getString("product_id")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("register"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("intrmanufacture_register.jsp?intrmanufacture_id=<%=rs.getString("intrmanufacture_id")%>")><%=demo.getLang("erp","质检调度")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
