...
String condition="check_tag='2' and purchase_tag='1' and qcs_apply_tag='0'";
String queue="order by register_time";
String validationXml="../../xml/purchase/purchase_purchase.xml";
String nickName="采购执行单";
String fileName="purchase.jsp";
...
%>
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../purchase/purchase/query.jsp?purchase_ID=<%=rs.getString("purchase_id")%>")><%=rs.getString("purchase_id")%></div>','<%=rs.getString("product_id")%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("register"))%>','<%=exchange.toHtml(rs.getString("register_time"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("purchase_register.jsp?purchase_id=<%=rs.getString("purchase_id")%>")><%=demo.getLang("erp","质检调度")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
