...
String rowSummary=demo.getLang("erp","符合条件质量标准总数：");
%>
...
nseer_grid.column_width=[180,100,180,100,100,70];
nseer_grid.auto='<%=demo.getLang("erp","标准名称")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?standard_id=<%=rs.getString("standard_id")%>")><%=rs.getString("standard_id")%></div>','<%=exchange.toHtml(rs.getString("standard_name"))%>','<%=exchange.toHtml(rs.getString("product_id"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("designer"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?standard_id=<%=rs.getString("standard_id")%>")><%=demo.getLang("erp","变更")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
