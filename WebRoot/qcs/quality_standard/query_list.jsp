...
String rowSummary=demo.getLang("erp","符合条件的质量标准总数：");
%>
...
nseer_grid.column_width=[40,180,200,180,200,70,80];
...
%>
['<%if(rs.getString("check_tag").equals("1")){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs.getString("id")%>⊙<%=rs.getString("used_tag")%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?standard_id=<%=rs.getString("standard_id")%>")><%=rs.getString("standard_id")%></div>','<%=exchange.toHtml(rs.getString("standard_name"))%>','<%=exchange.toHtml(rs.getString("product_id"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("designer"))%>','<span style="color:<%=color%>"><%=check_tag%></span>'],
<%k++;%>
</page:pages>
['']];
nseer_grid.init();
