...
String condition="check_tag='1' and excel_tag='2' and (dealwith_tag='0' or dealwith_tag='3')";
...
while(rs.next()){
String sql="select view_cycle from qcs_sample_details where sample_id='"+rs.getString("sample_id")+"'";
ResultSet rs1=qcsdb.executeQuery(sql);
if(rs1.next()){
if((now.getTime()-rs.getDate("lately_view_time").getTime())>=(rs1.getInt("view_cycle")*24*60*60*1000)){
%>
['<%=exchange.toHtml(rs.getString("sample_id"))%>','<%=rs.getString("quality_type")%>','<%=rs.getString("apply_id")%>','<%=exchange.toHtml(rs.getString("sampling_person"))%>','<%=exchange.toHtml(rs.getString("lately_view_time"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?sample_id=<%=rs.getString("sample_id")%>&&apply_id=<%=rs.getString("apply_id")%>")><%=demo.getLang("erp","检验登记")%></div>'],
<%}}}%>
['']];
nseer_grid.init();
</script>
