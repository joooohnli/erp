...
%>
<form id="mutiValidation" class="x-form" method="POST" action="../../qcs_sample_check_delete_ok">
...
<td <%=TD_STYLE3%> class="TD_STYLE3" height="10"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1" value="<%=demo.getLang("erp","确认")%>" name="B1"> 
<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="check.jsp?sample_id=<%=sample_id%>&&config_id=<%=config_id%>"></div></td>
</tr>
</table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
<tr <%=TR_STYLE1%> class="TR_STYLE1">
<td <%=TD_STYLE3%> class="TD_STYLE3" height="10"><%=demo.getLang("erp","该样品登记未通过审核，您确认吗？请选择处理：")%></td>
<td <%=TD_STYLE3%> class="TD_STYLE3" height="10"></td>
</tr>
<%
int i=1;
String sql6="select config_id from qcs_workflow where object_ID='"+sample_id+"' and config_id<'"+config_id+"' order by id";
