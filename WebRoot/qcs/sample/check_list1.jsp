...
nseer_grid.auto='<%=demo.getLang("erp","样品登记单编号")%>';
nseer_grid.data = [
...
%>
['<%=exchange.toHtml(rs.getString("sample_id"))%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=winopen("check.jsp?sample_id=<%=rs.getString("sample_id")%>&&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
