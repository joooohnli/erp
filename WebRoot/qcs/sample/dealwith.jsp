...
String sql="select * from qcs_sample where sample_id='"+sample_id+"'";
ResultSet rs=qcs_db.executeQuery(sql);
if(rs.next()){
%>
<form id="sample_dealwith" METHOD="POST" ENCTYPE="multipart/form-data">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE1%> class="TD_STYLE8"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="radio" name="dealwith_tag" value="1" checked><%=demo.getLang("erp","销毁")%>&nbsp;&nbsp;&nbsp;<input type="radio" name="dealwith_tag" value="2"><%=demo.getLang("erp","封存")%>&nbsp;&nbsp;&nbsp;<input type="radio" name="dealwith_tag" value="3"><%=demo.getLang("erp","继续检验")%>&nbsp;&nbsp;&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","提交")%>" onClick="showFormSubmit('dealwith','../../xml/qcs/qcs_sample.xml','sample_dealwith');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></div></td>
 </tr>
 </table>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="quality_type" name="quality_type" type="text" width="100%" value="<%=rs.getString("quality_type")%>" readonly /></td>
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","质检申请单编号")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="apply_id" name="apply_id" type="text" width="100%" value="<%=rs.getString("apply_id")%>" readonly /></td>
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","取样人")%>&nbsp;&nbsp;：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="sampling_person" name="sampling_person" type="text" width="37%" value="<%=rs.getString("sampling_person")%>" readonly /></td>
<td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","取样时间")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="sampling_time" name="sampling_time" type="text" width="37%" value="<%=rs.getString("sampling_time")%>" readonly /></td>
 </tr>
</table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
ResultSet rs1=qcsdb.executeQuery(sql);
int i=1;
while(rs1.next()){
%>
	addRow('bill_body');
	var table_obj=document.getElementById('bill_body');
	document.getElementById(table_obj.rows[0].cells[0].id+'<%=i%>').value='<%=rs1.getString("sample_lable")%>';
	document.getElementById(table_obj.rows[0].cells[0].id+'<%=i%>').onfocus=function(){this.blur();};
   	document.getElementById(table_obj.rows[0].cells[1].id+'<%=i%>').value='<%=rs1.getString("product_id")%>';
	document.getElementById(table_obj.rows[0].cells[1].id+'<%=i%>').onfocus=function(){this.blur();};
    document.getElementById(table_obj.rows[0].cells[2].id+'<%=i%>').value='<%=rs1.getString("product_name")%>';
	document.getElementById(table_obj.rows[0].cells[2].id+'<%=i%>').onfocus=function(){this.blur();};
    document.getElementById(table_obj.rows[0].cells[3].id+'<%=i%>').value='<%=rs1.getString("amount_unit")%>';
	document.getElementById(table_obj.rows[0].cells[3].id+'<%=i%>').onfocus=function(){this.blur();};
    document.getElementById(table_obj.rows[0].cells[4].id+'<%=i%>').value='<%=rs1.getString("sumtotal")%>';
	document.getElementById(table_obj.rows[0].cells[4].id+'<%=i%>').onfocus=function(){this.blur();};
    document.getElementById(table_obj.rows[0].cells[5].id+'<%=i%>').value='<%=rs1.getString("sampling_amount")%>';
	document.getElementById(table_obj.rows[0].cells[5].id+'<%=i%>').onfocus=function(){this.blur();};
    document.getElementById(table_obj.rows[0].cells[6].id+'<%=i%>').value='<%=rs1.getString("view_cycle")%>';
	document.getElementById(table_obj.rows[0].cells[6].id+'<%=i%>').onfocus=function(){this.blur();};
    document.getElementById(table_obj.rows[0].cells[7].id+'<%=i%>').value='<%=rs1.getString("deposit_limit")%>'; 
	document.getElementById(table_obj.rows[0].cells[7].id+'<%=i%>').onfocus=function(){this.blur();};
    document.getElementById(table_obj.rows[0].cells[8].id+'<%=i%>').value='<%=rs1.getString("deposit_place")%>';
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="register" name="register" type="text" width="37%" value="<%=rs.getString("register")%>" readonly /><input id="register_id" name="register_id" type="hidden" value="<%=rs.getString("register_id")%>"/></td>
...
<%=isPrint.printOrNot3(rs.getString("attachment1"),"附件&nbsp;&nbsp;&nbsp;",response,TR_STYLE1,TD_STYLE1,TD_STYLE2,rs.getString("id"),"qcs_sample","attachment1")%>
<%
...
<TABLE width="100%" height="100%" border="0" align="center" cellPadding="0" cellSpacing="0">
 ...
 <TD> 
<div class="cssDiv1">
<div class="cssDiv2"><%=demo.getLang("erp","恩信科技开源ERP")%></div></div>
