...
nseer_grid.columns =[
                       {name: '<%=demo.getLang("erp","方案编号")%>'},
                       {name: '<%=demo.getLang("erp","方案名称")%>'},
...
nseer_grid.column_width=[180,60,180,100,100,100,<%for(int i=1;i<workflow_amount;i++){%>70,<%}%>70];
nseer_grid.auto='<%=demo.getLang("erp","方案名称")%>';
nseer_grid.data = [
...
%>
['<%=exchange.toHtml(rs.getString("solution_id"))%>','<%=exchange.toHtml(rs.getString("solution_name"))%>','<%=exchange.toHtml(rs.getString("product_id"))%>','<%=exchange.toHtml(rs.getString("product_name"))%>','<%=exchange.toHtml(rs.getString("quality_standard_name"))%>','<%=exchange.toHtml(rs.getString("designer"))%>'
<%
for(int i=1;i<=workflow_amount;i++){
	String status="";
	if(rs2.next()){
...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?solution_id=<%=rs.getString("solution_id")%>&&config_id=<%=rs2.getString("config_id")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
%>
</page:pages>
