...
if(!rsa.next()){
	...
	String sql = "select id,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,register,register_time,qcs_result,attachment1,remark,stock_id,stock_name from qcs_stock where qcs_id='"+qcs_id+"' and check_tag='0'";
...
function TwoSubmit(){
var form=document.getElementById('stock_check');
if (form.Ref[0].checked){
form.action = "check_delete_reconfirm.jsp";
}else{
form.action = "../../qcs_stock_check_ok";
}
if (doValidate('../../xml/qcs/qcs_stock.xml','stock_check')){
form.submit();
}
}
</script>
<body onload="locateSelectDiv()">
<form id="stock_check" METHOD="POST" ENCTYPE="multipart/form-data">
...
<%=demo.getLang("erp","通过")%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onClick="TwoSubmit();" value=<%=demo.getLang("erp","确定")%>>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();" /></td>
 </tr>
 </table>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","库房名称")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="stock_name" name="stock_name" type="text" width="37%" value="<%=rs.getString("stock_name")%>"/><input id="stock_id" name="stock_id" type="hidden" value="<%=rs.getString("stock_id")%>"/></td>
