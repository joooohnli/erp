...
try{
	String sql = "select id,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,register,register_time,qcs_result,attachment1,remark,stock_id,stock_name from qcs_stock where qcs_id='"+qcs_id+"' and check_tag='1'";
...
<form id="stock_dealwith" METHOD="POST">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <%
 sql="select type_name,describe2 from qcs_config_public_char where kind_id='07' and describe1='7' and used_tag='1' order by describe1";
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input <%=INPUT_STYLE3%> class="INPUT_STYLE3" id="stock_name" name="stock_name" type="text" width="37%" value="<%=stock_name%>"/><input id="stock_id" name="stock_id" type="hidden" value="<%=stock_id%>"/></td>
...
<td <%=TD_STYLE2%> class="TD_STYLE2" width="87%" colspan="3"><textarea <%=TEXTAREA_STYLE1%> class="TEXTAREA_STYLE1" name="remark" /><%=remark%></textarea></td>
 ...
 <%=isPrint.printOrNot3(attachment1,"附件&nbsp;&nbsp;&nbsp;",response,TR_STYLE1,TD_STYLE1,TD_STYLE2,id,"qcs_stock","attachment1")%>
</table>
