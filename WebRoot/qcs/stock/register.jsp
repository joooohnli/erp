...
try{
	String sql = "select apply_id,product_id,product_name,demand_amount,stock_id,stock_name from qcs_apply_details where qcs_tag='0' and reason='7' and apply_id='"+apply_id+"'";
	ResultSet rs = qcs_db.executeQuery(sql);
    if(rs.next()){%>
<%String table_width1="820px";%>
<body onload="locateSelectDiv()">
<form id="stock_register" METHOD="POST" ENCTYPE="multipart/form-data">
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE1%> class="TD_STYLE8"><%=DgButton.getDraft("'stock_register','../../qcs_stock_register_draft_ok','../../xml/qcs/qcs_stock.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","保存")%>" onClick="showFormSubmit('register');">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back();"></td>
 </tr>
 </table>
...
String quality_way=getnamefromid.getNameFromID((String)session.getAttribute("unit_db_name"),"design_file","product_id",rs.getString("product_id"),"stock_qcs_way");
%>
<tr <%=TR_STYLE1%> class="TR_STYLE1">
