...
max.close();//关闭数据库
%>
['<%=str1[i]%>','<%=demo.getLang("erp","有")%>','<div style="text-decoration : underline;color:#3366FF" onclick=winopen("query_details_list.jsp?tablename=<%=str[i]%>&&human_ID=<%=human_ID%>")><%=demo.getLang("erp","查看")%></div>'],
<%
}else{%>
