...
['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?human_ID=<%=rs.getString("human_ID")%>")><%=rs.getString("human_ID")%></div>','<%=exchange.toHtml(rs.getString("human_name"))%>','<%=exchange.toHtml(rs.getString("sex"))%>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("register_choose.jsp?human_ID=<%=rs.getString("human_ID")%>&&human_name=<%=toUtf8String.utf8String(exchange.toURL(rs.getString("human_name")))%>")><%=demo.getLang("erp","发放")%></div>'],
</page:pages>
