...
nseer_pie_pie.strokecolor='#000';
    fillStyle="<v:TextBox inset='2pt,0pt,0pt,0pt' style='font-size:10pt;'>"+
                "<div align='left'><font color='#000'>系统登录共:"+this.nseerSum+"次</font></div></v:TextBox>";
...
fillStyle="<v:TextBox inset='0pt,3pt,0pt,0pt' style='font-size:10pt;'><div align='left'>"+this.nseerPieS[i][0]+"点至"+this.nseerPieS[i+1][0]+"点:"+nseerMakeCircle(this.nseerPieS[i][1],2)+"次</div></v:TextBox>";
}else{
	fillStyle="<v:TextBox inset='0pt,3pt,0pt,0pt' style='font-size:10pt;'><div align='left'>"+this.nseerPieS[i][0]+"点至"+this.nseerPieS[0][0]+"点:"+nseerMakeCircle(this.nseerPieS[i][1],2)+"次</div></v:TextBox>";
}
...
nseer_make_pie.innerHTML="<v:shape id='cake"+(i+1)+"' type='#Cake_3D'"+
                            ...
                            " fillcolor='"+this.nseerPieS[i][2]+"' onclick='view_info1("+access_num1+")' onmouseover='drawup(cake"+(i+1)+","+(jj + iii / 14)+",doc"+(i+1)+",circle"+(i+1)+")'; onmouseout='drawDown(cake"+(i+1)+","+(jj + iii / 14)+",doc"+(i+1)+",circle"+(i+1)+");'>"+
...
nseer_make_pie.innerHTML+="<v:shape id='doc"+(i+1)+"' type='#3dtxt'  style='position:absolute;left:"+doc_xx+"px;top:"+doc_yy+"px;z-index:2000;display:none;width:100; height:"+height1+";'"+
                            ...
                            "<v:textpath style='v-text-kern:t;font-size:20pt;' trim='t' fitpath='t'  string='"+this.nseerPieS[i][0]+"点:"+nseerMakeCircle(this.nseerPieS[i][1],2)+"次，占比例:"+nseerMakeCircle(this.Percent[i]*100,2)+"%'/> "+
