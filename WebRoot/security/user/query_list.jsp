...
String condition="forbid_tag!='2' and type='0'";
...
nseer_grid.column_width=[180,100,100,70,100,140];
nseer_grid.auto='<%=demo.getLang("erp","机构")%>';
nseer_grid.data = [
<page:pages rs="<%=rs%>" strPage="<%=strPage%>">
	
  ['<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../../hr/file/query.jsp?human_ID=<%=rs.getString("human_ID")%>")><%=rs.getString("human_ID")%></div>','<%=exchange.toHtml(rs.getString("human_name"))%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("../license/query.jsp?human_ID=<%=rs.getString("human_ID")%>")><%=exchange.toHtml(rs.getString("name"))%></div>','<%=exchange.toHtml(rs.getString("chain_name"))%>','<%=exchange.toHtml(rs.getString("human_major_first_kind_name"))%>','<%=exchange.toHtml(rs.getString("human_major_second_kind_name"))%>'],
</page:pages>
['']];
nseer_grid.init();
</script>
