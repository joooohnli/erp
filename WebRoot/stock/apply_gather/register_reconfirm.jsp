...
<div id="nseerGround" class="nseerGround">
	<form id="mutiValidation" method="POST"
		action="../../stock_apply_gather_register_ok"
		onSubmit="return doValidate('../../xml/stock/stock_apply_gather.xml','mutiValidation')&&CheckForm(this)">
		<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
			<tr <%=TR_STYLE1%> class="TR_STYLE1">
				<td <%=TD_STYLE1%> class="TD_STYLE8"><%=DgButton
									.getDraft(
											"'mutiValidation','../../stock_apply_gather_register_draft_ok','../../xml/stock/stock_apply_gather.xml'",
											request)%>&nbsp;
					<input type="submit" <%=SUBMIT_STYLE1%> class="SUBMIT_STYLE1"
						value="<%=demo.getLang("erp", "确认")%>">
					&nbsp;
					<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1"
						value="<%=demo.getLang("erp", "返回")%>" onClick="history.back();">
					&nbsp;
					<input type="reset" <%=RESET_STYLE1%> class="RESET_STYLE1"
		...
		<%
			double demand_amount = 0.0d;
					...
					for (int j = 1; j < product_ID.length; j++) {
						StringTokenizer tokenTO2 = new StringTokenizer(
								cost_price[j], ",");
						String cost_price2 = "";
						...
						cost_price_sum += Double.parseDouble(cost_price2)
								* Double.parseDouble(amount[j]);
					}
		%>
...
<%}%>
	return(true);
		...
		<input type="hidden" name="new_apply"
			value="<%=exchange.toHtml(new_apply)%>">
		...
		<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
			<tr <%=TR_STYLE1%> class="TR_STYLE1">
				<td <%=TD_STYLE5%> class="TD_STYLE5">
					<font size="4"><b><%=demo.getLang("erp", "入库申请单")%></b>
					</font>
				</td>
			</tr>
		</table>
		...
		<table <%=TABLE_STYLE4%> class="TABLE_STYLE4" id=theObjTable>
			<tr <%=TR_STYLE1%> class="TR_STYLE1">
				<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp", "入库人")%>&nbsp;&nbsp;：
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%">
					<input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3"
						name="gatherer_name" value="<%=gatherer_name%>">
				</td>
				<input name="gatherer_ID" type="hidden" value="<%=gatherer_ID%>">
				<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp", "入库理由")%>：
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%">
					<select <%=SELECT_STYLE1%> class="SELECT_STYLE1" name="reason">
						<%
						...
						%>
						<option value="<%=exchange.toHtml(rs5
									.getString("stock_name"))%>"
							selected><%=exchange.toHtml(rs5
									.getString("stock_name"))%></option>
						<%
							} else {
						%>
						<option value="<%=exchange.toHtml(rs5
									.getString("stock_name"))%>"><%=exchange.toHtml(rs5
									.getString("stock_name"))%></option>
						<%
							}
									}
			...
			<tr <%=TR_STYLE1%> class="TR_STYLE1">
				<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp", "是否退回")%>：
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%">
				...
				<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp", "退回时间")%>：
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%">
					<input name="demand_return_time" <%=INPUT_STYLE3%>
						class="INPUT_STYLE3" type="text" onfocus=""
						value="<%=exchange.toHtml(demand_return_time)%>" id="date_start">
				</td>
			</tr>
		</table>
		<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
		...
		<table <%=TABLE_STYLE5%> class="TABLE_STYLE5">
			...
			<%
				for (int i = 1; i < product_ID.length; i++) {
							...
							double subtotal = Double.parseDouble(cost_price3)
									* Double.parseDouble(amount[i]);
			%>
			<tr <%=TR_STYLE1%> class="TR_STYLE1">
				<td <%=TD_STYLE2%> class="TD_STYLE2"><%=i%></td>
				<td <%=TD_STYLE2%> class="TD_STYLE2">
					<input type="text" <%=INPUT_STYLE4%> class="INPUT_STYLE4"
						name="product_name<%=i%>" onFocus="this.blur()"
						value="<%=exchange.toHtml(product_name[i])%>">
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2">
					<input type="text" <%=INPUT_STYLE4%> class="INPUT_STYLE4"
						name="product_ID<%=i%>" onFocus="this.blur()"
						value="<%=product_ID[i]%>">
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2">
					<div><%=product_describe[i]%></div>
					<input type="hidden" <%=INPUT_STYLE4%> class="INPUT_STYLE4"
						name="product_describe<%=i%>" onFocus="this.blur()"
						value="<%=product_describe[i]%>">
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2">
					<input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5"
						name="amount<%=i%>" value="<%=amount[i]%>">
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2">
					<input type="text" <%=INPUT_STYLE4%> class="INPUT_STYLE4"
						name="amount_unit<%=i%>"
						value="<%=exchange.toHtml(amount_unit[i])%>" onFocus="this.blur()">
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2">
					<input name="cost_price<%=i%>" type="hidden"
						value="<%=cost_price[i]%>"><%=cost_price[i]%></td>
			...
			<%
						...
						stock_db.close();
			%>
			<input name="product_amount" type="hidden"
				value="<%=product_ID.length%>">
		</table>
		<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
			<tr <%=TR_STYLE1%> class="TR_STYLE1">
				<td <%=TD_STYLE5%> class="TD_STYLE5">
		...
		<TABLE <%=TABLE_STYLE4%> class="TABLE_STYLE4">
			<tr <%=TR_STYLE1%> class="TR_STYLE1">
				<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp", "总件数")%>&nbsp;&nbsp;：
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%">
					<input name="demand_amount" type="hidden"
						value="<%=demand_amount%>"><%=demand_amount%></td>
				<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp", "总金额")%>&nbsp;&nbsp;：
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%">
					<input name="cost_price_sum" type="hidden"
						value="<%=cost_price_sum%>"><%=cost_price_sum%></td>
			</tr>
			<tr <%=TR_STYLE1%> class="TR_STYLE1">
				<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp", "登记人")%>&nbsp;&nbsp;：
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%">
					<input type="text" <%=INPUT_STYLE3%> class="INPUT_STYLE3"
						name="register" value="<%=exchange.toHtml(register)%>">
				</td>
				<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp", "登记时间")%>：
				</td>
				<td <%=TD_STYLE2%> class="TD_STYLE2" width="40%">
					<input name="register_time" type="hidden" onfocus="setday(this)"
						value="<%=exchange.toHtml(register_time)%>"><%=exchange.toHtml(register_time)%></td>
			</tr>
			<tr <%=TR_STYLE1%> class="TR_STYLE1">
				<td <%=TD_STYLE1%> class="TD_STYLE8" width="9%"><%=demo.getLang("erp", "备注")%>&nbsp;&nbsp;&nbsp;&nbsp;：
				</td>
		...
		<tr <%=TR_STYLE1%> class="TR_STYLE1">
			<td <%=TD_STYLE3%> class="TD_STYLE3">
				<div <%=DIV_STYLE1%> class="DIV_STYLE1">
					<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1"
						value="<%=demo.getLang("erp", "返回")%>" onClick=location="register.jsp">
				</div>
			</td>
		</tr>
	</table>
