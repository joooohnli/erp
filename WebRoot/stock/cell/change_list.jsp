...
String rowSummary=demo.getLang("erp","当前可变更的配置单总数：");
int k=1;
%>
...
nseer_grid.column_width=[180,180,200,70,100,160,70];
...
%>
['<%=rs1.getString("design_ID")%>','<%=rs1.getString("product_ID")%>','<%=exchange.toHtml(rs1.getString("product_name"))%>','<%=exchange.toHtml(rs1.getString("type"))%>','<%=exchange.toHtml(rs1.getString("chain_name"))%>','<%=exchange.toHtml(register_time)%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("change.jsp?design_ID=<%=rs1.getString("design_ID")%>")><%=demo.getLang("erp","变更")%></div>'],
</page:pages>
['']];
nseer_grid.init();
</script>
