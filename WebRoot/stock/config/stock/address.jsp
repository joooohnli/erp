...
<% 
 String sql = "select * from stock_config_public_char where describe1='库房' order by stock_ID";
...
['<%=rs.getString("stock_ID")%>','<%=exchange.toHtml(rs.getString("stock_name"))%>','<%=rs.getString("responsible_person_ID")%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("address_change.jsp?stock_ID=<%=rs.getString("stock_ID")%>")><%=demo.getLang("erp","变更")%></div>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("address_delete_reconfirm.jsp?stock_ID=<%=rs.getString("stock_ID")%>")><%=demo.getLang("erp","释放")%></div>'],
<%
k++;
%>
</page:pages>
