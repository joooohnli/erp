...
%>
,'<div style="text-decoration : underline;color:#3366FF" onclick=id_link("check.jsp?pay_ID=<%=rs6.getString("pay_ID")%>&config_id=<%=rs2.getString("config_id")%>&paying_time=<%=rs6.getString("paying_time")%>")><%=demo.getLang("erp","审核")%></div>'<%}else{%>,'<%=demo.getLang("erp",status)%>'<%}}else{%>,''<%}}%>],
<%
}
	}
m++;
