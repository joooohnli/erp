...
<table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8"><%=DgButton.getDraft("'mutiValidation','../../stock_pay_register_draft_ok','../../xml/stock/stock_pay.xml'",request)%>&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" onclick="registerOk();" value="<%=demo.getLang("erp","提交")%>">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","打印")%>" onClick="javascript:winopen('register_print.jsp?pay_ID=<%=pay_ID%>')">&nbsp;<input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick="history.back()"></td> 
 </tr>
</table>
...
String sql8="select qualified_amount from stock_pre_paying where product_ID='"+rs6.getString("product_ID")+"' and pay_id='"+pay_ID+"' and stock_id='"+rs6.getString("stock_ID")+"'";
ResultSet rs8=stock_db1.executeQuery(sql8);
if(rs8.next()){
qualified_amount=rs8.getDouble("qualified_amount");
}
%>
<input name="qualified_amount" type="hidden" value="<%=qualified_amount%>">
...
<input name="paid_amount1" type="hidden" value="<%=rs6.getDouble("paid_amount")%>">
<input name="details_number" type="hidden" style="width: 100%;" value="<%=i%>">
...
<%if(serial_number_tag==1){%>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="register_choose.jsp?product_ID=<%=rs6.getString("product_ID")%>&&stock_ID=<%=rs6.getString("stock_ID")%>&&pay_ID=<%=pay_ID%>"><%=available.balanceAmount((String)session.getAttribute("unit_db_name"),rs6.getString("product_ID"),rs6.getString("stock_name"))%></a>&nbsp;</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(qualified_amount)%></td><input type="hidden" id="qualified_amount<%=j%>" value="<%=qualified_amount%>">
<%}

else if(serial_number_tag==2){%>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="register_choose_bat.jsp?product_ID=<%=rs6.getString("product_ID")%>&&stock_ID=<%=rs6.getString("stock_ID")%>&&pay_ID=<%=pay_ID%>"><%=available.balanceAmount((String)session.getAttribute("unit_db_name"),rs6.getString("product_ID"),rs6.getString("stock_name"))%></a>&nbsp;</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(qualified_amount)%></td><input type="hidden" id="qualified_amount<%=j%>" value="<%=qualified_amount%>">
<%}
...
<%}%>
 	...
 	<td <%=TD_STYLE2%> class="TD_STYLE2"><input type="hidden" id="paid_amount<%=j%>" value="<%=rs6.getDouble("paid_amount")%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs6.getDouble("paid_amount"))%></td>

<%if(rs6.getString("pay_check_tag").equals("0")&&!rs6.getString("pay_tag").equals("1")){
		 String sql5="select * from stock_paying_temp where pay_ID='"+pay_ID+"' and product_ID='"+rs6.getString("product_ID")+"' and stock_ID='"+rs6.getString("stock_ID")+"'";
		 ResultSet rs5=stock_db1.executeQuery(sql5);
		 if(rs5.next()){
		 %>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" id="amount<%=j%>" name="amount" value="<%=rs5.getDouble("amount")%>"></td>
	 ...
	 <%}else{%>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input type="text" <%=INPUT_STYLE5%> class="INPUT_STYLE5" id="amount<%=j%>" name="amount" ></td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><%if(serial_number_tag==1){%><input type="text" <%=INPUT_STYLE4%> class="INPUT_STYLE4" name="serial_number" size="<%=lll-1%>" style="width:75%;background-color:#FFFFCC"><input name="amount_number" type="hidden" size="<%=lll-1%>" value="">(S/N)<%}else if(serial_number_tag==2){%><input type="text" <%=INPUT_STYLE4%> class="INPUT_STYLE4" name="serial_number" size="<%=lll-1%>" style="width:75%;background-color:#FFFFCC">(B/N)<input name="amount_number" type="hidden" size="<%=lll-1%>" value=""><%}else{%><input name="serial_number" type="hidden" size="<%=lll-1%>" value=""><input name="amount_number" type="hidden" size="<%=lll-1%>" value=""><%}%></td>
<%}}else if(rs6.getString("pay_check_tag").equals("1")&&!rs6.getString("pay_tag").equals("1")){%>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input id="amount<%=j%>" name="amount" type="hidden" value=""><%=demo.getLang("erp","等待审核")%></td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input name="serial_number" type="hidden" value=""><input name="amount_number" type="hidden" size="<%=lll-1%>" value="">&nbsp;</td>
<%}else if(rs6.getString("pay_tag").equals("1")){%>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input id="amount<%=j%>" name="amount" type="hidden" value=""><%=demo.getLang("erp","完成")%></td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><input name="serial_number" type="hidden" value=""><input name="amount_number" type="hidden" size="<%=lll-1%>" value="">&nbsp;</td>
<%}%>
 </tr>
<%
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="demand_amount1" type="hidden" value="<%=rs.getDouble("demand_amount")%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs.getDouble("demand_amount"))%>&nbsp;</td>
	 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","已出库总件数")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="paid_amount" type="hidden" value="<%=rs.getDouble("paid_amount")%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs.getDouble("paid_amount"))%>&nbsp;</td>
 </tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
	 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","应出库总成本")%>：</td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="cost_price_sum" type="hidden" style="width: 100%;" value="<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("cost_price_sum"))%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("cost_price_sum"))%>&nbsp;</td>
	 <td <%=TD_STYLE1%> class="TD_STYLE8" width="13%"><%=demo.getLang("erp","已出库总成本")%>：</td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="paid_cost_price_sum" type="hidden" style="width: 100%;" value="<%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("paid_cost_price_sum"))%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerPrecision")).format(rs.getDouble("paid_cost_price_sum"))%>&nbsp;</td>
</tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
