...
%>
 ...
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE1%> class="TD_STYLE8"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="gather_list.jsp"></td>
 </tr>
 </table>
...
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" colspan="3" width="87%"><input name="gather_ID" type="hidden" value="<%=gather_ID%>"><%=gather_ID%></td>
 </tr>
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><input name="reasonexact" type="hidden" value="<%=exchange.toHtml(rs.getString("reasonexact"))%>"><%=exchange.toHtml(rs.getString("reasonexact"))%>/<%=rs.getString("reasonexact_details")%><input name="reasonexact_details" type="hidden" value="<%=rs.getString("reasonexact_details")%>">&nbsp;</td>
 </tr>
</table>
<table <%=TABLE_STYLE4%> class="TABLE_STYLE4">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<TABLE <%=TABLE_STYLE5%> class="TABLE_STYLE5">	
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="13%"><%=demo.getLang("erp","产品编号")%></td>
	 <td <%=TD_STYLE2%> class="TD_STYLE2" width="11%"><%=demo.getLang("erp","应入库件数")%></td>
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="11%"><%=demo.getLang("erp","已入库件数")%></td>
...
while(rs6.next()){
	if(rs6.getDouble("amount")==0){
		String sql7="update stock_gather_details set gather_tag='1' where gather_ID='"+gather_ID+"' and details_number='"+rs6.getString("details_number")+"'";
...
<%}else{%>
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=rs6.getString("address_group")%>&nbsp;</td>
<%}if(rs6.getString("gather_tag").equals("0")){%>
	 <td <%=TD_STYLE2%> class="TD_STYLE2"><a href="gather_product_details.jsp?gather_ID=<%=rs6.getString("gather_ID")%>&&product_ID=<%=rs6.getString("product_ID")%>&&reason=<%=toUtf8String.utf8String(exchange.toURL(reason))%>&&reasonexact=<%=toUtf8String.utf8String(exchange.toURL(reasonexact))%>&&reasonexact_details=<%=toUtf8String.utf8String(exchange.toURL(reasonexact_details))%>"><%=demo.getLang("erp","调度")%></a></td>
...
<tr <%=TR_STYLE1%> class="TR_STYLE1">
 ...
 <td <%=TD_STYLE2%> class="TD_STYLE2" width="37%"><%=rs.getDouble("gathered_amount")%>&nbsp;</td>
 </tr>
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
...
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE3%> class="TD_STYLE3"><div <%=DIV_STYLE1%> class="DIV_STYLE1"><input type="button" <%=BUTTON_STYLE1%> class="BUTTON_STYLE1" value="<%=demo.getLang("erp","返回")%>" onClick=location="gather_list.jsp"></div></td>
 </tr>
 </table>
<table <%=TABLE_STYLE2%> class="TABLE_STYLE2">
 <tr <%=TR_STYLE1%> class="TR_STYLE1">
