...
String sql9="select * from stock_workflow where object_ID='"+gather_ID+"' and check_tag='0' and config_id<'"+config_id+"'";
ResultSet rs9=stockdb.executeQuery(sql9);
if(!rs9.next()){
	if(vt.validata((String)session.getAttribute("unit_db_name"),"stock_transfer_gather","gather_ID",gather_ID,"check_tag").equals("0")){
try{
...
form.action = "check_delete_reconfirm.jsp?config_id=<%=config_id%>&gather_ID=<%=rs.getString("gather_ID")%>";
}else{
form.action = "../../stock_transfer_gather_check_ok?config_id=<%=config_id%>&gather_ID=<%=rs.getString("gather_ID")%>";
}
}
</script>
...
<form id="mutiValidation" method="POST" onSubmit="return doValidate('../../xml/stock/stock_transfer_gather.xml','mutiValidation')&&TwoSubmit(this)">
