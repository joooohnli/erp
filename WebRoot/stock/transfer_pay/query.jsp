...
if(rs.getString("check_tag").equals("1")&&rs.getString("transfer_pay_tag").equals("0")){
transfer_pay_tag="执行";
color="mediumseagreen";
}else if(rs.getString("check_tag").equals("1")&&rs.getString("transfer_pay_tag").equals("1")){
transfer_pay_tag="完成";
color="3333FF";
...
%>
 ...
 <table <%=TABLE_STYLE6%> class="TABLE_STYLE6">
	<tr <%=TR_STYLE1%> class="TR_STYLE1">
 <td <%=TD_STYLE2%> class="TD_STYLE2"><%=demo.getLang("erp","调出申请单编号：")%><font color="<%=color%>"><%=rs.getString("pay_ID")%><%=transfer_pay_tag%></font> 
 <%=demo.getLang("erp","其中审核状态：")%><font color="<%=color1%>"><%=check_tag%></font> <%=demo.getLang("erp","入库状态：")%><font color="<%=color2%>"><%=pay_tag%></font></td>
 </tr>
 </table>
...
<%
String sql2="select * from stock_transfer_pay_details where pay_ID='"+pay_ID+"' order by details_number";
