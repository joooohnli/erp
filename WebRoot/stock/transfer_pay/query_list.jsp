...
String validationXml="../../xml/stock/stock_transfer_pay.xml";
String nickName="内部调出申请单";
String fileName="query_list.jsp";
String rowSummary=demo.getLang("erp","符合条件的调出申请单总数：");
int k=1;
%>
...
nseer_grid.auto='<%=demo.getLang("erp","调出库")%>';
nseer_grid.data = [
...
transfer_pay_tag="等待";
}else if(rs1.getString("check_tag").equals("1")&&rs1.getString("transfer_pay_tag").equals("0")){
transfer_pay_tag="执行";
color="mediumseagreen";
}else if(rs1.getString("check_tag").equals("1")&&rs1.getString("transfer_pay_tag").equals("1")){
transfer_pay_tag="完成";
delete_tag="0";
color="3333FF";
}
...
%>
['<%if(delete_tag.equals("0")){%><input type="checkbox" id="draft_gar<%=k%>" name="row_id" value="<%=rs1.getString("id")%>⊙<%=delete_tag%>" style="height:10"><%}%>','<div style="text-decoration : underline;color:#3366FF" onclick=id_link("query.jsp?pay_ID=<%=rs1.getString("pay_ID")%>")><span style="color:<%=color%>"><%=rs1.getString("pay_ID")%></span></div>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("reasonexact"))%></span>','<span style="color:<%=color%>"><%=exchange.toHtml(rs1.getString("register_time"))%></span>','<span style="color:<%=color%>"><%=new java.text.DecimalFormat((String)application.getAttribute("nseerAmountPrecision")).format(rs1.getDouble("demand_amount"))%></span>','<span style="color:<%=color%>"><%=transfer_pay_tag%></span>','<span style="color:<%=color%>"><%=check_tag%></span>','<span style="color:<%=color%>"><%=pay_tag%></span>'],
<%k++;%>
</page:pages>
['']];
nseer_grid.init();
