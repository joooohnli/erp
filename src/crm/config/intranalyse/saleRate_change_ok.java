...
public class saleRate_change_ok extends HttpServlet{
...
retail_discount=retail_discount.replaceAll(",", "");
String sqll="select * from security_users where human_ID='"+human_ID+"'";
ResultSet rs=security_db.executeQuery(sqll);
if(!rs.next()){
response.sendRedirect("crm/config/intranalyse/saleRate_change_ok_a.jsp");
}else{
String sql2="update security_users set retail_discount='"+retail_discount+"' where human_ID='"+human_ID+"'";
security_db.executeUpdate(sql2);


response.sendRedirect("crm/config/intranalyse/saleRate_change_ok_b.jsp");
