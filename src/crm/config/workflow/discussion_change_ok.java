...
if(chain_id_array==null){
  	response.sendRedirect("crm/config/workflow/discussion_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select discussion_id from crm_discussion where check_tag='0'";
	ResultSet rs=crm_db1.executeQuery(sql);
	while(rs.next()){
		sql="update crm_workflow set check_tag='0' where object_ID='"+rs.getString("discussion_id")+"'";
	...
	crm_db.executeUpdate(sql1);	
response.sendRedirect("crm/config/workflow/discussion_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	crm_db.commit();
	crm_db1.commit();
	crm_db1.close();
