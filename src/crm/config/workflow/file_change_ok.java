...
if(chain_id_array==null){
  	response.sendRedirect("crm/config/workflow/file_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select customer_id from crm_file where check_tag='0'";
	ResultSet rs=crm_db1.executeQuery(sql);
	while(rs.next()){
		sql="update crm_workflow set check_tag='0' where object_ID='"+rs.getString("customer_id")+"'";
	...
	crm_db.executeUpdate(sql1);	
response.sendRedirect("crm/config/workflow/file_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	crm_db.commit();
	crm_db1.commit();
	crm_db1.close();
