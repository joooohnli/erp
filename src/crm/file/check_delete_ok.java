...
if(!rs6.next()&&vt.validata((String)dbSession.getAttribute("unit_db_name"),"crm_file","customer_ID",customer_ID,"excel_tag").equals("1")){
if(choice!=null){
	if(choice.equals("")){
	CheckRows del=new CheckRows();//报警这是属于审核未通过
	del.delRowCount((String)dbSession.getAttribute("unit_db_name"),"crm_file");//加ADD方法
	String sql = "update crm_file set check_tag='9' where customer_ID='"+customer_ID+"'" ;
	crm_db.executeUpdate(sql) ;
	sql = "delete from crm_workflow where object_ID='"+customer_ID+"'" ;
		crm_db.executeUpdate(sql) ;
	}else{

	sql6="select id from crm_workflow where object_ID='"+customer_ID+"' and config_id<'"+config_id+"' and config_id>='"+choice+"'";
...
response.sendRedirect("crm/file/check_delete_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("crm/file/check_delete_ok.jsp?finished_tag=2");
}
crm_db.commit();
crm_db1.commit();
crm_db.close();
