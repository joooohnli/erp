...
int which_time=0;
sql="select which_time from crm_order where order_ID='"+reasonexact+"'";
rset=crm_db.executeQuery(sql);
if(rset.next()){
...
response.sendRedirect("crm/invoice/register_ok.jsp?order_ID="+reasonexact+"&finished_tag=1");


}

}else{
response.sendRedirect("crm/invoice/register_ok.jsp?order_ID="+reasonexact+"&finished_tag=2");
}
}else{
