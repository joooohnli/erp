...
try{
	if(choice.equals("")){
	String sql2="update crm_order set check_tag='9',order_tag='0',check_time='"+check_time+"',checker='"+checker+"' where order_ID='"+order_ID+"' and check_tag='0'";
	crm_db.executeUpdate(sql2) ;
	sql2 = "delete from crm_workflow where object_ID='"+order_ID+"' and which_time='0'" ;
	crm_db.executeUpdate(sql2) ;
	}else{

	sql6="select id from crm_workflow where object_ID='"+order_ID+"' and config_id<'"+config_id+"' and config_id>='"+choice+"' and which_time='0'";
	...
	while(rs6.next()){
  	...
  	response.sendRedirect("crm/order/check_delete_ok.jsp?finished_tag=0");
}else{
	
response.sendRedirect("crm/order/check_delete_ok.jsp?finished_tag=3");

}
}else{
	response.sendRedirect("crm/order/check_delete_ok.jsp?finished_tag=2");
}
}else{
	response.sendRedirect("crm/order/check_delete_ok.jsp?finished_tag=1");
}
crm_db.commit();
crm_db1.commit();
crm_db.close();
