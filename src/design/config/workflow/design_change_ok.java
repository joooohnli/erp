...
public class design_change_ok extends HttpServlet{
...
if(chain_id_array==null){
  	response.sendRedirect("design/config/workflow/design_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select design_id from design_module where check_tag='0'";
	ResultSet rs=design_db1.executeQuery(sql);
	while(rs.next()){
		sql="update design_workflow set check_tag='0' where object_ID='"+rs.getString("design_id")+"'";
	...
	design_db.executeUpdate(sql1);	
response.sendRedirect("design/config/workflow/design_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	design_db.commit();
	design_db1.commit();
	design_db1.close();
