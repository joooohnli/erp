...
if(chain_id_array==null){
  	response.sendRedirect("design/config/workflow/file_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select product_id from design_file where check_tag='0'";
	...
	design_db.executeUpdate(sql1);	
response.sendRedirect("design/config/workflow/file_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	design_db.commit();
	design_db1.commit();
	design_db1.close();
