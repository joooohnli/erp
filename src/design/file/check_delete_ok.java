...
if(choice!=null){
	if(choice.equals("")){
	String sql = "update design_file set check_tag='9' where product_ID='"+product_ID+"'" ;
	design_db.executeUpdate(sql) ;
	sql = "delete from design_workflow where object_ID='"+product_ID+"'" ;
		design_db.executeUpdate(sql) ;
	}else{

	sql6="select id from design_workflow where object_ID='"+product_ID+"' and config_id<'"+config_id+"' and config_id>='"+choice+"'";
	rs6=design_db.executeQuery(sql6);
	while(rs6.next()){
		String sql = "update design_workflow set check_tag='0' where id='"+rs6.getString("id")+"'" ;
		design_db1.executeUpdate(sql) ;
	}
	}	
response.sendRedirect("design/file/check_delete_ok.jsp?finished_tag=0");
}else{
response.sendRedirect("design/file/check_delete_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("design/file/check_delete_ok.jsp?finished_tag=2");
}
design_db.commit();
design_db1.commit();
design_db.close();
