...
if(vt.validata((String)dbSession.getAttribute("unit_db_name"),"design_file","product_ID",product_ID,"check_tag").equals("9")||vt.validata((String)dbSession.getAttribute("unit_db_name"),"design_file","product_ID",product_ID,"check_tag").equals("5")){
...
if(rsList.size()==0){
	...
	if(!type.equals("物料")&&!type.equals("服务型产品")){//电子商务的配送信息发布
		...
		sql="update design_file set check_tag='0' where product_ID='"+product_ID+"'";
		design_db.executeUpdate(sql) ;
...
response.sendRedirect("draft/design/file_choose_attachment.jsp?product_ID="+product_ID+"");
	...
	response.sendRedirect("draft/design/file/uncheck_ok.jsp?finished_tag=1");
	}
}else{
...
response.sendRedirect("draft/design/file_ok.jsp?finished_tag=0");
