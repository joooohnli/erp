...
public class comment_check_ok extends HttpServlet{
...
if(vt.validata((String)session.getAttribute("unit_db_name"),"ecommerce_comment","comment_ID",comment_ID,"check_tag").equals("9")||vt.validata((String)session.getAttribute("unit_db_name"),"ecommerce_comment","comment_ID",comment_ID,"check_tag").equals("5")){
...
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	...
	String sqla = "update ecommerce_comment set kind='"+kind+"',name='"+name+"',sex='"+sex+"',company='"+company+"',address='"+address+"',tel='"+tel+"',email='"+email+"',comment='"+comment+"',reply='"+reply+"'";
...
String sql=sqla+sqlb;
	...
	if(rsList.size()==0){
		sql="update ecommerce_other set check_tag='1' where other_ID='"+comment_ID+"'";
		ecommerce_db.executeUpdate(sql);
	}else{
		sql="delete from ecommerce_workflow where object_ID='"+comment_ID+"'";
		ecommerce_db.executeUpdate(sql) ;		
		sql="update ecommerce_comment set check_tag='0' where comment_ID='"+comment_ID+"'";
		ecommerce_db.executeUpdate(sql) ;
		Iterator ite=rsList.iterator();
		while(ite.hasNext()){
		elem=(String[])ite.next();
		sql = "insert into ecommerce_workflow(config_id,object_ID,describe1,describe2) values ('"+elem[0]+"','"+comment_ID+"','"+elem[1]+"','"+elem[2]+"')" ;
		ecommerce_db.executeUpdate(sql) ;
		}
	}
response.sendRedirect("draft/ecommerce/comment_ok.jsp?finished_tag=4");

}else{
response.sendRedirect("draft/ecommerce/comment_ok.jsp?finished_tag=5");
}
