...
public class comment_draft_ok extends HttpServlet{
...
if(vt.validata((String)session.getAttribute("unit_db_name"),"ecommerce_comment","comment_ID",comment_ID,"check_tag").equals("5")||vt.validata((String)session.getAttribute("unit_db_name"),"ecommerce_comment","comment_ID",comment_ID,"check_tag").equals("9")){
...
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	...
	String sqla = "update ecommerce_comment set kind='"+kind+"',name='"+name+"',sex='"+sex+"',company='"+company+"',address='"+address+"',tel='"+tel+"',email='"+email+"',comment='"+comment+"',reply='"+reply+"',check_tag='5'";
...
response.sendRedirect("draft/ecommerce/comment_ok.jsp?finished_tag=0");

}else{
response.sendRedirect("draft/ecommerce/comment_ok.jsp?finished_tag=1");
}
