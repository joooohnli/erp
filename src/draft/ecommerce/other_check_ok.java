...
public class other_check_ok extends HttpServlet{
...
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	...
	String sqla = "update ecommerce_other set topic='"+topic+"',chain_id='"+chain_id+"',chain_name='"+chain_name+"',other_ID='"+other_ID+"',content='"+content+"'";
...
String sql=sqla+sqlb;
	...
	List rsList = GetWorkflow.getList(ecommerce_db, "ecommerce_config_workflow", "09");
	...
	if(rsList.size()==0){
		...
		ecommerce_db.executeUpdate(sql) ;		
		sql="update ecommerce_other set check_tag='0' where other_ID='"+other_ID+"'";
		ecommerce_db.executeUpdate(sql) ;
...
response.sendRedirect("draft/ecommerce/other_ok.jsp?finished_tag=4");
}else{
response.sendRedirect("draft/ecommerce/other_ok.jsp?finished_tag=5");
}
}catch(Exception ex){
	response.sendRedirect("draft/ecommerce/other_ok.jsp?finished_tag=6");
}
