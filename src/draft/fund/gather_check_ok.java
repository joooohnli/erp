...
response.sendRedirect("draft/fund/gather_ok.jsp?finished_tag=8");
}
}else{
	String sql="update fund_fund set check_tag='1' where fund_ID='"+fund_ID+"'";
		...
		response.sendRedirect("draft/fund/gather_ok.jsp?finished_tag=4");
	}
}
catch (Exception ex){
ex.printStackTrace();
...
response.sendRedirect("draft/fund/gather_ok.jsp?finished_tag=5");
}
}else{
