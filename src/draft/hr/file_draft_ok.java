...
public class file_draft_ok extends HttpServlet{//创建方法
...
response.sendRedirect("draft/hr/file_choose_picture.jsp?human_ID="+human_ID+"&tag=1");
...
response.sendRedirect("draft/hr/file_ok.jsp?finished_tag=1");
}
}else{
	...
	try{
		...
		response.sendRedirect("draft/hr/file_choose_attachment.jsp?human_ID="+human_ID+"&&tag="+tag+"");
	}catch(Exception ex){
	response.sendRedirect("draft/hr/file_ok.jsp?finished_tag=6");
