...
public class salaryStandard_garbage_ok extends HttpServlet{
//创建方法
...
String sqll="update hr_salary_standard set check_tag='0' where standard_ID='"+standard_ID+"'";
	...
	String sql2="update hr_salary_standard set standard_name='"+standard_name+"',major_type='"+major_type+"',designer='"+designer+"',register='"+register+"',register_time='"+register_time+"',salary_sum='"+salary_sum+"',check_tag='2' where standard_ID='"+standard_ID+"'";
...
response.sendRedirect("draft/hr/salaryStandard_ok.jsp?finished_tag=2");
...
response.sendRedirect("draft/hr/salaryStandard_ok.jsp?finished_tag=3");
}
hr_db.commit();
hr_db.close();
}else{
