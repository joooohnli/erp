...
public class salary_draft_ok extends HttpServlet{
//创建方法
...
if(rs.next()){
	...
	response.sendRedirect("draft/hr/salary_ok.jsp?finished_tag=0");
}else{
	response.sendRedirect("draft/hr/salary_ok.jsp?finished_tag=6");
}
}else{
	response.sendRedirect("draft/hr/salary_ok.jsp?finished_tag=1");
}
hr_db.commit();
hr_db.close();
}
