...
try{
	...
	logistics_db.executeUpdate(sql) ;
	/*****************************************************/
    List rsList = GetWorkflow.getList(logistics_db, "logistics_config_workflow", "01");
	String[] elem=new String[3];
if(rsList.size()==0){
		sql="update logistics_file set check_tag='1' where provider_ID='"+provider_ID+"'";
		logistics_db.executeUpdate(sql);
}else{
		sql="update logistics_file set check_tag='0' where provider_ID='"+provider_ID+"'";
		logistics_db.executeUpdate(sql) ;
...
response.sendRedirect("draft/logistics/file_choose_attachment.jsp?provider_ID="+provider_ID+"");
	...
	response.sendRedirect("draft/logistics/file_ok.jsp?finished_tag=4");
	}
	logistics_db.commit();	
	design_db.commit();	
	logistics_db.close();
