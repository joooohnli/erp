...
public class culture_check_ok extends HttpServlet{
...
if(!rs.next()){
	...
	response.sendRedirect("draft/oa/culture_ok.jsp?finished_tag=5");
}else{
...
List rsList = GetWorkflow.getList(oa_db, "oa_config_workflow", "05");
	...
	if(rsList.size()==0){
	String sqla = "update oa_culture set content='"+content+"',remark='"+remark+"',check_tag='1'";
...
String sqla = "update oa_culture set content='"+content+"',remark='"+remark+"',check_tag='0'";
String sqlb = " where culture_ID='"+culture_ID+"'" ;
if(attachment!=null){
for(int i=0;i<attachment.length;i++){
	sqla=sqla+","+attachment[i]+"=''";
...
response.sendRedirect("draft/oa/culture_ok.jsp?finished_tag=4");
}
oa_db.commit();
oa_db.close();
}catch(Exception ex){
