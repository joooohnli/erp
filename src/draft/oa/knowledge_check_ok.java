...
public class knowledge_check_ok extends HttpServlet{
...
if(!rs.next()){
	...
	response.sendRedirect("draft/oa/knowledge_ok.jsp?finished_tag=5");
}else{
...
String sqla = "update oa_knowledge set content='"+content+"',remark='"+remark+"',check_tag='1'";
...
String sql=sqla+sqlb;
	oa_db.executeUpdate(sql) ;
	  ...
	  String sql2="delete from oa_workflow where object_ID='"+knowledge_ID+"' and type_id='04'";
		oa_db.executeUpdate(sql2) ;

	String sqla = "update oa_knowledge set content='"+content+"',remark='"+remark+"',check_tag='0'";
	String sqlb = " where knowledge_ID='"+knowledge_ID+"'" ;
if(attachment!=null){
for(int i=0;i<attachment.length;i++){
	sqla=sqla+","+attachment[i]+"=''";
...
Iterator ite=rsList.iterator();
		...
		sql2 = "insert into oa_workflow(config_id,object_ID,describe1,describe2,type_id) values ('"+elem[0]+"','"+knowledge_ID+"','"+elem[1]+"','"+elem[2]+"','04')" ;
		oa_db.executeUpdate(sql2) ;
		}
	}
...
response.sendRedirect("draft/oa/knowledge_ok.jsp?finished_tag=4");
}
}catch(Exception ex){
