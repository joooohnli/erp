...
public class planing_check_ok extends HttpServlet{
...
String sql1="select * from oa_planing where planing_ID='"+planing_ID+"' and (check_tag='5' or check_tag='9')";
ResultSet rs=oa_db.executeQuery(sql1);
if(!rs.next()){
	response.sendRedirect("draft/oa/planing_ok.jsp?finished_tag=5");
}else{
...
String sql3="select * from oa_planing where planing_ID='"+planing_ID+"'";
...
List rsList = GetWorkflow.getList(oa_db, "oa_config_workflow", "01");
    ...
    String sqla = "update oa_planing set content='"+content+"',check_tag='1'";
...
String sql=sqla+sqlb;
    ...
    oa_db.executeUpdate(sql) ;
     ...
     String sql2="delete from oa_workflow where object_ID='"+planing_ID+"' and type_id='01'";
		oa_db.executeUpdate(sql2) ;
String sqla = "update oa_planing set content='"+content+"',check_tag='0'";
String sqlb = " where planing_ID='"+planing_ID+"'" ;
if(attachment!=null){
for(int i=0;i<attachment.length;i++){
	sqla=sqla+","+attachment[i]+"=''";
...
Iterator ite=rsList.iterator();
		...
		sql2 = "insert into oa_workflow(config_id,object_ID,describe1,describe2,type_id) values ('"+elem[0]+"','"+planing_ID+"','"+elem[1]+"','"+elem[2]+"','01')" ;
		oa_db.executeUpdate(sql2) ;
		}
	}

response.sendRedirect("draft/oa/planing_ok.jsp?finished_tag=4");
}
}catch(Exception ex){
	response.sendRedirect("draft/oa/planing_ok_a.jsp?planing_ID="+planing_ID);
}
