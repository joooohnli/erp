...
public class publicize_check_ok extends HttpServlet{
...
String sql1="select * from oa_publicize where publicize_ID='"+publicize_ID+"' and (check_tag='5' or check_tag='9')";
ResultSet rs=oa_db.executeQuery(sql1);
if(!rs.next()){
	
	response.sendRedirect("draft/oa/publicize_ok.jsp?finished_tag=5");
}else{
...
List rsList = GetWorkflow.getList(oa_db, "oa_config_workflow", "02");
	...
	String sqla = "update oa_publicize set content='"+content+"',remark='"+remark+"',check_tag='1'";
...
String sqla = "update oa_publicize set content='"+content+"',remark='"+remark+"',check_tag='0'";
	String sqlb = " where publicize_ID='"+publicize_ID+"'" ;
if(attachment!=null){
for(int i=0;i<attachment.length;i++){
	sqla=sqla+","+attachment[i]+"=''";
...
response.sendRedirect("draft/oa/publicize_ok.jsp?finished_tag=4");
}
}catch(Exception ex){
	response.sendRedirect("draft/oa/publicize_ok_a.jsp?publicize_ID="+publicize_ID);
}
oa_db.commit();
oa_db.close();
}else{
