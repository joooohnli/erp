...
public class publicize_draft_ok extends HttpServlet{
...
if(!rs.next()){
	response.sendRedirect("draft/oa/publicize_ok.jsp?finished_tag=1");
}else{
...
String remark=exchange.toHtml(bodyab);
	...
	String sqla = "update oa_publicize set register='"+register+"',register_time='"+register_time+"',content='"+content+"',remark='"+remark+"'";
...
String sql=sqla+sqlb;
	...
	response.sendRedirect("draft/oa/publicize_ok.jsp?finished_tag=0");
}
}catch(Exception ex){
