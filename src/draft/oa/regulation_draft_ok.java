...
public class regulation_draft_ok extends HttpServlet{
...
if(!rs.next()){
	...
	response.sendRedirect("draft/oa/regulation_ok.jsp?finished_tag=1");
}else{
...
String remark=exchange.toHtml(bodyab);
	...
	String sqla = "update oa_regulation set register='"+register+"',register_time='"+register_time+"',content='"+content+"',remark='"+remark+"'";
...
response.sendRedirect("draft/oa/regulation_ok.jsp?finished_tag=0");
}
}catch(Exception ex){
