...
if(purchase_db.conn((String)dbSession.getAttribute("unit_db_name"))&&stock_db.conn((String)dbSession.getAttribute("unit_db_name"))&&stock_db1.conn((String)dbSession.getAttribute("unit_db_name"))){
...
String sql = "update stock_apply_gather set reason='"+reason+"',register='"+register+"',register_time='"+register_time+"',demand_return_time='"+demand_return_time+"',check_time='"+check_time+"',checker='"+checker+"',remark='"+remark+"',demand_amount='"+demand_amount+"',cost_price_sum='"+cost_price_sum+"',not_return_tag='"+not_return_tag+"' where gather_ID='"+gather_ID+"'" ;
	...
	stock_db1.executeUpdate(sql) ;		
        ...
        String sql2="update stock_apply_gather set check_tag='0' where gather_ID='"+gather_ID+"'" ;
...
response.sendRedirect("draft/purchase/credit_ok.jsp?finished_tag=4");
	}else{
	response.sendRedirect("draft/purchase/credit_ok_b.jsp?gather_ID="+gather_ID+"");
	}
	}else{
	response.sendRedirect("draft/purchase/credit_ok_c.jsp?gather_ID="+gather_ID+"");
	}
	}else{
	response.sendRedirect("draft/purchase/credit_ok.jsp?finished_tag=5");
    }
