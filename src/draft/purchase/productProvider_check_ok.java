...
public class productProvider_check_ok extends HttpServlet{
...
if(m!=0){
response.sendRedirect("draft/purchase/productProvider_ok.jsp?finished_tag=6");
}else if(num==0&&provider_IDn.length==1){
	response.sendRedirect("draft/purchase/productProvider_ok.jsp?finished_tag=7");
	}else{
...
response.sendRedirect("draft/purchase/productProvider_ok.jsp?finished_tag=4");
}else{
response.sendRedirect("draft/purchase/productProvider_ok.jsp?finished_tag=5");
}
  }
		purchase_db.commit();	
purchase_db.close();
