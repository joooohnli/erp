...
//String gather_ID=count.ReadFile("/erp/WEB-INF/stockGathercount.txt");
//count.WriteFile("/erp/WEB-INF/stockGathercount.txt",gather_ID);

if(vt.validata((String)dbSession.getAttribute("unit_db_name"),"purchase_purchase","purchase_ID",purchase_ID,"check_tag").equals("5")){
try{
	String sql1 = "update purchase_purchase set check_tag='1' where purchase_ID='"+purchase_ID+"'" ;
...
ex.printStackTrace();
	...
	response.sendRedirect("draft/purchase/purchase_ok.jsp?finished_tag=4");
	}else{
	
	response.sendRedirect("draft/purchase/purchase_ok.jsp?finished_tag=5");
}
