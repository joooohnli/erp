...
public class accident_draft_ok extends HttpServlet{
...
if(!rs.next()){
  response.sendRedirect("draft/qcs/accident_ok.jsp?finished_tag=1");
}else{
...
sqla = "update qcs_accident set accident_time='"+accident_time+"',accident_name='"+accident_name+"',product_id='"+product_id+"',product_name='"+product_name+"',accident_outlines='"+accident_outlines+"',explanation='"+explanation+"',survey_result='"+survey_result+"',scene_measure='"+scene_measure+"',register_id='"+register_id+"',register='"+register+"',register_time='"+register_time+"',remark='"+remark+"',check_tag='5'";
