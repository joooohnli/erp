...
public class crmDeliver_check_ok extends HttpServlet{
...
try{
	...
	String sqla="select attachment1 from qcs_crm_deliver where qcs_id='"+qcs_id+"' and (check_tag='5' or check_tag='9')";
	ResultSet rs=qcs_db.executeQuery(sqla);
	if(!rs.next()){	
		response.sendRedirect("draft/qcs/crmDeliver_ok.jsp?finished_tag=5");
	}else{
...
String remark=exchange.toHtml(bodyab);
	...
	sqla = "update qcs_crm_deliver set apply_id='"+apply_id+"',stock_name='"+stock_name+"',product_id='"+product_id+"',product_name='"+product_name+"',qcs_amount='"+qcs_amount+"',qcs_time='"+qcs_time+"',quality_way='"+quality_way+"',quality_solution='"+quality_solution+"',sampling_standard='"+sampling_standard+"',sampling_amount='"+sampling_amount+"',accept='"+accept+"',reject='"+reject+"',qualified='"+qualified+"',unqualified='"+unqualified+"',changer_id='"+changer_id+"',qcs_result='"+qcs_result+"',changer='"+changer+"',register_time='"+register_time+"',remark='"+remark+"',check_tag='0'";
...
sql="delete from qcs_crm_deliver_details where qcs_id='"+qcs_id+"'";
qcs_db.executeUpdate(sql);
...
for(int i=0;i<item.length;i++){
	if(!item[i].equals("")){
	sql="insert into qcs_crm_deliver_details(qcs_id,item,default_basis,ready_basis,quality_method,analyse_method,standard_value,standard_max,standard_min,quality_value,sampling_amount_d,qualified_d,unqualified_d,quality_result,unqualified_reason,details_number) values('"+qcs_id+"','"+item[i]+"','"+default_basis[i]+"','"+ready_basis[i]+"','"+quality_method[i]+"','"+analyse_method[i]+"','"+standard_value[i]+"','"+standard_max[i]+"','"+standard_min[i]+"','"+quality_value[i]+"','"+sampling_amount_d[i]+"','"+qualified_d[i]+"','"+unqualified_d[i]+"','"+quality_result[i]+"','"+unqualified_reason[i]+"','"+i+"')";
	qcs_db.executeUpdate(sql);
}
}

List rsList = GetWorkflow.getList(qcs_db, "qcs_config_workflow", "08");
if(rsList.size()==0){
	sql="update qcs_crm_deliver set check_tag='1' where qcs_id='"+qcs_id+"'";
	qcs_db.executeUpdate(sql);
}else{
	sql="delete from qcs_workflow where object_ID='"+qcs_id+"'";
	qcs_db.executeUpdate(sql) ;
	sql="update qcs_crm_deliver set check_tag='0' where qcs_id='"+qcs_id+"'";
	qcs_db.executeUpdate(sql) ;
	...
	response.sendRedirect("draft/qcs/crmDeliver_ok.jsp?finished_tag=4");
}
qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("draft/qcs/crmDeliver_ok.jsp?finished_tag=7");
	}
}catch(Exception ex){
	response.sendRedirect("draft/qcs/crmDeliver_ok.jsp?finished_tag=6");
}
}else{
