...
public class crmDeliver_draft_ok extends HttpServlet{
...
try{
	...
	if(!rs.next()){	
		response.sendRedirect("draft/qcs/crmDeliver_ok.jsp?finished_tag=1");
	}else{
...
response.sendRedirect("draft/qcs/crmDeliver_ok.jsp?finished_tag=0");
}
qcs_db.commit();
qcs_db.close();
}else{
