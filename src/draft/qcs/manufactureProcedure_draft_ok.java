...
public class manufactureProcedure_draft_ok extends HttpServlet{
...
try{
	...
	if(!rs.next()){	
		response.sendRedirect("draft/qcs/manufactureProcedure_ok.jsp?finished_tag=1");
	}else{
...
String remark=exchange.toHtml(bodyab);
	...
	sqla = "update qcs_manufacture_procedure set apply_id='"+apply_id+"',product_id='"+product_id+"',product_name='"+product_name+"',qcs_amount='"+qcs_amount+"',qcs_time='"+qcs_time+"',quality_way='"+quality_way+"',accept='"+accept+"',reject='"+reject+"',procedure_name='"+procedure_name+"',quality_solution='"+quality_solution+"',sampling_standard='"+sampling_standard+"',sampling_amount='"+sampling_amount+"',qualified='"+qualified+"',unqualified='"+unqualified+"',checker_id='"+checker_id+"',qcs_result='"+qcs_result+"',register='"+register+"',register_time='"+register_time+"',remark='"+remark+"'";
...
response.sendRedirect("draft/qcs/manufactureProcedure_ok.jsp?finished_tag=0");
}
qcs_db.commit();
qcs_db.close();
}else{
