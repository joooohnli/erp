...
public class manufactureProduct_check_ok extends HttpServlet{
...
try{
	...
	String sqla="select attachment1 from qcs_manufacture_product where qcs_id='"+qcs_id+"' and (check_tag='5' or check_tag='9')";
	ResultSet rs=qcs_db.executeQuery(sqla);
	if(!rs.next()){	
		response.sendRedirect("draft/qcs/manufactureProduct_ok.jsp?finished_tag=5");
	}else{
...
String remark=exchange.toHtml(bodyab);
	...
	sqla = "update qcs_manufacture_product set apply_id='"+apply_id+"',product_id='"+product_id+"',product_name='"+product_name+"',qcs_amount='"+qcs_amount+"',qcs_time='"+qcs_time+"',quality_way='"+quality_way+"',quality_solution='"+quality_solution+"',sampling_standard='"+sampling_standard+"',sampling_amount='"+sampling_amount+"',accept='"+accept+"',reject='"+reject+"',qualified='"+qualified+"',unqualified='"+unqualified+"',register_id='"+register_id+"',qcs_result='"+qcs_result+"',register='"+register+"',register_time='"+register_time+"',remark='"+remark+"',check_tag='0'";
...
sql="delete from qcs_manufacture_product_details where qcs_id='"+qcs_id+"'";
qcs_db.executeUpdate(sql);
...
if(rsList.size()==0){
	...
	qcs_db.executeUpdate(sql) ;
	sql="update qcs_manufacture_product set check_tag='0' where qcs_id='"+qcs_id+"'";
	qcs_db.executeUpdate(sql) ;
...
response.sendRedirect("draft/qcs/manufactureProduct_ok.jsp?finished_tag=4");
}
qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("draft/qcs/manufactureProduct_ok.jsp?finished_tag=7");
	}
}catch(Exception ex){
	response.sendRedirect("draft/qcs/manufactureProduct_ok.jsp?finished_tag=6");
}
