...
public class manufactureProduct_draft_ok extends HttpServlet{
...
try{
	...
	if(!rs.next()){	
		response.sendRedirect("draft/qcs/manufactureProduct_ok.jsp?finished_tag=1");
	}else{
...
String remark=exchange.toHtml(bodyab);
	...
	sqla = "update qcs_manufacture_product set apply_id='"+apply_id+"',product_id='"+product_id+"',product_name='"+product_name+"',qcs_amount='"+qcs_amount+"',qcs_time='"+qcs_time+"',quality_way='"+quality_way+"',quality_solution='"+quality_solution+"',sampling_standard='"+sampling_standard+"',sampling_amount='"+sampling_amount+"',accept='"+accept+"',reject='"+reject+"',qualified='"+qualified+"',unqualified='"+unqualified+"',register_id='"+register_id+"',qcs_result='"+qcs_result+"',register='"+register+"',register_time='"+register_time+"',remark='"+remark+"',check_tag='5'";
...
response.sendRedirect("draft/qcs/manufactureProduct_ok.jsp?finished_tag=0");
}
qcs_db.commit();
qcs_db.close();
}else{
