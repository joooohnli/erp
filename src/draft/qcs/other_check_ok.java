...
try{
	...
	String sqla="select attachment1 from qcs_other where qcs_id='"+qcs_id+"' and (check_tag='5' or check_tag='9')";
	ResultSet rs=qcs_db.executeQuery(sqla);
	if(!rs.next()){	
		response.sendRedirect("draft/qcs/other_ok.jsp?finished_tag=5");
	}else{
...
List rsList = GetWorkflow.getList(qcs_db, "qcs_config_workflow", "12");
if(rsList.size()==0){
	sql="update qcs_other set check_tag='1' where qcs_id='"+qcs_id+"'";
	qcs_db.executeUpdate(sql);
}else{
	sql="delete from qcs_workflow where object_ID='"+qcs_id+"'";
	qcs_db.executeUpdate(sql) ;
	sql="update qcs_other set check_tag='0' where qcs_id='"+qcs_id+"'";
	qcs_db.executeUpdate(sql) ;
...
response.sendRedirect("draft/qcs/other_ok.jsp?finished_tag=4");
}
qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("draft/qcs/other_ok.jsp?finished_tag=7");
	}
}catch(Exception ex){
	response.sendRedirect("draft/qcs/other_ok.jsp?finished_tag=6");
}
