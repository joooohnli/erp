...
public class purchase_draft_ok extends HttpServlet{
...
try{
	...
	String sqla="select attachment1 from qcs_purchase where qcs_id='"+qcs_id+"' and (check_tag='5' or check_tag='9')";
	ResultSet rs=qcs_db.executeQuery(sqla);
	if(!rs.next()){	
		response.sendRedirect("draft/qcs/purchase_ok.jsp?finished_tag=1");
	}else{
...
response.sendRedirect("draft/qcs/purchase_ok.jsp?finished_tag=0");
}
qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("draft/qcs/purchase_ok.jsp?finished_tag=7");
	}

}catch(Exception ex){
	response.sendRedirect("draft/qcs/purchase_ok.jsp?finished_tag=6");
}
