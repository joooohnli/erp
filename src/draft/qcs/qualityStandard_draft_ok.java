...
public class qualityStandard_draft_ok extends HttpServlet{
...
if(!rs.next()){	
	response.sendRedirect("draft/qcs/qualityStandard_ok.jspfinished_tag=1");
}else{
...
String remark=exchange.toHtml(bodyab);
	String sqla = "update qcs_quality_standard set standard_name='"+standard_name+"',product_id='"+product_id+"',product_name='"+product_name+"',designer='"+designer+"',register='"+register+"',register_id='"+register_id+"',register_time='"+register_time+"',remark='"+remark+"',check_tag='5'";
...
response.sendRedirect("draft/qcs/qualityStandard_ok.jsp?finished_tag=0");
}
