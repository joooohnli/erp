...
public class sample_draft_ok extends HttpServlet{
...
try{
	...
	if(!rs.next()){	
		response.sendRedirect("draft/qcs/sample_ok.jsp?finished_tag=1");
	}else{
	...
	sqla = "update qcs_sample set apply_id='"+apply_id+"',lately_view_time='"+sampling_time+"',quality_type='"+quality_type+"',sampling_person='"+sampling_person+"',sampling_time='"+sampling_time+"',register='"+register+"',register_id='"+register_id+"',register_time='"+register_time+"',remark='"+remark+"',check_tag='5'";
...
response.sendRedirect("draft/qcs/sample_ok.jsp?finished_tag=0");
}
qcs_db.commit();
qcs_db.close();
