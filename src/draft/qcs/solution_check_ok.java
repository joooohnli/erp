...
public class solution_check_ok extends HttpServlet{
...
String sql1="select attachment1 from qcs_solution where solution_id='"+solution_id+"' and (check_tag='5' or check_tag='9')";
ResultSet rs=qcs_db.executeQuery(sql1);
if(!rs.next()){	
	response.sendRedirect("draft/qcs/solution_ok.jsp?finished_tag=5");
}else{
...
if(rsList.size()==0){
	...
	qcs_db.executeUpdate(sql) ;
	sql="update qcs_solution set check_tag='0' where solution_id='"+solution_id+"'";
	qcs_db.executeUpdate(sql) ;
...
response.sendRedirect("draft/qcs/solution_ok.jsp?finished_tag=4");
}
qcs_db.commit();
qcs_db.close();
}else{
