...
public class stock_check_ok extends HttpServlet{
...
try{
	...
	if(!rs.next()){	
		response.sendRedirect("draft/qcs/stock_ok.jsp?finished_tag=5");
	}else{
...
String remark=exchange.toHtml(bodyab);
	...
	sqla = "update qcs_stock set apply_id='"+apply_id+"',product_id='"+product_id+"',product_name='"+product_name+"',qcs_amount='"+qcs_amount+"',qcs_time='"+qcs_time+"',quality_way='"+quality_way+"',quality_solution='"+quality_solution+"',sampling_standard='"+sampling_standard+"',sampling_amount='"+sampling_amount+"',accept='"+accept+"',reject='"+reject+"',qualified='"+qualified+"',unqualified='"+unqualified+"',changer_id='"+changer_id+"',qcs_result='"+qcs_result+"',register='"+register+"',register_time='"+register_time+"',remark='"+remark+"',check_tag='0'";
...
if(rsList.size()==0){
	...
	qcs_db.executeUpdate(sql) ;
	sql="update qcs_stock set check_tag='0' where qcs_id='"+qcs_id+"'";
	qcs_db.executeUpdate(sql) ;
...
response.sendRedirect("draft/qcs/stock_ok.jsp?finished_tag=4");
}
qcs_db.commit();
qcs_db.close();
}else{
