...
public class applyGather_check_ok extends HttpServlet{
...
List rsList = GetWorkflow.getList(stock_db, "stock_config_workflow", "01");
...
if(num==0&&product_IDn.length==1){
	response.sendRedirect("draft/stock/applyGather_ok_a.jsp?gather_ID="+gather_ID+"");
}else{
//判断新
int m=0;
for(int i=1;i<=num;i++){
...
if(vt.validata((String)dbSession.getAttribute("unit_db_name"),"stock_apply_gather","gather_ID",gather_ID,"check_tag").equals("5")||vt.validata((String)dbSession.getAttribute("unit_db_name"),"stock_apply_gather","gather_ID",gather_ID,"check_tag").equals("9")){
if(p==0){
if(m==0){
try{
String time="";
...
String amount_unit=request.getParameter(tem_amount_unit) ;
	...
	String sql1 = "update stock_apply_gather_details set product_ID='"+product_ID+"',product_name='"+product_name+"',amount='"+amount+"',cost_price='"+cost_price+"',subtotal='"+subtotal+"' where gather_ID='"+gather_ID+"' and details_number='"+i+"'" ;
	...
	if(not_return_tag.equals("0")&&!reason.equals("库存初始")){
	String sql3="insert into stock_pay_details(pay_ID,details_number,product_ID,product_name,cost_price,subtotal,amount,unpay_amount) values('"+pay_ID+"','"+i+"','"+product_ID+"','"+product_name+"','"+cost_price+"','"+subtotal+"','"+amount+"','"+amount+"')";
...
String cost_price="";
	...
	String sql1 = "insert into stock_apply_gather_details(gather_ID,details_number,product_ID,product_name,product_describe,amount,amount_unit,cost_price,subtotal) values ('"+gather_ID+"','"+num+"','"+product_IDn[i]+"','"+product_namen[i]+"','"+product_describen[i]+"','"+amountn[i]+"','"+amount_unitn[i]+"','"+cost_price+"','"+subtotal+"')" ;
	stock_db.executeUpdate(sql1) ;
	if(rsList.size()==0){
	String sql2="insert into stock_gather_details(gather_ID,details_number,product_ID,product_name,cost_price,subtotal,amount,ungather_amount) values('"+stock_gather_ID+"','"+i+"','"+product_IDn[i]+"','"+product_namen[i]+"','"+cost_price+"','"+subtotal+"','"+amountn[i]+"','"+amountn[i]+"')";
	stock_db.executeUpdate(sql2) ;
	if(not_return_tag.equals("0")&&!reason.equals("库存初始")){
 String sql3="insert into stock_pay_details(pay_ID,details_number,product_ID,product_name,cost_price,subtotal,amount,unpay_amount) values('"+pay_ID+"','"+num+"','"+product_IDn[i]+"','"+product_namen[i]+"','"+cost_price+"','"+subtotal+"','"+amountn[i]+"','"+amountn[i]+"')";
	stock_db.executeUpdate(sql3) ;
	}
	}
	}
...
if(!vr.validata((String)dbSession.getAttribute("unit_db_name"),"stock_pay","reasonexact",gather_ID)){
	String sql5="insert into stock_pay(pay_ID,reason,reasonexact,reasonexact_details,demand_amount,cost_price_sum,register,register_time) values('"+pay_ID+"','"+reason+"','"+gather_ID+"','"+gatherer_name+"','"+demand_amount+"','"+cost_price_sum+"','"+checker+"','"+check_time+"')";
	stock_db.executeUpdate(sql5) ;
}
}
	}else{
		String sql = "update stock_apply_gather set reason='"+reason+"',register='"+register+"',register_time='"+register_time+"',demand_return_time='"+demand_return_time+"',check_time='"+check_time+"',checker='"+checker+"',remark='"+remark+"',demand_amount='"+demand_amount+"',cost_price_sum='"+cost_price_sum+"',not_return_tag='"+not_return_tag+"',check_tag='0' where gather_ID='"+gather_ID+"'" ;
		stock_db.executeUpdate(sql);
...
response.sendRedirect("draft/stock/applyGather_ok.jsp?finished_tag=6");
}
catch (Exception ex){
ex.printStackTrace();
}
}else{
response.sendRedirect("draft/stock/applyGather_ok.jsp?finished_tag=7");
}
}else{
response.sendRedirect("draft/stock/applyGather_ok.jsp?finished_tag=2");
}
}else{
response.sendRedirect("draft/stock/applyGather_ok.jsp?finished_tag=8");
}
}
stock_db.commit();
stock_db.close();
