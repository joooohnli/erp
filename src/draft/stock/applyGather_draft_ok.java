...
public class applyGather_draft_ok extends HttpServlet{
...
String sql = "update stock_apply_gather set reason='"+reason+"',register='"+register+"',register_time='"+register_time+"',demand_return_time='"+demand_return_time+"',checker='"+checker+"',remark='"+remark+"',demand_amount='"+demand_amount+"',cost_price_sum='"+cost_price_sum+"',not_return_tag='"+not_return_tag+"' where gather_ID='"+gather_ID+"'" ;
stock_db.executeUpdate(sql);
response.sendRedirect("draft/stock/applyGather_ok.jsp?finished_tag=0");

}catch (Exception ex){
ex.printStackTrace();
}
}else{
response.sendRedirect("draft/stock/applyGather_ok_b.jsp?gather_ID="+gather_ID+"");
}
}else{
response.sendRedirect("draft/stock/applyGather_ok_c.jsp?gather_ID="+gather_ID+"");
}
}else{
response.sendRedirect("draft/stock/applyGather_ok.jsp?finished_tag=3");
}
}
stock_db.commit();
stock_db.close();
