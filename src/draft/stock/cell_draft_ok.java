...
public class cell_draft_ok extends HttpServlet{
...
String sql = "update stock_cell set checker='"+checker+"',designer='"+designer+"',cell_describe='"+cell_describe+"',min_amount='"+min_amount+"',max_amount='"+max_amount+"',check_tag='5',serial_number_tag='"+serial_number_tag+"' where design_ID='"+design_ID+"'" ;
...
response.sendRedirect("draft/stock/cell_ok.jsp?finished_tag=0");
}else{
response.sendRedirect("draft/stock/cell_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("draft/stock/cell_ok.jsp?finished_tag=2");
}
