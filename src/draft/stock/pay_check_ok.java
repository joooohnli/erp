...
String sql88="select * from stock_paying_gathering where pay_ID='"+pay_ID+"' and check_tag='0' and paying_time='"+paying_time+"' order by details_number";
...
if(n==0){
String sql2="update stock_pre_paying set pay_check_tag='1' where pay_ID='"+pay_ID+"'";
stock_db.executeUpdate(sql2) ;
...
//**************************
if(rsList.size()==0){
		String sql4="update stock_paying_gathering set amount='"+amount[i]+"',paid_amount='"+amount22+"',balance_amount='"+balance_amount11+"',subtotal='"+subtotal+"',register='"+checker+"',register_time='"+check_time+"',check_tag='1' where id='"+id[i]+"'";
		stock_db.executeUpdate(sql4);
}else{
	String sql4="update stock_paying_gathering set amount='"+amount[i]+"',paid_amount='"+paid_amount[i]+"',balance_amount='"+balance_amount11+"',subtotal='"+subtotal+"',register='"+checker+"',register_time='"+check_time+"',check_tag='0' where id='"+id[i]+"'";
		stock_db.executeUpdate(sql4);
}
...
response.sendRedirect("draft/stock/pay_ok.jsp?finished_tag=0");
	...
	response.sendRedirect("draft/stock/pay_ok.jsp?finished_tag=7");
}
}
catch (Exception ex){
ex.printStackTrace();
