...
public class transferGather_draft_ok extends HttpServlet{
...
if(vt.validata((String)dbSession.getAttribute("unit_db_name"),"stock_transfer_gather","gather_ID",gather_ID,"check_tag").equals("5")||vt.validata((String)dbSession.getAttribute("unit_db_name"),"stock_transfer_gather","gather_ID",gather_ID,"check_tag").equals("9")){
if(p==0){
if(m==0){
String time="";
...
String cost_price="";
	...
	String sql1 = "insert into stock_transfer_gather_details(gather_ID,details_number,product_ID,product_name,product_describe,amount_unit,amount,cost_price,subtotal) values ('"+gather_ID+"','"+num+"','"+product_IDn[i]+"','"+product_namen[i]+"','"+product_describen[i]+"','"+amount_unitn[i]+"','"+amountn[i]+"','"+cost_pricen[i]+"','"+subtotal+"')" ;
	stock_db.executeUpdate(sql1) ;
	}
}
String sql = "update stock_transfer_gather set reasonexact='"+reasonexact+"',register='"+register+"',register_time='"+register_time+"',checker='"+checker+"',remark='"+remark+"',demand_amount='"+demand_amount+"',cost_price_sum='"+cost_price_sum+"' where gather_ID='"+gather_ID+"'" ;
stock_db.executeUpdate(sql);	
}
catch (Exception ex){
ex.printStackTrace();
}
response.sendRedirect("draft/stock/transferGather_ok.jsp?finished_tag=0");
}else{
response.sendRedirect("draft/stock/transferGather_ok_a.jsp?gather_ID="+gather_ID+"");
}
}else{
response.sendRedirect("draft/stock/transferGather_ok_b.jsp?gather_ID="+gather_ID+"");
}}else{
