...
public class transferPay_check_ok extends HttpServlet{
...
for(int i=1;i<product_IDn.length;i++){
	...
	String sql1 = "insert into stock_transfer_pay_details(pay_ID,details_number,product_ID,product_name,product_describe,amount_unit,amount,cost_price,subtotal) values ('"+pay_ID+"','"+num+"','"+product_IDn[i]+"','"+product_namen[i]+"','"+product_describen[i]+"','"+amount_unitn[i]+"','"+amountn[i]+"','"+cost_price+"','"+subtotal+"')" ;
	stock_db.executeUpdate(sql1) ;
if(rsList.size()==0){
	String sql2="insert into stock_pay_details(pay_ID,details_number,product_ID,product_name,cost_price,subtotal,amount,unpay_amount) values('"+stock_pay_ID+"','"+num+"','"+product_IDn[i]+"','"+product_namen[i]+"','"+cost_pricen[i]+"','"+subtotal+"','"+amountn[i]+"','"+amountn[i]+"')";
	stock_db.executeUpdate(sql2) ;
	String sql5="insert into stock_gather_details(gather_ID,details_number,product_ID,product_name,cost_price,subtotal,amount,ungather_amount) values('"+stock_gather_ID+"','"+num+"','"+product_IDn[i]+"','"+product_namen[i]+"','"+cost_price+"','"+subtotal+"','"+amountn[i]+"','"+amountn[i]+"')";
	stock_db.executeUpdate(sql5) ;
	}
	}
}
if(rsList.size()==0){
		String sql = "update stock_transfer_pay set reasonexact='"+reasonexact+"',register='"+register+"',register_time='"+register_time+"',check_time='"+check_time+"',checker='"+checker+"',remark='"+remark+"',demand_amount='"+demand_amount+"',cost_price_sum='"+cost_price_sum+"',check_tag='1' where pay_ID='"+pay_ID+"'" ;
...
String sql ="update stock_transfer_pay set reasonexact='"+reasonexact+"',register='"+register+"',register_time='"+register_time+"',check_time='"+check_time+"',checker='"+checker+"',remark='"+remark+"',demand_amount='"+demand_amount+"',cost_price_sum='"+cost_price_sum+"',check_tag='0' where pay_ID='"+pay_ID+"'" ;
		stock_db.executeUpdate(sql);
...
response.sendRedirect("draft/stock/transferPay_ok.jsp?finished_tag=5");
}else{
	...
	response.sendRedirect("draft/stock/transferPay_ok.jsp?finished_tag=6");
}
}
	stock_db.commit();
	stock_db.close();
