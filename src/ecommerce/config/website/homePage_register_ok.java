...
public class homePage_register_ok extends HttpServlet{
...
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	...
	java.io.File filea = new java.io.File(path+"ecommerce/homePage_template/" +file.getFileName());
	if(file.getSize()>d||filea.exists()) j++;
}
if(j!=0){
	response.sendRedirect("ecommerce/config/website/homePage_register_ok_a.jsp");
}else{
...
if(j!=0){
	response.sendRedirect("ecommerce/config/website/homePage_register_ok_c.jsp");
}else{
	file.saveAs(path+"portal/" +file.getFileName());
	file = mySmartUpload.getFiles().getFile(1);
	file_name[1]=file.getFileName();
	file.saveAs(path+"ecommerce/homePage_template/"+file.getFileName());
String sql="insert into ecommerce_web_template(topic,filename,attachment1,register,register_ID,register_time) values('"+topic+"','"+file_name[0]+"','"+file_name[1]+"','"+register+"','"+register_ID+"','"+register_time+"')";
ecommerce_db.executeUpdate(sql);
response.sendRedirect("ecommerce/config/website/homePage_register_ok_b.jsp");
}
}
ecommerce_db.commit();
ecommerce_db.close();
