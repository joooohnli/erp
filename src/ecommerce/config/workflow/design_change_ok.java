...
if(chain_id_array==null){
  	response.sendRedirect("ecommerce/config/workflow/design_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select product_ID from design_file where check_tag='1' and excel_tag!='3'";
	ResultSet rs=ecommerce_db1.executeQuery(sql);
	while(rs.next()){
		sql="update ecommerce_workflow set check_tag='0' where object_ID='"+rs.getString("product_ID")+"' and type_id='01'";
	...
	ecommerce_db.executeUpdate(sql1);	
response.sendRedirect("ecommerce/config/workflow/design_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	ecommerce_db.commit();
