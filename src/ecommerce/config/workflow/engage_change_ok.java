...
public class engage_change_ok extends HttpServlet{
...
if(chain_id_array==null){
  	response.sendRedirect("ecommerce/config/workflow/engage_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
		...
		if(!chain_id_array[j].equals("")){
				StringTokenizer token=new StringTokenizer(chain_id_array[j],"/");
					while(token.hasMoreTokens()){
						String release_id=token.nextToken();
						String human_name=token.nextToken();
						describe1+=release_id+", ";
						describe2+=human_name+", ";
					}
		}
	
}
String sql="select release_id from hr_major_release where check_tag='0'";
	ResultSet rs=ecommerce_db1.executeQuery(sql);
	while(rs.next()){
		sql="update ecommerce_workflow set check_tag='0' where object_ID='"+rs.getString("release_id")+"' and type_id='01'";
	...
	ecommerce_db.executeUpdate(sql1);	
response.sendRedirect("ecommerce/config/workflow/engage_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	ecommerce_db.commit();
