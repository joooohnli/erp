...
public class link_change_ok extends HttpServlet{
...
if(chain_id_array==null){
  	response.sendRedirect("ecommerce/config/workflow/link_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select link_ID from ecommerce_link where check_tag='0'";
	ResultSet rs=ecommerce_db1.executeQuery(sql);
	while(rs.next()){
		sql="update ecommerce_workflow set check_tag='0' where object_ID='"+rs.getString("link_ID")+"' and type_id='01'";
	...
	ecommerce_db.executeUpdate(sql1);	
response.sendRedirect("ecommerce/config/workflow/link_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	ecommerce_db.commit();
