...
try{
	...
	if(rsList.size()==0){//产品档案无工作流
		...
		if(rsList1.size()==0){//电子商务的配送信息的发布审核无工作流
			sql = "update design_file set excel_tag='3' where product_ID='"+product_ID+"'" ;//excel_tag为3表示所有审核工作流审核通过
			design_db.executeUpdate(sql);
     	}else{
...
response.sendRedirect("ecommerce/design/change_choose_attachment.jsp?product_ID="+product_ID+"");
}else{
	response.sendRedirect("ecommerce/design/change_ok_a.jsp?product_ID="+product_ID+"");
	}
}else{
response.sendRedirect("ecommerce/design/change_ok_b.jsp?product_ID="+product_ID+"");
}}else{
response.sendRedirect("ecommerce/design/change_ok_c.jsp");
}
design_db.commit();
design_db.close();
}else{
