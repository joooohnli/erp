...
String sql6="select * from ecommerce_workflow where object_ID='"+product_ID+"' and ((check_tag='0' and config_id<'"+config_id+"') or (check_tag='1' and config_id='"+config_id+"')) and type_id='01'";
...
if(choice!=null){
	if(choice.equals("")){
	String sql = "update design_file set promotion='"+promotion+"',opinion='"+opinion+"',excel_tag='9' where product_ID='"+product_ID+"'" ;
	design_db.executeUpdate(sql) ;
	sql = "update ecommerce_workflow set check_tag='9',checker_ID='"+checker_ID+"',checker='"+checker+"',check_time='"+check_time+"' where object_ID='"+product_ID+"' and config_id='"+config_id+"' and type_id='01'" ;
	design_db.executeUpdate(sql);
	}else{

	sql6="select * from ecommerce_workflow where object_ID='"+product_ID+"' and config_id<'"+config_id+"' and config_id>='"+choice+"' and type_id='01'";
...
response.sendRedirect("ecommerce/design/check_delete_ok_a.jsp");
}else{
response.sendRedirect("ecommerce/design/check_delete_ok_b.jsp?product_ID="+product_ID+"&config_id="+config_id+"");
}
}else{
response.sendRedirect("ecommerce/design/check_delete_ok_c.jsp");
}
design_db.commit();
design_db1.commit();
design_db.close();
