...
String bodyd = new String(request.getParameter("opinion4").getBytes("UTF-8"),"UTF-8");
String opinion4=exchange.toHtml(bodyd);
...
String sql6="select * from ecommerce_workflow where object_ID='"+product_ID+"' and type_id='05' and ((check_tag='0' and config_id<'"+config_id+"') or (check_tag='1' and config_id='"+config_id+"'))";

ResultSet rs6=design_db.executeQuery(sql6);
if(!rs6.next()&&vt.validata((String)session.getAttribute("unit_db_name"),"design_file","product_ID",product_ID,"excel_tag4").equals("1")){
...
String column_group=getInsertSql.sql((String)dbSession.getAttribute("unit_db_name"),"design_file");
	sql6="select * from ecommerce_workflow where object_ID='"+product_ID+"' and config_id<'"+config_id+"' and type_id='05'";
	...
	String sql = "update design_file set chain_id='"+chain_id+"',chain_name='"+chain_name+"',product_ID='"+product_ID+"',product_name='"+product_name+"',factory_name='"+factory_name+"',product_class='"+product_class+"',type='"+type+"',twin_name='"+twin_name+"',twin_ID='"+twin_ID+"',personal_unit='"+personal_unit+"',personal_value='"+personal_value+"',warranty='"+warranty+"',lifecycle='"+lifecycle+"',product_nick='"+product_nick+"',list_price='"+list_price+"',cost_price='"+cost_price+"',register='"+register+"',provider_group='"+provider_group+"',product_describe='"+product_describe+"',responsible_person_name='"+responsible_person_name+"',responsible_person_ID='"+responsible_person_ID+"',amount_unit='"+amount_unit+"',promotion4='"+promotion4+"',opinion4='"+opinion4+"' where product_ID='"+product_ID+"'" ;
	design_db.executeUpdate(sql) ;

	sql = "update ecommerce_workflow set check_tag='1',checker_ID='"+checker_ID+"',checker='"+checker+"',check_time='"+check_time+"' where object_ID='"+product_ID+"' and config_id='"+config_id+"' and type_id='05'";
		design_db.executeUpdate(sql) ;
	sql6="select * from ecommerce_workflow where object_ID='"+product_ID+"' and check_tag='0' and type_id='05'";
	rs6=design_db.executeQuery(sql6);
	if(!rs6.next()){
		sql = "update design_file set excel_tag4='3' where product_ID='"+product_ID+"'" ;
		design_db.executeUpdate(sql) ;
	}
response.sendRedirect("ecommerce/logistics/check_choose_attachment.jsp?product_ID="+product_ID+"");
}else{
response.sendRedirect("ecommerce/logistics/check_ok_a.jsp?product_ID="+product_ID+"&config_id="+config_id+"");
	}
}else{
response.sendRedirect("ecommerce/logistics/check_ok_b.jsp?product_ID="+product_ID+"&config_id="+config_id+"");
}
}else{
response.sendRedirect("ecommerce/logistics/check_ok_c.jsp");
}
	  design_db.commit();
	  design_db.close();
}else{
