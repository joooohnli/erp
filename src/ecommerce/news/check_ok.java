...
String sql6="select id from ecommerce_workflow where object_ID='"+news_ID+"' and ((check_tag='0' and config_ID<'"+config_ID+"') or (check_tag='1' and config_ID='"+config_ID+"'))";
ResultSet rs6=ecommerce_db.executeQuery(sql6);
if(!rs6.next()){

if(vt.validata((String)session.getAttribute("unit_db_name"),"ecommerce_news","news_ID",news_ID,"check_tag").equals("0")){
...
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	...
	String sqla = "update ecommerce_news set topic='"+topic+"',chain_id='"+chain_id+"',chain_name='"+chain_name+"',news_ID='"+news_ID+"',content='"+content+"',opinion='"+opinion+"'";
...
String sql=sqla+sqlb;
	...
	sql = "update ecommerce_workflow set check_tag='1',checker_ID='"+checker_ID+"',checker='"+checker+"',check_time='"+check_time+"' where object_ID='"+news_ID+"' and config_id='"+config_ID+"'" ;
     ecommerce_db.executeUpdate(sql) ;
	sql6="select * from ecommerce_workflow where object_ID='"+news_ID+"' and check_tag='0'";
	rs6=ecommerce_db.executeQuery(sql6);
	if(!rs6.next()){
		sql = "update ecommerce_news set check_tag='1',excel_tag='3',checker_ID='"+checker_ID+"',checker='"+checker+"',check_time='"+check_time+"' where news_ID='"+news_ID+"'" ;
		ecommerce_db.executeUpdate(sql) ;
	}	
response.sendRedirect("ecommerce/news/check_ok.jsp?finished_tag=0");

}else{
response.sendRedirect("ecommerce/news/check_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("ecommerce/news/check_ok.jsp?finished_tag=2");
}
ecommerce_db.commit();
ecommerce_db.close();
	...
	ex.printStackTrace();
response.sendRedirect("ecommerce/news/check_ok_b.jsp?config_id="+config_ID+"&news_ID="+news_ID+"");
}
