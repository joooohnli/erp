...
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	...
	String sqla = "insert into ecommerce_news(register,register_ID,register_time,news_ID,topic,content,chain_id,chain_name,check_tag";
	String sqlb = ") values ('"+register+"','"+register_ID+"','"+register_time+"','"+news_ID+"','"+topic+"','"+content+"','"+chain_id+"','"+chain_name+"','5'" ;   
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
response.sendRedirect("ecommerce/news/register_draft_ok.jsp?finished_tag=1");
	ecommerce_db.commit();
	ecommerce_db1.commit();
	ecommerce_db.close();
	ecommerce_db1.close();
}catch(Exception ex){
	response.sendRedirect("ecommerce/news/register_draft_ok.jsp?finished_tag=0");
	}
}else{
