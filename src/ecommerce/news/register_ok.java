...
String news_ID=NseerId.getId(getClass().getResource("").toString(),(String)dbSession.getAttribute("unit_db_name"));
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	...
	String sqla = "insert into ecommerce_news(register,register_ID,register_time,news_ID,topic,content,chain_id,chain_name";
	String sqlb = ") values ('"+register+"','"+register_ID+"','"+register_time+"','"+news_ID+"','"+topic+"','"+content+"','"+chain_id+"','"+chain_name+"'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
String sql=sqla+sqlb+")";
	...
	sql="update ecommerce_config_news_kind set delete_tag='1' where file_id='"+chain_id+"'";
	ecommerce_db.executeUpdate(sql);//delete_tag为1说明分类被使用	
	List rsList = (List)new java.util.ArrayList();
	String[] elem=new String[3];
	 sql="select id,describe1,describe2 from ecommerce_config_workflow where type_id='07'";
...
if(rsList.size()==0){
		...
		sql = "insert into ecommerce_workflow(config_id,object_ID,describe1,describe2,type_id) values ('"+elem[0]+"','"+news_ID+"','"+elem[1]+"','"+elem[2]+"','07')" ;

		ecommerce_db.executeUpdate(sql) ;
		}
	}
response.sendRedirect("ecommerce/news/register_ok_b.jsp");
	ecommerce_db.commit();
	ecommerce_db1.commit();
	ecommerce_db.close();
	ecommerce_db1.close();
}catch(Exception ex){
	response.sendRedirect("ecommerce/news/register_ok_a.jsp");
	}
}else{
