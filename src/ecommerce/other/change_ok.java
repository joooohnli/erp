...
if(vt.validata((String)session.getAttribute("unit_db_name"),"ecommerce_other","other_ID",other_ID,"check_tag").equals("1")){

String sqloo= "select * from ecommerce_other where other_ID='"+other_ID+"'";
	 ResultSet rsoo = ecommerce_db.executeQuery(sqloo);
	 if(rsoo.next()){
	String sqll="insert into ecommerce_other_dig(chain_id,chain_name,other_ID,topic,content,attachment1,attachment2,attachment3,opinion,CHECK_TAG,FILE_CHANGE_AMOUNT,REGISTER,CHECKER,CHANGER,REGISTER_ID,CHECKER_ID,CHANGER_ID,REGISTER_TIME,CHECK_TIME,CHANGE_TIME,LATELY_CHANGE_TIME,EXCEL_TAG) values('"+rsoo.getString("chain_id")+"','"+rsoo.getString("chain_name")+"','"+rsoo.getString("other_ID")+"','"+rsoo.getString("topic")+"','"+rsoo.getString("content")+"','"+rsoo.getString("attachment1")+"','"+rsoo.getString("attachment2")+"','"+rsoo.getString("attachment3")+"','"+rsoo.getString("opinion")+"','"+rsoo.getString("CHECK_TAG")+"','"+rsoo.getString("FILE_CHANGE_AMOUNT")+"','"+rsoo.getString("REGISTER")+"','"+rsoo.getString("CHECKER")+"','"+rsoo.getString("CHANGER")+"','"+rsoo.getString("REGISTER_ID")+"','"+rsoo.getString("CHECKER_ID")+"','"+rsoo.getString("CHANGER_ID")+"','"+rsoo.getString("REGISTER_TIME")+"','"+rsoo.getString("CHECK_TIME")+"','"+rsoo.getString("CHANGE_TIME")+"','"+rsoo.getString("LATELY_CHANGE_TIME")+"','"+rsoo.getString("EXCEL_TAG")+"')";
...
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	...
	String sqla = "update ecommerce_other set chain_id='"+chain_id+"',chain_name='"+chain_name+"',other_ID='"+other_ID+"',content='"+content+"',changer_ID='"+changer_ID+"',changer='"+changer+"',change_time='"+change_time+"',lately_change_time='"+lately_change_time+"',file_change_amount='"+change_amount+"'";
...
List rsList = GetWorkflow.getList(ecommerce_db, "ecommerce_config_workflow", "08");
	String[] elem=new String[3];
if(rsList.size()==0){
		sql="update ecommerce_other set check_tag='1' where other_ID='"+other_ID+"'";
		ecommerce_db.executeUpdate(sql);
	}else{
		sql="delete from ecommerce_workflow where object_ID='"+other_ID+"'";
		ecommerce_db.executeUpdate(sql) ;
		sql="update ecommerce_other set check_tag='0',excel_tag='0' where other_ID='"+other_ID+"'";
       
		ecommerce_db.executeUpdate(sql) ;
		Iterator ite=rsList.iterator();
		while(ite.hasNext()){
		elem=(String[])ite.next();
		sql = "insert into ecommerce_workflow(config_id,object_ID,describe1,describe2) values ('"+elem[0]+"','"+other_ID+"','"+elem[1]+"','"+elem[2]+"')" ;
		
		ecommerce_db.executeUpdate(sql) ;
		}
	}

response.sendRedirect("ecommerce/other/change_ok.jsp?finished_tag=1");
}else{
response.sendRedirect("ecommerce/other/change_ok.jsp?finished_tag=0");
}
}catch(Exception ex){
response.sendRedirect("ecommerce/other/change_ok_b.jsp?other_ID="+other_ID+"");
}
