...
if(choice!=null){
	if(choice.equals("")){
	String sql = "update ecommerce_other set check_tag='9',excel_tag='9' where other_ID='"+other_ID+"'" ;
	...
	ecommerce_db.executeUpdate(sql) ;
       ...
       sql6="select id from ecommerce_workflow where object_ID='"+other_ID+"' and config_id<'"+config_ID+"' and    config_id>='"+choice+"'";
...
response.sendRedirect("ecommerce/other/check_delete_ok.jsp?finished_tag=3");
}else{
response.sendRedirect("ecommerce/other/check_delete_ok.jsp?finished_tag=2");
}
}else{
response.sendRedirect("ecommerce/other/check_delete_ok.jsp?finished_tag=0");
}
}else{
response.sendRedirect("ecommerce/other/check_delete_ok.jsp?finished_tag=1");
}
ecommerce_db.commit();
