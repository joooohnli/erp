...
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	...
	String sqla = "update ecommerce_contact set chain_id='"+chain_id+"',chain_name='"+chain_name+"',email='"+email+"',tel='"+tel+"',fax='"+fax+"',address='"+address+"',postcode='"+postcode+"',qq1='"+qq1+"',qq2='"+qq2+"',qq3='"+qq3+"',qq4='"+qq4+"',check_tag='0',changer_ID='"+changer_ID+"',changer='"+changer+"',change_time='"+change_time+"'";
...
response.sendRedirect("ecommerce/website/contact/change_ok.jsp?finished_tag=0");

}else{
response.sendRedirect("ecommerce/website/contact/change_ok.jsp?finished_tag=1");
}
ecommerce_db.commit();
ecommerce_db.close();

}catch(Exception ex){
response.sendRedirect("ecommerce/website/contact/change_ok_b.jsp?contact_ID="+contact_ID+"");
}
