...
try{
	...
	String sqla = "insert into ecommerce_contact(contact_ID,chain_id,chain_name,address,tel,fax,postcode,register,register_time,email,qq1,qq2,qq3,qq4";
	String sqlb=") values ('"+contact_ID+"','"+chain_id+"','"+chain_name+"','"+address+"','"+tel+"','"+fax+"','"+postcode+"','"+register+"','"+register_time+"','"+email+"','"+qq1+"','"+qq2+"','"+qq3+"','"+qq4+"'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
String sql=sqla+sqlb+")";
	...
	sql="select id,describe1,describe2 from ecommerce_config_workflow where type_id='11'";
...
if(rsList.size()==0){
		...
		sql = "insert into ecommerce_workflow(config_id,object_ID,describe1,describe2,type_id) values ('"+elem[0]+"','"+contact_ID+"','"+elem[1]+"','"+elem[2]+"','11')" ;
		ecommerce_db.executeUpdate(sql) ;
		}
	}
response.sendRedirect("ecommerce/website/contact/register_ok_b.jsp");
}
catch (Exception ex){
	ex.printStackTrace();
}
}else{
response.sendRedirect("ecommerce/website/contact/register_ok_c.jsp");
}
 ecommerce_db.commit();
 ecommerce_db.close();
}catch(Exception ex){
