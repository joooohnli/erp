...
if(choice!=null){
if(choice.equals("")){
	String sql = "update ecommerce_link set opinion='"+opinion+"',check_tag='9' where link_ID='"+link_ID+"'" ;
	...
	ecommerce_db.executeUpdate(sql) ;
       ...
       sql6="select id from ecommerce_workflow where object_ID='"+link_ID+"' and config_id<'"+config_ID+"' and    config_id>='"+choice+"'";
...
response.sendRedirect("ecommerce/website/link/check_delete_ok.jsp?finished_tag=3");
}else{
response.sendRedirect("ecommerce/website/link/check_delete_ok.jsp?finished_tag=2");
}
}else{
response.sendRedirect("ecommerce/website/link/check_delete_ok.jsp?finished_tag=0");
}
}else{
response.sendRedirect("ecommerce/website/link/check_delete_ok.jsp?finished_tag=1");
}
ecommerce_db.commit();
