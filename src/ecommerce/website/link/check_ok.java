...
String sql6="select id from ecommerce_workflow where object_ID='"+link_ID+"' and ((check_tag='0' and config_ID<'"+config_ID+"') or (check_tag='1' and config_ID='"+config_ID+"'))";
ResultSet rs6=ecommerce_db.executeQuery(sql6);
if(!rs6.next()){
	if(vt.validata((String)session.getAttribute("unit_db_name"),"ecommerce_link","link_ID",link_ID,"check_tag").equals("0")){
...
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	...
	String sqla = "update ecommerce_link set link_ID='"+link_ID+"',topic='"+topic+"',url='"+url+"',content='"+content+"',opinion='"+opinion+"'";
...
sql = "update ecommerce_workflow set check_tag='1',checker_ID='"+checker_ID+"',checker='"+checker+"',check_time='"+check_time+"' where object_ID='"+link_ID+"' and config_id='"+config_ID+"'" ;
    ecommerce_db.executeUpdate(sql) ;
	sql6="select * from ecommerce_workflow where object_ID='"+link_ID+"' and check_tag='0'";
	rs6=ecommerce_db.executeQuery(sql6);
	if(!rs6.next()){
		sql = "update ecommerce_link set check_tag='1',excel_tag='3',checker_ID='"+checker_ID+"',checker='"+checker+"',check_time='"+check_time+"' where link_ID='"+link_ID+"'" ;
		
		ecommerce_db.executeUpdate(sql) ;
	}

response.sendRedirect("ecommerce/website/link/check_ok.jsp?finished_tag=0");

}else{
response.sendRedirect("ecommerce/website/link/check_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("ecommerce/website/link/check_ok.jsp?finished_tag=2");
}
ecommerce_db.commit();
ecommerce_db.close();

}catch(Exception ex){
response.sendRedirect("ecommerce/website/link/check_ok_b.jsp?link_ID="+link_ID+"&config_id="+config_ID+"");
	}
