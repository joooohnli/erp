...
public class homePage_ok extends HttpServlet{
...
response.sendRedirect("ecommerce/website/template/homePage_ok_a.jsp");

}else{
		String sql = "update ecommerce_web_base set home_page='"+cols+"' where unit_id='"+unit_id+"'" ;
    	ecommerce_db.executeUpdate(sql) ;
	response.sendRedirect("ecommerce/website/template/homePage_ok_b.jsp");
	}
ecommerce_db.commit();
ecommerce_db.close();
}else{
