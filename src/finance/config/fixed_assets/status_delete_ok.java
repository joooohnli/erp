...
public class status_delete_ok extends HttpServlet{
...
while(tk.hasMoreTokens()){
	  temp=tk.nextToken();
	  sql = "select id from finance_config_public_char where used_tag='0' and id="+temp;
	  rs=finance_db.executeQuery(sql);
if(rs.next()){	 
  sql = "delete from finance_config_public_char where used_tag='0' and id="+temp;
...
count1++;
		  ...
		  sql = "select id from finance_config_public_char where used_tag='0' and id="+id_group;
		  rs=finance_db.executeQuery(sql);
		  
	if(rs.next()){	 
	  sql = "delete from finance_config_public_char where used_tag='0' and id="+id_group;
	  finance_db.executeUpdate(sql);
	  count++;
	}
	count1++;
