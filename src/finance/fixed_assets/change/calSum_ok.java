...
public class calSum_ok extends HttpServlet{
...
String cb_calsum=request.getParameter("cb_calsum");
String ca_calsum=request.getParameter("ca_calsum");
...
ResultSet rs=null;
    ...
    sql = "insert into finance_fa_change(changebill_id,file_id,file_name,start_time,specification,cb_caled_sum,ca_caled_sum,change_reason,change_time,changer,change_kind) values('"+changebill_id+"','"+file_id+"','"+file_name+"','"+start_time+"','"+specification+"','"+cb_calsum+"','"+ca_calsum+"','"+change_reason+"','"+change_time+"','"+changer+"','累计折旧调整')";
    	finance_db.executeUpdate(sql); 
    	
    sql="select * from finance_fa_file where file_id='"+file_id+"'";
    	rs=finance_db.executeQuery(sql);
    if(rs.next()){
    	double ca_net_value=Double.parseDouble(rs.getString("original_value"))-Double.parseDouble(ca_calsum);
      ...
      sql="update finance_fa_file set caled_sum='"+ca_calsum+"',net_value='"+ca_net_value+"',change_time='"+change_time+"' where file_id='"+file_id+"'";	
      finance_db.executeUpdate(sql);
    }
		finance_db.commit();
		finance_db.close();
