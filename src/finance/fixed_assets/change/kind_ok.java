...
public class kind_ok extends HttpServlet{
...
ResultSet rs=null;
	...
	String ca_type_id=ca_type.split(" ")[0];
	String ca_type_name=ca_type.split(" ")[1];
	String changebill_id=NseerId.getId(getClass().getResource("").toString(),(String)dbSession.getAttribute("unit_db_name"));
    sql = "insert into finance_fa_change(changebill_id,file_id,file_name,start_time,specification,cb_type,ca_type,change_reason,change_time,changer,change_kind) values('"+changebill_id+"','"+file_id+"','"+file_name+"','"+start_time+"','"+specification+"','"+cb_type+"','"+ca_type+"','"+change_reason+"','"+change_time+"','"+changer+"','类别调整')";
    ...
    if(rs.next()){
      ...
      sql="update finance_fa_file set type_id='"+ca_type_id+"',type_name='"+ca_type_name+"',change_time='"+change_time+"' where file_id='"+file_id+"'";	
      finance_db.executeUpdate(sql);
    }
		finance_db.commit();
		finance_db.close();
