...
public class life_ok extends HttpServlet{
...
String cb_lifecycle=request.getParameter("cb_lifecycle");
String ca_lifecycle=request.getParameter("ca_lifecycle");
...
ResultSet rs=null;
    ...
    sql = "insert into finance_fa_change(changebill_id,file_id,file_name,start_time,specification,cb_lifecycle,ca_lifecycle,change_reason,change_time,changer,change_kind) values('"+changebill_id+"','"+file_id+"','"+file_name+"','"+start_time+"','"+specification+"','"+cb_lifecycle+"','"+ca_lifecycle+"','"+change_reason+"','"+change_time+"','"+changer+"','使用年限调整')";
    ...
    if(rs.next()){
    	...
    	finance_db.executeUpdate(sql); 	 	
      sql="update finance_fa_file set lifecycle='"+ca_lifecycle+"',change_time='"+change_time+"' where file_id='"+file_id+"'";	
      finance_db.executeUpdate(sql);
    }
		finance_db.commit();
		finance_db.close();
