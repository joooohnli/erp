...
public class reduceReady_ok extends HttpServlet{
...
String pre_sub=request.getParameter("pre_sub");
...
ResultSet rs=null;
    ...
    sql = "insert into finance_fa_change(changebill_id,file_id,file_name,start_time,specification,sum_presub,sum_re_presub,ca_net_value,pre_sub,currency,exchange_rate,cb_original_value,cb_caled_sum,change_reason,change_time,changer,change_kind) values('"+changebill_id+"','"+file_id+"','"+file_name+"','"+start_time+"','"+specification+"','"+sum_presub+"','"+sum_re_presub+"','"+ca_net_value+"','"+pre_sub+"','"+currency+"','"+currency_rate+"','"+original_value+"','"+caled_sum+"','"+change_reason+"','"+change_time+"','"+changer+"','计提减值准备')";
    ...
    if(rs.next()){
      ...
      sql="update finance_fa_file set sum_presub='"+sum_presub+"',net_value='"+ca_net_value+"',change_time='"+change_time+"' where file_id='"+file_id+"'";	
      finance_db.executeUpdate(sql);
    }
		finance_db.commit();
		finance_db.close();
