...
public class status_ok extends HttpServlet{
...
ResultSet rs=null;
    ...
    sql = "insert into finance_fa_change(changebill_id,file_id,file_name,start_time,specification,cb_status_id,ca_status_id,change_reason,change_time,changer,change_kind) values('"+changebill_id+"','"+file_id+"','"+file_name+"','"+start_time+"','"+specification+"','"+cb_status+"','"+ca_status+"','"+change_reason+"','"+change_time+"','"+changer+"','使用状态变动')";
    ...
    if(rs.next()){
      ...
      sql="update finance_fa_file set status_id='"+ca_status+"',change_time='"+change_time+"' where file_id='"+file_id+"'";	
      finance_db.executeUpdate(sql);
    }
		finance_db.commit();
		finance_db.close();
