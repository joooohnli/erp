...
public class workSum_ok extends HttpServlet{
...
String cb_work_total=request.getParameter("cb_work_total");
String ca_work_total=request.getParameter("ca_work_total");
...
ResultSet rs=null;
    ...
    sql = "insert into finance_fa_change(changebill_id,file_id,file_name,start_time,specification,cb_work_total,ca_work_total,change_reason,change_time,changer,change_kind) values('"+changebill_id+"','"+file_id+"','"+file_name+"','"+start_time+"','"+specification+"','"+cb_work_total+"','"+ca_work_total+"','"+change_reason+"','"+change_time+"','"+changer+"','工作总量调整')";
    ...
    if(rs.next()){
      ...
      sql="update finance_fa_file set work_total='"+ca_work_total+"',change_time='"+change_time+"' where file_id='"+file_id+"'";	
      finance_db.executeUpdate(sql);
    }
		finance_db.commit();
		finance_db.close();
