...
sql="select voucher_in_month_id from finance_voucher where voucher_in_month_id='"+voucher_id+"' && account_period='"+account_period+"'";
...
for(int i=0;i<rowCount;i++){
	...
	sql="select name from finance_config_summary where name='"+summary[i]+"'";
	rs=finance_db.executeQuery(sql);
	if(!rs.next()){
		sql="insert into finance_config_summary(name) values('"+summary[i]+"')";
	...
	finance_db.executeUpdate(sql);
    String sql1="update finance_config_file_kind set delete_tag='1' where chain_id='"+file_id+"'";
...
response.sendRedirect("finance/voucher/registerNew_ok_a.jsp");
}else{
	response.sendRedirect("finance/voucher/registerNew_ok_b.jsp?err_number="+err_count);
}
}else{
	response.sendRedirect("finance/voucher/registerNew_ok_b.jsp?err_number=6");
}
}else{
	response.sendRedirect("finance/voucher/registerNew_ok_b.jsp?err_number=14");
}
}else{
	response.sendRedirect("finance/voucher/registerNew_ok_b.jsp?err_number=7");
}
	}else{
	response.sendRedirect("finance/voucher/registerNew_ok_b.jsp?err_number=2");
		}

}else{
	response.sendRedirect("finance/voucher/registerNew_ok_b.jsp?err_number=1");
}
}else{
	response.sendRedirect("finance/voucher/registerNew_ok_b.jsp?err_number=15");
}
finance_db.commit();
finance_db.close();
}else{
