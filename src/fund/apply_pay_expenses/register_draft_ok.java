...
if(!vr.validata((String)dbSession.getAttribute("unit_db_name"),"fund_apply_pay","register_time",register_time)){
	String sql = "insert into fund_apply_pay(apply_pay_ID,chain_ID,chain_name,human_ID,human_name,register_time,demand_cost_price_sum,currency_name,personal_unit,remark,register,check_tag,excel_tag,reason) values ('"+apply_pay_ID+"','"+chain_ID+"','"+chain_name+"','"+human_ID+"','"+human_name+"','"+register_time+"','"+demand_cost_price_sum+"','"+currency_name+"','"+personal_unit+"','"+remark+"','"+register+"','5','2','费用')" ;
	fund_db.executeUpdate(sql) ;
}

response.sendRedirect("fund/apply_pay_expenses/register_draft_ok.jsp?finished_tag=0");
}
catch (Exception ex){
	ex.printStackTrace();
}
}else{
response.sendRedirect("fund/apply_pay_expenses/register_draft_ok.jsp?finished_tag=1");
}
}else{	
response.sendRedirect("fund/apply_pay_expenses/register_draft_ok.jsp?finished_tag=2");
}
fund_db.commit();
hr_db.commit();
fund_db.close();
