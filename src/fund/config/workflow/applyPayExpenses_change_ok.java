...
public class applyPayExpenses_change_ok extends HttpServlet{
...
if(chain_id_array==null){
  	response.sendRedirect("fund/config/workflow/applyPayExpenses_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select apply_pay_ID from fund_apply_pay where check_tag='0'";
	ResultSet rs=crm_db1.executeQuery(sql);
	while(rs.next()){
		sql="update fund_workflow set check_tag='0' where object_ID='"+rs.getString("apply_pay_ID")+"'";
	...
	crm_db.executeUpdate(sql1);	
response.sendRedirect("fund/config/workflow/applyPayExpenses_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	crm_db.commit();
	crm_db1.commit();
	crm_db1.close();
