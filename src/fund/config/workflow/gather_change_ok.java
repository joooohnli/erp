...
if(chain_id_array==null){
  	response.sendRedirect("fund/config/workflow/gather_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
int f=0;
String sql="select fund_id,gather_time from fund_fund where check_tag='1' and fund_tag!='1' and reason='收款' and fund_execute_tag='1'";
	...
	while(rs4.next()){
		if(rs4.getString("execute_check_tag").equals("1")&&!rs4.getString("execute_details_tag").equals("2")){
			if(f==0){
				...
				pList.add(rs.getString("gather_time"));
				f++;
			}
...
for(int i=0;i<fList.size();i++){
	sql="update fund_workflow set check_tag='0' where object_ID='"+fList.get(i)+"' and gather_time='"+pList.get(i)+"'";
	...
	fund_db.executeUpdate(sql1);	
response.sendRedirect("fund/config/workflow/gather_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	fund_db.commit();
	fund_db1.commit();
	fund_db1.close();
