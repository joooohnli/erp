...
public class registerPay_ok extends HttpServlet{
//创建方法
...
String executing_cost_price_sum=request.getParameter("executing_cost_price_sum") ;
...
if(demand_cost_price_sum1==Double.parseDouble(demand_cost_price_sum)&&m==0){
...
if(fund_ID.equals("")){
	...
	fund_ID=NseerId.getId("fund/pay",(String)dbSession.getAttribute("unit_db_name"));
...
String sql3="insert into fund_fund(fund_ID,chain_ID,chain_name,file_chain_ID,file_chain_name,funder,funder_ID,reason,demand_cost_price_sum,apply_ID_group,choice_ID_group,currency_name,personal_unit,register_time,designer,register,remark,check_tag,excel_tag,fund_execute_tag,executing_cost_price_sum) values('"+fund_ID+"','"+chain_ID+"','"+chain_name+"','"+file_chain_ID+"','"+file_chain_name+"','"+funder+"','"+funder_ID+"','付款','"+demand_cost_price_sum+"','"+apply_ID_group+"','"+choice_group+"','"+currency_name+"','"+personal_unit+"','"+register_time+"','"+designer+"','"+register+"','"+remark+"','1','2','1','"+executing_cost_price_sum+"')";
fund_db.executeUpdate(sql3);
}

response.sendRedirect("fund/fund/registerPay_ok_a.jsp");
}else{

response.sendRedirect("fund/fund/registerPay_ok_b.jsp");
...
response.sendRedirect("fund/fund/registerPay_ok_c.jsp");

}}else{
	
response.sendRedirect("fund/fund/registerPay_ok_d.jsp");
}
fund_db.commit();
fund_db.close();
}else{
