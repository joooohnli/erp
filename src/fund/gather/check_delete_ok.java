...
if(!rs6.next()){
String sql8="select id from fund_executing where fund_ID='"+fund_ID+"' and check_tag='0' and gather_time='"+gather_time+"'";
...
try{
	...
	sql2 = "delete from fund_workflow where object_ID='"+fund_ID+"' and gather_time='"+gather_time+"'" ;
	fund_db.executeUpdate(sql2) ;
	}else{

	sql6="select id from fund_workflow where object_ID='"+fund_ID+"' and config_id<'"+config_id+"' and config_id>='"+choice+"' and gather_time='"+gather_time+"'";
	...
	while(rs6.next()){
  	...
  	response.sendRedirect("fund/gather/check_delete_ok.jsp?finished_tag=0");
}else{
	
response.sendRedirect("fund/gather/check_delete_ok.jsp?finished_tag=3");

}
}else{
	response.sendRedirect("fund/gather/check_delete_ok.jsp?finished_tag=2");
}
}else{
	response.sendRedirect("fund/gather/check_delete_ok.jsp?finished_tag=1");
}
fund_db.commit();
fund_db1.commit();
fund_db.close();
