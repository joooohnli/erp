...
for(int i=0;i<value1.length;i++){
	String id=value1[i];
	String sql="select invoice_gar_tag,intrmanufacture_id from "+tableName+" where id='"+id+"' and (invoice_check_tag='2' or invoice_gar_tag='1') and gar_tag='0'";
	ResultSet rs=qcs_db.executeQuery(sql);
	if(rs.next()){
		if(rs.getString("invoice_gar_tag").equals("1")){//强制完成
			sql="delete from intrmanufacture_purchasing where intrmanufacture_id='"+rs.getString("intrmanufacture_id")+"'";
				qcs_db.executeUpdate(sql);
			sql="update "+tableName+" set invoice_gar_tag='3' where id='"+id+"'";//两边的记录不显示
				qcs_db.executeUpdate(sql);
		}else{//处理从垃圾箱过来的记录
			sql="delete from intrmanufacture_purchasing where intrmanufacture_id='"+rs.getString("intrmanufacture_id")+"' and check_tag='0'";
				qcs_db.executeUpdate(sql);
