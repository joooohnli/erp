...
try{
	String sql2="update hr_bonus set bonus_worth='"+bonus_worth+"',checker='"+checker+"',check_time='"+check_time+"',remark='"+remark+"' where human_ID='"+human_ID+"' and check_tag='0'";
		hr_db.executeUpdate(sql2);
	String sql = "update hr_workflow set checker='"+checker+"',checker_ID='"+checker_ID+"',check_time='"+check_time+"',check_tag='1' where object_ID='"+human_ID+"' and bonus_time='"+bonus_time+"' and config_id='"+config_id+"' and type_id='06'" ;
	hr_db.executeUpdate(sql);
	sql="select id from hr_workflow where object_ID='"+human_ID+"' and bonus_time='"+bonus_time+"' and check_tag='0' and type_id='06'";
	ResultSet rset=hr_db.executeQuery(sql);
	if(!rset.next()){
		sql="update hr_bonus set check_tag='1' where human_ID='"+human_ID+"' and check_tag='0'";
	hr_db.executeUpdate(sql);
...
response.sendRedirect("hr/bonus/check_ok_a.jsp");
}else{
response.sendRedirect("hr/bonus/check_ok_b.jsp?human_ID="+human_ID+"");
}
}else{
response.sendRedirect("hr/bonus/check_ok_c.jsp");
}
}else{
response.sendRedirect("hr/bonus/check_ok.jsp?finished_tag=0");
}
hr_db.commit();
hr_db.close();
}else{
