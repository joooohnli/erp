...
try{
	if(del[i-1]!=null){
		real_del[n]=del[i-1];
		n++;
	String sql2="select * from hr_config_question_second_kind where id='"+del[i-1]+"'" ;
...
if(rs2.next()){
	...
	String sql3="select * from hr_questiones where first_kind_ID='"+first_kind_ID[i-1]+"' and second_kind_ID='"+second_kind_ID[i-1]+"'" ;
ResultSet rs3=hr_db.executeQuery(sql3) ;
if(!rs3.next()){
	String sql = "delete from hr_config_question_second_kind where id='"+del[i-1]+"'" ;
