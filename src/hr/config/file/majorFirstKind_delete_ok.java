...
try{
	if(del[i-1]!=null){
		real_del[n]=del[i-1];
		n++;
	String sql2="select * from hr_config_major_first_kind where id='"+del[i-1]+"'" ;
...
if(rs2.next()){
	...
	String sql3="select * from hr_config_major_second_kind where first_kind_ID='"+first_kind_ID[i-1]+"' and second_kind_name!=''" ;
ResultSet rs3=hr_db.executeQuery(sql3) ;
if(!rs3.next()){
	String sql = "delete from hr_config_major_first_kind where id='"+del[i-1]+"'" ;
...
if(n==0){
	response.sendRedirect("hr/config/file/majorFirstKind.jsp");
}else{
if(m<n&&m!=0){
...
response.sendRedirect("hr/config/file/majorFirstKind_delete_ok_c.jsp");
  }else if(m==n){

response.sendRedirect("hr/config/file/majorFirstKind_delete_ok_a.jsp");

	  }else if(m==0){


response.sendRedirect("hr/config/file/majorFirstKind_delete_ok_b.jsp");
