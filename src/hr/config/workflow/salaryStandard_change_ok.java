...
public class salaryStandard_change_ok extends HttpServlet{
...
if(chain_id_array==null){
	response.sendRedirect("hr/config/workflow/salaryStandard_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select standard_id from hr_salary_standard where check_tag='0'";
	ResultSet rs=hr_db1.executeQuery(sql);
	while(rs.next()){
		sql="update hr_workflow set check_tag='0' where object_ID='"+rs.getString("standard_id")+"' and type_id='"+type_id+"'";
	...
	hr_db.executeUpdate(sql1);
response.sendRedirect("hr/config/workflow/salaryStandard_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	hr_db.commit();
	hr_db1.commit();
	hr_db1.close();
