...
if(choice!=null){
	if(choice.equals("")){
	String sql = "update hr_salary_standard set check_tag='9' where standard_ID='"+standard_ID+"'" ;
	hr_db.executeUpdate(sql) ;
	sql = "delete from hr_workflow where object_ID='"+standard_ID+"' and type_id='02'" ;
		hr_db.executeUpdate(sql) ;
	}else{

	sql6="select id from hr_workflow where object_ID='"+standard_ID+"' and type_id='02' and config_id<'"+config_id+"' and config_id>='"+choice+"'";
	rs6=hr_db.executeQuery(sql6);
	while(rs6.next()){
		String sql = "update hr_workflow set check_tag='0' where id='"+rs6.getString("id")+"' and type_id='02'" ;
		hr_db1.executeUpdate(sql) ;
	}
	}
response.sendRedirect("hr/salary_standard/check_delete_ok.jsp?finished_tag=0");
}else{
response.sendRedirect("hr/salary_standard/check_delete_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("hr/salary_standard/check_delete_ok.jsp?finished_tag=2");
}
hr_db.commit();
hr_db1.commit();
hr_db.close();
