...
public class ValidatorDup8p extends HttpServlet {
	...
	public void service(HttpServletRequest request,HttpServletResponse response) throws IOException,ServletException{
		try{
			...
			ValidatorDupBean8p vdb=new ValidatorDupBean8p();
			if(!vdb.isDup(session,table_name,fn,value)){
				
				out.println(demo.getLang("erp","该编号不存在"));
			}
		}catch(Exception ex){
		ex.printStackTrace();
		}
