...
public class ValidatorDupBean4 {
	private boolean a;
	public boolean isDup(HttpSession session,String tn,String fn,String value,String values,String valued){
		try{
		...
		tokenTO = new StringTokenizer(valued,"/");        
		while(tokenTO.hasMoreTokens()) {
			second_kind_ID = tokenTO.nextToken();
			second_kind_name = tokenTO.nextToken();
			
		}
		nseer_db db = new nseer_db((String)session.getAttribute("unit_db_name"));
		String sql="select * from "+tn+" where "+fn+"='"+value+"' and first_kind_ID='"+first_kind_ID+"' and second_kind_ID='"+second_kind_ID+"' and third_kind_name!=''";
