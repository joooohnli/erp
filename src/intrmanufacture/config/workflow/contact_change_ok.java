...
if(chain_id_array==null){
  	response.sendRedirect("intrmanufacture/config/workflow/contact_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select contact_id from intrmanufacture_contact where check_tag='0'";
	ResultSet rs=intrmanufacture_db1.executeQuery(sql);
	while(rs.next()){
		sql="update intrmanufacture_workflow set check_tag='0' where object_ID='"+rs.getString("contact_id")+"'";
	...
	intrmanufacture_db.executeUpdate(sql1);	
response.sendRedirect("intrmanufacture/config/workflow/contact_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
