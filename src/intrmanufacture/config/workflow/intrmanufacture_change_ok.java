...
public class intrmanufacture_change_ok extends HttpServlet{
...
if(chain_id_array==null){
  	response.sendRedirect("intrmanufacture/config/workflow/intrmanufacture_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select intrmanufacture_id from intrmanufacture_intrmanufacture where check_tag='1'";
	ResultSet rs=intrmanufacture_db1.executeQuery(sql);
	while(rs.next()){
		sql="update intrmanufacture_workflow set check_tag='0' where object_ID='"+rs.getString("intrmanufacture_id")+"'";
	...
	intrmanufacture_db.executeUpdate(sql1);	
response.sendRedirect("intrmanufacture/config/workflow/intrmanufacture_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
