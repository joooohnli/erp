...
if(!vt.validata((String)dbSession.getAttribute("unit_db_name"),"intrmanufacture_discussion","id",id,"check_tag").equals("1")){
if(n==0){
String time="";
...
double order_profit_bonus_subtotal=0.0d;
	...
	String sql1 = "update intrmanufacture_discussion_details set product_ID='"+product_ID+"',product_name='"+product_name+"',product_describe='"+product_describe+"',list_price='"+list_price+"',amount='"+amount1+"',cost_price='"+cost_price+"',off_discount='"+off_discount+"',subtotal='"+subtotal+"' where discussion_ID='"+discussion_ID+"' and details_number='"+i+"'" ;
...
String sql2="update intrmanufacture_discussion set sale_price_sum='"+sale_price_sum+"',cost_price_sum='"+cost_price_sum+"',modify_tag='0',discussion_tag='1',discussion_status='等待' where id='"+id+"'";
intrmanufacture_db.executeUpdate(sql2) ;

	sql = "update intrmanufacture_workflow set checker='"+checker+"',checker_ID='"+checker_ID+"',check_time='"+check_time+"',check_tag='1' where object_ID='"+discussion_ID+"' and config_id='"+config_id+"'" ;
		intrmanufacture_db.executeUpdate(sql) ;

	sql="select id from intrmanufacture_workflow where object_id='"+discussion_ID+"' and check_tag='0'";
	ResultSet rset=intrmanufacture_db.executeQuery(sql) ;
	if(!rset.next()){
		sql="update intrmanufacture_discussion set check_tag='1' where discussion_ID='"+discussion_ID+"'";
		intrmanufacture_db.executeUpdate(sql) ;
	}
}
catch (Exception ex){
...
response.sendRedirect("intrmanufacture/discussion/check_choose_attachment.jsp?discussion_ID="+discussion_ID+"");
	}else{
response.sendRedirect("intrmanufacture/discussion/check_ok_a.jsp?id="+id+"&&discussion_ID="+discussion_ID+"&&config_id="+config_id+"");
	}
}else{
response.sendRedirect("intrmanufacture/discussion/check_ok_b.jsp");
}
}else{
