...
if(!rs6.next()&&vt.validata((String)dbSession.getAttribute("unit_db_name"),"intrmanufacture_file","provider_ID",provider_ID,"excel_tag").equals("1")){
if(choice!=null){
	if(choice.equals("")){
	String sql = "update intrmanufacture_file set check_tag='9' where provider_ID='"+provider_ID+"'" ;
	purchase_db.executeUpdate(sql) ;
	sql = "delete from intrmanufacture_workflow where object_ID='"+provider_ID+"'" ;
		purchase_db.executeUpdate(sql) ;
	}else{

	sql6="select id from intrmanufacture_workflow where object_ID='"+provider_ID+"' and config_id<'"+config_id+"' and config_id>='"+choice+"'";
...
response.sendRedirect("intrmanufacture/file/check_delete_ok.jsp?finished_tag=0");
}else{
response.sendRedirect("intrmanufacture/file/check_delete_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("intrmanufacture/file/check_delete_ok.jsp?finished_tag=2");
}
purchase_db.commit();
purchase_db1.commit();
purchase_db.close();
