...
public class discussion_change_ok extends HttpServlet{
...
if(chain_id_array==null){
  	response.sendRedirect("logistics/config/workflow/discussion_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
for(int j=0;j<chain_id_array.length;j++){
	...
	String sql="select discussion_id from logistics_discussion where check_tag='0'";
	ResultSet rs=logistics_db1.executeQuery(sql);
	while(rs.next()){
		sql="update logistics_workflow set check_tag='0' where object_ID='"+rs.getString("discussion_id")+"'";
	...
	logistics_db.executeUpdate(sql1);	
response.sendRedirect("logistics/config/workflow/discussion_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	logistics_db.commit();
