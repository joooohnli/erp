...
if(chain_id_array==null){//判断数组是否为空
  	response.sendRedirect("logistics/config/workflow/file_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
for(int j=0;j<chain_id_array.length;j++){
	...
	String sql="select provider_id from logistics_file where check_tag='0'";
	ResultSet rs=logistics_db1.executeQuery(sql);
	while(rs.next()){
		sql="update logistics_workflow set check_tag='0' where object_ID='"+rs.getString("provider_id")+"'";
	...
	logistics_db.executeUpdate(sql1);	
response.sendRedirect("logistics/config/workflow/file_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	logistics_db.commit();
