...
if(vt.validata((String)dbSession.getAttribute("unit_db_name"),"logistics_discussion","discussion_ID",discussion_ID,"modify_tag").equals("0")){
...
try{
	String sql16="select * from logistics_discussion where discussion_ID='"+discussion_ID+"'";
...
response.sendRedirect("logistics/discussion/change_choose_attachment.jsp?discussion_ID="+discussion_ID+"");
	}else{		
response.sendRedirect("logistics/discussion/change_ok_a.jsp?discussion_ID="+discussion_ID+"");
}
}else{		
response.sendRedirect("logistics/discussion/change_ok_b.jsp?discussion_ID="+discussion_ID+"");
}
logistics_db.commit();
logistics_db.close();
}else{
