...
if(choice!=null){
	if(choice.equals("")){
	String sql = "update logistics_discussion set check_tag='9',discussion_tag='1',modify_tag='0',check_time='"+check_time+"',checker='"+checker+"' where discussion_ID='"+discussion_ID+"' and check_tag='0'" ;
	logistics_db.executeUpdate(sql) ;
    sql = "delete from logistics_workflow where object_ID='"+discussion_ID+"'" ;
		logistics_db.executeUpdate(sql) ;
	}else{

	sql6="select id from logistics_workflow where object_ID='"+discussion_ID+"' and config_id<'"+config_id+"' and config_id>='"+choice+"'";
...
response.sendRedirect("logistics/discussion/check_delete_ok.jsp?finished_tag=0");
}else{
response.sendRedirect("logistics/discussion/check_delete_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("logistics/discussion/check_delete_ok.jsp?finished_tag=3");
}
}else{
	response.sendRedirect("logistics/discussion/check_delete_ok.jsp?finished_tag=2");
}
logistics_db.commit();
