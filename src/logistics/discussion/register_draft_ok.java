...
String real_cost_price="";
	...
	logistics_db.executeUpdate(sql1) ;
		...
		String sql="update logistics_discussion set sale_price_sum='"+sale_price_sum+"',cost_price_sum='"+cost_price_sum+"',real_cost_price_sum='"+real_cost_price_sum+"' where discussion_ID='"+discussion_ID+"'";
		logistics_db.executeUpdate(sql) ;
response.sendRedirect("logistics/discussion/register_choose_attachment.jsp?discussion_ID="+discussion_ID+"");	
}
catch (Exception ex){
out.println("error"+ex);
}
}else{	
	response.sendRedirect("logistics/discussion/register_draft_ok.jsp?finished_tag=0");
	}
	logistics_db.commit();
	logistics_db.close();
}else{
