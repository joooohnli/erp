...
response.sendRedirect("logistics/product_providers/register_ok_a.jsp");
}else{
response.sendRedirect("logistics/product_providers/register_ok_b.jsp?product_ID="+product_ID+"&&product_name="+toUtf8String.utf8String(exchange.toURL(product_name))+"");
	}
	}else{
	response.sendRedirect("logistics/product_providers/register_ok_c.jsp");
	}
	logistics_db.commit();
	logistics_db.close();	
}else{
