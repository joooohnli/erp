...
String real_cost_price="";
	...
	String sql = "insert into manufacture_apply(apply_ID,product_ID,product_name,product_describe,amount,remark,register,register_time,type,pay_id_group,check_tag) values('"+apply_ID+"','"+product_ID+"','"+product_name+"','"+product_describe+"','"+amount+"','"+remark+"','"+register+"','"+register_time+"','"+type+"','"+reason+"','5')";
...
ex.printStackTrace();
	...
	response.sendRedirect("manufacture/apply/newRegister_draft_ok.jsp?finished_tag=0");
}else{
	
	response.sendRedirect("manufacture/apply/newRegister_draft_ok.jsp?finished_tag=1");
}
