...
public class designProcedure_change_ok extends HttpServlet{
...
if(chain_id_array==null){
  	response.sendRedirect("manufacture/config/workflow/designProcedure_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select design_id from manufacture_design_procedure where check_tag='0'";
	ResultSet rs=manufacture_db1.executeQuery(sql);
	while(rs.next()){
		sql="update manufacture_workflow set check_tag='0' where object_ID='"+rs.getString("design_id")+"'";
	...
	manufacture_db.executeUpdate(sql1);	
response.sendRedirect("manufacture/config/workflow/designProcedure_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
