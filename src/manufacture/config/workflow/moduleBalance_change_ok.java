 ...
 */
package manufacture.config.workflow;
...
public class moduleBalance_change_ok extends HttpServlet{
...
if(chain_id_array==null){
  	response.sendRedirect("manufacture/config/workflow/moduleBalance_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select manufacture_id from manufacture_module_balance where check_tag='0'";
	ResultSet rs=manufacture_db1.executeQuery(sql);
	while(rs.next()){
		sql="update manufacture_workflow set check_tag='0' where object_ID='"+rs.getString("manufacture_id")+"'";
		manufacture_db.executeUpdate(sql);
	}
	String sql1="update manufacture_config_workflow set describe1='"+describe1+"',describe2='"+describe2+"' where id='"+id+"'";
	manufacture_db.executeUpdate(sql1);
	sql1="update manufacture_workflow set describe1='"+describe1+"',describe2='"+describe2+"' where config_id='"+id+"' and check_tag='0'";
	manufacture_db.executeUpdate(sql1);	
response.sendRedirect("manufacture/config/workflow/moduleBalance_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
