...
String sql="";
	...
	response.sendRedirect("manufacture/design_module/register_draft_ok.jsp?finished_tag=0");
}else if(rs2.next()){
	response.sendRedirect("manufacture/design_module/register_draft_ok.jsp?finished_tag=1");
}else{
	...
	if(rs3.next()){
String sql4="update manufacture_design_procedure set design_module_tag='5',module_cost_price_sum='"+rs3.getString("cost_price_sum")+"' where design_ID='"+design_ID+"'";
manufacture_db.executeUpdate(sql4) ;
}

		 sql = "update manufacture_design_procedure set design_module_tag='5',design_module_change_tag='0' where design_ID='"+design_ID+"'" ;
	manufacture_db.executeUpdate(sql) ;
		sql="update manufacture_design_procedure_details set design_module_change_tag='1',design_module_tag='5' where design_ID='"+design_ID+"'";
	manufacture_db.executeUpdate(sql) ;
response.sendRedirect("manufacture/design_module/register_draft_ok.jsp?finished_tag=2");
}
  design_db.commit();
