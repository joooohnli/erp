...
if(num==0&&procedure_namen.length==1){
	...
	response.sendRedirect("manufacture/design_procedure/change_ok_a.jsp?design_ID="+design_ID+"");
}else{
	if(m!=0){
	
	response.sendRedirect("manufacture/design_procedure/change_ok_b.jsp?design_ID="+design_ID+"");
	}else{
for(int i=1;i<=num;i++){
...
try{
	String sql = "update manufacture_design_procedure set design_ID='"+design_ID+"',product_ID='"+product_ID+"',product_name='"+product_name+"',change_time='"+change_time+"',changer='"+changer+"',designer='"+designer+"',procedure_describe='"+procedure_describea+"' where design_ID='"+design_ID+"'" ;
...
response.sendRedirect("manufacture/design_procedure/change_ok_c.jsp");
}else{
	
	response.sendRedirect("manufacture/design_procedure/change_ok_d.jsp?design_ID="+design_ID+"");
}
  }
}
