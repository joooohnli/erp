...
if(choice!=null){
	if(choice.equals("")){
	String sql = "update manufacture_design_procedure set check_tag='9',check_time='"+check_time+"',checker='"+checker+"' where design_ID='"+design_ID+"' and check_tag='0'" ;
...
response.sendRedirect("manufacture/design_procedure/check_delete_ok.jsp?finished_tag=0");
}else{
response.sendRedirect("manufacture/design_procedure/check_delete_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("manufacture/design_procedure/check_delete_ok.jsp?finished_tag=3");
}
}else{
	response.sendRedirect("manufacture/design_procedure/check_delete_ok.jsp?finished_tag=2");
}
