...
String sql1="select * from oa_culture where culture_ID='"+culture_ID+"' and check_tag='1'";
ResultSet rs=oa_db.executeQuery(sql1);
if(!rs.next()){
	
	response.sendRedirect("oa/culture/change_ok_a.jsp?culture_ID="+culture_ID+"");
}else{
...
String sql3="select * from oa_culture where culture_ID='"+culture_ID+"'";
	ResultSet rs3=oa_db.executeQuery(sql3);
	if(rs3.next()){
		String sql4="insert into oa_culture_dig(culture_ID,subject,type,chain_id,chain_name,CONTENT,attachment1,attachment2,remark,register,register_ID,CHECKER,CHECKER_ID,CHANGER,CHANGER_ID,register_TIME,CHECK_TIME,CHANGE_TIME,LATELY_CHANGE_TIME,change_amount,CHECK_TAG) values('"+rs3.getString("culture_ID")+"','"+rs3.getString("subject")+"','"+rs3.getString("type")+"','"+rs3.getString("chain_id")+"','"+rs3.getString("chain_name")+"','"+rs3.getString("CONTENT")+"','"+rs3.getString("attachment1")+"','"+rs3.getString("attachment2")+"','"+rs3.getString("remark")+"','"+rs3.getString("register")+"','"+rs3.getString("register_ID")+"','"+rs3.getString("CHECKER")+"','"+rs3.getString("CHECKER_ID")+"','"+rs3.getString("CHANGER")+"','"+rs3.getString("CHANGER_ID")+"','"+rs3.getString("register_TIME")+"','"+rs3.getString("CHECK_TIME")+"','"+rs3.getString("CHANGE_TIME")+"','"+rs3.getString("LATELY_CHANGE_TIME")+"','"+rs3.getString("change_amount")+"','"+rs3.getString("CHECK_TAG")+"')";
		oa_db.executeUpdate(sql4) ;
	}

List rsList = GetWorkflow.getList(oa_db, "oa_config_workflow", "05");
	String[] elem=new String[3];
	if(rsList.size()==0){

	String sqla = "update oa_culture set changer='"+changer+"',changer_ID='"+changer_ID+"',change_time='"+change_time+"',content='"+content+"',remark='"+remark+"',lately_change_time='"+lately_change_time+"',change_amount='"+change_amount+"',check_tag='1'";
...
String sql=sqla+sqlb;
	oa_db.executeUpdate(sql) ;
		 ...
		 String sql2="delete from oa_workflow where object_ID='"+culture_ID+"' and type_id='05'";
		oa_db.executeUpdate(sql2) ;
String sqla = "update oa_culture set changer='"+changer+"',changer_ID='"+changer_ID+"',change_time='"+change_time+"',content='"+content+"',remark='"+remark+"',lately_change_time='"+lately_change_time+"',change_amount='"+change_amount+"',check_tag='0'";
	String sqlb = " where culture_ID='"+culture_ID+"'" ;
if(attachment!=null){
for(int i=0;i<attachment.length;i++){
	sqla=sqla+","+attachment[i]+"=''";
...
Iterator ite=rsList.iterator();
		...
		sql2 = "insert into oa_workflow(config_id,object_ID,describe1,describe2,type_id) values ('"+elem[0]+"','"+culture_ID+"','"+elem[1]+"','"+elem[2]+"','05')" ;
		oa_db.executeUpdate(sql2) ;
		}
	}

	
response.sendRedirect("oa/culture/change_ok_b.jsp");
}
oa_db.commit();
oa_db.close();
}catch(Exception ex){
response.sendRedirect("oa/culture/change_ok_c.jsp?culture_ID="+culture_ID+"");
}
}else{
