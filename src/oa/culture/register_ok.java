...
if(rs.next()){
	...
	response.sendRedirect("oa/culture/register_ok_a.jsp");
}else{
...
String remark=exchange.toHtml(bodyab);
	...
	String sqla = "insert into oa_culture(register,register_ID,register_time,culture_ID,subject,type,content,chain_id,chain_name,remark";
	String sqlb = ") values ('"+register+"','"+register_ID+"','"+register_time+"','"+culture_ID+"','"+subject+"','"+culture_type+"','"+content+"','"+chain_id+"','"+chain_name+"','"+remark+"'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
String sql2="update oa_culture set check_tag='1' where culture_ID='"+culture_ID+"'";
...
Iterator ite=rsList.iterator();
		...
		sql = "insert into oa_workflow(config_id,object_ID,describe1,describe2,type_id) values ('"+elem[0]+"','"+culture_ID+"','"+elem[1]+"','"+elem[2]+"','05')" ;
		oa_db.executeUpdate(sql);
}
}
response.sendRedirect("oa/culture/register_ok_b.jsp");
}
oa_db.commit();
oa_db.close();
}catch(Exception ex){
	response.sendRedirect("oa/culture/register_ok_c.jsp");
}
}else{
