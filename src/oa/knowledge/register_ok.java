...
int filenum1=count.read((String)dbSession.getAttribute("unit_db_name"),"oaknowledgecount");
String knowledge_ID=filenum1+"";
count.write((String)dbSession.getAttribute("unit_db_name"),"oaknowledgecount",filenum1);
String sql1="select * from oa_knowledge where knowledge_ID='"+knowledge_ID+"'";
ResultSet rs=oa_db.executeQuery(sql1);
if(rs.next()){
	response.sendRedirect("oa/knowledge/register_ok_a.jsp");
}else{
...
String knowledge_type = mySmartUpload.getRequest().getParameter("knowledge_type");
...
String sqla = "insert into oa_knowledge(register,register_ID,register_time,knowledge_ID,subject,type,content,chain_id,chain_name,remark";
	String sqlb = ") values ('"+register+"','"+register_ID+"','"+register_time+"','"+knowledge_ID+"','"+subject+"','"+knowledge_type+"','"+content+"','"+chain_id+"','"+chain_name+"','"+remark+"'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
List rsList = GetWorkflow.getList(oa_db, "oa_config_workflow", "04");
String[] elem=new String[3];
	 if(rsList.size()==0){

String sql2="update oa_knowledge set check_tag='1' where knowledge_ID='"+knowledge_ID+"'";
...
Iterator ite=rsList.iterator();
		...
		sql = "insert into oa_workflow(config_id,object_ID,describe1,describe2,type_id) values ('"+elem[0]+"','"+knowledge_ID+"','"+elem[1]+"','"+elem[2]+"','04')" ;
		oa_db.executeUpdate(sql);
}
}

	
response.sendRedirect("oa/knowledge/register_ok_b.jsp");
}
}catch(Exception ex){
	response.sendRedirect("oa/knowledge/register_ok_c.jsp");
	}
oa_db.commit();
oa_db.close();
}else{
