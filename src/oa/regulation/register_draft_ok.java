...
if(rs.next()){
	response.sendRedirect("oa/regulation/register_draft_ok.jsp?finished_tag=0");
}else{
...
String chain_name=Divide1.getName(fileKind_chain);
	...
	String sqla = "insert into oa_regulation(register,register_ID,register_time,regulation_ID,subject,type,content,chain_id,chain_name,remark,check_tag";
	String sqlb = ") values ('"+register+"','"+register_ID+"','"+register_time+"','"+regulation_ID+"','"+subject+"','"+regulation_type+"','"+content+"','"+chain_id+"','"+chain_name+"','"+remark+"','5'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
response.sendRedirect("oa/regulation/register_draft_ok.jsp?finished_tag=1");

}
}catch(Exception ex){
	response.sendRedirect("oa/regulation/register_draft_ok.jsp?finished_tag=2");
	}
oa_db.commit();
oa_db.close();
}else{
