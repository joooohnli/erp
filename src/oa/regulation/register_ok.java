...
int filenum1=count.read((String)dbSession.getAttribute("unit_db_name"),"oaregulationcount");
	String regulation_ID=filenum1+"";
	count.write((String)dbSession.getAttribute("unit_db_name"),"oaregulationcount",filenum1);
String sql1="select * from oa_regulation where regulation_ID='"+regulation_ID+"'";
ResultSet rs=oa_db.executeQuery(sql1);
if(rs.next()){
	response.sendRedirect("oa/regulation/register_ok_a.jsp");
}else{
...
String regulation_type = mySmartUpload.getRequest().getParameter("regulation_type");
...
String chain_name=Divide1.getName(fileKind_chain);
	...
	String sqla = "insert into oa_regulation(register,register_ID,register_time,regulation_ID,subject,type,content,chain_id,chain_name,remark";
	String sqlb = ") values ('"+register+"','"+register_ID+"','"+register_time+"','"+regulation_ID+"','"+subject+"','"+regulation_type+"','"+content+"','"+chain_id+"','"+chain_name+"','"+remark+"'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
String sql2="update oa_regulation set check_tag='1' where regulation_ID='"+regulation_ID+"'";
...
Iterator ite=rsList.iterator();
		...
		sql = "insert into oa_workflow(config_id,object_ID,describe1,describe2,type_id) values ('"+elem[0]+"','"+regulation_ID+"','"+elem[1]+"','"+elem[2]+"','03')" ;
		oa_db.executeUpdate(sql);
}
}
...
response.sendRedirect("oa/regulation/register_ok_b.jsp");

}
}catch(Exception ex){
	response.sendRedirect("oa/regulation/register_ok_c.jsp");
	}
oa_db.commit();
oa_db.close();
}else{
