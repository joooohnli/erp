...
String file=details_number+myFile.getFileName().substring(myFile.getFileName().length()-4).toLowerCase();
	...
	response.sendRedirect("portal/engage/register1_choose_attachment.jsp?details_number="+details_number+"");

}catch(Exception ex){
response.sendRedirect("portal/engage/register1_ok_a.jsp?details_number="+details_number+"");
}
hr_db.commit();
hr_db.close();
}else{
