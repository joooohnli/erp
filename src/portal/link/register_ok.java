...
if(rset1.next()){
		response.sendRedirect("portal/link/register_ok_c.jsp");
}else{
...
List rsList = (List)new java.util.ArrayList();
	...
	String link_ID=NseerId.getId("ecommerce/website/link",(String)dbSession.getAttribute("unit_db_name"));
...
response.sendRedirect("portal/link/register_ok_b.jsp");	
}
	ecommerce_db.commit();
	...
	response.sendRedirect("portal/link/register_ok_a.jsp");
	}
}else{
