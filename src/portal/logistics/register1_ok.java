...
String provider_ID=NseerId.getId("logistics/file",(String)dbSession.getAttribute("unit_db_name"));
...
if(rset.next()){
		response.sendRedirect("portal/logistics/register1_ok_a.jsp");
}else{
	...
	System.out.println(sql+"ddd");
	logistics_db.executeUpdate(sql) ;
	String[] email_box={contact_person1_email};
	mail.send(email_box,"smtp.sina.com.cn","yhuser@sina.com","123456",demo.getLang("erp","注册信息"),"欢迎您注册成为我们的配送单位，您的档案编号是:"+provider_ID);
	...
	response.sendRedirect("portal/logistics/register1_choose_attachment.jsp?provider_ID="+provider_ID+"");
	}
}else{

response.sendRedirect("portal/logistics/register1_ok_b.jsp");
