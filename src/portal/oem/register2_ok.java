...
public synchronized void doPost(HttpServletRequest request,HttpServletResponse response) throws IOException,ServletException{
	...
	String discussion_ID=NseerId.getId("intrmanufacture/discussion",(String)dbSession.getAttribute("unit_db_name"));
	...
	try{
		...
		ResultSet rs=intrmanufacture_db.executeQuery(sql3);
		if(rs.next()){
		String sql = "insert into intrmanufacture_discussion(discussion_ID,chain_id,chain_name,provider_ID,provider_name,demand_contact_person_tel,demand_pay_time,register_time,remark,check_tag,excel_tag) values ('"+discussion_ID+"','"+rs.getString("chain_id")+"','"+rs.getString("chain_name")+"','"+provider_ID+"','"+rs.getString("provider_name")+"','"+demand_contact_person_tel+"','"+demand_pay_time+"','"+register_time+"','"+remark+"','0','2')" ;
		intrmanufacture_db.executeUpdate(sql) ;
		}
	String id="";
	String sqlb="select id from intrmanufacture_discussion where discussion_ID='"+discussion_ID+"'";
	rs=intrmanufacture_db.executeQuery(sqlb);
		if(rs.next()){
			id=rs.getString("id");
		}
	...
	String real_cost_price="";
		...
		String sql1 = "insert into intrmanufacture_discussion_details(discussion_ID,details_number,product_ID,product_name,product_describe,list_price,amount,cost_price,real_cost_price,off_discount,subtotal,amount_unit) values ('"+discussion_ID+"','"+i+"','"+product_ID+"','"+product_name+"','"+product_describe+"','"+list_price+"','"+amount+"','"+cost_price+"','"+real_cost_price+"','"+off_discount+"','"+subtotal+"','"+amount_unit+"')" ;
		intrmanufacture_db.executeUpdate(sql1) ;
	}
		List rsList = (List)new java.util.ArrayList();
		String[] elem=new String[3];
		String sql="select id,describe1,describe2 from intrmanufacture_config_workflow where type_id='02'";
		...
		if(rsList.size()==0){
			sql="update intrmanufacture_discussion set modify_tag='0',discussion_tag='1',discussion_status='等待' where discussion_ID='"+discussion_ID+"'";
			intrmanufacture_db.executeUpdate(sql) ;
		}else{
	...
	String sql2="update intrmanufacture_discussion set sale_price_sum='"+sale_price_sum+"',cost_price_sum='"+cost_price_sum+"',real_cost_price_sum='"+real_cost_price_sum+"' where discussion_ID='"+discussion_ID+"'";
	intrmanufacture_db.executeUpdate(sql2) ;
response.sendRedirect("portal/oem/register2_choose_attachment.jsp?discussion_ID="+discussion_ID+"");	
}
catch (Exception ex){
out.println("error"+ex);
	...
	response.sendRedirect("portal/oem/register2_ok_a.jsp?provider_ID="+provider_ID);
	}
