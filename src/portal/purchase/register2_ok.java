...
public synchronized void doPost(HttpServletRequest request,HttpServletResponse response) throws IOException,ServletException{
	...
	String discussion_ID=NseerId.getId("purchase/discussion",(String)dbSession.getAttribute("unit_db_name"));
	...
	try{
		...
		if(rs.next()){
		String sql = "insert into purchase_discussion(discussion_ID,chain_id,chain_name,provider_ID,provider_name,demand_contact_person_tel,demand_pay_time,register_time,register,remark,check_tag,excel_tag) values ('"+discussion_ID+"','"+rs.getString("chain_id")+"','"+rs.getString("chain_name")+"','"+provider_ID+"','"+rs.getString("provider_name")+"','"+demand_contact_person_tel+"','"+demand_pay_time+"','"+register_time+"','"+register+"','"+remark+"','0','2')" ;
...
response.sendRedirect("portal/purchase/register2_choose_attachment.jsp?discussion_ID="+discussion_ID+"");	
}
catch (Exception ex){
out.println("error"+ex);
	...
	response.sendRedirect("portal/purchase/register2_ok_a.jsp?provider_ID="+provider_ID);
	}
	purchase_db.commit();
	purchase_db.close();
}else{
