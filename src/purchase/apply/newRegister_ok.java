...
String real_cost_price="";
	...
	String sql = "insert into purchase_apply(apply_ID,product_ID,product_name,product_describe,amount,remark,register,register_time,new_tag,pay_ID_group) values('"+apply_ID+"','"+product_ID+"','"+product_name+"','"+product_describe+"','"+amount+"','"+remark+"','"+register+"','"+register_time+"','1','新发生')";
	purchase_db.executeUpdate(sql) ;
	}
	if(rsList.size()==0){
		String sql="update purchase_apply set check_tag='1' where apply_ID='"+apply_ID+"'";
		purchase_db.executeUpdate(sql) ;
		}else{
		Iterator ite=rsList.iterator();
		while(ite.hasNext()){
		String[] elem=(String[])ite.next();
		String sql = "insert into purchase_workflow(config_id,object_ID,describe1,describe2) values ('"+elem[0]+"','"+apply_ID+"','"+elem[1]+"','"+elem[2]+"')" ;
		purchase_db.executeUpdate(sql);
		}
	}
}

response.sendRedirect("purchase/apply/newRegister_ok_a.jsp");
	}else{
response.sendRedirect("purchase/apply/newRegister_ok_b.jsp");
}
