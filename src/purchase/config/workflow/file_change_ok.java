...
if(chain_id_array==null){
  	response.sendRedirect("purchase/config/workflow/file_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select provider_id from purchase_file where check_tag='0'";
	ResultSet rs=purchase_db1.executeQuery(sql);
	while(rs.next()){
		sql="update purchase_workflow set check_tag='0' where object_ID='"+rs.getString("provider_id")+"'";
	...
	purchase_db.executeUpdate(sql1);	
response.sendRedirect("purchase/config/workflow/file_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	purchase_db.commit();
	purchase_db1.commit();
	purchase_db1.close();
