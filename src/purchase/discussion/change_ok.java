...
if(vt.validata((String)dbSession.getAttribute("unit_db_name"),"purchase_discussion","discussion_ID",discussion_ID,"modify_tag").equals("0")){
...
response.sendRedirect("purchase/discussion/change_choose_attachment.jsp?discussion_ID="+discussion_ID+"");
	}else{		
response.sendRedirect("purchase/discussion/change_ok_a.jsp?discussion_ID="+discussion_ID+"");
}
}else{		
response.sendRedirect("purchase/discussion/change_ok_b.jsp?discussion_ID="+discussion_ID+"");
}
purchase_db.commit();
purchase_db.close();
}else{
