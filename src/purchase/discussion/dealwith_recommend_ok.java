...
Data dt1=new Data((String)dbSession.getAttribute("unit_db_name"),"purchase_file","provider_ID",provider_ID);
...
try{
product_providers_recommend_ID=NseerId.getId("purchase/product_providers",(String)dbSession.getAttribute("unit_db_name"));
...
design_db.executeUpdate(sql3) ;
	...
	String sql1 = "insert into purchase_product_providers_recommend_details(product_providers_recommend_ID,details_number,provider_ID,provider_name,provider_class,provider_tel,provider_web,type,contact_person) values ('"+product_providers_recommend_ID+"','1','"+provider_ID+"','"+provider_name+"','"+provider_class+"','"+provider_tel1+"','"+provider_web+"','"+type+"','"+contact_person1+"')" ;
	purchase_db.executeUpdate(sql1) ;
	sql1 = "update purchase_discussion_details set dealwith_tag='1' where discussion_ID='"+discussion_ID+"' and product_ID='"+product_ID+"'" ;
	purchase_db.executeUpdate(sql1) ;
}
catch (Exception ex){
ex.printStackTrace();
...
purchase_db.executeUpdate(sql4) ;
	...
	response.sendRedirect("purchase/discussion/dealwith.jsp?discussion_ID="+discussion_ID);

	}else{

String sqq="select * from purchase_product_providers_recommend where product_ID='"+product_ID+"' and (check_tag='1' or check_tag='0')";
ResultSet rs=purchase_db.executeQuery(sqq) ;
if(rs.next()){
	product_providers_recommend_ID=rs.getString("product_providers_recommend_id");
	sqq="select * from purchase_product_providers_recommend_details where product_providers_recommend_id='"+rs.getString("product_providers_recommend_id")+"' and provider_ID='"+provider_ID+"'";
	rs=purchase_db.executeQuery(sqq) ;
	if(rs.next()){
		String sql1 = "update purchase_discussion_details set dealwith_tag='1' where discussion_ID='"+discussion_ID+"' and product_ID='"+product_ID+"'" ;
	purchase_db.executeUpdate(sql1) ;
	}else{
		sqq="select * from purchase_product_providers_recommend_details where product_providers_recommend_id='"+product_providers_recommend_ID+"' order by details_number desc";
		rs=purchase_db.executeQuery(sqq) ;
		if(rs.next()){
			int details_number=rs.getInt("details_number")+1;
			String sql1 = "insert into purchase_product_providers_recommend_details(product_providers_recommend_ID,details_number,provider_ID,provider_name,provider_class,provider_tel,provider_web,type,contact_person) values ('"+product_providers_recommend_ID+"','"+details_number+"','"+provider_ID+"','"+provider_name+"','"+provider_class+"','"+provider_tel1+"','"+provider_web+"','"+type+"','"+contact_person1+"')" ;
			purchase_db.executeUpdate(sql1) ;
			sql1 = "update purchase_discussion_details set dealwith_tag='1' where discussion_ID='"+discussion_ID+"' and product_ID='"+product_ID+"'" ;
			purchase_db.executeUpdate(sql1) ;
		}
	}
	sqq = "update purchase_product_providers_recommend set change_time='"+time+"',changer='"+processer+"',recommender='"+recommender+"',check_tag='0',change_tag='1' where product_providers_recommend_ID='"+product_providers_recommend_ID+"'" ;
	purchase_db.executeUpdate(sqq) ;
}
String sql3="update design_file set recommend_provider_tag='1' where product_ID='"+product_ID+"'";
design_db.executeUpdate(sql3) ;
