...
response.sendRedirect("purchase/file/register_ok_a.jsp");
	...
	response.sendRedirect("purchase/file/register_choose_attachment.jsp?provider_ID="+provider_ID+"");
	}
	
	}else{
	
	response.sendRedirect("purchase/file/register_draft_ok.jsp?finished_tag=1");
	}
	purchase_db.commit();	
	design_db.commit();
purchase_db.close();
