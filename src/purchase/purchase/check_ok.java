...
String sql6="select id from purchase_workflow where object_ID='"+purchase_ID+"' and ((check_tag='0' and config_ID<'"+config_ID+"') or (check_tag='1' and config_ID='"+config_ID+"'))";
...
try{
    String sql9 = "update purchase_workflow set checker='"+checker+"',checker_ID='"+checker_ID+"',check_time='"+check_time+"',check_tag='1' where object_ID='"+purchase_ID+"' and config_ID='"+config_ID+"'" ;
	purchase_db.executeUpdate(sql9);
	sql9="select id from purchase_workflow where object_ID='"+purchase_ID+"' and check_tag='0'";
	ResultSet rset=purchase_db.executeQuery(sql9);
	if(!rset.next()){
		String sqll = "update purchase_purchase set purchase_tag='1',invoice_tag='1',gather_tag='1',stock_gather_tag='1',pay_tag='1',check_tag='2' where purchase_ID='"+purchase_ID+"'" ;
    ...
    sql9="select * from purchase_details where purchase_ID='"+purchase_ID+"'";
    ...
    if(vrn.validata((String)dbSession.getAttribute("unit_db_name"),"stock_gather","reasonexact",purchase_ID)<record_number){    	
		...
		sql6="insert into stock_gather_details(gather_ID,details_number,product_ID,product_name,amount,ungather_amount,cost_price,subtotal) values('"+gather_ID+"','1','"+product_ID+"','"+product_name+"','"+rs9.getString("demand_amount")+"','"+rs9.getString("demand_amount")+"','"+rs9.getString("demand_price")+"','"+rs9.getString("demand_cost_price_sum")+"')";
...
ex.printStackTrace();
	...
	response.sendRedirect("purchase/purchase/check_ok.jsp?finished_tag=0");
	}else{	
	response.sendRedirect("purchase/purchase/check_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("purchase/purchase/check_ok.jsp?finished_tag=2");
}
	purchase_db1.commit();
	stock_db.commit();
fund_db.commit();
