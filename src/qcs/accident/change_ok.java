...
String sqla="select check_tag,dealwith_tag,track_tag,attachment1 from qcs_accident where accident_id='"+accident_id+"'";
ResultSet rs=qcs_db.executeQuery(sqla);
if(!rs.next()){
  response.sendRedirect("qcs/accident/change_ok.jsp?finished_tag=0");
} else if(rs.getString("check_tag").equals("0")){
	response.sendRedirect("qcs/accident/change_ok.jsp?finished_tag=2");
	}else if(rs.getString("dealwith_tag").equals("0")){
	response.sendRedirect("qcs/accident/change_ok.jsp?finished_tag=3");
	}else if(rs.getString("track_tag").equals("1")){
	response.sendRedirect("qcs/accident/change_ok.jsp?finished_tag=4");
	}
else{
...
String body5 = new String(mySmartUpload.getRequest().getParameter("records").getBytes("UTF-8"),"UTF-8");
String records=exchange.toHtml(body5);


String spoorer = mySmartUpload.getRequest().getParameter("spoorer");
String spoorer_id = mySmartUpload.getRequest().getParameter("spoorer_id");
String spoor_time = mySmartUpload.getRequest().getParameter("spoor_time");

sqla = "update qcs_accident set accident_time='"+accident_time+"',accident_name='"+accident_name+"',product_id='"+product_id+"',product_name='"+product_name+"',accident_outlines='"+accident_outlines+"',explanation='"+explanation+"',survey_result='"+survey_result+"',spoorer_id='"+spoorer_id+"',spoorer='"+spoorer+"',spoor_time='"+spoor_time+"',records='"+records+"',track_tag='1'";
...
response.sendRedirect("qcs/accident/change_ok.jsp?finished_tag=1");
}
qcs_db.commit();
qcs_db.close();
}else{
