...
public class stock_change_ok extends HttpServlet{
...
if(chain_id_array==null){
	response.sendRedirect("qcs/config/workflow/stock_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
...
String sql="select qcs_id from qcs_stock where check_tag='0'";
	...
	qcs_db.executeUpdate(sql1);
response.sendRedirect("qcs/config/workflow/stock_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	qcs_db.commit();
	qcs_db1.commit();
	qcs_db1.close();
