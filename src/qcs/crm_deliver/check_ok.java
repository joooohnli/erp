...
try{
	...
	String sqla="select attachment1 from qcs_crm_deliver where qcs_id='"+qcs_id+"' and check_tag='0'";
	ResultSet rs=qcs_db.executeQuery(sqla);
	if(!rs.next()){	
		response.sendRedirect("qcs/crm_deliver/check_ok.jsp?finished_tag=0");
	}else{
...
if(!rset.next()){
	sql="update qcs_crm_deliver set check_tag='1' where qcs_id='"+qcs_id+"' and check_tag='0'";
	qcs_db.executeUpdate(sql);
}
response.sendRedirect("qcs/crm_deliver/check_ok.jsp?finished_tag=1");
}
qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/crm_deliver/check_ok.jsp?finished_tag=2");
	}
}catch(Exception ex){
