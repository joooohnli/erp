...
String remark=exchange.toHtml(bodyab);
	String sqla = "insert into qcs_crm_deliver(qcs_id,stock_id,stock_name,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,qcs_result,register,register_time,remark";
	String sqlb = ") values ('"+qcs_id+"','"+stock_id+"','"+stock_name+"','"+apply_id+"','"+product_id+"','"+product_name+"','"+qcs_amount+"','"+qcs_time+"','"+quality_way+"','"+quality_solution+"','"+sampling_standard+"','"+sampling_amount+"','"+accept+"','"+reject+"','"+qualified+"','"+unqualified+"','"+qcs_result+"','"+register+"','"+register_time+"','"+remark+"'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
response.sendRedirect("qcs/crm_deliver/register_ok.jsp?finished_tag=0");

qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/crm_deliver/register_ok.jsp?finished_tag=1");
	}
}catch(Exception ex){
	response.sendRedirect("qcs/crm_deliver/register_ok.jsp?finished_tag=2");
}
