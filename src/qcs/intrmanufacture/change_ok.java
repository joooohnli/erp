...
try{
	...
	String sqla="select attachment1 from qcs_intrmanufacture where qcs_id='"+qcs_id+"' and check_tag='1'";
	ResultSet rs=qcs_db.executeQuery(sqla);
	if(!rs.next()){	
		response.sendRedirect("qcs/intrmanufacture/change_ok.jsp?finished_tag=0");
	}else{
...
if(rsList.size()==0){
	...
	qcs_db.executeUpdate(sql) ;
	sql="update qcs_intrmanufacture set check_tag='0' where qcs_id='"+qcs_id+"'";
	qcs_db.executeUpdate(sql) ;
...
response.sendRedirect("qcs/intrmanufacture/change_ok.jsp?finished_tag=1");
}
qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/intrmanufacture/change_ok.jsp?finished_tag=2");
	}
}catch(Exception ex){
	response.sendRedirect("qcs/intrmanufacture/change_ok.jsp?finished_tag=3");
}
