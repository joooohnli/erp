...
String remark=exchange.toHtml(bodyab);
	String sqla = "insert into qcs_intrmanufacture(qcs_id,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,qcs_result,register,register_time,remark,check_tag";
	String sqlb = ") values ('"+qcs_id+"','"+apply_id+"','"+product_id+"','"+product_name+"','"+qcs_amount+"','"+qcs_time+"','"+quality_way+"','"+quality_solution+"','"+sampling_standard+"','"+sampling_amount+"','"+accept+"','"+reject+"','"+qualified+"','"+unqualified+"','"+qcs_result+"','"+register+"','"+register_time+"','"+remark+"','5'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
response.sendRedirect("qcs/intrmanufacture/register_draft_ok.jsp?finished_tag=0");

qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/intrmanufacture/register_draft_ok.jsp?finished_tag=1");
	}
}catch(Exception ex){
	response.sendRedirect("qcs/intrmanufacture/register_draft_ok.jsp?finished_tag=2");
}
