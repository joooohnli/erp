...
if(!rs6.next()){
String sql1="select id from qcs_item where item_id='"+item_id+"' and check_tag='0'";
ResultSet rs=qcs_db.executeQuery(sql1);
if(!rs.next()){	
	response.sendRedirect("qcs/item/check_delete_ok.jsp?finished_tag=3");
}else{
	if(choice!=null){
	if(choice.equals("")){
	String sql = "update qcs_item set checker='"+checker+"',checker_id='"+checker_id+"',check_time='"+check_time+"',check_tag='9' where item_id='"+item_id+"'";
	qcs_db.executeUpdate(sql);
	sql = "delete from qcs_workflow where object_ID='"+item_id+"'" ;
	qcs_db.executeUpdate(sql) ;
	}else{
	sql6="select id from qcs_workflow where object_ID='"+item_id+"' and config_id<'"+config_id+"' and config_id>='"+choice+"'";
	...
	response.sendRedirect("qcs/item/check_delete_ok.jsp?finished_tag=0");
	}else{
	response.sendRedirect("qcs/item/check_delete_ok.jsp?finished_tag=1");
	}	
//response.sendRedirect("qcs/sampling_standard/check_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("qcs/item/check_delete_ok.jsp?finished_tag=2");
}
qcs_db1.commit();
qcs_db.commit();
qcs_db1.close();
