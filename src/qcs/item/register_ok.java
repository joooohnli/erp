...
String remark=exchange.toHtml(bodyab);
	String sqla = "insert into qcs_item(item_id,item_name,analyse_method,default_basis,ready_basis,quality_method,quality_equipment,sampling_standard,defect_class,important,register_id,register,register_time,designer,quality_value,remark";
	String sqlb = ") values ('"+item_id+"','"+item_name+"','"+analyse_method+"','"+default_basis+"','"+ready_basis+"','"+quality_method+"','"+quality_equipment+"','"+sampling_standard+"','"+defect_class+"','"+important+"','"+register_id+"','"+register+"','"+register_time+"','"+designer+"','"+QV_hidden+"','"+remark+"'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
response.sendRedirect("qcs/item/register_ok.jsp?finished_tag=0");
qcs_db.commit();
qcs_db.close();
}
catch(Exception ex){
	response.sendRedirect("qcs/item/register_ok.jsp?finished_tag=1");
}
}else{
