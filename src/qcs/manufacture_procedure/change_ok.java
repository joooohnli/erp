...
try{
	...
	String sqla="select attachment1 from qcs_manufacture_procedure where qcs_id='"+qcs_id+"' and check_tag='1'";
	ResultSet rs=qcs_db.executeQuery(sqla);
	if(!rs.next()){	
		response.sendRedirect("qcs/manufacture_procedure/change_ok.jsp?finished_tag=0");
	}else{
...
String remark=exchange.toHtml(bodyab);
	...
	sqla = "update qcs_manufacture_procedure set apply_id='"+apply_id+"',product_id='"+product_id+"',product_name='"+product_name+"',qcs_amount='"+qcs_amount+"',qcs_time='"+qcs_time+"',quality_way='"+quality_way+"',accept='"+accept+"',reject='"+reject+"',procedure_name='"+procedure_name+"',quality_solution='"+quality_solution+"',sampling_standard='"+sampling_standard+"',sampling_amount='"+sampling_amount+"',qualified='"+qualified+"',unqualified='"+unqualified+"',changer_id='"+changer_id+"',qcs_result='"+qcs_result+"',changer='"+changer+"',change_time='"+change_time+"',remark='"+remark+"',check_tag='0'";
...
List rsList = GetWorkflow.getList(qcs_db, "qcs_config_workflow", "10");
if(rsList.size()==0){
	sql="update qcs_manufacture_procedure set check_tag='1' where qcs_id='"+qcs_id+"'";
	qcs_db.executeUpdate(sql);
	sql="update manufacture_procedure set qcs_tag='1' where manufacture_id='"+manufacture_id+"' and qcs_tag='0' and procedure_name='"+procedure_name+"'";
	qcs_db.executeUpdate(sql);
}else{
	sql="delete from qcs_workflow where object_ID='"+qcs_id+"'";
	qcs_db.executeUpdate(sql) ;
	sql="update manufacture_procedure set qcs_tag='0' where manufacture_id='"+manufacture_id+"' and procedure_name='"+procedure_name+"'";
	qcs_db.executeUpdate(sql);
	sql="update qcs_manufacture_procedure set check_tag='0' where qcs_id='"+qcs_id+"'";
	qcs_db.executeUpdate(sql) ;
...
response.sendRedirect("qcs/manufacture_procedure/change_ok.jsp?finished_tag=1");
}
qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/manufacture_procedure/change_ok.jsp?finished_tag=2");
	}
}catch(Exception ex){
	response.sendRedirect("qcs/manufacture_procedure/change_ok.jsp?finished_tag=3");
}
