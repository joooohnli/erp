...
try{
	...
	String sqla="select attachment1 from qcs_manufacture_procedure where qcs_id='"+qcs_id+"' and check_tag='0'";
	ResultSet rs=qcs_db.executeQuery(sqla);
	if(!rs.next()){	
		response.sendRedirect("qcs/manufacture_procedure/check_ok.jsp?finished_tag=0");
	}else{
...
if(!rset.next()){
	sql="update qcs_manufacture_procedure set check_tag='1' where qcs_id='"+qcs_id+"' and check_tag='0'";
	qcs_db.executeUpdate(sql);
}
response.sendRedirect("qcs/manufacture_procedure/check_ok.jsp?finished_tag=1");
}
qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/manufacture_procedure/check_ok.jsp?finished_tag=2");
	}
}else{
response.sendRedirect("qcs/manufacture_procedure/check_ok.jsp?finished_tag=4");
}

}catch(Exception ex){
	response.sendRedirect("qcs/manufacture_procedure/check_ok.jsp?finished_tag=3");
}
