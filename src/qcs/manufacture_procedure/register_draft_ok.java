...
String remark=exchange.toHtml(bodyab);
	String sqla = "insert into qcs_manufacture_procedure(manufacture_id,qcs_id,apply_id,product_id,product_name,qcs_amount,qcs_time,procedure_name,quality_way,quality_solution,accept,reject,sampling_standard,sampling_amount,qualified,unqualified,qcs_result,register,register_time,remark,check_tag";
	String sqlb = ") values ('"+manufacture_id+"','"+qcs_id+"','"+apply_id+"','"+product_id+"','"+product_name+"','"+qcs_amount+"','"+qcs_time+"','"+procedure_name+"','"+quality_way+"','"+quality_solution+"','"+accept+"','"+reject+"','"+sampling_standard+"','"+sampling_amount+"','"+qualified+"','"+unqualified+"','"+qcs_result+"','"+register+"','"+register_time+"','"+remark+"','5'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
response.sendRedirect("qcs/manufacture_procedure/register_draft_ok.jsp?finished_tag=0");

qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/manufacture_procedure/register_draft_ok.jsp?finished_tag=1");
	}
}catch(Exception ex){
	response.sendRedirect("qcs/manufacture_procedure/register_draft_ok.jsp?finished_tag=2");
}
