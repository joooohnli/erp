...
try{
	...
	String sqla="select attachment1 from qcs_manufacture_product where qcs_id='"+qcs_id+"' and check_tag='1'";
	ResultSet rs=qcs_db.executeQuery(sqla);
	if(!rs.next()){	
		response.sendRedirect("qcs/manufacture_product/change_ok.jsp?finished_tag=0");
	}else{
...
response.sendRedirect("qcs/manufacture_product/change_ok.jsp?finished_tag=1");
}
qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/manufacture_product/change_ok.jsp?finished_tag=2");
	}
}catch(Exception ex){
	response.sendRedirect("qcs/manufacture_product/change_ok.jsp?finished_tag=3");
}
