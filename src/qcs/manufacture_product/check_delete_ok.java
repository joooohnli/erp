...
if(!rs6.next()){
String sql1="select id from qcs_manufacture_product where qcs_id='"+qcs_id+"' and check_tag='0'";
ResultSet rs=qcs_db.executeQuery(sql1);
if(!rs.next()){
	response.sendRedirect("qcs/manufacture_product/check_delete_ok.jsp?finished_tag=3");
}else{
	if(choice!=null){
	if(choice.equals("")){
	String sql = "update qcs_manufacture_product set checker='"+checker+"',checker_id='"+checker_id+"',check_time='"+check_time+"',check_tag='9' where qcs_id='"+qcs_id+"'";
	...
	response.sendRedirect("qcs/manufacture_product/check_delete_ok.jsp?finished_tag=0");
	}else{
	response.sendRedirect("qcs/manufacture_product/check_delete_ok.jsp?finished_tag=1");
	}	
}
}else{
response.sendRedirect("qcs/manufacture_product/check_delete_ok.jsp?finished_tag=2");
}
qcs_db.commit();
qcs_db1.commit();
qcs_db.close();
