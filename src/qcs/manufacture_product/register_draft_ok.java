...
String remark=exchange.toHtml(bodyab);
	String sqla = "insert into qcs_manufacture_product(qcs_id,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,qcs_result,register,register_time,remark,check_tag";
...
response.sendRedirect("qcs/manufacture_product/register_draft_ok.jsp?finished_tag=0");
qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/manufacture_product/register_draft_ok.jsp?finished_tag=1");
	}
}catch(Exception ex){
	response.sendRedirect("qcs/manufacture_product/register_draft_ok.jsp?finished_tag=2");
}
