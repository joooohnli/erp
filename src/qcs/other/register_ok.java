...
String time=formatter.format(now);String 
qcs_id=NseerId.getId(getClass().getResource("").toString(),(String)dbSession.getAttribute("unit_db_name"));
...
String remark=exchange.toHtml(bodyab);
	String sqla = "insert into qcs_other(qcs_id,apply_id,product_id,product_name,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,qcs_result,register,register_time,remark";
...
response.sendRedirect("qcs/other/register_ok.jsp?finished_tag=0");

qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/other/register_ok.jsp?finished_tag=1");
	}
}catch(Exception ex){
	response.sendRedirect("qcs/other/register_ok.jsp?finished_tag=2");
}
