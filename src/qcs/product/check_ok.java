...
if(!rs6.next()){
String sql1="select attachment1 from qcs_product_config where qcs_id='"+qcs_id+"' and check_tag='0'";
ResultSet rs=qcs_db.executeQuery(sql1);
if(!rs.next()){	
	response.sendRedirect("qcs/product/check_ok.jsp?finished_tag=0");
}else{
...
if(!rset.next()){
	sql="update qcs_product_config set check_tag='1' where qcs_id='"+qcs_id+"' and check_tag='0'";
	qcs_db.executeUpdate(sql);
...
response.sendRedirect("qcs/product/check_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("qcs/product/check_ok.jsp?finished_tag=3");
}
qcs_db.commit();
qcs_db.close();
}
