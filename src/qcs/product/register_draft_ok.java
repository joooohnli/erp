...
String remark=exchange.toHtml(bodyab);
	String sqla = "insert into qcs_product_config(qcs_id,product_id,product_name,purchase_qcs_way,intrmanu_qcs_way,crm_deliver_qcs_way,crm_return_qcs_way,manu_product_qcs_way,manu_procedure_qcs_way,stock_qcs_way,other_qcs_way,register_id,register,register_time,remark,check_tag";
	String sqlb = ") values ('"+qcs_id+"','"+product_id+"','"+product_name+"','"+purchase_qcs_way+"','"+intrmanu_qcs_way+"','"+crm_deliver_qcs_way+"','"+crm_return_qcs_way+"','"+manu_product_qcs_way+"','"+manu_procedure_qcs_way+"','"+stock_qcs_way+"','"+other_qcs_way+"','"+register_id+"','"+register+"','"+register_time+"','"+remark+"','5'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
response.sendRedirect("qcs/product/register_draft_ok.jsp?finished_tag=0");
qcs_db.commit();
qcs_db.close();
}catch(Exception ex){
	response.sendRedirect("qcs/product/register_draft_ok.jsp?finished_tag=1");
}
}else{
