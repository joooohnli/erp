...
String sql1="select attachment1 from qcs_quality_standard where standard_id='"+standard_id+"' and check_tag='1'";
ResultSet rs=qcs_db.executeQuery(sql1);
if(!rs.next()){
	
	response.sendRedirect("qcs/quality_standard/change_ok.jsp?finished_tag=0");
}else{
...
String remark=exchange.toHtml(bodyab);
	String sqla = "update qcs_quality_standard set standard_name='"+standard_name+"',product_id='"+product_id+"',product_name='"+product_name+"',designer='"+designer+"',changer='"+changer+"',changer_id='"+changer_id+"',change_time='"+change_time+"',remark='"+remark+"',check_tag='0'";
...
response.sendRedirect("qcs/quality_standard/change_ok.jsp?finished_tag=1");
}
qcs_db.commit();
qcs_db.close();
}else{
	response.sendRedirect("qcs/quality_standard/change_ok.jsp?finished_tag=3");
}
}catch(Exception ex){
	ex.printStackTrace();
	response.sendRedirect("qcs/quality_standard/change_ok.jsp?finished_tag=2");
}
}else{
