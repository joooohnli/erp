...
if(!rs6.next()){
String sql1="select attachment1 from qcs_quality_standard where standard_id='"+standard_id+"' and check_tag='0'";
ResultSet rs=qcs_db.executeQuery(sql1);
if(!rs.next()){	
	response.sendRedirect("qcs/quality_standard/check_ok.jsp?finished_tag=0");
}else{
...
String remark=exchange.toHtml(bodyab);
	String sqla = "update qcs_quality_standard set standard_name='"+standard_name+"',product_id='"+product_id+"',product_name='"+product_name+"',designer='"+designer+"',checker='"+checker+"',checker_id='"+checker_id+"',check_time='"+check_time+"',remark='"+remark+"'";
...
sql = "update qcs_workflow set checker='"+checker+"',checker_ID='"+checker_id+"',check_time='"+check_time+"',check_tag='1' where object_ID='"+standard_id+"' and config_id='"+config_id+"'" ;
	...
	if(!rset.next()){
	sql="update qcs_quality_standard set check_tag='1' where standard_id='"+standard_id+"' and check_tag='0'";
	qcs_db.executeUpdate(sql);
	}
response.sendRedirect("qcs/quality_standard/check_ok.jsp?finished_tag=2");
}
}else{
response.sendRedirect("qcs/quality_standard/check_ok.jsp?finished_tag=5");
}
qcs_db.commit();
qcs_db.close();
}else{
	response.sendRedirect("qcs/quality_standard/check_ok.jsp?finished_tag=3");
}
}
catch(Exception ex){
	ex.printStackTrace();
	response.sendRedirect("qcs/quality_standard/check_ok.jsp?finished_tag=4");
}
}else{
