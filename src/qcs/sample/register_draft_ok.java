...
String remark=exchange.toHtml(bodyab);
	String sqla = "insert into qcs_sample(sample_id,lately_view_time,quality_type,apply_id,sampling_person,sampling_time,register,register_time,remark,check_tag";
	String sqlb = ") values ('"+sample_id+"','"+sampling_time+"','"+quality_type+"','"+apply_id+"','"+sampling_person+"','"+sampling_time+"','"+register+"','"+register_time+"','"+remark+"','5'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
response.sendRedirect("qcs/sample/register_draft_ok.jsp?finished_tag=0");
qcs_db.commit();
qcs_db.close();
}else{
	response.sendRedirect("qcs/sample/register_draft_ok.jsp?finished_tag=2");
}
}catch(Exception ex){
	//ex.printStackTrace();
	response.sendRedirect("qcs/sample/register_draft_ok.jsp?finished_tag=1");
}
}else{
