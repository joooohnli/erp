...
String sql1="select attachment1 from qcs_sampling_standard where standard_id='"+standard_id+"' and check_tag='1'";
ResultSet rs=qcs_db.executeQuery(sql1);
if(!rs.next()){
	
	response.sendRedirect("qcs/sampling_standard/change_ok.jsp?finished_tag=0");
}else{
...
if(aql_id==null) aql_id="";
	...
	String sqla = "update qcs_sampling_standard set standard_name='"+standard_name+"',changer_id='"+changer_id+"',changer='"+changer+"',change_time='"+change_time+"',sampling_method='"+method_name+"',designer='"+designer+"',quality_level='"+level_id+"',mil_std='"+class_id+"',aql='"+aql_id+"',remark='"+remark+"',check_tag='0'";
...
response.sendRedirect("qcs/sampling_standard/change_ok.jsp?finished_tag=1");
}
qcs_db.commit();
qcs_db.close();
}
	...
	ex.printStackTrace();
	response.sendRedirect("qcs/sampling_standard/change_ok.jsp?finished_tag=2");
}
}else{
