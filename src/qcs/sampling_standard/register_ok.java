...
if(aql_id==null) aql_id="";
	...
	String sqla = "insert into qcs_sampling_standard(standard_id,standard_name,register_ID,register,register_time,sampling_method,designer,quality_level,mil_std,aql,remark";
	String sqlb = ") values ('"+standard_id+"','"+standard_name+"','"+register_ID+"','"+register+"','"+register_time+"','"+method_name+"','"+designer+"','"+level_id+"','"+class_id+"','"+aql_id+"','"+remark+"'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
response.sendRedirect("qcs/sampling_standard/register_ok.jsp?finished_tag=0");

qcs_db.commit();
qcs_db.close();
}
catch(Exception ex){
	response.sendRedirect("qcs/sampling_standard/register_ok.jsp?finished_tag=1");
}
}else{
