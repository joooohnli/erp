...
String sql1="select attachment1 from qcs_solution where solution_id='"+solution_id+"' and check_tag='1'";
ResultSet rs=qcs_db.executeQuery(sql1);
if(!rs.next()){	
	response.sendRedirect("qcs/solution/change_ok.jsp?finished_tag=0");
}else{
...
if(rsList.size()==0){
	...
	sql="delete from qcs_workflow where object_ID='"+solution_id+"'";
	qcs_db.executeUpdate(sql) ;
...
response.sendRedirect("qcs/solution/change_ok.jsp?finished_tag=1");
}
qcs_db.commit();
qcs_db.close();
}else{
	response.sendRedirect("qcs/solution/change_ok.jsp?finished_tag=3");
}
}
catch(Exception ex){
	ex.printStackTrace();
	response.sendRedirect("qcs/solution/change_ok.jsp?finished_tag=2");
}
}else{
