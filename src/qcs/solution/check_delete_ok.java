...
if(!rs6.next()){
String sql1="select id from qcs_solution where solution_id='"+solution_id+"' and check_tag='0'";
ResultSet rs=qcs_db.executeQuery(sql1);
if(!rs.next()){
	response.sendRedirect("qcs/solution/check_delete_ok.jsp?finished_tag=0");
}else{
	if(choice!=null){
	if(choice.equals("")){
	String sql = "update qcs_solution set checker='"+checker+"',checker_id='"+checker_id+"',check_time='"+check_time+"',check_tag='9' where solution_id='"+solution_id+"'";
	qcs_db.executeUpdate(sql) ;
	sql = "delete from qcs_workflow where object_ID='"+solution_id+"'" ;
	qcs_db.executeUpdate(sql) ;
	}else{
	sql6="select id from qcs_workflow where object_ID='"+solution_id+"' and config_id<'"+config_id+"' and config_id>='"+choice+"'";
	...
	response.sendRedirect("qcs/solution/check_delete_ok.jsp?finished_tag=1");
	}else{
	response.sendRedirect("qcs/solution/check_delete_ok.jsp?finished_tag=2");
	}
	}
}else{
response.sendRedirect("qcs/solution/check_delete_ok.jsp?finished_tag=3");
}
qcs_db.commit();
qcs_db1.commit();
qcs_db.close();
