...
String remark=exchange.toHtml(bodyab);
	String sqla = "insert into qcs_solution(quality_standard_id,quality_standard_name,solution_id,solution_name,product_id,product_name,register_id,register,register_time,designer,remark,check_tag";
	String sqlb = ") values ('"+quality_standard_id+"','"+quality_standard_name+"','"+solution_id+"','"+solution_name+"','"+product_id+"','"+product_name+"','"+register_id+"','"+register+"','"+register_time+"','"+designer+"','"+remark+"','5'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
response.sendRedirect("qcs/solution/register_draft_ok.jsp?finished_tag=0");

qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/solution/register_draft_ok.jsp?finished_tag=2");
	}
}

catch(Exception ex){
	response.sendRedirect("qcs/solution/register_draft_ok.jsp?finished_tag=1");
}
