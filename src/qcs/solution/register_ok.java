...
String solution_id=NseerId.getId(getClass().getResource("").toString(),(String)dbSession.getAttribute("unit_db_name"));
...
String remark=exchange.toHtml(bodyab);
	String sqla = "insert into qcs_solution(quality_standard_id,quality_standard_name,solution_id,solution_name,product_id,product_name,register_id,register,register_time,designer,remark";
	String sqlb = ") values ('"+quality_standard_id+"','"+quality_standard_name+"','"+solution_id+"','"+solution_name+"','"+product_id+"','"+product_name+"','"+register_id+"','"+register+"','"+register_time+"','"+designer+"','"+remark+"'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
List rsList = GetWorkflow.getList(qcs_db, "qcs_config_workflow", "04");
if(rsList.size()==0){
	sql="update qcs_solution set check_tag='1' where solution_id='"+solution_id+"'";
	qcs_db.executeUpdate(sql);
}else{
	Iterator ite=rsList.iterator();
	while(ite.hasNext()){
		String[] elem=(String[])ite.next();
		sql = "insert into qcs_workflow(config_id,object_ID,describe1,describe2) values ('"+elem[0]+"','"+solution_id+"','"+elem[1]+"','"+elem[2]+"')" ;
		qcs_db.executeUpdate(sql);
	}
}
response.sendRedirect("qcs/solution/register_ok.jsp?finished_tag=0");

qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/solution/register_ok.jsp?finished_tag=2");
	}
}

catch(Exception ex){
	response.sendRedirect("qcs/solution/register_ok.jsp?finished_tag=1");
}
