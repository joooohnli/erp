...
String remark=exchange.toHtml(bodyab);
	String sqla = "insert into qcs_stock(qcs_id,apply_id,product_id,product_name,stock_name,stock_id,qcs_amount,qcs_time,quality_way,quality_solution,sampling_standard,sampling_amount,accept,reject,qualified,unqualified,qcs_result,register,register_time,remark";
	String sqlb = ") values ('"+qcs_id+"','"+apply_id+"','"+product_id+"','"+product_name+"','"+stock_name+"','"+stock_id+"','"+qcs_amount+"','"+qcs_time+"','"+quality_way+"','"+quality_solution+"','"+sampling_standard+"','"+sampling_amount+"','"+accept+"','"+reject+"','"+qualified+"','"+unqualified+"','"+qcs_result+"','"+register+"','"+register_time+"','"+remark+"'" ;
for(int i=0;i<mySmartUpload.getFiles().getCount();i++){
	int p=i+1;
...
List rsList = GetWorkflow.getList(qcs_db, "qcs_config_workflow", "11");
if(rsList.size()==0){
	sql="update qcs_stock set check_tag='1' where qcs_id='"+qcs_id+"'";
	qcs_db.executeUpdate(sql);
}else{
...
response.sendRedirect("qcs/stock/register_ok.jsp?finished_tag=0");
qcs_db.commit();
qcs_db.close();
}else{
	 response.sendRedirect("qcs/stock/register_ok.jsp?finished_tag=1");
	}
}catch(Exception ex){
	response.sendRedirect("qcs/stock/register_ok.jsp?finished_tag=2");
}
}else{
