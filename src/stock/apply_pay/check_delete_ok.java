...
try{
if(choice.equals("")){//***********************
	String sql2="update stock_apply_pay set check_tag='9',check_time='"+check_time+"',checker='"+checker+"' where pay_ID='"+pay_ID+"'";
...
response.sendRedirect("stock/apply_pay/check_delete_ok.jsp?finished_tag=0");
}else{
response.sendRedirect("stock/apply_pay/check_delete_ok.jsp?finished_tag=3");
}
}else{
response.sendRedirect("stock/apply_pay/check_delete_ok.jsp?finished_tag=2");
}
}else{
response.sendRedirect("stock/apply_pay/check_delete_ok.jsp?finished_tag=1");
}
stock_db.commit();
stock_db1.commit();
stock_db.close();
