...
response.sendRedirect("stock/cell/register_draft_ok.jsp?finished_tag=0");
}else{
	session.setAttribute("product_name",product_name);
response.sendRedirect("stock/cell/register_draft_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("stock/cell/register_draft_ok.jsp?finished_tag=2");
}
stock_db.commit();
stock_db.close();
}else{
