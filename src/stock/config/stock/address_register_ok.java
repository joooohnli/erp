...
if(n!=0){
response.sendRedirect("stock/config/stock/address_register_ok_a.jsp");
}else{
      String sqll="select * from stock_config_public_char where stock_ID='"+stock_ID+"' or stock_name='"+stock_name+"'";
ResultSet rs=stock_db.executeQuery(sqll);
if(rs.next()){
response.sendRedirect("stock/config/stock/address_register_ok_b.jsp");
}else{
String sql = "insert into stock_config_public_char(stock_ID,stock_name,responsible_person_ID,responsible_person,describe1) values('"+stock_ID+"','"+stock_name+"','"+responsible_person_ID+"','"+responsible_person+"','库房')" ;
stock_db.executeUpdate(sql) ;
int filenum=count.read((String)dbSession.getAttribute("unit_db_name"),"stockstockcount");
count.write((String)dbSession.getAttribute("unit_db_name"),"stockstockcount",filenum);
response.sendRedirect("stock/config/stock/address_register_ok_c.jsp");
}
}
stock_db.commit();
stock_db.close();
