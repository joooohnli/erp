...
public class applyGather_change_ok extends HttpServlet{
...
if(chain_id_array==null){//判断数组是否为空
  	response.sendRedirect("stock/config/workflow/applyGather_change_ok_a.jsp?id="+id+"&type_id="+type_id);
}else{
String describe1="";
String describe2="";
for(int j=0;j<chain_id_array.length;j++){
	...
	String sql="select gather_id from stock_apply_gather where check_tag='0'";
	...
	stock_db.executeUpdate(sql1);	
response.sendRedirect("stock/config/workflow/applyGather_change_ok_b.jsp?id="+id+"&type_id="+type_id);
}
	stock_db.commit();
	stock_db1.commit();
	stock_db1.close();
