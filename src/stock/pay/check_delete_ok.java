...
if(!rs6.next()){
String sql8="select * from stock_paying_gathering where pay_ID='"+pay_ID+"' and check_tag='0' and paying_time='"+paying_time+"'";
ResultSet rs8=stock_db.executeQuery(sql8);
if(rs8.next()){
try{
	if(choice.equals("")){
	String sql2="update stock_pre_paying set pay_check_tag='9' where pay_ID='"+pay_ID+"'";
	stock_db.executeUpdate(sql2) ;
	sql2 = "delete from stock_workflow where object_ID='"+pay_ID+"' and paying_time='"+paying_time+"'" ;
	stock_db.executeUpdate(sql2) ;
	}else{

	sql6="select id from stock_workflow where object_ID='"+pay_ID+"' and config_id<'"+config_id+"' and config_id>='"+choice+"' and paying_time='"+paying_time+"'";
	...
	while(rs6.next()){
  	...
  	response.sendRedirect("stock/pay/check_delete_ok.jsp?finished_tag=0");
}else{
	
response.sendRedirect("stock/pay/check_delete_ok.jsp?finished_tag=3");

}
}else{
	response.sendRedirect("stock/pay/check_delete_ok.jsp?finished_tag=2");
}
}else{
	response.sendRedirect("stock/pay/check_delete_ok.jsp?finished_tag=1");
}
stock_db.commit();
stock_db1.commit();
stock_db.close();
