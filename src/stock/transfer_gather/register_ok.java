...
//**********************************************************
if(rsList.size()==0){
	String nseer_sql="update stock_transfer_gather set cost_price_sum='"+cost_price_sum+"',demand_amount='"+demand_amount+"',check_tag='1' where gather_ID='"+gather_ID+"'";
...
if(!vr.validata((String)dbSession.getAttribute("unit_db_name"),"stock_gather","reasonexact",gather_ID)){
	...
	stock_db.executeUpdate(sql4) ;
		...
		String nseer_sql="update stock_transfer_gather set cost_price_sum='"+cost_price_sum+"',demand_amount='"+demand_amount+"',check_tag='0' where gather_ID='"+gather_ID+"'";
		stock_db.executeUpdate(nseer_sql);
		Iterator ite=rsList.iterator();
		while(ite.hasNext()){
		elem=(String[])ite.next();
		nseer_sql = "insert into stock_workflow(config_id,object_ID,describe1,describe2) values ('"+elem[0]+"','"+gather_ID+"','"+elem[1]+"','"+elem[2]+"')";
		stock_db.executeUpdate(nseer_sql);
		}
}
}
...
response.sendRedirect("stock/transfer_gather/register_ok_a.jsp");
}else{
