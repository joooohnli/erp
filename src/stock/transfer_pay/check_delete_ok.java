...
try{
if(choice.equals("")){//***********************
	String sql2="update stock_transfer_pay set check_tag='9',check_time='"+check_time+"',checker='"+checker+"' where pay_ID='"+pay_ID+"'";
	stock_db.executeUpdate(sql2) ;
	sql2 = "delete from stock_workflow where object_ID='"+pay_ID+"'" ;
	stock_db.executeUpdate(sql2) ;
	}else{

	sql6="select id from stock_workflow where object_ID='"+pay_ID+"' and config_id<'"+config_id+"' and config_id>='"+choice+"'";
...
response.sendRedirect("stock/transfer_pay/check_delete_ok.jsp?finished_tag=0");
}else{
response.sendRedirect("stock/transfer_pay/check_delete_ok.jsp?finished_tag=3");
}
}else{
response.sendRedirect("stock/transfer_pay/check_delete_ok.jsp?finished_tag=2");
}
}else{
response.sendRedirect("stock/transfer_pay/check_delete_ok.jsp?finished_tag=1");
}
stock_db.commit();
stock_db1.commit();
stock_db.close();
