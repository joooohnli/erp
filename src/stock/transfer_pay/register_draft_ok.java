...
String sql = "insert into stock_transfer_pay(pay_ID,reason,reasonexact,register_time,remark,register,check_tag,excel_tag) values ('"+pay_ID+"','内部调出','"+reasonexact+"','"+register_time+"','"+remark+"','"+register+"','5','2')" ;
...
String cost_price="";
	...
	String nseer_sql="update stock_transfer_pay set cost_price_sum='"+cost_price_sum+"',demand_amount='"+demand_amount+"' where pay_ID='"+pay_ID+"'";
...
response.sendRedirect("stock/transfer_pay/register_draft_ok.jsp?finished_tag=0");
}else{
response.sendRedirect("stock/transfer_pay/register_draft_ok.jsp?finished_tag=1");
}
}else{
response.sendRedirect("stock/transfer_pay/register_draft_ok.jsp?finished_tag=2");
}
stock_db.commit();
	stock_db.close();
}else{
